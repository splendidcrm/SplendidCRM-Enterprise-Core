...
if not exists (select * from INFORMATION_SCHEMA.COLUMNS where TABLE_NAME = 'CALLS' and COLUMN_NAME = 'EMAIL_REMINDER_TIME') begin -- then
	print 'alter table CALLS add EMAIL_REMINDER_TIME int null';
	alter table CALLS add EMAIL_REMINDER_TIME int null;
end -- if;
GO
...
if not exists (select * from INFORMATION_SCHEMA.COLUMNS where TABLE_NAME = 'CALLS' and COLUMN_NAME = 'SMS_REMINDER_TIME') begin -- then
	print 'alter table CALLS add SMS_REMINDER_TIME int null';
	alter table CALLS add SMS_REMINDER_TIME int null;
end -- if;
GO
...
-- 11/07/2017 Paul.  Add IS_PRIVATE for use by a large customer. 
if not exists (select * from INFORMATION_SCHEMA.COLUMNS where TABLE_NAME = 'CALLS' and COLUMN_NAME = 'IS_PRIVATE') begin -- then
	print 'alter table CALLS add IS_PRIVATE bit null';
	alter table CALLS add IS_PRIVATE bit null;
end -- if;
GO

if exists (select * from INFORMATION_SCHEMA.TABLES where TABLE_NAME = 'CALLS_AUDIT') begin -- then
	if not exists (select * from INFORMATION_SCHEMA.COLUMNS where TABLE_NAME = 'CALLS_AUDIT' and COLUMN_NAME = 'IS_PRIVATE') begin -- then
		print 'alter table CALLS_AUDIT add IS_PRIVATE bit null';
		alter table CALLS_AUDIT add IS_PRIVATE bit null;
	end -- if;
end -- if;
GO

-- 11/29/2017 Paul.  Add ASSIGNED_SET_ID for Dynamic User Assignment. 
if not exists (select * from INFORMATION_SCHEMA.COLUMNS where TABLE_NAME = 'CALLS' and COLUMN_NAME = 'ASSIGNED_SET_ID') begin -- then
	print 'alter table CALLS add ASSIGNED_SET_ID uniqueidentifier null';
	alter table CALLS add ASSIGNED_SET_ID uniqueidentifier null;

	create index IDX_CALLS_ASSIGNED_SET_ID on dbo.CALLS (ASSIGNED_SET_ID, DELETED, ID)
end -- if;
GO

if exists (select * from INFORMATION_SCHEMA.TABLES where TABLE_NAME = 'CALLS_AUDIT') begin -- then
	if not exists (select * from INFORMATION_SCHEMA.COLUMNS where TABLE_NAME = 'CALLS_AUDIT' and COLUMN_NAME = 'ASSIGNED_SET_ID') begin -- then
		print 'alter table CALLS_AUDIT add ASSIGNED_SET_ID uniqueidentifier null';
		alter table CALLS_AUDIT add ASSIGNED_SET_ID uniqueidentifier null;
	end -- if;
end -- if;
GO
