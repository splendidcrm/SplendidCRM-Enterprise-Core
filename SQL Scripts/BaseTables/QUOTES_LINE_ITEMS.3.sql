...
if not exists (select * from INFORMATION_SCHEMA.TABLES where TABLE_NAME = 'QUOTES_LINE_ITEMS' and TABLE_TYPE = 'BASE TABLE')
  begin
	print 'Create Table dbo.QUOTES_LINE_ITEMS';
	Create Table dbo.QUOTES_LINE_ITEMS
		( ID                                 uniqueidentifier not null default(newid()) constraint PK_QUOTES_LINE_ITEMS primary key
	...
	alter table dbo.QUOTES_LINE_ITEMS add constraint FK_QUOTES_LINE_ITEMS_QUOTE foreign key ( QUOTE_ID ) references dbo.QUOTES ( ID )
  end
GO
