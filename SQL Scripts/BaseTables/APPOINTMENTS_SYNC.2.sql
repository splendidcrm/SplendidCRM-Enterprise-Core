...
-- drop table APPOINTMENTS_SYNC;
if not exists (select * from INFORMATION_SCHEMA.TABLES where TABLE_NAME = 'APPOINTMENTS_SYNC' and TABLE_TYPE = 'BASE TABLE')
  begin
	print 'Create Table dbo.APPOINTMENTS_SYNC';
	Create Table dbo.APPOINTMENTS_SYNC
		( ID                                 uniqueidentifier not null default(newid()) constraint PK_APPOINTMENTS_SYNC primary key
