if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spSqlBuildNonArchiveRelatedView' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spSqlBuildNonArchiveRelatedView;
GO
 ...
 * 
 *********************************************************************************************************************/
-- 06/26/2018 Paul.  This procedure is to create views for an non-archived table to an archived table. 
Create Procedure dbo.spSqlBuildNonArchiveRelatedView
...
as
  begin
	set nocount on
	print 'spSqlBuildNonArchiveRelatedView ' + @MODULE_NAME;
	...
	if @SPLENDID_FIELDS = 6 begin -- then
		-- 06/26/2018 Paul.  This table is not in the archive database. 
		exec dbo.spSqlTableExists @EXISTS out, @ARCHIVE_TABLE, null;
		if @EXISTS = 1 begin -- then
			-- 12/08/2017 Paul.  Module TABLE_NAME may be invalid, so just to vwSqlTables to validate. 
			-- 06/26/2018 Paul.  Use RELATE_TABLE = @TABLE_NAME so that we access the other side. 
			declare ARCHIVE_RELATED_CURSOR cursor for
			select vwSqlTables.TABLE_NAME                  as RELATED_TABLE
			     , vwMODULES_ARCHIVE_RELATED.RELATED_TABLE as LEFT_TABLE
			     , vwMODULES_ARCHIVE_RELATED.TABLE_NAME    as RIGHT_TABLE
			 ...
			 where vwMODULES_ARCHIVE_RELATED.RELATED_TABLE = @TABLE_NAME
			 order by 3;
			...
			while @@FETCH_STATUS = 0 begin -- do
				...
				*/
				if exists (select * from vwSqlViews where VIEW_NAME = 'vw' + @RIGHT_TABLE + '_ARCHIVE') begin -- then
					...
					--	set @COMMAND = @COMMAND + '     , DOCUMENT_REVISIONS.REVISION          as SELECTED_REVISION' + @CRLF;
					--end else 
					if not exists(select * from vwSqlColumns where ObjectName = @RELATED_VIEW and ColumnName = @SINGULAR_RIGHT + '_NAME') begin -- then
						set @COMMAND = @COMMAND + '     , ' + @RELATED_VIEW + '.NAME             as ' + @SINGULAR_RIGHT + '_NAME' + @CRLF;
					end -- if;
					set @COMMAND = @COMMAND + '  from            ' + @ARCHIVE_DATABASE_DOT + 'dbo.' + @ARCHIVE_TABLE + ' ' + @ARCHIVE_TABLE + @CRLF;
					if @RELATED_TABLE = @RIGHT_TABLE begin -- then
...
Grant Execute on dbo.spSqlBuildNonArchiveRelatedView to public;
GO
