if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spQUOTES_MassDelete' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spQUOTES_MassDelete;
GO
 ...
 * 
 *********************************************************************************************************************/
Create Procedure dbo.spQUOTES_MassDelete
...
as
  begin
	...
	while @CurrentPosR <= len(@ID_LIST) begin -- do
		...
		exec dbo.spQUOTES_Delete @ID, @MODIFIED_USER_ID;
	end -- while;
  end
GO
 
Grant Execute on dbo.spQUOTES_MassDelete to public;
GO
