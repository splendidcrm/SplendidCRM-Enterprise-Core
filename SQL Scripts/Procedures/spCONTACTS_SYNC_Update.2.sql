if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spCONTACTS_SYNC_Update' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spCONTACTS_SYNC_Update;
GO
...
Create Procedure dbo.spCONTACTS_SYNC_Update
...
as
  begin
	...
	if @SERVICE_NAME = N'ConstantContact' or @SERVICE_NAME = N'HubSpot' or @SERVICE_NAME = N'iContact' or @SERVICE_NAME = N'Marketo' or @SERVICE_NAME = N'MailChimp' or @SERVICE_NAME = N'GetResponse' begin -- then
		...
		exec dbo.spCONTACTS_USERS_Update @MODIFIED_USER_ID, @LOCAL_ID, @ASSIGNED_USER_ID;
	end -- if;
...
Grant Execute on dbo.spCONTACTS_SYNC_Update to public;
GO
