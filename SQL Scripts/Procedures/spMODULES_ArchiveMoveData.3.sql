if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spMODULES_ArchiveMoveData' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spMODULES_ArchiveMoveData;
GO
...
Create Procedure dbo.spMODULES_ArchiveMoveData
...
as
  begin
	...
	declare @RELATED_MODULE      nvarchar(80);
	...
	print 'spMODULES_ArchiveMoveData: ' + @MODULE_NAME;
	...
	if len(@ID_LIST) = 0 begin -- then
		...
		-- 10/10/2017 Paul.  Make sure to delete the trackers. 
		set @Command = 'delete from TRACKER where ITEM_ID in (' + @ID_VALUES  + ')';
		exec(@Command);

		set @CUSTOM_NAME = @TABLE_NAME + '_CSTM';
		exec dbo.spMODULES_ARCHIVE_LOG_InsertList @MODIFIED_USER_ID, @TABLE_NAME, N'Move', @ID_LIST, @ARCHIVE_RULE_ID;
		exec dbo.spSqlMoveArchiveData @MODIFIED_USER_ID, @CUSTOM_NAME, @ID_LIST, null, null, null, @ARCHIVE_DATABASE;
		exec dbo.spSqlMoveArchiveData @MODIFIED_USER_ID, @TABLE_NAME , @ID_LIST, null, null, null, @ARCHIVE_DATABASE;

		-- 02/08/2020 Paul.  Include Audit tables. 
		set @AUDIT_TABLE_NAME  = @TABLE_NAME + '_AUDIT';
		set @AUDIT_CUSTOM_NAME = @TABLE_NAME + '_CSTM_AUDIT';
		exec dbo.spSqlMoveArchiveData @MODIFIED_USER_ID, @AUDIT_CUSTOM_NAME, @ID_LIST, null, null, null, @ARCHIVE_DATABASE;
		exec dbo.spSqlMoveArchiveData @MODIFIED_USER_ID, @AUDIT_TABLE_NAME , @ID_LIST, null, null, null, @ARCHIVE_DATABASE;
		if @TABLE_NAME = 'QUOTES' begin -- then
			...
			exec dbo.spSqlMoveArchiveData @MODIFIED_USER_ID, @RELATED_CUSTOM_NAME, @ID_LIST, @RELATED_TABLE_NAME, @RELATED_FIELD_NAME, null, @ARCHIVE_DATABASE;
			exec dbo.spSqlMoveArchiveData @MODIFIED_USER_ID, @RELATED_TABLE_NAME , @ID_LIST, @RELATED_TABLE_NAME, @RELATED_FIELD_NAME, null, @ARCHIVE_DATABASE;

			-- 02/08/2020 Paul.  Include Audit tables. 
			set @AUDIT_RELATED_TABLE_NAME  = @RELATED_TABLE_NAME + '_AUDIT';
			set @AUDIT_RELATED_CUSTOM_NAME = @RELATED_TABLE_NAME + '_CSTM_AUDIT';
			exec dbo.spSqlMoveArchiveData @MODIFIED_USER_ID, @AUDIT_RELATED_CUSTOM_NAME, @ID_LIST, @AUDIT_RELATED_TABLE_NAME, @RELATED_FIELD_NAME, null, @ARCHIVE_DATABASE;
			exec dbo.spSqlMoveArchiveData @MODIFIED_USER_ID, @AUDIT_RELATED_TABLE_NAME , @ID_LIST, @AUDIT_RELATED_TABLE_NAME, @RELATED_FIELD_NAME, null, @ARCHIVE_DATABASE;
		...
		if exists(select * from vwMODULES_ARCHIVE_RELATED where MODULE_NAME = @MODULE_NAME and RELATED_NAME = 'Activities') begin -- then
			exec dbo.spMODULES_ArchiveMoveActivities @MODIFIED_USER_ID, @MODULE_NAME, @ID_LIST;
		end -- if;
		...
		while @@FETCH_STATUS = 0 begin -- do
			print 'spMODULES_ArchiveMoveData: ' + @MODULE_NAME + '.' + @RELATED_MODULE_NAME;
			...
			if @EXISTS = 0 begin -- then
				print 'spMODULES_ArchiveMoveData: ' + @RELATED_ARCHIVE + ' does not exist';
			end -- if;
			if @EXISTS = 1 begin -- then
				...
				--print 'spMODULES_ArchiveMoveData: ' + @RELATIONSHIP_TABLE + '.' + @SINGULAR_RIGHT_KEY;
				if exists (select * from vwSqlTables where TABLE_NAME = @RELATIONSHIP_TABLE) begin -- then
					...
					if len(@RELATED_ID_LIST) > 0 begin -- then
						set @RELATED_CUSTOM_NAME = @RELATED_TABLE_NAME + '_CSTM';
						exec dbo.spMODULES_ARCHIVE_LOG_InsertOnly @MODIFIED_USER_ID, @RELATED_TABLE_NAME, N'Move';
						exec dbo.spSqlMoveArchiveData @MODIFIED_USER_ID, @RELATED_CUSTOM_NAME, @RELATED_ID_LIST, null, null, null, @ARCHIVE_DATABASE;
						exec dbo.spSqlMoveArchiveData @MODIFIED_USER_ID, @RELATED_TABLE_NAME , @RELATED_ID_LIST, null, null, null, @ARCHIVE_DATABASE;

						-- 02/08/2020 Paul.  Include Audit tables. 
						set @AUDIT_RELATED_TABLE_NAME  = @RELATED_TABLE_NAME + '_AUDIT';
						set @AUDIT_RELATED_CUSTOM_NAME = @RELATED_TABLE_NAME + '_CSTM_AUDIT';
						exec dbo.spSqlMoveArchiveData @MODIFIED_USER_ID, @AUDIT_RELATED_CUSTOM_NAME, @RELATED_ID_LIST, null, null, null, @ARCHIVE_DATABASE;
						exec dbo.spSqlMoveArchiveData @MODIFIED_USER_ID, @AUDIT_RELATED_TABLE_NAME , @RELATED_ID_LIST, null, null, null, @ARCHIVE_DATABASE;
						if @TABLE_NAME = 'QUOTES' begin -- then
							set @RELATED_TABLE_NAME  = 'QUOTES_LINE_ITEMS';
							set @RELATED_CUSTOM_NAME = @RELATED_TABLE_NAME + '_CSTM';
							set @RELATED_FIELD_NAME  = 'QUOTE_ID';
							exec dbo.spMODULES_ARCHIVE_LOG_InsertOnly @MODIFIED_USER_ID, @RELATED_TABLE_NAME, N'Move';
							exec dbo.spSqlMoveArchiveData @MODIFIED_USER_ID, @RELATED_CUSTOM_NAME, @RELATED_ID_LIST, @RELATED_TABLE_NAME, @RELATED_FIELD_NAME, null, @ARCHIVE_DATABASE;
							exec dbo.spSqlMoveArchiveData @MODIFIED_USER_ID, @RELATED_TABLE_NAME , @RELATED_ID_LIST, @RELATED_TABLE_NAME, @RELATED_FIELD_NAME, null, @ARCHIVE_DATABASE;

							-- 02/08/2020 Paul.  Include Audit tables. 
							set @AUDIT_RELATED_TABLE_NAME  = @RELATED_TABLE_NAME + '_AUDIT';
							set @AUDIT_RELATED_CUSTOM_NAME = @RELATED_TABLE_NAME + '_CSTM_AUDIT';
							exec dbo.spSqlMoveArchiveData @MODIFIED_USER_ID, @AUDIT_RELATED_CUSTOM_NAME, @RELATED_ID_LIST, @AUDIT_RELATED_TABLE_NAME, @RELATED_FIELD_NAME, null, @ARCHIVE_DATABASE;
							exec dbo.spSqlMoveArchiveData @MODIFIED_USER_ID, @AUDIT_RELATED_TABLE_NAME , @RELATED_ID_LIST, @AUDIT_RELATED_TABLE_NAME, @RELATED_FIELD_NAME, null, @ARCHIVE_DATABASE;
						end else if @TABLE_NAME = 'ORDERS' begin -- then
							set @RELATED_TABLE_NAME  = 'ORDERS_LINE_ITEMS';
							set @RELATED_CUSTOM_NAME = @RELATED_TABLE_NAME + '_CSTM';
							set @RELATED_FIELD_NAME  = 'ORDER_ID';
						...
						if exists(select * from vwMODULES_ARCHIVE_RELATED where MODULE_NAME = @RELATED_MODULE_NAME and RELATED_NAME = 'Activities') begin -- then
							exec dbo.spMODULES_ArchiveMoveActivities @MODIFIED_USER_ID, @RELATED_MODULE_NAME, @RELATED_ID_LIST;
						end -- if;
					end -- if;
				end -- if;
			end -- if;
...
Grant Execute on dbo.spMODULES_ArchiveMoveData to public;
GO

/*
begin tran;
declare @ID_LIST varchar(8000);
set @ID_LIST = '4DBFE34B-1701-46BC-BA27-4DB027EFBF56';
exec dbo.spMODULES_ArchiveMoveData null, 'Accounts', @ID_LIST;
rollback tran;
*/
