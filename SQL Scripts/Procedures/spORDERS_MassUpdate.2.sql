if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spORDERS_MassUpdate' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spORDERS_MassUpdate;
GO
...
Create Procedure dbo.spORDERS_MassUpdate
	...
	while @CurrentPosR <= len(@ID_LIST) begin -- do
		...
		if @TAG_SET_NAME is not null and len(@TAG_SET_NAME) > 0 begin -- then
			if @TAG_SET_ADD = 1 begin -- then
				exec dbo.spTAG_SETS_AddSet       @MODIFIED_USER_ID, @ID, N'Orders', @TAG_SET_NAME;
			end else begin
		...
		-- BEGIN Oracle Exception
			update ORDERS
			   set MODIFIED_USER_ID   = @MODIFIED_USER_ID 
			     ...
			     , ORDER_STAGE        = isnull(@ORDER_STAGE       , ORDER_STAGE       )
			     , DATE_ORDER_DUE     = isnull(@DATE_ORDER_DUE    , DATE_ORDER_DUE    )
			     , DATE_ORDER_SHIPPED = isnull(@DATE_ORDER_SHIPPED, DATE_ORDER_SHIPPED)
			     , TEAM_ID            = isnull(@TEAM_ID           , TEAM_ID           )
...
Grant Execute on dbo.spORDERS_MassUpdate to public;
GO
