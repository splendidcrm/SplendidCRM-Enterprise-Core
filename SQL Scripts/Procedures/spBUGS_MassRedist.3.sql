if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spBUGS_MassRedist' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spBUGS_MassRedist;
GO
...
Create Procedure dbo.spBUGS_MassRedist
	( @ID_LIST           varchar(max)
	...
	while @CurrentPosR <= len(@ID_LIST) begin -- do
		...
		-- BEGIN Oracle Exception
			select @RELATED_LIST = @RELATED_LIST + (case when len(@RELATED_LIST) > 0 then  ',' else  '' end) + cast(ID as char(36))
			  from DOCUMENTS
			 where ID               in (select DOCUMENT_ID from DOCUMENTS_BUGS where BUG_ID = @ID and DELETED = 0)
			   and ASSIGNED_USER_ID = @EXISTING_USER_ID
			   and DELETED          = 0;
		-- END Oracle Exception
		if len(@RELATED_LIST) > 0 begin -- then
			exec dbo.spDOCUMENTS_MassRedist @RELATED_LIST, @MODIFIED_USER_ID, @ASSIGNED_USER_ID, @TEAM_ID, @EXISTING_USER_ID, N'Bugs';
		end -- if;
...
Grant Execute on dbo.spBUGS_MassRedist to public;
GO
