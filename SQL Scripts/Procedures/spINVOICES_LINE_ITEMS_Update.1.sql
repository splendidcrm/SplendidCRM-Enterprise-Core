if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spINVOICES_LINE_ITEMS_Update' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spINVOICES_LINE_ITEMS_Update;
GO
...
Create Procedure dbo.spINVOICES_LINE_ITEMS_Update
...
as
  begin
	...
	-- BEGIN Oracle Exception
		-- 07/07/2007 Paul.  Use exchange rate from invoice. 
	...
	if @POSITION is null begin -- then
		-- BEGIN Oracle Exception
			select @POSITION = isnull(max(POSITION) + 1, 0)
			  from vwINVOICES_LINE_ITEMS
			 where INVOICE_ID = @INVOICE_ID;
		-- END Oracle Exception
	end -- if;
	
	if @LINE_ITEM_TYPE = N'Comment' begin -- then
		if not exists(select * from INVOICES_LINE_ITEMS where ID = @ID) begin -- then
	...
	if not exists(select * from INVOICES_LINE_ITEMS_CSTM where ID_C = @ID) begin -- then
		insert into INVOICES_LINE_ITEMS_CSTM ( ID_C ) values ( @ID );
	end -- if;
  end
GO

Grant Execute on dbo.spINVOICES_LINE_ITEMS_Update to public;
GO
