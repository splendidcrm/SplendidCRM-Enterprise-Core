if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spCONTACTS_MassUpdate' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spCONTACTS_MassUpdate;
GO
...
Create Procedure dbo.spCONTACTS_MassUpdate
	...
	while @CurrentPosR <= len(@ID_LIST) begin -- do
		...
		if @TAG_SET_NAME is not null and len(@TAG_SET_NAME) > 0 begin -- then
			if @TAG_SET_ADD = 1 begin -- then
				exec dbo.spTAG_SETS_AddSet       @MODIFIED_USER_ID, @ID, N'Contacts', @TAG_SET_NAME;
			end else begin
		...
		if dbo.fnIsEmptyGuid(@ACCOUNT_ID) = 0 begin -- then
			...
			-- 05/06/2007 Paul.  The account and contact IDs were switched. 
...
Grant Execute on dbo.spCONTACTS_MassUpdate to public;
GO
