if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spPROJECT_MassRedist' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spPROJECT_MassRedist;
GO
...
Create Procedure dbo.spPROJECT_MassRedist
	( @ID_LIST           varchar(max)
...
as
  begin
	...
	while @CurrentPosR <= len(@ID_LIST) begin -- do
		...
		-- 04/17/2013 Paul.  Reassign related Project Tasks that were assigned to the old user. 
		set @RELATED_LIST = '';
		-- BEGIN Oracle Exception
		...
		if len(@RELATED_LIST) > 0 begin -- then
			exec dbo.spPROJECT_TASK_MassRedist @RELATED_LIST, @MODIFIED_USER_ID, @ASSIGNED_USER_ID, @TEAM_ID, @EXISTING_USER_ID, N'Project';
		end -- if;
...
Grant Execute on dbo.spPROJECT_MassRedist to public;
GO
