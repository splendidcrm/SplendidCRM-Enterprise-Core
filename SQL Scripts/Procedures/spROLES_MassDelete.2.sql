if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spROLES_MassDelete' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spROLES_MassDelete;
GO
 ...
 * 
 *********************************************************************************************************************/
Create Procedure dbo.spROLES_MassDelete
...
as
  begin
	...
	while @CurrentPosR <= len(@ID_LIST) begin -- do
		...
		exec dbo.spROLES_Delete @ID, @MODIFIED_USER_ID;
	end -- while;
  end
GO
 
Grant Execute on dbo.spROLES_MassDelete to public;
GO
