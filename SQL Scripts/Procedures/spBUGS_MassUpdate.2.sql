if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spBUGS_MassUpdate' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spBUGS_MassUpdate;
GO
...
Create Procedure dbo.spBUGS_MassUpdate
	...
	while @CurrentPosR <= len(@ID_LIST) begin -- do
		...
		if @TAG_SET_NAME is not null and len(@TAG_SET_NAME) > 0 begin -- then
			if @TAG_SET_ADD = 1 begin -- then
				exec dbo.spTAG_SETS_AddSet       @MODIFIED_USER_ID, @ID, N'Bugs', @TAG_SET_NAME;
			end else begin
...
Grant Execute on dbo.spBUGS_MassUpdate to public;
GO
