if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spCONTRACTS_MassUpdate' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spCONTRACTS_MassUpdate;
GO
...
Create Procedure dbo.spCONTRACTS_MassUpdate
	...
	while @CurrentPosR <= len(@ID_LIST) begin -- do
		...
		if @TAG_SET_NAME is not null and len(@TAG_SET_NAME) > 0 begin -- then
			if @TAG_SET_ADD = 1 begin -- then
				exec dbo.spTAG_SETS_AddSet       @MODIFIED_USER_ID, @ID, N'Contracts', @TAG_SET_NAME;
			end else begin
		...
		-- BEGIN Oracle Exception
			update CONTRACTS
			   set MODIFIED_USER_ID     = @MODIFIED_USER_ID 
			     ...
			     , COMPANY_SIGNED_DATE  = isnull(@COMPANY_SIGNED_DATE , COMPANY_SIGNED_DATE )
			     , CUSTOMER_SIGNED_DATE = isnull(@CUSTOMER_SIGNED_DATE, CUSTOMER_SIGNED_DATE)
			     , TEAM_ID              = isnull(@TEAM_ID             , TEAM_ID             )
...
Grant Execute on dbo.spCONTRACTS_MassUpdate to public;
GO
