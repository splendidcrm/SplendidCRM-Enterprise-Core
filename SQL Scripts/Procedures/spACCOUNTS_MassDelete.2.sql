if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spACCOUNTS_MassDelete' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spACCOUNTS_MassDelete;
GO
 ...
 * 
 *********************************************************************************************************************/
Create Procedure dbo.spACCOUNTS_MassDelete
...
as
  begin
	...
	while @CurrentPosR <= len(@ID_LIST) begin -- do
		...
		exec dbo.spACCOUNTS_Delete @ID, @MODIFIED_USER_ID;
	end -- while;
  end
GO
 
Grant Execute on dbo.spACCOUNTS_MassDelete to public;
GO
