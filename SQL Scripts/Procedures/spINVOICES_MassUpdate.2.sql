if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spINVOICES_MassUpdate' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spINVOICES_MassUpdate;
GO
...
Create Procedure dbo.spINVOICES_MassUpdate
	...
	while @CurrentPosR <= len(@ID_LIST) begin -- do
		...
		if @TAG_SET_NAME is not null and len(@TAG_SET_NAME) > 0 begin -- then
			if @TAG_SET_ADD = 1 begin -- then
				exec dbo.spTAG_SETS_AddSet       @MODIFIED_USER_ID, @ID, N'Invoices', @TAG_SET_NAME;
			end else begin
...
Grant Execute on dbo.spINVOICES_MassUpdate to public;
GO
