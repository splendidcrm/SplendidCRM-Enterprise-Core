if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spMEETINGS_Update' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spMEETINGS_Update;
GO
...
Create Procedure dbo.spMEETINGS_Update
...
as
  begin
	...
	if @REPEAT_TYPE = N'Daily' or (@REPEAT_TYPE = N'Weekly' and (@TEMP_REPEAT_DOW is not null and len(@TEMP_REPEAT_DOW) > 0)) or @REPEAT_TYPE = N'Monthly'  or @REPEAT_TYPE = N'Yearly' begin -- then
		...
		-- 01/20/2015 Paul.  Prevent a repeated record from generating other repeats. 
		if exists(select * from MEETINGS where ID = @ID and REPEAT_PARENT_ID is null and DELETED = 0) begin -- then
			exec dbo.spCALENDAR_Next @DATE_TIME, @REPEAT_TYPE, @TEMP_REPEAT_INTERVAL, @TEMP_REPEAT_DOW, @REPEAT_INDEX, @TEMP_DATE_TIME output, @WEEK_START output, @WEEKDAY output;
			...
			while @WEEKDAY >= 0 and (@TEMP_REPEAT_COUNT = 0 or ((@REPEAT_TYPE = 'Weekly' and @REPEAT_INDEX < @TEMP_REPEAT_COUNT) or (@REPEAT_TYPE <> 'Weekly' and @REPEAT_INDEX < @TEMP_REPEAT_COUNT))) and @REPEAT_INDEX < @MAX_REPEAT_COUNT and (@TEMP_REPEAT_UNTIL is null or @TEMP_DATE_TIME <= @TEMP_REPEAT_UNTIL) begin -- do
				...
				if @REPEAT_ID is null begin -- then
					...
					insert into MEETINGS_CSTM ( ID_C ) values ( @REPEAT_ID );
				end else begin
					update MEETINGS
					   set MODIFIED_USER_ID    = @MODIFIED_USER_ID   
					     , DATE_MODIFIED       =  getdate()          
				...
				if @INVITEE_LIST is not null begin -- then
					exec dbo.spMEETINGS_InviteeMassUpdate @MODIFIED_USER_ID, @REPEAT_ID, @INVITEE_LIST, 1;
				end -- if;
				if dbo.fnIsEmptyGuid(@ASSIGNED_USER_ID) = 0 begin -- then
					if @MODIFIED_USER_ID = @ASSIGNED_USER_ID begin -- then
						if not exists(select * from MEETINGS_USERS where MEETING_ID = @REPEAT_ID and USER_ID = @MODIFIED_USER_ID and ACCEPT_STATUS = N'accept' and DELETED = 0) begin -- then
							exec dbo.spMEETINGS_USERS_Update @MODIFIED_USER_ID, @REPEAT_ID, @ASSIGNED_USER_ID, 1, N'accept';
						end -- if;
					end else begin
						exec dbo.spMEETINGS_USERS_Update @MODIFIED_USER_ID, @REPEAT_ID, @ASSIGNED_USER_ID, 1, null;
					end -- if;
				end -- if;
				...
				-- BEGIN Oracle Exception
					update MEETINGS_USERS
					   set MODIFIED_USER_ID  = @MODIFIED_USER_ID 
					     ...
					     , DATE_MODIFIED_UTC =  getutcdate()     
					     , DELETED           = 1                 
					 where MEETING_ID in (select ID from MEETINGS where REPEAT_PARENT_ID = @ID and DATE_MODIFIED_UTC < @REPEAT_UTC and DATE_START > getdate() and DELETED = 0);
				-- END Oracle Exception
	...
	if @@ERROR = 0 begin -- then
		exec dbo.spTAG_SETS_NormalizeSet @MODIFIED_USER_ID, @ID, N'Meetings', @TAG_SET_NAME;
	end -- if;
  end
GO

Grant Execute on dbo.spMEETINGS_Update to public;
GO
