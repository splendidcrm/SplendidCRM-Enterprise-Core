if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spORDERS_MassDelete' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spORDERS_MassDelete;
GO
 ...
 * 
 *********************************************************************************************************************/
Create Procedure dbo.spORDERS_MassDelete
...
as
  begin
	...
	while @CurrentPosR <= len(@ID_LIST) begin -- do
		...
		exec dbo.spORDERS_Delete @ID, @MODIFIED_USER_ID;
	end -- while;
  end
GO
 
Grant Execute on dbo.spORDERS_MassDelete to public;
GO
