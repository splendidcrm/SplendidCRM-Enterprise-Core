if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spDOCUMENTS_CONTENT_ReadOffset' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spDOCUMENTS_CONTENT_ReadOffset;
GO
...
Create Procedure dbo.spDOCUMENTS_CONTENT_ReadOffset
	( @ID                   uniqueidentifier
	, @FILE_OFFSET          int
	, @READ_SIZE            int
...
-- #if SQL_Server /*
	...
	-- 	readtext DOCUMENT_REVISIONS.CONTENT @FILE_POINTER @FILE_OFFSET @READ_SIZE;
	-- end -- if;
-- #endif SQL_Server */
...
Grant Execute on dbo.spDOCUMENTS_CONTENT_ReadOffset to public;
GO
