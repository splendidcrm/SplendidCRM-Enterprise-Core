if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spTEAM_NOTICES_MassDelete' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spTEAM_NOTICES_MassDelete;
GO
 ...
 * 
 *********************************************************************************************************************/
Create Procedure dbo.spTEAM_NOTICES_MassDelete
...
as
  begin
	...
	while @CurrentPosR <= len(@ID_LIST) begin -- do
		...
		exec dbo.spTEAM_NOTICES_Delete @ID, @MODIFIED_USER_ID;
	end -- while;
  end
GO
 
Grant Execute on dbo.spTEAM_NOTICES_MassDelete to public;
GO
