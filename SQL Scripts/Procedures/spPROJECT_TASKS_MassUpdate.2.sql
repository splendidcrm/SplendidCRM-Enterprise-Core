if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spPROJECT_TASKS_MassUpdate' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spPROJECT_TASKS_MassUpdate;
GO
...
Create Procedure dbo.spPROJECT_TASKS_MassUpdate
	...
	while @CurrentPosR <= len(@ID_LIST) begin -- do
		...
		if @ASSIGNED_SET_ADD = 1 and @ASSIGNED_SET_ID is not null begin -- then
			-- BEGIN Oracle Exception
				...
				-- 02/01/2019 Paul.  PROJECT_TASKS is not the correct table name. 
		...
		-- 02/06/2013 Paul.  TEAM_SET_ID was not being updated until now. 
		-- 05/31/2018 Paul.  Add ASSIGNED_SET_ID for Dynamic User Assignment. 
		-- BEGIN Oracle Exception
			update PROJECT_TASK
			   set MODIFIED_USER_ID  = @MODIFIED_USER_ID 
			     ...
			     , DATE_DUE          = isnull(@DATE_DUE        , DATE_DUE        )
			     , TIME_DUE          = isnull(@TIME_DUE        , TIME_DUE        )
...
Grant Execute on dbo.spPROJECT_TASKS_MassUpdate to public;
GO
