if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spQUOTES_UpdateTotals' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spQUOTES_UpdateTotals;
GO
...
-- 01/22/2013 Paul.  Change procedure to perform a single QUOTES update. 
...
Create Procedure dbo.spQUOTES_UpdateTotals
	( @ID               uniqueidentifier
...
as
  begin
	...
	if exists(select * from vwQUOTES where ID = @ID) begin -- then
		select @TAX_RATE = min(VALUE) / 100
		  from      QUOTES
		 inner join vwTAX_RATES
		         on vwTAX_RATES.ID = QUOTES.TAXRATE_ID
		 where QUOTES.ID = @ID;
		...
		if @TAX_LINE_ITEMS = 1 begin -- then
			select @SUBTOTAL      = (select sum(QUANTITY * UNIT_PRICE    ) from vwQUOTES_LINE_ITEMS where QUOTE_ID = QUOTES.ID and (NAME is not null or PRODUCT_TEMPLATE_ID is not null) and (LINE_ITEM_TYPE is null or LINE_ITEM_TYPE not in (N'Comment', N'Subtotal')))
			     , @DISCOUNT      = (select sum(           DISCOUNT_PRICE) from vwQUOTES_LINE_ITEMS where QUOTE_ID = QUOTES.ID and (NAME is not null or PRODUCT_TEMPLATE_ID is not null) and (LINE_ITEM_TYPE is null or LINE_ITEM_TYPE not in (N'Comment', N'Subtotal')) and DISCOUNT_PRICE is not null)
			     , @TAX           = (select sum(           TAX           ) from vwQUOTES_LINE_ITEMS where QUOTE_ID = QUOTES.ID and (NAME is not null or PRODUCT_TEMPLATE_ID is not null) and (LINE_ITEM_TYPE is null or LINE_ITEM_TYPE not in (N'Comment', N'Subtotal')) and TAXRATE_ID is not null)
			...
			select @SUBTOTAL      = (select sum( QUANTITY * UNIT_PRICE                                           ) from vwQUOTES_LINE_ITEMS where QUOTE_ID = QUOTES.ID and (NAME is not null or PRODUCT_TEMPLATE_ID is not null) and (LINE_ITEM_TYPE is null or LINE_ITEM_TYPE not in (N'Comment', N'Subtotal')))
			     , @DISCOUNT      = (select sum(                         isnull(DISCOUNT_PRICE, 0.0)             ) from vwQUOTES_LINE_ITEMS where QUOTE_ID = QUOTES.ID and (NAME is not null or PRODUCT_TEMPLATE_ID is not null) and (LINE_ITEM_TYPE is null or LINE_ITEM_TYPE not in (N'Comment', N'Subtotal')) and DISCOUNT_PRICE is not null)
			     , @TAX           = (select sum((QUANTITY * UNIT_PRICE - isnull(DISCOUNT_PRICE, 0.0)) * @TAX_RATE) from vwQUOTES_LINE_ITEMS where QUOTE_ID = QUOTES.ID and (NAME is not null or PRODUCT_TEMPLATE_ID is not null) and (LINE_ITEM_TYPE is null or LINE_ITEM_TYPE not in (N'Comment', N'Subtotal')) and TAX_CLASS = 'Taxable')
			     , @EXCHANGE_RATE = isnull(nullif(EXCHANGE_RATE, 0.0), 1.0)
			     , @SHIPPING      = isnull(SHIPPING, 0.0)
			  from QUOTES
			 where ID = @ID;
...
Grant Execute on dbo.spQUOTES_UpdateTotals to public;
GO
