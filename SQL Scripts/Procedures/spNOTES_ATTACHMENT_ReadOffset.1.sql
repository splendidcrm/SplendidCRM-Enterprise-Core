if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spNOTES_ATTACHMENT_ReadOffset' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spNOTES_ATTACHMENT_ReadOffset;
GO
...
Create Procedure dbo.spNOTES_ATTACHMENT_ReadOffset
	( @ID                   uniqueidentifier
	, @FILE_OFFSET          int
	, @READ_SIZE            int
...
-- #if SQL_Server /*
	...
	-- select @FILE_SIZE    = datalength(ATTACHMENT)
	--      , @FILE_POINTER = textptr(ATTACHMENT)
	...
	-- 	readtext NOTE_ATTACHMENTS.ATTACHMENT @FILE_POINTER @FILE_OFFSET @READ_SIZE;
	-- end -- if;
-- #endif SQL_Server */
...
Grant Execute on dbo.spNOTES_ATTACHMENT_ReadOffset to public;
GO
