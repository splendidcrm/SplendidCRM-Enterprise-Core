if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spPAYMENTS_MassUpdate' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spPAYMENTS_MassUpdate;
GO
...
Create Procedure dbo.spPAYMENTS_MassUpdate
...
as
  begin
	...
	while @CurrentPosR <= len(@ID_LIST) begin -- do
		...
		-- 09/11/2007 Paul.  SugarCRM 4.2 only updates the date, not the time. 
		-- 05/31/2018 Paul.  Add ASSIGNED_SET_ID for Dynamic User Assignment. 
		-- BEGIN Oracle Exception
			update PAYMENTS
			   set MODIFIED_USER_ID  = @MODIFIED_USER_ID 
			     ...
			     , PAYMENT_DATE      = isnull(dbo.fnDateAdd_Time(PAYMENT_DATE, @PAYMENT_DATE), PAYMENT_DATE)
			     , ASSIGNED_USER_ID  = isnull(@ASSIGNED_USER_ID, ASSIGNED_USER_ID)
...
Grant Execute on dbo.spPAYMENTS_MassUpdate to public;
GO
