if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spCASES_MassDelete' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spCASES_MassDelete;
GO
 ...
 * 
 *********************************************************************************************************************/
Create Procedure dbo.spCASES_MassDelete
...
as
  begin
	...
	while @CurrentPosR <= len(@ID_LIST) begin -- do
		...
		exec dbo.spCASES_Delete @ID, @MODIFIED_USER_ID;
	end -- while;
  end
GO
 
Grant Execute on dbo.spCASES_MassDelete to public;
GO
