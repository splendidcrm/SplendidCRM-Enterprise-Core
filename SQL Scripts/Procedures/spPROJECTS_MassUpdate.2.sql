if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spPROJECTS_MassUpdate' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spPROJECTS_MassUpdate;
GO
...
Create Procedure dbo.spPROJECTS_MassUpdate
	...
	while @CurrentPosR <= len(@ID_LIST) begin -- do
		...
		if @TAG_SET_NAME is not null and len(@TAG_SET_NAME) > 0 begin -- then
			if @TAG_SET_ADD = 1 begin -- then
				exec dbo.spTAG_SETS_AddSet       @MODIFIED_USER_ID, @ID, N'Project', @TAG_SET_NAME;
			end else begin
		...
		-- BEGIN Oracle Exception
			update PROJECT
			   set MODIFIED_USER_ID     = @MODIFIED_USER_ID 
			     ...
			     , ESTIMATED_START_DATE = isnull(@ESTIMATED_START_DATE, ESTIMATED_START_DATE)
			     , ESTIMATED_END_DATE   = isnull(@ESTIMATED_END_DATE  , ESTIMATED_END_DATE  )
...
Grant Execute on dbo.spPROJECTS_MassUpdate to public;
GO
