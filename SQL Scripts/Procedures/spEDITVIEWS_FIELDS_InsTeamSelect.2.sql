if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spEDITVIEWS_FIELDS_InsTeamSelect' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spEDITVIEWS_FIELDS_InsTeamSelect;
GO
...
-- 01/08/2018 Paul.  FIELD_TYPE needed to be TeamSelect.  MODULE_TYPE is Teams. 
-- 01/15/2018 Paul.  Include @FORMAT_TAB_INDEX field. 
Create Procedure dbo.spEDITVIEWS_FIELDS_InsTeamSelect
...
as
  begin
	...
	set @DATA_LABEL  = N'.LBL_TEAM_SET_NAME';
	set @DATA_FIELD  = N'TEAM_SET_NAME';
	set @MODULE_TYPE = N'Teams';
...
Grant Execute on dbo.spEDITVIEWS_FIELDS_InsTeamSelect to public;
GO
