if exists (select * from INFORMATION_SCHEMA.VIEWS where TABLE_NAME = 'vwORDERS_ACTIVITIES')
	Drop View dbo.vwORDERS_ACTIVITIES;
GO
...
Create View dbo.vwORDERS_ACTIVITIES
as
select TASKS.ID
  ...
  from            ORDERS
       inner join TASKS
               on TASKS.PARENT_ID              = ORDERS.ID
              and TASKS.PARENT_TYPE            = N'Orders'
              and TASKS.DELETED                = 0
...
select MEETINGS.ID
  ...
  from            ORDERS
       inner join MEETINGS
               on MEETINGS.PARENT_ID           = ORDERS.ID
              and MEETINGS.PARENT_TYPE         = N'Orders'
...
select CALLS.ID
  ...
  from            ORDERS
       inner join CALLS
               on CALLS.PARENT_ID              = ORDERS.ID
              and CALLS.PARENT_TYPE            = N'Orders'
              and CALLS.DELETED                = 0
  left outer join ORDERS_ACCOUNTS                ORDERS_ACCOUNTS
               on ORDERS_ACCOUNTS.ORDER_ID     = ORDERS.ID
              and ORDERS_ACCOUNTS.ACCOUNT_ROLE = N'Bill To'
...
select EMAILS.ID
  ...
  from            ORDERS
       inner join EMAILS
               on EMAILS.PARENT_ID             = ORDERS.ID
              and EMAILS.PARENT_TYPE           = N'Orders'
              and EMAILS.DELETED               = 0
  left outer join EMAILS_ORDERS
               on EMAILS_ORDERS.ORDER_ID       = ORDERS.ID
              and EMAILS_ORDERS.EMAIL_ID       = EMAILS.ID
...
select EMAILS.ID
  ...
  from            ORDERS
       inner join EMAILS_ORDERS
              ...
              and EMAILS_ORDERS.DELETED        = 0
       inner join EMAILS
               on EMAILS.ID                    = EMAILS_ORDERS.EMAIL_ID
              and EMAILS.DELETED               = 0
...
select NOTES.ID
     ...
     , ORDERS.ID                      as ORDER_ID
     , ORDERS.NAME                    as ORDER_NAME
     , ACCOUNTS.ASSIGNED_USER_ID      as ORDER_ASSIGNED_USER_ID
     , ORDERS.ASSIGNED_SET_ID         as ORDER_ASSIGNED_SET_ID
     , CONTACTS.ID                    as CONTACT_ID
     , CONTACTS.ASSIGNED_USER_ID      as CONTACT_ASSIGNED_USER_ID
     , CONTACTS.ASSIGNED_SET_ID       as CONTACT_ASSIGNED_SET_ID
  ...
  from            ORDERS
       inner join NOTES
               on NOTES.PARENT_ID              = ORDERS.ID
              and NOTES.PARENT_TYPE            = N'Orders'
              and NOTES.DELETED                = 0
  left outer join ORDERS_ACCOUNTS                ORDERS_ACCOUNTS
               on ORDERS_ACCOUNTS.ORDER_ID     = ORDERS.ID
              and ORDERS_ACCOUNTS.ACCOUNT_ROLE = N'Bill To'
...
select SMS_MESSAGES.ID
  ...
  from            ORDERS
       inner join SMS_MESSAGES
               on SMS_MESSAGES.PARENT_ID       = ORDERS.ID
              and SMS_MESSAGES.PARENT_TYPE     = N'Orders'
...
select TWITTER_MESSAGES.ID
  ...
  from            ORDERS
       inner join TWITTER_MESSAGES
               on TWITTER_MESSAGES.PARENT_ID   = ORDERS.ID
              and TWITTER_MESSAGES.PARENT_TYPE = N'Orders'
...
select CHAT_MESSAGES.ID
  ...
  from            ORDERS
       inner join CHAT_MESSAGES
               on CHAT_MESSAGES.PARENT_ID      = ORDERS.ID
              and CHAT_MESSAGES.PARENT_TYPE    = N'Orders'
...
select CHAT_MESSAGES.ID
  ...
  from            ORDERS
       inner join CHAT_CHANNELS
               on CHAT_CHANNELS.PARENT_ID      = ORDERS.ID
              and CHAT_CHANNELS.PARENT_TYPE    = N'Orders'
              and CHAT_CHANNELS.DELETED        = 0
       inner join CHAT_MESSAGES
...
Grant Select on dbo.vwORDERS_ACTIVITIES to public;
GO
