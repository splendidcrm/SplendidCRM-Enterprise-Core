if exists (select * from INFORMATION_SCHEMA.VIEWS where TABLE_NAME = 'vwCONTRACTS_ACTIVITIES')
	Drop View dbo.vwCONTRACTS_ACTIVITIES;
GO
...
Create View dbo.vwCONTRACTS_ACTIVITIES
as
select TASKS.ID
  ...
  from            CONTRACTS
       inner join TASKS
               on TASKS.PARENT_ID          = CONTRACTS.ID
              and TASKS.PARENT_TYPE        = N'Contracts'
              and TASKS.DELETED            = 0
...
select MEETINGS.ID
  ...
  from            CONTRACTS
       inner join MEETINGS
               on MEETINGS.PARENT_ID           = CONTRACTS.ID
              and MEETINGS.PARENT_TYPE         = N'Contracts'
...
select CALLS.ID
  ...
  from            CONTRACTS
       inner join CALLS
               on CALLS.PARENT_ID          = CONTRACTS.ID
              and CALLS.PARENT_TYPE        = N'Contracts'
              and CALLS.DELETED            = 0
  left outer join TEAMS
...
select EMAILS.ID
  ...
  from            CONTRACTS
       inner join EMAILS
               on EMAILS.PARENT_ID              = CONTRACTS.ID
              and EMAILS.PARENT_TYPE            = N'Contracts'
              and EMAILS.DELETED                = 0
  left outer join EMAILS_CONTRACTS
               on EMAILS_CONTRACTS.CONTRACT_ID  = CONTRACTS.ID
              and EMAILS_CONTRACTS.EMAIL_ID     = EMAILS.ID
              and EMAILS_CONTRACTS.DELETED      = 0
  left outer join TEAMS
  ...
  left outer join ASSIGNED_SETS
              ...
              and ASSIGNED_SETS.DELETED         = 0
 where CONTRACTS.DELETED = 0
   and EMAILS_CONTRACTS.ID is null
union all
select EMAILS.ID
     , EMAILS.ID                      as ACTIVITY_ID
     , N'Emails'                      as ACTIVITY_TYPE
     ...
     , ASSIGNED_SETS.ASSIGNED_SET_LIST as ASSIGNED_SET_LIST
  from            CONTRACTS
       inner join EMAILS_CONTRACTS
               on EMAILS_CONTRACTS.CONTRACT_ID  = CONTRACTS.ID
              and EMAILS_CONTRACTS.DELETED      = 0
       inner join EMAILS
               on EMAILS.ID                     = EMAILS_CONTRACTS.EMAIL_ID
              and EMAILS.DELETED                = 0
  left outer join TEAMS
               on TEAMS.ID                      = EMAILS.TEAM_ID
              and TEAMS.DELETED                 = 0
...
select NOTES.ID
  ...
  from            CONTRACTS
       inner join NOTES
               on NOTES.PARENT_ID          = CONTRACTS.ID
              and NOTES.PARENT_TYPE        = N'Contracts'
              and NOTES.DELETED            = 0
  left outer join TEAMS
...
select SMS_MESSAGES.ID
  ...
  from            CONTRACTS
       inner join SMS_MESSAGES
               on SMS_MESSAGES.PARENT_ID        = CONTRACTS.ID
              and SMS_MESSAGES.PARENT_TYPE      = N'Contracts'
...
select TWITTER_MESSAGES.ID
  ...
  from            CONTRACTS
       inner join TWITTER_MESSAGES
               on TWITTER_MESSAGES.PARENT_ID   = CONTRACTS.ID
              and TWITTER_MESSAGES.PARENT_TYPE = N'Contracts'
...
select CHAT_MESSAGES.ID
  ...
  from            CONTRACTS
       inner join CHAT_MESSAGES
               on CHAT_MESSAGES.PARENT_ID      = CONTRACTS.ID
              and CHAT_MESSAGES.PARENT_TYPE    = N'Contracts'
...
select CHAT_MESSAGES.ID
  ...
  from            CONTRACTS
       inner join CHAT_CHANNELS
               on CHAT_CHANNELS.PARENT_ID      = CONTRACTS.ID
              and CHAT_CHANNELS.PARENT_TYPE    = N'Contracts'
              and CHAT_CHANNELS.DELETED        = 0
       inner join CHAT_MESSAGES
...
Grant Select on dbo.vwCONTRACTS_ACTIVITIES to public;
GO
