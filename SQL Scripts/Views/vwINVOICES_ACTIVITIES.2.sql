if exists (select * from INFORMATION_SCHEMA.VIEWS where TABLE_NAME = 'vwINVOICES_ACTIVITIES')
	Drop View dbo.vwINVOICES_ACTIVITIES;
GO
...
Create View dbo.vwINVOICES_ACTIVITIES
as
select TASKS.ID
     ...
     , INVOICES_ACCOUNTS.ACCOUNT_ID   as ACCOUNT_ID
  ...
  from            INVOICES
       inner join TASKS
               on TASKS.PARENT_ID                = INVOICES.ID
              and TASKS.PARENT_TYPE              = N'Invoices'
              and TASKS.DELETED                  = 0
...
select MEETINGS.ID
  ...
  from            INVOICES
       inner join MEETINGS
               on MEETINGS.PARENT_ID              = INVOICES.ID
              and MEETINGS.PARENT_TYPE            = N'Invoices'
...
select CALLS.ID
  ...
  from            INVOICES
       inner join CALLS
               on CALLS.PARENT_ID                = INVOICES.ID
              and CALLS.PARENT_TYPE              = N'Invoices'
              and CALLS.DELETED                  = 0
  left outer join INVOICES_ACCOUNTS
               on INVOICES_ACCOUNTS.INVOICE_ID   = INVOICES.ID
              and INVOICES_ACCOUNTS.ACCOUNT_ROLE = N'Bill To'
...
select EMAILS.ID
  ...
  from            INVOICES
       inner join EMAILS
               on EMAILS.PARENT_ID               = INVOICES.ID
              and EMAILS.PARENT_TYPE             = N'Invoices'
              and EMAILS.DELETED                 = 0
  left outer join EMAILS_INVOICES
               on EMAILS_INVOICES.INVOICE_ID     = INVOICES.ID
              and EMAILS_INVOICES.EMAIL_ID       = EMAILS.ID
...
select EMAILS.ID
  ...
  from            INVOICES
       inner join EMAILS_INVOICES
              ...
              and EMAILS_INVOICES.DELETED        = 0
       inner join EMAILS
               on EMAILS.ID                      = EMAILS_INVOICES.EMAIL_ID
              and EMAILS.DELETED                 = 0
  left outer join INVOICES_ACCOUNTS
               on INVOICES_ACCOUNTS.INVOICE_ID   = INVOICES.ID
              and INVOICES_ACCOUNTS.ACCOUNT_ROLE = N'Bill To'
...
select NOTES.ID
     ...
     , INVOICES.ID                    as INVOICE_ID
     , INVOICES.NAME                  as INVOICE_NAME
     , ACCOUNTS.ASSIGNED_USER_ID      as INVOICE_ASSIGNED_USER_ID
     , INVOICES.ASSIGNED_SET_ID       as INVOICE_ASSIGNED_SET_ID
     , CONTACTS.ID                    as CONTACT_ID
     , CONTACTS.ASSIGNED_USER_ID      as CONTACT_ASSIGNED_USER_ID
     , CONTACTS.ASSIGNED_SET_ID       as CONTACT_ASSIGNED_SET_ID
  ...
  from            INVOICES
       inner join NOTES
               on NOTES.PARENT_ID                = INVOICES.ID
              and NOTES.PARENT_TYPE              = N'Invoices'
              and NOTES.DELETED                  = 0
  left outer join INVOICES_ACCOUNTS
               on INVOICES_ACCOUNTS.INVOICE_ID   = INVOICES.ID
              and INVOICES_ACCOUNTS.ACCOUNT_ROLE = N'Bill To'
...
select SMS_MESSAGES.ID
  ...
  from            INVOICES
       inner join SMS_MESSAGES
               on SMS_MESSAGES.PARENT_ID         = INVOICES.ID
              and SMS_MESSAGES.PARENT_TYPE       = N'Invoices'
...
select TWITTER_MESSAGES.ID
  ...
  from            INVOICES
       inner join TWITTER_MESSAGES
               on TWITTER_MESSAGES.PARENT_ID     = INVOICES.ID
              and TWITTER_MESSAGES.PARENT_TYPE   = N'Invoices'
...
select CHAT_MESSAGES.ID
  ...
  from            INVOICES
       inner join CHAT_MESSAGES
               on CHAT_MESSAGES.PARENT_ID        = INVOICES.ID
              and CHAT_MESSAGES.PARENT_TYPE      = N'Invoices'
...
select CHAT_MESSAGES.ID
  ...
  from            INVOICES
       inner join CHAT_CHANNELS
               on CHAT_CHANNELS.PARENT_ID        = INVOICES.ID
              and CHAT_CHANNELS.PARENT_TYPE      = N'Invoices'
              and CHAT_CHANNELS.DELETED          = 0
       inner join CHAT_MESSAGES
...
Grant Select on dbo.vwINVOICES_ACTIVITIES to public;
GO
