if exists (select * from INFORMATION_SCHEMA.VIEWS where TABLE_NAME = 'vwCONTACTS_CALLS_Soap')
	Drop View dbo.vwCONTACTS_CALLS_Soap;
GO
...
Create View dbo.vwCONTACTS_CALLS_Soap
as
select CALLS_CONTACTS.CONTACT_ID as PRIMARY_ID
     , CALLS_CONTACTS.CALL_ID    as RELATED_ID
...
select CONTACTS.ID               as PRIMARY_ID
 ...
 inner join CALLS
        ...
        and CALLS.DELETED     = CONTACTS.DELETED
        and CALLS.PARENT_TYPE = N'Contacts'

GO

Grant Select on dbo.vwCONTACTS_CALLS_Soap to public;
GO
