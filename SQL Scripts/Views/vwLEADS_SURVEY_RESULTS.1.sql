if exists (select * from INFORMATION_SCHEMA.VIEWS where TABLE_NAME = 'vwLEADS_SURVEY_RESULTS')
	Drop View dbo.vwLEADS_SURVEY_RESULTS;
GO
...
Create View dbo.vwLEADS_SURVEY_RESULTS
as
select SURVEY_RESULTS.ID
  ...
  from            SURVEY_RESULTS
       ...
       inner join LEADS
               on LEADS.ID         = SURVEY_RESULTS.PARENT_ID
              and LEADS.DELETED    = 0
 where SURVEY_RESULTS.DELETED = 0

GO

Grant Select on dbo.vwLEADS_SURVEY_RESULTS to public;
GO
