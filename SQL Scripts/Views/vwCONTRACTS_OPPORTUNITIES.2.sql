if exists (select * from INFORMATION_SCHEMA.VIEWS where TABLE_NAME = 'vwCONTRACTS_OPPORTUNITIES')
	Drop View dbo.vwCONTRACTS_OPPORTUNITIES;
GO
...
Create View dbo.vwCONTRACTS_OPPORTUNITIES
as
select CONTRACTS.ID                 as CONTRACT_ID
  ...
  from           CONTRACTS
      ...
      inner join vwOPPORTUNITIES
              on vwOPPORTUNITIES.ID                  = CONTRACTS_OPPORTUNITIES.OPPORTUNITY_ID
 where CONTRACTS.DELETED = 0

GO

Grant Select on dbo.vwCONTRACTS_OPPORTUNITIES to public;
GO
