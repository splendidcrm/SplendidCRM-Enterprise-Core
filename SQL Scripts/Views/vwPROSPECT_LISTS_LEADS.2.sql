if exists (select * from INFORMATION_SCHEMA.VIEWS where TABLE_NAME = 'vwPROSPECT_LISTS_LEADS')
	Drop View dbo.vwPROSPECT_LISTS_LEADS;
GO
...
Create View dbo.vwPROSPECT_LISTS_LEADS
as
select PROSPECT_LISTS.ID               as PROSPECT_LIST_ID
  ...
  from            PROSPECT_LISTS
       ...
       inner join vwLEADS
               on vwLEADS.ID                                = PROSPECT_LISTS_PROSPECTS.RELATED_ID
 where PROSPECT_LISTS.DELETED = 0

GO

Grant Select on dbo.vwPROSPECT_LISTS_LEADS to public;
GO
