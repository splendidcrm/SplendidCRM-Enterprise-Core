if exists (select * from INFORMATION_SCHEMA.VIEWS where TABLE_NAME = 'vwACCOUNTS_ACTIVITIES')
	Drop View dbo.vwACCOUNTS_ACTIVITIES;
GO
...
-- 02/13/2009 Paul.  Notes should assume the ownership of the parent record (i.e. the account record). 
...
-- 01/11/2018 Paul.  Remove EMAILS_ACCOUNTS as it is already in the vwEMAILS_RELATED_ACCOUNTS view. 
Create View dbo.vwACCOUNTS_ACTIVITIES
as
select TASKS.ID
  ...
  from            ACCOUNTS
       inner join vwTASKS_RELATED_ACCOUNTS
               on vwTASKS_RELATED_ACCOUNTS.ACCOUNT_ID = ACCOUNTS.ID
       inner join TASKS
               on TASKS.ID                 = vwTASKS_RELATED_ACCOUNTS.ID
...
select MEETINGS.ID
     ...
     , ASSIGNED_SETS.ASSIGNED_SET_LIST as ASSIGNED_SET_LIST
  from            ACCOUNTS
       inner join vwMEETINGS_RELATED_ACCOUNTS
               on vwMEETINGS_RELATED_ACCOUNTS.ACCOUNT_ID = ACCOUNTS.ID
       inner join MEETINGS
               on MEETINGS.ID                  = vwMEETINGS_RELATED_ACCOUNTS.ID
  left outer join TEAMS
...
select CALLS.ID
     ...
     , ASSIGNED_SETS.ASSIGNED_SET_LIST as ASSIGNED_SET_LIST
  from            ACCOUNTS
       inner join vwCALLS_RELATED_ACCOUNTS
               on vwCALLS_RELATED_ACCOUNTS.ACCOUNT_ID = ACCOUNTS.ID
       inner join CALLS
               on CALLS.ID                 = vwCALLS_RELATED_ACCOUNTS.ID
  left outer join TEAMS
...
select EMAILS.ID
     ...
     , ASSIGNED_SETS.ASSIGNED_SET_LIST as ASSIGNED_SET_LIST
  from            ACCOUNTS
       inner join vwEMAILS_RELATED_ACCOUNTS
               on vwEMAILS_RELATED_ACCOUNTS.ACCOUNT_ID = ACCOUNTS.ID
       inner join EMAILS
               on EMAILS.ID                  = vwEMAILS_RELATED_ACCOUNTS.ID
  left outer join TEAMS
...
select NOTES.ID
     ...
     , ASSIGNED_SETS.ASSIGNED_SET_LIST as ASSIGNED_SET_LIST
  from            ACCOUNTS
       inner join vwNOTES_RELATED_ACCOUNTS
               on vwNOTES_RELATED_ACCOUNTS.ACCOUNT_ID = ACCOUNTS.ID
       inner join NOTES
               on NOTES.ID                 = vwNOTES_RELATED_ACCOUNTS.ID
  left outer join TEAMS
...
select SMS_MESSAGES.ID
     ...
     , ASSIGNED_SETS.ASSIGNED_SET_LIST as ASSIGNED_SET_LIST
  from            ACCOUNTS
       inner join vwSMS_MESSAGES_RELATED_ACCOUNTS
               on vwSMS_MESSAGES_RELATED_ACCOUNTS.ACCOUNT_ID = ACCOUNTS.ID
       inner join SMS_MESSAGES
               on SMS_MESSAGES.ID            = vwSMS_MESSAGES_RELATED_ACCOUNTS.ID
  left outer join TEAMS
...
select TWITTER_MESSAGES.ID
     ...
     , ASSIGNED_SETS.ASSIGNED_SET_LIST as ASSIGNED_SET_LIST
  from            ACCOUNTS
       inner join vwTWITTER_MESSAGES_RELATED_ACCOUNTS
               on vwTWITTER_MESSAGES_RELATED_ACCOUNTS.ACCOUNT_ID = ACCOUNTS.ID
       inner join TWITTER_MESSAGES
               on TWITTER_MESSAGES.ID          = vwTWITTER_MESSAGES_RELATED_ACCOUNTS.ID
  left outer join TEAMS
...
select CHAT_MESSAGES.ID
     ...
     , ASSIGNED_SETS.ASSIGNED_SET_LIST as ASSIGNED_SET_LIST
  from            ACCOUNTS
       inner join vwCHAT_MESSAGES_RELATED_ACCOUNTS
               on vwCHAT_MESSAGES_RELATED_ACCOUNTS.ACCOUNT_ID = ACCOUNTS.ID
       inner join CHAT_MESSAGES
               on CHAT_MESSAGES.ID             = vwCHAT_MESSAGES_RELATED_ACCOUNTS.ID
...
Grant Select on dbo.vwACCOUNTS_ACTIVITIES to public;
GO
