if exists (select * from INFORMATION_SCHEMA.VIEWS where TABLE_NAME = 'vwQUOTES_ACTIVITIES')
	Drop View dbo.vwQUOTES_ACTIVITIES;
GO
...
-- 05/08/2008 Paul.  Add related accounts as they are used in the subpanel data.
...
Create View dbo.vwQUOTES_ACTIVITIES
as
select TASKS.ID
     ...
     , QUOTES_ACCOUNTS.ACCOUNT_ID     as ACCOUNT_ID
  ...
  from            QUOTES
       inner join TASKS
               on TASKS.PARENT_ID              = QUOTES.ID
              and TASKS.PARENT_TYPE            = N'Quotes'
              and TASKS.DELETED                = 0
  left outer join QUOTES_ACCOUNTS
...
select MEETINGS.ID
  ...
  from            QUOTES
       inner join MEETINGS
               on MEETINGS.PARENT_ID           = QUOTES.ID
              and MEETINGS.PARENT_TYPE         = N'Quotes'
...
select CALLS.ID
  ...
  from            QUOTES
       inner join CALLS
               on CALLS.PARENT_ID              = QUOTES.ID
              and CALLS.PARENT_TYPE            = N'Quotes'
              and CALLS.DELETED                = 0
  left outer join QUOTES_ACCOUNTS
               on QUOTES_ACCOUNTS.QUOTE_ID     = QUOTES.ID
              and QUOTES_ACCOUNTS.ACCOUNT_ROLE = N'Bill To'
...
select EMAILS.ID
  ...
  from            QUOTES
       inner join EMAILS
               on EMAILS.PARENT_ID              = QUOTES.ID
              and EMAILS.PARENT_TYPE            = N'Quotes'
              and EMAILS.DELETED                = 0
  left outer join EMAILS_QUOTES
               on EMAILS_QUOTES.QUOTE_ID        = QUOTES.ID
              and EMAILS_QUOTES.EMAIL_ID        = EMAILS.ID
              and EMAILS_QUOTES.DELETED         = 0
  left outer join QUOTES_ACCOUNTS
               on QUOTES_ACCOUNTS.QUOTE_ID      = QUOTES.ID
              and QUOTES_ACCOUNTS.ACCOUNT_ROLE  = N'Bill To'
              and QUOTES_ACCOUNTS.DELETED       = 0
...
select EMAILS.ID
     ...
     , ASSIGNED_SETS.ASSIGNED_SET_LIST as ASSIGNED_SET_LIST
  from            QUOTES
       inner join EMAILS_QUOTES
               on EMAILS_QUOTES.QUOTE_ID       = QUOTES.ID
              and EMAILS_QUOTES.DELETED        = 0
       inner join EMAILS
               on EMAILS.ID                    = EMAILS_QUOTES.EMAIL_ID
              and EMAILS.DELETED               = 0
  left outer join QUOTES_ACCOUNTS
               on QUOTES_ACCOUNTS.QUOTE_ID     = QUOTES.ID
              and QUOTES_ACCOUNTS.ACCOUNT_ROLE = N'Bill To'
...
select NOTES.ID
     ...
     , QUOTES.ID                      as QUOTE_ID
     , QUOTES.NAME                    as QUOTE_NAME
     , ACCOUNTS.ASSIGNED_USER_ID      as QUOTE_ASSIGNED_USER_ID
     , QUOTES.ASSIGNED_SET_ID         as QUOTE_ASSIGNED_SET_ID
     , CONTACTS.ID                    as CONTACT_ID
     , CONTACTS.ASSIGNED_USER_ID      as CONTACT_ASSIGNED_USER_ID
     , CONTACTS.ASSIGNED_SET_ID       as CONTACT_ASSIGNED_SET_ID
  ...
  from            QUOTES
       inner join NOTES
               on NOTES.PARENT_ID              = QUOTES.ID
              and NOTES.PARENT_TYPE            = N'Quotes'
              and NOTES.DELETED                = 0
  left outer join QUOTES_ACCOUNTS
               on QUOTES_ACCOUNTS.QUOTE_ID     = QUOTES.ID
              and QUOTES_ACCOUNTS.ACCOUNT_ROLE = N'Bill To'
...
select SMS_MESSAGES.ID
  ...
  from            QUOTES
       inner join SMS_MESSAGES
               on SMS_MESSAGES.PARENT_ID        = QUOTES.ID
              and SMS_MESSAGES.PARENT_TYPE      = N'Quotes'
...
select TWITTER_MESSAGES.ID
  ...
  from            QUOTES
       inner join TWITTER_MESSAGES
               on TWITTER_MESSAGES.PARENT_ID    = QUOTES.ID
              and TWITTER_MESSAGES.PARENT_TYPE  = N'Quotes'
...
select CHAT_MESSAGES.ID
  ...
  from            QUOTES
       inner join CHAT_MESSAGES
               on CHAT_MESSAGES.PARENT_ID      = QUOTES.ID
              and CHAT_MESSAGES.PARENT_TYPE    = N'Quotes'
...
select CHAT_MESSAGES.ID
  ...
  from            QUOTES
       inner join CHAT_CHANNELS
               on CHAT_CHANNELS.PARENT_ID       = QUOTES.ID
              and CHAT_CHANNELS.PARENT_TYPE     = N'Quotes'
              and CHAT_CHANNELS.DELETED         = 0
       inner join CHAT_MESSAGES
...
Grant Select on dbo.vwQUOTES_ACTIVITIES to public;
GO
