if exists (select * from INFORMATION_SCHEMA.VIEWS where TABLE_NAME = 'vwBUGS_ACTIVITIES')
	Drop View dbo.vwBUGS_ACTIVITIES;
GO
...
Create View dbo.vwBUGS_ACTIVITIES
as
select TASKS.ID
  ...
  from            BUGS
       inner join TASKS
               on TASKS.PARENT_ID          = BUGS.ID
              and TASKS.PARENT_TYPE        = N'Bugs'
              and TASKS.DELETED            = 0
...
select MEETINGS.ID
  ...
  from            BUGS
       inner join MEETINGS
               on MEETINGS.PARENT_ID           = BUGS.ID
              and MEETINGS.PARENT_TYPE         = N'Bugs'
...
select CALLS.ID
  ...
  from            BUGS
       inner join CALLS
               on CALLS.PARENT_ID          = BUGS.ID
              and CALLS.PARENT_TYPE        = N'Bugs'
              and CALLS.DELETED            = 0
  left outer join TEAMS
...
select EMAILS.ID
  ...
  from            BUGS
       inner join EMAILS
               on EMAILS.PARENT_ID         = BUGS.ID
              and EMAILS.PARENT_TYPE       = N'Bugs'
              and EMAILS.DELETED           = 0
  left outer join EMAILS_BUGS
               on EMAILS_BUGS.BUG_ID       = BUGS.ID
              and EMAILS_BUGS.EMAIL_ID     = EMAILS.ID
...
select EMAILS.ID
  ...
  from            BUGS
       inner join EMAILS_BUGS
              ...
              and EMAILS_BUGS.DELETED      = 0
       inner join EMAILS
               on EMAILS.ID                = EMAILS_BUGS.EMAIL_ID
              and EMAILS.DELETED           = 0
  left outer join TEAMS
               on TEAMS.ID                 = EMAILS.TEAM_ID
              and TEAMS.DELETED            = 0
...
select NOTES.ID
  ...
  from            BUGS
       inner join NOTES
               on NOTES.PARENT_ID          = BUGS.ID
              and NOTES.PARENT_TYPE        = N'Bugs'
              and NOTES.DELETED            = 0
  left outer join TEAMS
...
select SMS_MESSAGES.ID
  ...
  from            BUGS
       inner join SMS_MESSAGES
               on SMS_MESSAGES.PARENT_ID   = BUGS.ID
              and SMS_MESSAGES.PARENT_TYPE = N'Bugs'
...
select TWITTER_MESSAGES.ID
  ...
  from            BUGS
       inner join TWITTER_MESSAGES
               on TWITTER_MESSAGES.PARENT_ID   = BUGS.ID
              and TWITTER_MESSAGES.PARENT_TYPE = N'Bugs'
...
select CHAT_MESSAGES.ID
  ...
  from            BUGS
       inner join CHAT_MESSAGES
               on CHAT_MESSAGES.PARENT_ID      = BUGS.ID
              and CHAT_MESSAGES.PARENT_TYPE    = N'Bugs'
...
select CHAT_MESSAGES.ID
  ...
  from            BUGS
       inner join CHAT_CHANNELS
               on CHAT_CHANNELS.PARENT_ID      = BUGS.ID
              and CHAT_CHANNELS.PARENT_TYPE    = N'Bugs'
              and CHAT_CHANNELS.DELETED        = 0
       inner join CHAT_MESSAGES
...
Grant Select on dbo.vwBUGS_ACTIVITIES to public;
GO
