if exists (select * from INFORMATION_SCHEMA.VIEWS where TABLE_NAME = 'vwPROJECT_TASKS_ACTIVITIES')
	Drop View dbo.vwPROJECT_TASKS_ACTIVITIES;
GO
...
Create View dbo.vwPROJECT_TASKS_ACTIVITIES
as
select TASKS.ID
     ...
     , PROJECT_TASK.ASSIGNED_USER_ID  as PROJECT_TASK_ASSIGNED_USER_ID
     , PROJECT_TASK.ASSIGNED_SET_ID   as PROJECT_TASK_ASSIGNED_SET_ID
  ...
  from            PROJECT_TASK
       inner join TASKS
               on TASKS.PARENT_ID          = PROJECT_TASK.ID
              and TASKS.PARENT_TYPE        = N'ProjectTask'
              and TASKS.DELETED            = 0
...
select MEETINGS.ID
  ...
  from            PROJECT_TASK
       inner join MEETINGS
               on MEETINGS.PARENT_ID           = PROJECT_TASK.ID
              and MEETINGS.PARENT_TYPE         = N'ProjectTask'
...
select CALLS.ID
  ...
  from            PROJECT_TASK
       inner join CALLS
               on CALLS.PARENT_ID          = PROJECT_TASK.ID
              and CALLS.PARENT_TYPE        = N'ProjectTask'
              and CALLS.DELETED            = 0
  left outer join TEAMS
...
select EMAILS.ID
  ...
  from            PROJECT_TASK
       inner join EMAILS
               on EMAILS.PARENT_ID                     = PROJECT_TASK.ID
              and EMAILS.PARENT_TYPE                   = N'ProjectTask'
              and EMAILS.DELETED                       = 0
  left outer join EMAILS_PROJECT_TASKS
               on EMAILS_PROJECT_TASKS.PROJECT_TASK_ID = PROJECT_TASK.ID
              and EMAILS_PROJECT_TASKS.EMAIL_ID        = EMAILS.ID
...
select EMAILS.ID
  ...
  from            PROJECT_TASK
       inner join EMAILS_PROJECT_TASKS
              ...
              and EMAILS_PROJECT_TASKS.DELETED         = 0
       inner join EMAILS
               on EMAILS.ID                            = EMAILS_PROJECT_TASKS.EMAIL_ID
              and EMAILS.DELETED                       = 0
  left outer join TEAMS
               on TEAMS.ID                             = EMAILS.TEAM_ID
              and TEAMS.DELETED                        = 0
...
select NOTES.ID
  ...
  from            PROJECT_TASK
       inner join NOTES
               on NOTES.PARENT_ID          = PROJECT_TASK.ID
              and NOTES.PARENT_TYPE        = N'ProjectTask'
              and NOTES.DELETED            = 0
  left outer join TEAMS
...
select SMS_MESSAGES.ID
  ...
  from            PROJECT_TASK
       inner join SMS_MESSAGES
               on SMS_MESSAGES.PARENT_ID               = PROJECT_TASK.ID
              and SMS_MESSAGES.PARENT_TYPE             = N'ProjectTask'
...
select TWITTER_MESSAGES.ID
  ...
  from            PROJECT_TASK
       inner join TWITTER_MESSAGES
               on TWITTER_MESSAGES.PARENT_ID   = PROJECT_TASK.ID
              and TWITTER_MESSAGES.PARENT_TYPE = N'ProjectTask'
...
select CHAT_MESSAGES.ID
  ...
  from            PROJECT_TASK
       inner join CHAT_MESSAGES
               on CHAT_MESSAGES.PARENT_ID      = PROJECT_TASK.ID
              and CHAT_MESSAGES.PARENT_TYPE    = N'ProjectTask'
...
select CHAT_MESSAGES.ID
  ...
  from            PROJECT_TASK
       inner join CHAT_CHANNELS
               on CHAT_CHANNELS.PARENT_ID      = PROJECT_TASK.ID
              and CHAT_CHANNELS.PARENT_TYPE    = N'ProjectTask'
              and CHAT_CHANNELS.DELETED        = 0
       inner join CHAT_MESSAGES
...
Grant Select on dbo.vwPROJECT_TASKS_ACTIVITIES to public;
GO
