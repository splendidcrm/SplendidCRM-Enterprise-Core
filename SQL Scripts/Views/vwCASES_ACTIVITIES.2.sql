if exists (select * from INFORMATION_SCHEMA.VIEWS where TABLE_NAME = 'vwCASES_ACTIVITIES')
	Drop View dbo.vwCASES_ACTIVITIES;
GO
...
Create View dbo.vwCASES_ACTIVITIES
as
select TASKS.ID
  ...
  from            CASES
       inner join TASKS
               on TASKS.PARENT_ID          = CASES.ID
              and TASKS.PARENT_TYPE        = N'Cases'
              and TASKS.DELETED            = 0
...
select MEETINGS.ID
  ...
  from            CASES
       inner join MEETINGS
               on MEETINGS.PARENT_ID           = CASES.ID
              and MEETINGS.PARENT_TYPE         = N'Cases'
...
select CALLS.ID
  ...
  from            CASES
       inner join CALLS
               on CALLS.PARENT_ID        = CASES.ID
              and CALLS.PARENT_TYPE      = N'Cases'
              and CALLS.DELETED          = 0
  left outer join TEAMS
...
select EMAILS.ID
  ...
  from            CASES
       inner join EMAILS
               on EMAILS.PARENT_ID         = CASES.ID
              and EMAILS.PARENT_TYPE       = N'Cases'
              and EMAILS.DELETED           = 0
  left outer join EMAILS_CASES
               on EMAILS_CASES.CASE_ID     = CASES.ID
              and EMAILS_CASES.EMAIL_ID    = EMAILS.ID
...
select EMAILS.ID
  ...
  from            CASES
       inner join EMAILS_CASES
              ...
              and EMAILS_CASES.DELETED     = 0
       inner join EMAILS
               on EMAILS.ID                = EMAILS_CASES.EMAIL_ID
              and EMAILS.DELETED           = 0
  left outer join TEAMS
               on TEAMS.ID                 = EMAILS.TEAM_ID
              and TEAMS.DELETED            = 0
...
select SMS_MESSAGES.ID
  ...
  from            CASES
       inner join SMS_MESSAGES
               on SMS_MESSAGES.PARENT_ID   = CASES.ID
              and SMS_MESSAGES.PARENT_TYPE = N'Cases'
...
select TWITTER_MESSAGES.ID
  ...
  from            CASES
       inner join TWITTER_MESSAGES
               on TWITTER_MESSAGES.PARENT_ID   = CASES.ID
              and TWITTER_MESSAGES.PARENT_TYPE = N'Cases'
...
select CHAT_MESSAGES.ID
  ...
  from            CASES
       inner join CHAT_MESSAGES
               on CHAT_MESSAGES.PARENT_ID      = CASES.ID
              and CHAT_MESSAGES.PARENT_TYPE    = N'Cases'
...
select CHAT_MESSAGES.ID
  ...
  from            CASES
       inner join CHAT_CHANNELS
               on CHAT_CHANNELS.PARENT_ID      = CASES.ID
              and CHAT_CHANNELS.PARENT_TYPE    = N'Cases'
              and CHAT_CHANNELS.DELETED        = 0
       inner join CHAT_MESSAGES
...
Grant Select on dbo.vwCASES_ACTIVITIES to public;
GO
