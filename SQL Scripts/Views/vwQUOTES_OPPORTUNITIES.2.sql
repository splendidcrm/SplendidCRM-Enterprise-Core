if exists (select * from INFORMATION_SCHEMA.VIEWS where TABLE_NAME = 'vwQUOTES_OPPORTUNITIES')
	Drop View dbo.vwQUOTES_OPPORTUNITIES;
GO
...
Create View dbo.vwQUOTES_OPPORTUNITIES
as
select QUOTES.ID                   as QUOTE_ID
     ...
     , vwOPPORTUNITIES.*
  from           QUOTES
      inner join QUOTES_OPPORTUNITIES
              on QUOTES_OPPORTUNITIES.QUOTE_ID = QUOTES.ID
             and QUOTES_OPPORTUNITIES.DELETED  = 0
      inner join vwOPPORTUNITIES
              on vwOPPORTUNITIES.ID            = QUOTES_OPPORTUNITIES.OPPORTUNITY_ID
 where QUOTES.DELETED = 0

GO

Grant Select on dbo.vwQUOTES_OPPORTUNITIES to public;
GO
