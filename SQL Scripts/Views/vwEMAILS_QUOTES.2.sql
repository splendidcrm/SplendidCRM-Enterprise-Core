if exists (select * from INFORMATION_SCHEMA.VIEWS where TABLE_NAME = 'vwEMAILS_QUOTES')
	Drop View dbo.vwEMAILS_QUOTES;
GO
...
Create View dbo.vwEMAILS_QUOTES
as
select EMAILS.ID               as EMAIL_ID
  ...
  from            EMAILS
       inner join EMAILS_QUOTES
               on EMAILS_QUOTES.EMAIL_ID = EMAILS.ID
              and EMAILS_QUOTES.DELETED  = 0
       inner join vwQUOTES
               on vwQUOTES.ID            = EMAILS_QUOTES.QUOTE_ID
...
select EMAILS.ID               as EMAIL_ID
  ...
  from            EMAILS
       inner join vwQUOTES
               on vwQUOTES.ID            = EMAILS.PARENT_ID
  left outer join EMAILS_QUOTES
               on EMAILS_QUOTES.EMAIL_ID = EMAILS.ID
              and EMAILS_QUOTES.QUOTE_ID = vwQUOTES.ID
              and EMAILS_QUOTES.DELETED  = 0
 where EMAILS.DELETED     = 0
...
Grant Select on dbo.vwEMAILS_QUOTES to public;
GO
