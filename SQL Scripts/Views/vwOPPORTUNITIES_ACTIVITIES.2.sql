if exists (select * from INFORMATION_SCHEMA.VIEWS where TABLE_NAME = 'vwOPPORTUNITIES_ACTIVITIES')
	Drop View dbo.vwOPPORTUNITIES_ACTIVITIES;
GO
...
Create View dbo.vwOPPORTUNITIES_ACTIVITIES
as
select TASKS.ID
  ...
  from            OPPORTUNITIES
       inner join TASKS
               on TASKS.PARENT_ID          = OPPORTUNITIES.ID
...
select MEETINGS.ID
  ...
  from            OPPORTUNITIES
       inner join MEETINGS
               on MEETINGS.PARENT_ID           = OPPORTUNITIES.ID
...
select CALLS.ID
  ...
  from            OPPORTUNITIES
       inner join CALLS
               on CALLS.PARENT_ID          = OPPORTUNITIES.ID
...
select EMAILS.ID
  ...
  from            OPPORTUNITIES
       inner join EMAILS
               on EMAILS.PARENT_ID                    = OPPORTUNITIES.ID
              and EMAILS.PARENT_TYPE                  = N'Opportunities'
              and EMAILS.DELETED                      = 0
  left outer join EMAILS_OPPORTUNITIES
               on EMAILS_OPPORTUNITIES.OPPORTUNITY_ID = OPPORTUNITIES.ID
              and EMAILS_OPPORTUNITIES.EMAIL_ID       = EMAILS.ID
...
select EMAILS.ID
  ...
  from            OPPORTUNITIES
       inner join EMAILS_OPPORTUNITIES
              ...
              and EMAILS_OPPORTUNITIES.DELETED        = 0
       inner join EMAILS
               on EMAILS.ID                = EMAILS_OPPORTUNITIES.EMAIL_ID
              and EMAILS.DELETED           = 0
  left outer join TEAMS
               on TEAMS.ID                 = EMAILS.TEAM_ID
              and TEAMS.DELETED            = 0
...
select NOTES.ID
  ...
  from            OPPORTUNITIES
       inner join NOTES
               on NOTES.PARENT_ID          = OPPORTUNITIES.ID
...
select SMS_MESSAGES.ID
  ...
  from            OPPORTUNITIES
       inner join SMS_MESSAGES
               on SMS_MESSAGES.PARENT_ID              = OPPORTUNITIES.ID
...
select TWITTER_MESSAGES.ID
  ...
  from            OPPORTUNITIES
       inner join TWITTER_MESSAGES
               on TWITTER_MESSAGES.PARENT_ID   = OPPORTUNITIES.ID
...
select CHAT_MESSAGES.ID
  ...
  from            OPPORTUNITIES
       inner join CHAT_MESSAGES
               on CHAT_MESSAGES.PARENT_ID      = OPPORTUNITIES.ID
...
select CHAT_MESSAGES.ID
  ...
  from            OPPORTUNITIES
       inner join CHAT_CHANNELS
               on CHAT_CHANNELS.PARENT_ID      = OPPORTUNITIES.ID
              and CHAT_CHANNELS.PARENT_TYPE    = N'Opportunities'
              and CHAT_CHANNELS.DELETED        = 0
       inner join CHAT_MESSAGES
...
Grant Select on dbo.vwOPPORTUNITIES_ACTIVITIES to public;
GO
