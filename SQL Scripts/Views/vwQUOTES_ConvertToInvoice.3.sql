if exists (select * from INFORMATION_SCHEMA.VIEWS where TABLE_NAME = 'vwQUOTES_ConvertToInvoice')
	Drop View dbo.vwQUOTES_ConvertToInvoice;
GO
...
-- 05/01/2009 Paul.  We want to be able to copy Quotes custom fields to Invoices custom fields, 
-- so we must join to the QUOTES_CSTM table and just expect a matching set of fields in INVOICES_CSTM. 
...
Create View dbo.vwQUOTES_ConvertToInvoice
as
select cast(null as nvarchar(30))     as INVOICE_NUM
     , cast(null as nvarchar(25))     as INVOICE_STAGE
     , DATE_QUOTE_EXPECTED_CLOSED     as DUE_DATE
     , QUOTES.TOTAL                   as AMOUNT_DUE
     , QUOTES.TOTAL_USDOLLAR          as AMOUNT_DUE_USDOLLAR
     , QUOTES.ID                      as QUOTE_ID
     , QUOTES.NAME                    as QUOTE_NAME
     , cast(null as uniqueidentifier) as ORDER_ID
     , cast(null as nvarchar(100))    as ORDER_NAME
     , QUOTES.ID
     , QUOTES.QUOTE_NUM
     , QUOTES.NAME
     , QUOTES.QUOTE_TYPE
...
Grant Select on dbo.vwQUOTES_ConvertToInvoice to public;
GO
