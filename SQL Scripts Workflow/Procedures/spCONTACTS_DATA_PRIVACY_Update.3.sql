if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spCONTACTS_DATA_PRIVACY_Update' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spCONTACTS_DATA_PRIVACY_Update;
GO
 ...
 * 
 *********************************************************************************************************************/
Create Procedure dbo.spCONTACTS_DATA_PRIVACY_Update
...
as
  begin
	...
	exec dbo.spSqlTableExists @ARCHIVE_EXISTS out, 'CONTACTS_ARCHIVE', @ARCHIVE_DATABASE;

	if exists(select * from CONTACTS where ID = @CONTACT_ID) begin -- then
		select @EMAIL1 = EMAIL1
		     , @EMAIL2 = EMAIL2
		  from CONTACTS
		 where ID = @CONTACT_ID;
	end else if @ARCHIVE_EXISTS = 1 begin -- then
		exec dbo.spSqlTableRecordExists @RECORD_EXISTS out, 'CONTACTS_ARCHIVE', 'ID', @CONTACT_ID, @ARCHIVE_DATABASE;
		if @RECORD_EXISTS = 1 begin -- then
			set @COMMAND = '';
			set @COMMAND = @COMMAND + 'select @EMAIL1 = EMAIL1' + @CRLF;
			set @COMMAND = @COMMAND + '     , @EMAIL2 = EMAIL2' + @CRLF;
			set @COMMAND = @COMMAND + '  from ' + @ARCHIVE_DATABASE_DOT + 'dbo.CONTACTS_ARCHIVE' + @CRLF;
			...
			exec sp_executesql @COMMAND, @PARAM_DEFINTION, @EMAIL1 = @EMAIL1 output, @EMAIL2 = @EMAIL2 output, @ID = @CONTACT_ID;
		end -- if;
	end -- if;

	if not exists(select * from CONTACTS_DATA_PRIVACY where CONTACT_ID = @CONTACT_ID and DATA_PRIVACY_ID = @DATA_PRIVACY_ID and DELETED = 0) begin -- then
		insert into CONTACTS_DATA_PRIVACY
			( ID               
			, CREATED_BY       
			, DATE_ENTERED     
			, MODIFIED_USER_ID 
	...
	-- 07/03/2018 Paul.  Also include any other contacts with the same email. 
	insert into CONTACTS_DATA_PRIVACY
		( ID              
		, CREATED_BY      
		, DATE_ENTERED    
	...
	select newid()           
	 ...
	 where (EMAIL1 in (@EMAIL1, @EMAIL2) or EMAIL2 in (@EMAIL1, @EMAIL2))
	   and not exists(select * from CONTACTS_DATA_PRIVACY where DATA_PRIVACY_ID = @DATA_PRIVACY_ID and CONTACT_ID = CONTACTS.ID and CONTACTS_DATA_PRIVACY.DELETED = 0);

	declare DATA_PRIVACY_LEADS_CURSOR cursor for
	select LEADS.ID
	  from      CONTACTS
	 inner join LEADS
	         on LEADS.CONTACT_ID = CONTACTS.ID
	         or LEADS.EMAIL1     in (CONTACTS.EMAIL1, CONTACTS.EMAIL2)
	         or LEADS.EMAIL2     in (CONTACTS.EMAIL1, CONTACTS.EMAIL2)
	 where CONTACTS.ID = @CONTACT_ID;
	open DATA_PRIVACY_LEADS_CURSOR;
	fetch next from DATA_PRIVACY_LEADS_CURSOR into @LEAD_ID;
	while @@FETCH_STATUS = 0 and @@ERROR = 0 begin -- do
		exec dbo.spLEADS_DATA_PRIVACY_Update @MODIFIED_USER_ID, @LEAD_ID, @DATA_PRIVACY_ID;
		fetch next from DATA_PRIVACY_LEADS_CURSOR into @LEAD_ID;
	end -- while;
	close DATA_PRIVACY_LEADS_CURSOR;
	deallocate DATA_PRIVACY_LEADS_CURSOR;

	if @ARCHIVE_EXISTS = 1 begin -- then
		set @COMMAND = '';
		set @COMMAND = @COMMAND + 'insert into CONTACTS_DATA_PRIVACY' + @CRLF;
		...
		set @COMMAND = @COMMAND + '   and not exists(select * from CONTACTS_DATA_PRIVACY where DATA_PRIVACY_ID = @DATA_PRIVACY_ID and CONTACT_ID = CONTACTS_ARCHIVE.ID and CONTACTS_DATA_PRIVACY.DELETED = 0)' + @CRLF;
	...
	exec dbo.spSqlTableExists @EXISTS out, 'LEADS_ARCHIVE', @ARCHIVE_DATABASE;
	if @EXISTS = 1 begin -- then
		...
		set @COMMAND = @COMMAND + ' where (EMAIL1 in (@EMAIL1, @EMAIL2) or EMAIL2 in (@EMAIL1, @EMAIL2) or CONTACT_ID = @CONTACT_ID) ' + @CRLF;
		...
		set @PARAM_DEFINTION = N'@DATA_PRIVACY_ID uniqueidentifier, @EMAIL1 nvarchar(100), @EMAIL2 nvarchar(100), @CONTACT_ID uniqueidentifier, @MODIFIED_USER_ID uniqueidentifier';
		exec sp_executesql @COMMAND, @PARAM_DEFINTION, @DATA_PRIVACY_ID = @DATA_PRIVACY_ID, @EMAIL1 = @EMAIL1, @EMAIL2 = @EMAIL2, @CONTACT_ID = @CONTACT_ID, @MODIFIED_USER_ID = @MODIFIED_USER_ID;
	end -- if;
...
Grant Execute on dbo.spCONTACTS_DATA_PRIVACY_Update to public;
GO
