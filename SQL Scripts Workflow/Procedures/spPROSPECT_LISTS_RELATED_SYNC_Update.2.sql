if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spPROSPECT_LISTS_RELATED_SYNC_Update' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spPROSPECT_LISTS_RELATED_SYNC_Update;
GO
 ...
 * 
 *********************************************************************************************************************/
Create Procedure dbo.spPROSPECT_LISTS_RELATED_SYNC_Update
...
as
  begin
	...
	-- 02/19/2018 Paul.  Instead of using the PROSPECT_LISTS_PROSPECTS table directly, use the view so that we get the DATE_MODIFIED_UTC of the related record. 
	...
	if dbo.fnIsEmptyGuid(@ID) = 1 begin -- then
		set @ID = newid();
		insert into PROSPECT_LISTS_PROSPECTS_SYNC
			( ID                      
			, CREATED_BY              
			, DATE_ENTERED            
			, MODIFIED_USER_ID        
		...
		update PROSPECT_LISTS_PROSPECTS_SYNC
...
Grant Execute on dbo.spPROSPECT_LISTS_RELATED_SYNC_Update to public;
GO
