if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spWWF_INSTANCE_STATES_ReadOffset' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spWWF_INSTANCE_STATES_ReadOffset;
GO
...
Create Procedure dbo.spWWF_INSTANCE_STATES_ReadOffset
	( @ID                   uniqueidentifier
	, @FILE_OFFSET          int
	, @READ_SIZE            int
...
-- #if SQL_Server /*
	...
	-- 	readtext WWF_INSTANCE_STATES.STATE @FILE_POINTER @FILE_OFFSET @READ_SIZE;
	-- end -- if;
-- #endif SQL_Server */
...
Grant Execute on dbo.spWWF_INSTANCE_STATES_ReadOffset to public;
GO
