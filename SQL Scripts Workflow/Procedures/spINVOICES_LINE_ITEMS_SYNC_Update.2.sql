if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spINVOICES_LINE_ITEMS_SYNC_Update' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spINVOICES_LINE_ITEMS_SYNC_Update;
GO
 ...
 * 
 *********************************************************************************************************************/
Create Procedure dbo.spINVOICES_LINE_ITEMS_SYNC_Update
...
as
  begin
	...
	if not exists(select* from INVOICES_LINE_ITEMS where ID = @LOCAL_ID and DELETED = 0) begin -- then
		raiserror(N'INVOICES_LINE_ITEMS_SYNC_Update: Could not find @LOCAL_ID value.', 16, 1);
		return;
	end -- if;
	...
	if dbo.fnIsEmptyGuid(@ID) = 1 begin -- then
		set @ID = newid();
		insert into INVOICES_LINE_ITEMS_SYNC
			( ID                      
			, CREATED_BY              
			, DATE_ENTERED            
			, MODIFIED_USER_ID        
...
Grant Execute on dbo.spINVOICES_LINE_ITEMS_SYNC_Update to public;
GO
