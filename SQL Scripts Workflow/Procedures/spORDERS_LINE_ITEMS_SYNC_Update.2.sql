if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spORDERS_LINE_ITEMS_SYNC_Update' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spORDERS_LINE_ITEMS_SYNC_Update;
GO
 ...
 * 
 *********************************************************************************************************************/
Create Procedure dbo.spORDERS_LINE_ITEMS_SYNC_Update
...
as
  begin
	...
	if dbo.fnIsEmptyGuid(@ID) = 1 begin -- then
		set @ID = newid();
		insert into ORDERS_LINE_ITEMS_SYNC
			( ID                      
			, CREATED_BY              
			, DATE_ENTERED            
			, MODIFIED_USER_ID        
...
Grant Execute on dbo.spORDERS_LINE_ITEMS_SYNC_Update to public;
GO
