if exists (select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME = 'spPROSPECT_LISTS_SYNC_Update' and ROUTINE_TYPE = 'PROCEDURE')
	Drop Procedure dbo.spPROSPECT_LISTS_SYNC_Update;
GO
 ...
 * 
 *********************************************************************************************************************/
Create Procedure dbo.spPROSPECT_LISTS_SYNC_Update
...
as
  begin
	...
	if dbo.fnIsEmptyGuid(@ID) = 1 begin -- then
		set @ID = newid();
		insert into PROSPECT_LISTS_SYNC
			( ID                      
			, CREATED_BY              
			, DATE_ENTERED            
			, MODIFIED_USER_ID        
...
Grant Execute on dbo.spPROSPECT_LISTS_SYNC_Update to public;
GO
