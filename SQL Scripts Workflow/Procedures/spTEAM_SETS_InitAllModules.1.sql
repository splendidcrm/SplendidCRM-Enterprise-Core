...
as
  begin
	...
	-- 02/27/2020 Paul.  First create a TEAM_SET_ID for each team. 
	declare INIT_SETS_TEAMS_CURSOR cursor for
	select ID
	  from TEAMS
	 order by NAME;
	open INIT_SETS_TEAMS_CURSOR;
	fetch next from INIT_SETS_TEAMS_CURSOR into @TEAM_ID;
	while @@FETCH_STATUS = 0 begin -- while
		set @TEAM_SET_ID = null;
		if @TEST = 0 begin -- then
			exec dbo.spTEAM_SETS_NormalizeSet @TEAM_SET_ID out, null, @TEAM_ID, @TEAM_SET_LIST;
		end -- if;
		fetch next from INIT_SETS_TEAMS_CURSOR into @TEAM_ID;
	end -- while;
	close INIT_SETS_TEAMS_CURSOR;
	deallocate INIT_SETS_TEAMS_CURSOR;
	
	declare INIT_SETS_MODULES_CURSOR cursor for
	select TABLE_NAME
	  from vwSqlTables
	 where TABLE_NAME in (select ObjectName from vwSqlColumns where ColumnName = 'TEAM_ID')
	   and TABLE_NAME in (select ObjectName from vwSqlColumns where ColumnName = 'TEAM_SET_ID')
	   and TABLE_NAME not like '%_AUDIT'
	   and TABLE_NAME not like '%_STREAM'
	 order by TABLE_NAME;
	...
	while @@FETCH_STATUS = 0 begin -- while
		...
		if exists(select * from vwSqlTables where TABLE_NAME = @CUSTOM_NAME) begin -- then
			...
			set @Command = @Command + ' inner join TEAM_SETS'                          + @CRLF;
			set @Command = @Command + '         on TEAM_SETS.TEAM_SET_LIST = cast(' + @TABLE_NAME + '.TEAM_ID as char(36))' + @CRLF;
			set @Command = @Command + '        and TEAM_SETS.DELETED           = 0'    + @CRLF;
			set @Command = @Command + ' where ' + @TABLE_NAME + '.TEAM_SET_ID is null' + @CRLF;
			set @Command = @Command + '   and ' + @TABLE_NAME + '.DELETED         = 0;'    + @CRLF;
		end -- if;
		set @Command = @Command + 'update ' + @TABLE_NAME                              + @CRLF;
		set @Command = @Command + '   set TEAM_SET_ID   = TEAM_SETS.ID'        + @CRLF;
...
-- select * from TEAM_SETS 
/*
begin tran;
exec dbo.spTEAM_SETS_InitAllModules ;
commit tran;
*/
