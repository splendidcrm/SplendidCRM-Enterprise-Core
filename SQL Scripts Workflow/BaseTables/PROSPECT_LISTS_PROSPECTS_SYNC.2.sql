...
-- drop table PROSPECT_LISTS_PROSPECTS_SYNC;
if not exists (select * from INFORMATION_SCHEMA.TABLES where TABLE_NAME = 'PROSPECT_LISTS_PROSPECTS_SYNC' and TABLE_TYPE = 'BASE TABLE')
  begin
	print 'Create Table dbo.PROSPECT_LISTS_PROSPECTS_SYNC';
	Create Table dbo.PROSPECT_LISTS_PROSPECTS_SYNC
		( ID                                 uniqueidentifier not null default(newid()) constraint PK_PROSPECT_LISTS_PROSPECTS_SYNC primary key
	...
	create index IDX_PROSPECT_LISTS_PROS_SYNC on dbo.PROSPECT_LISTS_PROSPECTS_SYNC (ASSIGNED_USER_ID, DELETED, SERVICE_NAME, REMOTE_KEY, LOCAL_ID)
	-- 11/18/2020 Paul.  Index is needed when getting list of CRM records to sync. 
