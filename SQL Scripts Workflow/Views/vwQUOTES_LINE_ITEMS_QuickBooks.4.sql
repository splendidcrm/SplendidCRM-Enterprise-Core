if exists (select * from INFORMATION_SCHEMA.VIEWS where TABLE_NAME = 'vwQUOTES_LINE_ITEMS_QuickBooks')
	Drop View dbo.vwQUOTES_LINE_ITEMS_QuickBooks;
GO
...
Create View dbo.vwQUOTES_LINE_ITEMS_QuickBooks
as
select vwQUOTES_LINE_ITEMS.*
 ...
 where vwQUOTES_LINE_ITEMS.MFT_PART_NUM is not null
   and PRODUCT_TEMPLATE_ID in (select SYNC_LOCAL_ID from vwPRODUCT_TEMPLATES_SYNC where SYNC_SERVICE_NAME = N'QuickBooks')
--   and QUOTE_ID            in (select SYNC_LOCAL_ID from vwQUOTES_SYNC            where SYNC_SERVICE_NAME = N'QuickBooks')

GO

Grant Select on dbo.vwQUOTES_LINE_ITEMS_QuickBooks to public;
GO
