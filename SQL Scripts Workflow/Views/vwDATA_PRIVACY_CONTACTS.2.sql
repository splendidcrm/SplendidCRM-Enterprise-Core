if exists (select * from INFORMATION_SCHEMA.VIEWS where TABLE_NAME = 'vwDATA_PRIVACY_CONTACTS')
	Drop View dbo.vwDATA_PRIVACY_CONTACTS;
GO
 ...
 * 
 *********************************************************************************************************************/
Create View dbo.vwDATA_PRIVACY_CONTACTS
as
select DATA_PRIVACY.ID               as DATA_PRIVACY_ID
  ...
  from            DATA_PRIVACY
       inner join CONTACTS_DATA_PRIVACY
               on CONTACTS_DATA_PRIVACY.DATA_PRIVACY_ID = DATA_PRIVACY.ID
              and CONTACTS_DATA_PRIVACY.DELETED         = 0
       inner join vwCONTACTS
               on vwCONTACTS.ID                         = CONTACTS_DATA_PRIVACY.CONTACT_ID
 where DATA_PRIVACY.DELETED = 0

GO

Grant Select on dbo.vwDATA_PRIVACY_CONTACTS to public;
GO
