...
namespace SplendidCRM.Controllers.Administration.WorkflowAlertShells
{
	[Authorize]
	[SplendidSessionAuthorize]
	[ApiController]
	[Route("Administration/WorkflowAlertShells/Rest.svc")]
	public class RestController : ControllerBase
	{
		public const string MODULE_NAME = "WorkflowAlertShells";
		...
		private List<Dictionary<string, object>> WorkflowFilterColumns(string sBaseURI, TimeZone T10n, L10N L10n, string MODULE_NAME, string TABLE_ALIAS)
		{
			...
			{
				dtColumns = SplendidCache.WorkflowFilterColumns(sMODULE_TABLE).Copy();
				...
				//vwColumns.RowFilter = "NAME in ('" + sTableAlias + ".ID', '" + sTableAlias + ".CREATED_BY_ID', '" + sTableAlias + ".MODIFIED_USER_ID', '" + sTableAlias + ".ASSIGNED_USER_ID', '" + sTableAlias + ".TEAM_ID', '" + sTableAlias + ".PARENT_ID', '" + sTableAlias + ".ACCOUNT_ID', '" + sTableAlias + ".CONTACT_ID')";
				...
				string[] arrIncludedColumns = new string[] {"ID", "CREATED_BY_ID", "MODIFIED_USER_ID", "ASSIGNED_USER_ID", "TEAM_ID", "PARENT_ID", "ACCOUNT_ID", "CONTACT_ID", "TEAM_SET_ID", "ASSIGNED_SET_ID"};
				StringBuilder sbIncludedColumns = new StringBuilder();
				sbIncludedColumns.Append("NAME in (");
				for ( int i = 0; i < arrIncludedColumns.Length; i++ )
				{
					if ( i > 0 )
						sbIncludedColumns.Append(", ");
					sbIncludedColumns.Append("'" + sTableAlias + "." + arrIncludedColumns[i] + "'");
				}
				System.Text.RegularExpressions.Regex reg = new System.Text.RegularExpressions.Regex(@"[^A-Za-z0-9_]");
				arrIncludedColumns = Sql.ToString(Application["CONFIG.Workflow.EmailColumns"]).Split(',');
				for ( int i = 0; i < arrIncludedColumns.Length; i++ )
				{
					sbIncludedColumns.Append(", ");
					sbIncludedColumns.Append("'" + sTableAlias + "." + reg.Replace(arrIncludedColumns[i], "") + "'");
				}
				sbIncludedColumns.Append(")");
				vwColumns.RowFilter = sbIncludedColumns.ToString();
			}
		...
		private Dictionary<string, object> AlertModuleFilterSource(string sBaseURI, TimeZone T10n, L10N L10n, string MODULE_NAME, string RELATED, string TYPE)
		{
			bool bDebug = Sql.ToBoolean(Application["CONFIG.show_sql"]);
			Dictionary<string, object> dict = Session["WorkflowFilterBuilder.AlertModuleFilterSource." + MODULE_NAME] as Dictionary<string, object>;
#if DEBUG
			dict = null;
#endif
			if ( dict == null )
			{
				...
				Session["WorkflowFilterBuilder.AlertModuleFilterSource." + MODULE_NAME] = dict;
			}
			return dict;
		}
		...
		public Dictionary<string, object> GetQueryBuilderState(string Modules, string MODULE_NAME, string RELATED, string TYPE)
		{
			string sBaseURI = Request.Scheme + "://" + Request.Host.Host + Request.Path.Value;
			TimeZone T10n = TimeZone.CreateTimeZone( Sql.ToGuid( Application["CONFIG.default_timezone"] ) );
			int  nACLACCESS = Security.GetUserAccess("WorkflowAlertShells", "edit");
			if ( !Security.IsAuthenticated() || !Sql.ToBoolean(Application["Modules." + "WorkflowAlertShells" + ".RestEnabled"]) || nACLACCESS < 0 )
			{
				// 09/06/2017 Paul.  Include module name in error. 
				throw(new Exception(L10n.Term("ACL.LBL_INSUFFICIENT_ACCESS") + ": " + "Reports"));
			}
			...
			Dictionary<string, object> dictModuleFilterSource = AlertModuleFilterSource(sBaseURI, T10n, L10n, MODULE_NAME, RELATED, TYPE);
			...
			List<Dictionary<string, object>> FILTER_COLUMN_LIST = null;
			if ( !Sql.IsEmptyString(RELATED) )
			{
				string sRELATED_MODULE    = RELATED.Split(' ')[0];
				string sRELATED_ALIAS     = RELATED.Split(' ')[1];
				FILTER_COLUMN_LIST = WorkflowFilterColumns(sBaseURI, T10n, L10n, sRELATED_MODULE, sRELATED_ALIAS);
			...
			else
			{
				...
				FILTER_COLUMN_LIST = WorkflowFilterColumns(sBaseURI, T10n, L10n, MODULE_NAME, sTABLE_ALIAS);
				results.Add("FILTER_COLUMN_LIST", FILTER_COLUMN_LIST);
			}
		...
		private XomlDocument BuildWorkflowXOML(HttpApplicationState Application, RdlDocument rdl, string BaseModule, Guid gPARENT_ID, string sWORKFLOW_TYPE, string sALERT_NAME, string sALERT_TYPE, string sALERT_TEXT, string sSOURCE_TYPE, Guid gCUSTOM_TEMPLATE_ID, Guid gASSIGNED_USER_ID, Guid gTEAM_ID, string sTEAM_SET_LIST, StringBuilder sbErrors)
		{
			...
			if ( xoml.DocumentElement != null )
			{
				try
				{
					...
					WorkflowBuilder.BuildAlertXOML(xoml, null, rdl, sWORKFLOW_TYPE, BaseModule, sMODULE_TABLE, sALERT_NAME, sALERT_TYPE, sALERT_TEXT, sSOURCE_TYPE, gCUSTOM_TEMPLATE_ID, 2, gASSIGNED_USER_ID, gTEAM_ID, sTEAM_SET_LIST);
				}
				catch(Exception ex)
				{
		...
		[DotNetLegacyData]
		[HttpPut("[action]")]
		public string BuildAlertXOML([FromBody] Dictionary<string, object> dict)
		{
			string sModuleName = "WorkflowAlertShells";
			...
			string sALERT_NAME           = String.Empty;
			string sALERT_TYPE           = String.Empty;
			string sALERT_TEXT           = String.Empty;
			string sSOURCE_TYPE          = String.Empty;
			Guid   gCUSTOM_TEMPLATE_ID   = Guid.Empty;
			...
			Dictionary<string, object> dictAttachmentXml    = null;
			foreach ( string sColumnName in dict.Keys )
			{
				switch ( sColumnName )
				{
					...
					case "ALERT_NAME"          :  sALERT_NAME           = Sql.ToString(dict[sColumnName]);  break;
					case "ALERT_TYPE"          :  sALERT_TYPE           = Sql.ToString(dict[sColumnName]);  break;
					case "ALERT_TEXT"          :  sALERT_TEXT           = Sql.ToString(dict[sColumnName]);  break;
					case "SOURCE_TYPE"         :  sSOURCE_TYPE          = Sql.ToString(dict[sColumnName]);  break;
					case "CUSTOM_TEMPLATE_ID"  :  gCUSTOM_TEMPLATE_ID   = Sql.ToGuid  (dict[sColumnName]);  break;
					...
					case "attachmentXml"       :  dictAttachmentXml     = dict[sColumnName] as Dictionary<string, object>;  break;
				}
			}
		...
		if ( sSOURCE_TYPE == "normal message" )
			gCUSTOM_TEMPLATE_ID = Guid.Empty;
			...
			rdl.SetWorkflowAttachmentCustomProperty(dictAttachmentXml   );
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				XomlDocument xoml = BuildWorkflowXOML(Application, rdl, sBASE_MODULE, gPARENT_ID, sWORKFLOW_TYPE, sALERT_NAME, sALERT_TYPE, sALERT_TEXT, sSOURCE_TYPE, gCUSTOM_TEMPLATE_ID, gASSIGNED_USER_ID, gTEAM_ID, sTEAM_SET_LIST, sbErrors);
