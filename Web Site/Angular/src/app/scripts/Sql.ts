...
export default class Sql
{
	...
	static ToDateTime(sDate: string)
	{
	...
	static IsDate(sDate: string)
	{
		let dt = new Date(sDate);
		return isDate(dt);
	}

	static IsEmail(sEmail: string)
	{
		let filter = /^([\w-\.]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([\w-]+\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)$/;
		return filter.test(sEmail);
	}

	static ToBoolean(b: any)
	{
	...
	// 06/14/2015 Paul.  We should be replacing all and not just the first item. 
	static EscapeSQLLike(str: string)
	{
		if (str != null)
		{
			str = Sql.replaceAll(str, '\\', '\\\\');
	...
	// 06/14/2015 Paul.  We should be replacing all and not just the first item. 
	static EscapeJavaScript(str: string)
	{
		if (str != null)
		{
			str = Sql.replaceAll(str, '\\', '\\\\');
	...
	static ReplaceEntities(str: string)
	{
	...
	static NormalizePhone(str: string)
{
	...
	static AppendLikeParameters(cmd: any, sField: string, oValue: any, bOrClause?: boolean)
	{
	...
	static AppendParameterWithNull(cmd: any, sField: string, oValue: any, bOrClause?: boolean)
	{
		let ControlChars = { CrLf: '\r\n' };
		if ( bOrClause === undefined )
		{
	...
	static AppendGuids(cmd: any, sField: string, oValue: any)
	{
		let ControlChars = { CrLf: '\r\n' };
	...
	static AppendParameter(cmd: any, sField: string, oValue: any, bOrClause?: boolean)
	{
		let ControlChars = { CrLf: '\r\n' };
		if ( bOrClause === undefined )
		{
	...
	static ParseFormData(sParameters: string)
	{
	...
	// 05/15/2022 Paul.  TODO. 
	static DataPrivacyErasedPill(L10n: L10nService)
	{
		return '<span class="Erased">' + L10n.Term('DataPrivacy.LBL_ERASED_VALUE') + '</span>';
	}

	// 05/15/2022 Paul.  TODO. 
	static SelectGridColumns(SplendidCache: SplendidCacheService, layout: any[])
	{
	...
	static SearchGridColumns(layout: any)
	{
		let arrSelectFields  = new Array();
	...
	// 05/15/2022 Paul.  TODO.
	static IsDataPrivacyErasedField(Crm_Config: CrmConfigService, rdr: any, sFIELD_NAME: string)
	{
	...
	static DataPrivacyErasedPillElement(L10n: L10nService)
	{
		let pill: string = '<span class="Erased">' + L10n.Term('DataPrivacy.LBL_ERASED_VALUE') + '</span>';
		return pill
	}

	static DataPrivacyErasedField(Crm_Config: CrmConfigService, L10n: L10nService, rdr: any, sFIELD_NAME: string)
	{
		...
		if ( rdr != null )
		{
			...
			{
				if ( Sql.IsDataPrivacyErasedField(Crm_Config, rdr, sFIELD_NAME) )
				{
					sDISPLAY_NAME = Sql.DataPrivacyErasedPillElement(L10n);
				}
			}
		}
	...
	static SetPageTitle(Crm_Config: CrmConfigService, L10n: L10nService, MODULE_NAME: string, rdr: any, sFIELD_NAME: string)
	{
