...
export default class SearchBuilder
{
	...
	public EscapeSql(str: string)
	{
		// 06/14/2015 Paul.  We should be replacing all and not just the first item. 
		return Sql.replaceAll(str, '\'', '\'\'');
	}

	public EscapeLike(str: string)
	{
		// 06/14/2015 Paul.  We should be replacing all and not just the first item. 
	...
	public ParseInput(sInput: string)
	{
		try
		{
	...
	public isAlphaNum(c: any)
	{
		return /^\w$/.test(c);
	}

	public BuildQuery(sCondition: string, sField: string, sInput?: string)
	{
		var sbSqlQuery  = '';
		var nNotFlag    = 0;
		var bOrFlag     = false;
	...
	public ApplyFilter(sField: string, sInput: string)
	{
		var nSuccess    = 0;
		var nNotFlag    = 0;
		var bOrFlag     = false;
