...
export default class ACL_FIELD_ACCESS
{
	...
	constructor(Security: SecurityService, nACLACCESS: number, gOWNER_ID: string)
	{
	...
	public static GetUserFieldSecurity(SplendidCache: SplendidCacheService, Security: SecurityService, sMODULE_NAME: string, sDATA_FIELD: string, gOWNER_ID: string): ACL_FIELD_ACCESS
	{
		let nACLACCESS: number = SplendidCache.GetUserFieldSecurity(sMODULE_NAME, sDATA_FIELD);
		let acl       : ACL_FIELD_ACCESS = new ACL_FIELD_ACCESS(Security, nACLACCESS, gOWNER_ID);
		return acl;
	}
}
