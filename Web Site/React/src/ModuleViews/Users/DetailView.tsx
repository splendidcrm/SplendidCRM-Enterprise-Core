...
@observer
class UsersDetailView extends React.Component<IDetailViewProps, IDetailViewState>
{
	...
	{
		super(props);
		let DETAIL_NAME = props.MODULE_NAME + '.DetailView';
	...
	async componentDidMount()
	{
		...
		try
		{
			...
			if ( status == 1 )
			{
				...
				// 05/28/2020 Paul.  After editing a User Signature, the user is returned to Users/View/000000.  
				// 04/18/2021 Paul.  Don't redirect on precompile as it breaks it. 
				if ( this.props.ID == Security.USER_ID() && !this.props.isPrecompile )
				{
	...
	private _onButtonsLoaded = async () =>
	{
		...
		if ( this.headerButtons.current != null )
		{
			let bIsAdmin: boolean = SplendidCache.AdminUserAccess('Users', 'view') >= 0;
			this.headerButtons.current.ShowButton('Edit'          , bIsAdmin);
			this.headerButtons.current.ShowButton('Duplicate'     , bIsAdmin);
			this.headerButtons.current.ShowButton('ChangePassword', bIsAdmin);
			// 10/09/2020 Paul.  ResetDefaults was never previously coded. 
			this.headerButtons.current.ShowButton('ResetDefaults' , bIsAdmin);
		}
	}
