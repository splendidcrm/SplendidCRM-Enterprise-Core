...
interface ISubPanelViewProps extends RouteComponentProps<any>
{
	...
	// 07/02/2021 Paul.  ACLRoles ByUser does not allow Make Primary. 
	disableMakePrimary? : boolean;
}
...
interface ISubPanelViewState
{
	...
	open             : boolean;
	customView       : any;
	ACLACCESS_Edit   : boolean;
	subPanelVisible  : boolean;
}

@observer
class UsersACLRoles extends React.Component<ISubPanelViewProps, ISubPanelViewState>
{
	...
	constructor(props: ISubPanelViewProps)
	{
		...
		// 11/10/2020 Paul.  A customer preferred the old icons instead of the new fontawesome icons. 
	...
	private Load = async (sMODULE_NAME, sSORT_FIELD, sSORT_DIRECTION, sSELECT, sFILTER, rowSEARCH_VALUES, nTOP, nSKIP, bADMIN_MODE?, archiveView?) =>
	{
		...
		// 05/28/2020 Paul.  The ROLE_ID field is not normally returned as part of the layout, so we have to add manually. 
		let arrSELECT: string[] = sSELECT.split(',');
		if ( arrSELECT.indexOf('ROLE_ID') < 0 )
		{
			arrSELECT.push('ROLE_ID');
		}
		...
		if ( d.results )
		{
			for ( let i: number = 0; i < d.results.length; i++ )
			{
				...
				d.results[i]['ID'] = L10n.Term(d.results[i]['ROLE_ID']);
			}
		}
		return d;
	}

	private _onMakePrimary = async (row) =>
	{
		const { PARENT_ID, RELATED_MODULE } = this.state;
		//console.log((new Date()).toISOString() + ' ' + this.constructor.name + '._onMakePrimary', row);
		try
		{
			let obj: any = new Object();
			obj['USER_ID'] = PARENT_ID;
			obj['ROLE_ID'] = row['ID'];
			let sBody: string = JSON.stringify(obj);
			let res  = await CreateSplendidRequest('Administration/Rest.svc/UserRoleMakeDefault', 'POST', 'application/json; charset=utf-8', sBody);
	...
	private makePrimaryColumnFormatter = (cell, row, rowIndex, formatExtraData) =>
	{
		const { ACLACCESS_Edit } = this.state;
		let IS_PRIMARY_ROLE: boolean = Sql.ToBoolean(row['IS_PRIMARY_ROLE']);
		if ( IS_PRIMARY_ROLE || !ACLACCESS_Edit )
		{
			return null;
		}
		else
		{
			return (
			<span>
				<input type='submit' value={ L10n.Term('ACLRoles.LBL_MAKE_PRIMARY_ROLE') } onClick={ (e) => { e.preventDefault(); this._onMakePrimary(row); } } />
			</span>);
		}
	}
	...
	{
		const { disableMakePrimary } = this.props;
		const { RELATED_MODULE, ACLACCESS_Edit } = this.state;
		// 04/20/2017 Paul.  Build DataTables columns. 
		let arrDataTableColumns = [];
		let objDataColumn: any = {};
		let disableView  : boolean = !ACLACCESS_Edit;
		let disableEdit  : boolean = !ACLACCESS_Edit;
		let disableRemove: boolean = !ACLACCESS_Edit;
		...
		// 11/10/2020 Paul.  Manually add Make Primary button
		// 07/02/2021 Paul.  ACLRoles ByUser does not allow Make Primary. 
		if ( !disableMakePrimary )
		{
			objDataColumn =
			{
				key            : 'editview',
				text           : null,
				dataField      : 'empty2',
				formatter      : this.makePrimaryColumnFormatter,
	...
	public render()
	{
		...
		const { RELATED_MODULE, GRID_NAME, TABLE_NAME, SORT_FIELD, SORT_DIRECTION, PRIMARY_FIELD, PRIMARY_ID, error, showCancel, showFullForm, showTopButtons, showBottomButtons, showSearch, showInlineEdit, item, popupOpen, multiSelect, archiveView, open, customView, ACLACCESS_Edit, subPanelVisible } = this.state;
...
export default withRouter(UsersACLRoles);
