...
@observer
class UsersUserSignatures extends React.Component<ISubPanelViewProps, ISubPanelViewState>
{
		...
		try
		{
			if ( this._isMounted )
			{
				if ( sCommandName == 'Create' || EndsWith(sCommandName, '.Create') )
				{
					...
					*/
					// 05/28/2020 Paul.  Inline Create is not supported at this time, so just redirect to the full form. 
	...
	public render()
	{
		...
		// 05/28/2020 Paul.  Always allow delete is this not a realtionship to be removed. 
		let cbRemove = this._onRemove;
		if ( SplendidCache.IsInitialized  )
		{
			...
			// 05/28/2020 Paul.  Always allow a user to edit their own record. 
			let ACLACCESS_Edit: boolean = (SplendidCache.AdminUserAccess('Users', 'edit') >= 0) || (PRIMARY_ID == Security.USER_ID()) ;
...
export default withRouter(UsersUserSignatures);
