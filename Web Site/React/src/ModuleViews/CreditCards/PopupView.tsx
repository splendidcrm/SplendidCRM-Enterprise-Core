...
class CreditCardsPopupView extends React.Component<IPopupViewProps, IPopupViewState>
{
	...
	{
		super(props);
		//console.log((new Date()).toISOString() + ' ' + this.constructor.name + '.constructor', props.rowDefaultSearch);
		let TITLE             : string = '';
	...
	private Load = async (sMODULE_NAME, sSORT_FIELD, sSORT_DIRECTION, sSELECT, sFILTER, rowSEARCH_VALUES, nTOP, nSKIP, bADMIN_MODE?, archiveView?) =>
	{
		...
		//console.log((new Date()).toISOString() + ' ' + this.constructor.name + '.Load', rowDefaultSearch);
		if ( rowDefaultSearch != null && rowDefaultSearch.ACCOUNT_ID != null )
			sFILTER = "ACCOUNT_ID eq '" + rowDefaultSearch.ACCOUNT_ID  + "'";
...
export default withRouter(CreditCardsPopupView);
