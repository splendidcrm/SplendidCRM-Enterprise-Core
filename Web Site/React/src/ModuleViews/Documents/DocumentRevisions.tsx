...
@observer
class DocumentsDocumentRevisions extends React.Component<ISubPanelViewProps, ISubPanelViewState>
{
	...
	private Page_Command = async (sCommandName, sCommandArguments) =>
	{
		...
		try
		{
			if ( this._isMounted )
			{
				...
				// 11/24/2021 Paul.  Separate command to create revision.  The ASP.Net code uses ID instead of PARENT_ID, so lets do the same here. 
				else if ( sCommandName == 'Documents.CreateRevision' )
				{
					this.props.history.push(`/Reset/DocumentRevisions/Edit/${PARENT_ID}`);
				}
	...
	{
		//console.log((new Date()).toISOString() + ' ' + this.constructor.name + '._onHyperLinkCallback ' + MODULE_NAME, ID, URL, row);
		window.location.href = Credentials.RemoteServer + 'Documents/Document.aspx?id=' + ID;
	}
...
export default withRouter(DocumentsDocumentRevisions);
