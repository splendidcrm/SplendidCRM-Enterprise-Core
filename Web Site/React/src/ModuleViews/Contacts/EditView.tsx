...
@observer
export default class ContactsEditView extends React.Component<IEditViewProps, IEditViewState>
{
	...
	private load = async () =>
	{
		...
		try
		{
			...
			if ( Sql.IsEmptyGuid(ID) && Sql.IsEmptyGuid(DuplicateID) && Sql.IsEmptyGuid(ConvertID) )
			{
				...
				rowDefaultSearch['EMAIL_OPT_OUT'   ] = Crm_Config.ToBoolean('default_email_opt_out');
				rowDefaultSearch['DO_NOT_CALL'     ] = Crm_Config.ToBoolean('default_do_not_call'  );
			}
			const layout       = EditView_LoadLayout(EDIT_NAME);
			const layoutPortal = EditView_LoadLayout(MODULE_NAME + '.PortalView');
	...
	{
		//console.log((new Date()).toISOString() + ' ' + this.constructor.name + '.ConvertItem: ' + sSOURCE_MODULE_NAME, sSOURCE_ID);
			...
			try
			{
				...
				if ( this._isMounted && item != null )
				{
					...
					if ( sPARENT_TYPE == 'Accounts' )
					{
						...
						rowDefaultSearch['PRIMARY_ADDRESS_STREET'    ] = item['BILLING_ADDRESS_STREET'     ];
						rowDefaultSearch['PRIMARY_ADDRESS_CITY'      ] = item['BILLING_ADDRESS_CITY'       ];
						rowDefaultSearch['PRIMARY_ADDRESS_STATE'     ] = item['BILLING_ADDRESS_STATE'      ];
						rowDefaultSearch['PRIMARY_ADDRESS_POSTALCODE'] = item['BILLING_ADDRESS_POSTALCODE' ];
						rowDefaultSearch['PRIMARY_ADDRESS_COUNTRY'   ] = item['BILLING_ADDRESS_COUNTRY'    ];
						// 10/07/2010 Paul.  Also populate the alt address with the shipping address. 
						rowDefaultSearch['ALT_ADDRESS_STREET'        ] = item['SHIPPING_ADDRESS_STREET'    ];
						rowDefaultSearch['ALT_ADDRESS_CITY'          ] = item['SHIPPING_ADDRESS_CITY'      ];
						rowDefaultSearch['ALT_ADDRESS_STATE'         ] = item['SHIPPING_ADDRESS_STATE'     ];
						rowDefaultSearch['ALT_ADDRESS_POSTALCODE'    ] = item['SHIPPING_ADDRESS_POSTALCODE'];
						rowDefaultSearch['ALT_ADDRESS_COUNTRY'       ] = item['SHIPPING_ADDRESS_COUNTRY'   ];
						// 04/03/2013 Paul.  A customer suggested that we copy phone numbers. 
						rowDefaultSearch['PHONE_WORK'                ] = item['PHONE_OFFICE'               ];
						rowDefaultSearch['PHONE_FAX'                 ] = item['PHONE_FAX'                  ];
						rowDefaultSearch['PHONE_OTHER'               ] = item['PHONE_ALTERNATE'            ];
					}
					else if ( sPARENT_TYPE == 'Contacts' )
					{
						rowDefaultSearch['REPORTS_TO_ID'             ] = item['ID'                         ];
						rowDefaultSearch['REPORTS_TO_NAME'           ] = item['NAME'                       ];
					}
	...
	private _onUpdate = async (PARENT_FIELD: string, DATA_VALUE: any, item?: any) =>
	{
		...
		// 08/09/2019  Paul.  The following are inherent dependencies. 
		if ( PARENT_FIELD == 'PRIMARY_ADDRESS_POSTALCODE' && item != null )
		{
			if ( !Sql.IsEmptyString(item.CITY) )
			{
				this.UpdateDependancy('PRIMARY_ADDRESS_CITY', item.CITY, 'value', item);
			}
			if ( !Sql.IsEmptyString(item.STATE) )
			{
				this.UpdateDependancy('PRIMARY_ADDRESS_STATE', item.STATE, 'value', item);
			}
			if ( !Sql.IsEmptyString(item.COUNTRY) )
			{
				this.UpdateDependancy('PRIMARY_ADDRESS_COUNTRY', item.COUNTRY, 'value', item);
			}
		}
		else if ( PARENT_FIELD == 'ALT_ADDRESS_POSTALCODE' && item != null )
		{
			if ( !Sql.IsEmptyString(item.CITY) )
			{
				this.UpdateDependancy('ALT_ADDRESS_CITY', item.CITY, 'value', item);
			}
			if ( !Sql.IsEmptyString(item.STATE) )
			{
				this.UpdateDependancy('ALT_ADDRESS_STATE', item.STATE, 'value', item);
			}
			if ( !Sql.IsEmptyString(item.COUNTRY) )
			{
				this.UpdateDependancy('ALT_ADDRESS_COUNTRY', item.COUNTRY, 'value', item);
			}
		}
		else if ( PARENT_FIELD == 'ACCOUNT_ID' )
		{
			// 11/19/2019 Paul.  Change to allow return of SQL. 
			...
			{
				this.UpdateDependancy('PRIMARY_ADDRESS_STREET'    , account['BILLING_ADDRESS_STREET'     ], 'value', item);
				this.UpdateDependancy('PRIMARY_ADDRESS_CITY'      , account['BILLING_ADDRESS_CITY'       ], 'value', item);
				this.UpdateDependancy('PRIMARY_ADDRESS_STATE'     , account['BILLING_ADDRESS_STATE'      ], 'value', item);
				this.UpdateDependancy('PRIMARY_ADDRESS_POSTALCODE', account['BILLING_ADDRESS_POSTALCODE' ], 'value', item);
				this.UpdateDependancy('PRIMARY_ADDRESS_COUNTRY'   , account['BILLING_ADDRESS_COUNTRY'    ], 'value', item);
				this.UpdateDependancy('ALT_ADDRESS_STREET'        , account['SHIPPING_ADDRESS_STREET'    ], 'value', item);
				this.UpdateDependancy('ALT_ADDRESS_CITY'          , account['SHIPPING_ADDRESS_CITY'      ], 'value', item);
				this.UpdateDependancy('ALT_ADDRESS_STATE'         , account['SHIPPING_ADDRESS_STATE'     ], 'value', item);
				this.UpdateDependancy('ALT_ADDRESS_POSTALCODE'    , account['SHIPPING_ADDRESS_POSTALCODE'], 'value', item);
				this.UpdateDependancy('ALT_ADDRESS_COUNTRY'       , account['SHIPPING_ADDRESS_COUNTRY'   ], 'value', item);
			}
		}
	}
		...
		try
		{
			...
			switch (sCommandName)
			{
				...
				case 'SaveConcurrency':
				{
					...
					if ( Crm_Config.ToBoolean('portal_on') )
					{
						if ( !Sql.IsEmptyString(row['PORTAL_PASSWORD']) || !Sql.IsEmptyString(row['PORTAL_PASSWORD_CONFIRM']) )
						{
							if ( Sql.ToString(row['PORTAL_PASSWORD']) != Sql.ToString(row['PORTAL_PASSWORD_CONFIRM']) )
							{
								nInvalidFields++;
								this.setState({ error: L10n.Term('Contacts.ERR_PORTAL_PASSWORDS') });
							}
						}
					}
					if ( nInvalidFields == 0 )
					{
						...
						try
						{
							...
							else if ( !Sql.IsEmptyString(queryParams['CALL_ID']) )
							{
								let sPRIMARY_MODULE = 'Calls'            ;
								let sPRIMARY_ID     = queryParams['CALL_ID'];
								let sRELATED_MODULE = 'Contacts'         ;
								let sRELATED_ID     = row.ID             ;
								await UpdateRelatedItem(sPRIMARY_MODULE, sPRIMARY_ID, sRELATED_MODULE, sRELATED_ID);
								history.push(`/Reset/Calls/View/${queryParams['CALL_ID']}`);
							}
							else if ( !Sql.IsEmptyString(queryParams['MEETING_ID']) )
							{
								let sPRIMARY_MODULE = 'Meetings'            ;
								let sPRIMARY_ID     = queryParams['MEETING_ID'];
								let sRELATED_MODULE = 'Contacts'            ;
								let sRELATED_ID     = row.ID                ;
								await UpdateRelatedItem(sPRIMARY_MODULE, sPRIMARY_ID, sRELATED_MODULE, sRELATED_ID);
								history.push(`/Reset/Meetings/View/${queryParams['MEETING_ID']}`);
							}
							else
							{
	...
	public render()
	{
		...
		const { item, layout, layoutPortal, EDIT_NAME, SUB_TITLE, error } = this.state;
		...
		if ( SplendidCache.IsInitialized )
		{
			...
			<React.Fragment>
				...
				{ Crm_Config.ToBoolean('portal_on') && layoutPortal
				? <div>
					<h4>{ L10n.Term('Contacts.LBL_PORTAL_INFORMATION') }</h4>
					{ SplendidDynamic_EditView.AppendEditViewFields(item, layoutPortal, this.refMap, callback, this._createDependency, null, this._onChange, this._onUpdate, onSubmit, 'tabForm', this.Page_Command) }
				</div>
				: null
				}
				{ !callback && headerButtons
