...
class ContactsListView extends React.Component<IListViewProps, IListViewState>
{
	...
	private exportHeader  = React.createRef<ExportHeader>();
	...
	private Grid_Command = async (sCommandName: string, sCommandArguments: any) =>
	{
		...
		else if ( sCommandName == 'PhoneBurner.BeginDial' )
		{
			try
			{
				let obj: any =
				...
				if ( obj.ID.length > 0 )
				{
					...
					let res  = await CreateSplendidRequest('Administration/PhoneBurner/Rest.svc/BeginDialSession', 'POST', 'application/octet-stream', sBody);
					let json = await GetSplendidResult(res);
					let url = json.d;
					window.open(url, '_blank', 'scrollbars=yes,resizable=yes,status=yes,location=yes,toolbar=yes,menubar=no');
					this.exportHeader.current.SetStatus('');
				}
				else
				{
					this.exportHeader.current.SetStatus(L10n.Term('.LBL_LISTVIEW_NO_SELECTED'));
				}
			}
			catch(error)
			{
...
export default withRouter(ContactsListView);
