...
@observer
export default class ThreadsEditView extends React.Component<IEditViewProps, IEditViewState>
{
	...
	public get data (): any
	{
		...
		if ( !Sql.IsEmptyGuid(this.FORUM_ID) )
		{
			row['FORUM_ID'] = this.FORUM_ID;
		}
	...
	private load = async () =>
	{
		...
		try
		{
			...
			if ( !Sql.IsEmptyGuid(queryParams['FORUM_ID']) )
			{
				this.FORUM_ID   = queryParams['FORUM_ID'];
			}
				...
				case 'SaveConcurrency':
				{
					...
					if ( nInvalidFields == 0 )
					{
						...
						try
						{
							...
							if ( !Sql.IsEmptyGuid(this.FORUM_ID) )
							{
								history.push(`/Reset/Forums/View/${this.FORUM_ID}`);
							}
							else if ( !Sql.IsEmptyString(this.PARENT_TYPE) && !Sql.IsEmptyGuid(this.PARENT_ID) )
							{
