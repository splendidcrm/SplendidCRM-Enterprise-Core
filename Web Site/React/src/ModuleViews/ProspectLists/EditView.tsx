...
import { EditView_LoadItem, EditView_LoadLayout, EditView_ActivateTab, EditView_ConvertItem, EditView_HideField } from '../../scripts/EditView';
...
@observer
export default class ProspectListsEditView extends React.Component<IEditViewProps, IEditViewState>
{
	...
	private ALLOW_DYNAMIC_LIST   : boolean = true;

	public get data (): any
	{
		const { ID } = this.props;
		let isDuplicate = location.pathname.includes('Duplicate');
		let row: any      = Object.assign({ID: isDuplicate ? null : ID}, this.queryBuilder.current.data);
		// 10/15/2019 Paul.  The PARENT_ID is set, but not edited. 
	...
	private load = async () =>
	{
		...
		try
		{
			...
			if ( this._isMounted )
			{
				// 02/02/2022 Paul.  Make sure to hide the dynamic list for the Community Edition. 
				if ( Crm_Config.ToString('service_level') == 'Community' )
				{
					EditView_HideField(layout, 'DYNAMIC_LIST', true);
				}
				this.setState(
				{
					layout: layout,
		...
		if ( !Sql.IsEmptyString(sID) )
		{
			try
			{
				...
				if ( item != null )
				{
					...
					if ( Sql.ToBoolean(item['DYNAMIC_LIST']) )
					{
						try
						{
							let res = await CreateSplendidRequest('Rest.svc/GetModuleTable?TableName=vwPROSPECT_LISTS_EditSQL&$filter=' + encodeURIComponent('ID eq \'' + sID + '\''), 'GET');
							let json = await GetSplendidResult(res);
							if ( json.d != null && json.d.results != null && json.d.results.length > 0 )
							{
								item.DYNAMIC_SQL = json.d.results[0].DYNAMIC_SQL;
								item.DYNAMIC_RDL = json.d.results[0].DYNAMIC_RDL;
							}
						}
						catch(error)
						{
						...
						// 08/22/2012 Paul.  If the RDL is blank, but the SQL is not, then prevent access to the builder as this is a read-only SQL. 
						if ( Sql.IsEmptyString(item["DYNAMIC_RDL"]) && !Sql.IsEmptyString(item["DYNAMIC_SQL"]) && Sql.ToBoolean(item["DYNAMIC_LIST"]) )
						{
							this.ALLOW_DYNAMIC_LIST = false;
							//console.log((new Date()).toISOString() + ' ' + this.constructor.name + '.LoadItem Dynamic Disabled', item);
						}
					}
				}
				if ( this._isMounted )
	...
	private ConvertItem = async (sMODULE_NAME: string, sSOURCE_MODULE_NAME: string, sSOURCE_ID: string) =>
	{
		if ( !Sql.IsEmptyString(sSOURCE_ID) )
		{
			try
			{
				...
				if ( item != null )
				{
					// 02/08/2022 Paul.  We need a second query to get the dynamic list info. 
					if ( Sql.ToBoolean(item['DYNAMIC_LIST']) )
					{
						try
						{
							let res = await CreateSplendidRequest('Rest.svc/GetModuleTable?TableName=vwPROSPECT_LISTS_EditSQL&$filter=' + encodeURIComponent('ID eq \'' + sSOURCE_ID + '\''), 'GET');
							let json = await GetSplendidResult(res);
							if ( json.d != null && json.d.results != null && json.d.results.length > 0 )
							{
								item.DYNAMIC_SQL = json.d.results[0].DYNAMIC_SQL;
				...
				case 'SaveConcurrency':
				{
					...
					if ( nInvalidFields == 0 )
					{
						...
						try
						{
							...
							// 06/01/2021 Paul. Dynamic lists require a separate save operation using spPROSPECT_LISTS_SQL_Update. 
							if ( Sql.ToBoolean(row['DYNAMIC_LIST']) )
							{
								let sBody = JSON.stringify(row);
								let sUrl: string = 'ProspectLists/Rest.svc/UpdateModule';
							...
							if ( !Sql.IsEmptyString(this.PARENT_TYPE) && !Sql.IsEmptyGuid(this.PARENT_ID) )
							{
	...
	private _onFieldDidMount = (DATA_FIELD: string, component: any): void =>
	{
		...
		try
		{
			if ( DATA_FIELD == 'DYNAMIC_LIST' )
			{
				if ( item != null )
				{
					// 08/22/2012 Paul.  If the RDL is blank, but the SQL is not, then prevent access to the builder as this is a read-only SQL. 
					component.updateDependancy(null, this.ALLOW_DYNAMIC_LIST, 'enabled', null);
				}
			}
		}
		catch(error)
	...
	public render()
	{
		...
		if ( SplendidCache.IsInitialized )
		{
			...
			// 02/08/2022 Paul.  Hide/show QueryBuilder based on DYNAMIC_LIST flag. 
			return (
			<div style={ {width: '100%'} }>
				...
				{ Sql.ToBoolean(currentItem['DYNAMIC_LIST']) && this.ALLOW_DYNAMIC_LIST
				? <QueryBuilder
					row={ currentItem }
					Modules='Contacts,Leads,Prospects,Users,Accounts'
					DATA_FIELD='DYNAMIC_RDL'
					UserSpecific={ false }
