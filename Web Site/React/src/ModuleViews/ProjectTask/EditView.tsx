...
@observer
export default class ProjectTaskEditView extends React.Component<IEditViewProps, IEditViewState>
{
	...
	private LoadParent = async (sPARENT_TYPE: string, sID: string) =>
	{
		...
		if ( !Sql.IsEmptyString(sPARENT_TYPE) && !Sql.IsEmptyString(sID) )
		{
			try
			{
				...
				{
					rowDefaultSearch = {};
					if ( sPARENT_TYPE == 'Project' )
					{
						rowDefaultSearch['PROJECT_ID'       ] = item['ID'               ];
						rowDefaultSearch['PROJECT_NAME'     ] = item['NAME'             ];
					}
