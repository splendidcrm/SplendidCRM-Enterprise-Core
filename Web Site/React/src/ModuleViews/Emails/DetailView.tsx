...
@observer
class EmailsDetailView extends React.Component<IDetailViewProps, IDetailViewState>
{
		...
		try
		{
			...
			if ( this._isMounted )
			{
				...
				if ( item != null )
				{
					...
					EMAIL_STATUS = Sql.ToString(item['STATUS']).toLowerCase();
					...
					switch ( EMAIL_TYPE )
					{
						...
						case 'inbound' :
							...
							{
								let sBody: string = JSON.stringify({ ID: sID });
								let res  = await CreateSplendidRequest('Rest.svc/UpdateEmailReadStatus', 'POST', 'application/json; charset=utf-8', sBody);
	...
	{
		const { item, MODULE_NAME, EMAIL_TYPE, EMAIL_STATUS } = this.state;
		...
		if ( this.headerButtons.current != null )
		{
			...
			switch ( EMAIL_TYPE )
			{
				...
				case 'out'     :
					...
					this.headerButtons.current.ShowButton('Duplicate', EMAIL_STATUS == 'send_error');
					this.headerButtons.current.ShowButton("Delete"   , (nDELETE_ACLACCESS >= 0));
					break;
				case 'sent'    :
			...
			<React.Fragment>
				...
				<div id="content">
					...
					<div style={ {display: 'flex', flexFlow: 'row wrap', flex: '1 0 100%'} }>
						<div id='ctlDetailView_Emails_DetailView_ATTACHMENTS_LABEL' className='tabDetailViewDL' style={ {width: '15%'} }>
							{ L10n.Term('Emails.LBL_ATTACHMENTS') }
						</div>
						<div id='ctlDetailView_Emails_DetailView_ATTACHMENTS' className='tabDetailViewDF' style={ {width: '85%'} }>
...
export default withRouter(EmailsDetailView);
