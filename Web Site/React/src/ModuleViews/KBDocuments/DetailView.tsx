...
@observer
class KBDocumentsDetailView extends React.Component<IDetailViewProps, IDetailViewState>
{
			...
			<React.Fragment>
				...
				<div id="content">
					...
					<div style={ {display: 'flex', flexFlow: 'row wrap', flex: '1 0 100%'} }>
						<div id='ctlDetailView_KBDocuments_DetailView_ATTACHMENTS_LABEL' className='tabDetailViewDL' style={ {width: '15%'} }>
							{ L10n.Term('KBDocuments.LBL_ATTACHMENTS') }
						</div>
						<div id='ctlDetailView_KBDocuments_DetailView_ATTACHMENTS' className='tabDetailViewDF' style={ {width: '85%'} }>
						{ item && item.ATTACHMENTS
						? item.ATTACHMENTS.map((attachment, index) => 
							{
								return (
								<div>
									<a id={ attachment.ID } key={ attachment.ID } href={ Credentials.RemoteServer + 'Notes/Attachment.aspx?ID=' + attachment.ID } target='_blank'>{ attachment.FILENAME }</a>
								</div>);
							})
						: null
						}
					...
					<br />
					<div style={ {display: 'flex', flexFlow: 'row wrap', flex: '1 0 100%'} }>
						<div id='ctlDetailView_KBDocuments_DetailView_IMAGES_LABEL' className='tabDetailViewDL' style={ {width: '15%'} }>
							{ L10n.Term('KBDocuments.LBL_IMAGES') }
						</div>
						<div id='ctlDetailView_KBDocuments_DetailView_IMAGES' className='tabDetailViewDF' style={ {width: '85%'} }>
						{ item && item.IMAGES
						? item.IMAGES.map((attachment, index) => 
							{
								return (
								<div>
									<a id={ attachment.ID } key={ attachment.ID } href={ Credentials.RemoteServer + 'KBDocuments/Image.aspx?ID=' + attachment.ID } target='_blank'>{ attachment.FILENAME }</a>
								</div>);
							})
						: null
						}
...
export default withRouter(KBDocumentsDetailView);
