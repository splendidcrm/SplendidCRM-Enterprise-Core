...
@observer
export default class KBDocumentsEditView extends React.Component<IEditViewProps, IEditViewState>
{
	...
	private _onAddImage = () =>
	{
		...
		//console.log((new Date()).toISOString() + ' ' + this.constructor.name + '._onAddImage');
		if ( !IMAGES )
		{
			IMAGES = [];
		}
	...
	private _onImage = (e, index) =>
	{
		let { IMAGES } = this.state;
		//console.log((new Date()).toISOString() + ' ' + this.constructor.name + '._onImage', index);
		try
		{
			let FILE_NAME: string = e.target.value;
			let files = e.target.files;
		...
		{
			console.error((new Date()).toISOString() + ' ' + this.constructor.name + '._onImage', error);
		}
	}

	private _onRemoveImage = (e, index) =>
	{
		let { IMAGES } = this.state;
	...
	public render()
	{
		...
		const { item, ATTACHMENTS, IMAGES, layout, EDIT_NAME, SUB_TITLE, error } = this.state;
			...
			<React.Fragment>
				...
				<div className='tabForm' style={ {width: '100%'} }>
					<div className='tabEditView' style={ {display: 'flex', flexFlow: 'row wrap', width: '100%'} }>
						<div style={ {display: 'flex', flexFlow: 'row wrap', flex: '1 0 50%'} }>
							<div id='ctlEditView_KBDocuments_EditView_ATTACHMENTS_LABEL' className='dataLabel' style={ {width: '30%'} }>
								{ L10n.Term('KBDocuments.LBL_ATTACHMENTS') }
							</div>
							<div id='ctlEditView_KBDocuments_EditView_ATTACHMENTS' className='dataField' style={ {width: '70%'} }>
								{ ATTACHMENTS
								...
								<div>
									<button className='button' onClick={ this._onAddAttachment }>{ L10n.Term('KBDocuments.LBL_ADD_FILE') }</button>
								</div>
							</div>
						</div>
						<div style={ {display: 'flex', flexFlow: 'row wrap', flex: '1 0 50%'} }>
							<div id='ctlEditView_KBDocuments_EditView_IMAGES_LABEL' className='dataLabel' style={ {width: '30%'} }>
								{ L10n.Term('KBDocuments.LBL_IMAGES') }
							</div>
							<div id='ctlEditView_KBDocuments_EditView_IMAGES' className='dataField' style={ {width: '70%'} }>
								{ IMAGES
								? IMAGES.map((attachment, index) => 
								{
									...
									else if ( attachment.FILENAME === undefined )
									{
										...
										<div>
											<input
												type='file'
												onChange={ (e) => this._onImage(e, index) }
											/>
										</div>);
									}
									else
									{
										...
										<div>
											...
											<span style={ {cursor: 'pointer'} } onClick={ (e) => this._onRemoveImage(e, index) }>
												<FontAwesomeIcon icon='minus' size='lg' style={ {marginLeft: '4px'} } />
											</span>
										</div>);
									}
								...
								<div>
									<button className='button' onClick={ this._onAddImage }>{ L10n.Term('KBDocuments.LBL_ADD_FILE') }</button>
								</div>
							</div>
						</div>
					</div>
