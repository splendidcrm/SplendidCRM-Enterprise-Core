...
@observer
class MeetingsDetailView extends React.Component<IDetailViewProps, IDetailViewState>
{
	...
	private Page_Command = async (sCommandName, sCommandArguments) =>
	{
		...
		switch ( sCommandName )
		{
			...
			case 'EditAllRecurrences':
			{
				history.push(`/Reset/${MODULE_NAME}/Edit/${sCommandArguments}`);
				break;
			}
			case 'DeleteRecurrences':
			{
				try
				{
					await DeleteModuleRecurrences(MODULE_NAME, ID);
					history.push(`/Reset/${MODULE_NAME}/List`);
				}
				catch(error)
				{
	...
	{
		const { ID } = this.props;
		const { item, layout, MODULE_NAME, DETAIL_NAME, SUB_TITLE, auditOpen, activitiesOpen, archiveExists, error } = this.state;
		...
		if ( SplendidCache.IsInitialized && layout && item )
		{
			...
			<React.Fragment>
				...
				<DumpSQL SQL={ __sql } />
				<div id="content">
					{ !Sql.IsEmptyGuid(ID) && (!Sql.IsEmptyGuid(item['REPEAT_PARENT_ID']) || !Sql.IsEmptyString(item['REPEAT_TYPE']))
					? <table id='tblEditAllRecurrences' className='tabDetailView'>
						<tr>
							{ !Sql.IsEmptyGuid(item['REPEAT_PARENT_ID'])
							? <td id='tdEditAllRecurrences'>
								&nbsp;<a id='lnkRepeatParent' href='#' onClick={ (e) => { e.preventDefault(); this.Page_Command('EditAllRecurrences', item['REPEAT_PARENT_ID']); } }>{ L10n.Term('Calendar.LBL_EDIT_ALL_RECURRENCES') }</a>
							</td>
							: null
							}
							{ !Sql.IsEmptyString(item['REPEAT_TYPE'])
							? <td id='tdDeleteAllRecurrences'>
								&nbsp;<a id='lnkDeleteRecurrences' href='#' onClick={ (e) => { e.preventDefault(); this.Page_Command('DeleteRecurrences', null); } }>{ L10n.Term('Calendar.LBL_REMOVE_ALL_RECURRENCES') }</a>
							</td>
							: null
							}
						</tr>
...
export default withRouter(MeetingsDetailView);
