...
@observer
export default class MeetingsEditView extends React.Component<IEditViewProps, IEditViewState>
{
	...
	async componentDidUpdate(prevProps: IEditViewProps)
	{
		...
		// 04/09/2021 Paul.  Create framework to allow pre-compile of all modules. 
		else
		{
	...
	private load = async () =>
	{
		...
		try
		{
			...
			if ( Sql.IsEmptyGuid(ID) && Sql.IsEmptyGuid(DuplicateID) && Sql.IsEmptyGuid(ConvertID) )
			{
				...
				// 10/21/2020 Paul.  Default to 1 hour to match ASP.NET code. 
				rowDefaultSearch['DURATION_MINUTES'] = 0;
				rowDefaultSearch['DURATION_HOURS'  ] = 1;
