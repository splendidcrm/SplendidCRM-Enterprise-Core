...
@observer
export default class NotesEditView extends React.Component<IEditViewProps, IEditViewState>
{
	...
	private load = async () =>
	{
		...
		try
		{
			...
			if ( !Sql.IsEmptyGuid(queryParams['CONTACT_ID']) )
			{
				let CONTACT_ID  : string = queryParams['CONTACT_ID'];
				let CONTACT_NAME: string = await Crm_Modules.ItemName('Contacts', CONTACT_ID);
				if ( !Sql.IsEmptyString(CONTACT_NAME) )
				{
					rowDefaultSearch = 
					{
						CONTACT_ID  ,
