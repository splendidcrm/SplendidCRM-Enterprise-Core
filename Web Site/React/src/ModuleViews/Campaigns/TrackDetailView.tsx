...
interface ITrackDetailViewProps extends RouteComponentProps<any>
{
	ID         : string;
...
interface ITrackDetailViewState
{
	__total         : number;
	__sql           : string;
	item            : any;
...
@observer
class TrackDetailView extends React.Component<ITrackDetailViewProps, ITrackDetailViewState>
{
	private _isMounted     : boolean = false;
	private refMap         : Record<string, React.RefObject<DetailComponent<any, any>>>;
	private headerButtons    = React.createRef<HeaderButtons>();

	constructor(props: ITrackDetailViewProps)
	{
		super(props);
	...
	async componentDidUpdate(prevProps: ITrackDetailViewProps)
	{
	...
	private load = async () =>
	{
		...
		try
		{
			...
			if ( status == 1 )
			{
				// 08/03/2019 Paul.  The Campaign status/tracking page shows the standard detail view at the top. 
				const layout = DetailView_LoadLayout(MODULE_NAME + '.DetailView');
	...
	public render()
	{
		...
		if ( SplendidCache.IsInitialized && layout && item )
		{
			...
			<div>
				{ headerButtons
				? React.createElement(headerButtons, { MODULE_NAME, ID, SUB_TITLE, enableFavorites: true, error, enableHelp: true, helpName: 'DetailView', ButtonStyle: 'ModuleHeader', VIEW_NAME: DETAIL_NAME, LINK_NAME: MODULE_NAME + '.TrackLinkView', row: item, Page_Command: this.Page_Command, showButtons: true, showProcess: true, history: this.props.history, location: this.props.location, match: this.props.match, ref: this.headerButtons })
				: null
				}
				<DumpSQL SQL={ __sql } />
				...
				<div id="content">
					...
					<br />
					<ResponseByRecipientActivity ID={ ID } />
					<br />
...
export default withRouter(TrackDetailView);
