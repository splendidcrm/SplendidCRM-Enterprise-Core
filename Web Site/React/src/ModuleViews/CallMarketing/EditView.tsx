...
@observer
export default class CallMarketingEditView extends React.Component<IEditViewProps, IEditViewState>
{
	...
	private load = async () =>
	{
		const { MODULE_NAME, ID, rowDefaultSearch, DuplicateID, ConvertModule, ConvertID } = this.props;
		...
		try
		{
			...
			if ( !Sql.IsEmptyGuid(queryParams['CAMPAIGN_ID']) )
			{
				...
				rowDefaultSearch = await Crm_Modules.LoadParent('Campaigns', this.CAMPAIGN_ID);
				bParentFound = true;
			}
			if ( Sql.IsEmptyGuid(ID) && Sql.IsEmptyGuid(DuplicateID) && Sql.IsEmptyGuid(ConvertID) )
			{
				...
				// 11/13/2020 Paul.  new DynamicControl(this, "DATE_START").DateValue = DateTime.Today.AddHours(12);
				let dtDATE_START: Date = new Date();
				dtDATE_START.setHours(12);
				dtDATE_START.setMinutes(0);
				dtDATE_START.setSeconds(0);
				dtDATE_START.setMilliseconds(0);
				rowDefaultSearch['DATE_START'] = ToJsonDate(dtDATE_START);
			}
	...
	public render()
	{
		...
		if ( SplendidCache.IsInitialized )
		{
			...
			<div style={ {width: '100%'} }>
				...
				? React.createElement(headerButtons, { MODULE_NAME, ID, SUB_TITLE, error, enableHelp: true, helpName: 'DetailView', ButtonStyle: 'EditHeader', VIEW_NAME: EDIT_NAME, row: item, Page_Command: this.Page_Command, showButtons: !isSearchView && !isUpdatePanel, history: this.props.history, location: this.props.location, match: this.props.match, ref: this.headerButtons })
				: null
				}
				<DumpSQL SQL={ __sql } />
