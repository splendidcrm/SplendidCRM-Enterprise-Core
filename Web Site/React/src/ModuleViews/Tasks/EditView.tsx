...
@observer
export default class TasksEditView extends React.Component<IEditViewProps, IEditViewState>
{
				...
				case 'SaveConcurrency':
				{
					...
					if ( nInvalidFields == 0 )
					{
						...
						try
						{
							...
							else if ( !Sql.IsEmptyGuid(this.CONTACT_ID) )
							{
								history.push(`/Reset/Contacts/View/${this.CONTACT_ID}`);
							}
							else
							{
	...
	{
		if ( DATA_FIELD == 'PARENT_ID' )
		{
			//console.log((new Date()).toISOString() + ' ' + this.constructor.name + '._onFieldDidMount ' + DATA_FIELD, this.refMap['PARENT_ID']);
			let LIST_VALUES      : any[]   = [];
			...
			if ( arrLIST != null )
			{
				...
				{
					// 01/08/2020 Paul.  Contacts are excluded from the list as there is a separate CONTACT_ID field. 
					if ( arrLIST[i] == 'Contacts' )
						continue;
					let opt4 = { key: arrLIST[i], text: L10n.ListTerm(LIST_NAME, arrLIST[i]) };
					LIST_VALUES.push(opt4);
				}
			}
			component.updateDependancy(null, LIST_VALUES, 'LIST_VALUES', null);
		}
	}
