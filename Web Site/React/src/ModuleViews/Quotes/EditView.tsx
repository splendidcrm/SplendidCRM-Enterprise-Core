...
import QuotesLineItems                        from './QuotesLineItems'                         ;
...
@observer
export default class QuotesEditView extends React.Component<IEditViewProps, IEditViewState>
{
	...
	private lineItems     = React.createRef<QuotesLineItems>();
	...
	private load = async () =>
	{
		...
		try
		{
			...
			if ( Sql.IsEmptyGuid(ID) && Sql.IsEmptyGuid(DuplicateID) && Sql.IsEmptyGuid(ConvertID) && Sql.IsEmptyGuid(this.OPPORTUNITY_ID) )
			{
				...
				// 04/29/2007 Paul.  Default expected closed to 1 month away. 
				rowDefaultSearch['DATE_QUOTE_EXPECTED_CLOSED'] = mnt.toDate();
			}
	...
	private _onFieldDidMount = (DATA_FIELD: string, component: any): void =>
	{
		...
		try
		{
			if ( item )
			{
				...
				else if ( DATA_FIELD == 'SHIPPING_CONTACT_ID' )
				{
					let rowDefaultSearch: any =
					{
	...
	public render()
	{
		...
		// 02/04/2020 Paul.  Must include the other conversions, OPPORTUNITY_ID. 
		if ( layout == null || (item == null && (!Sql.IsEmptyGuid(ID) || !Sql.IsEmptyGuid(DuplicateID) || !Sql.IsEmptyGuid(ConvertID) || !Sql.IsEmptyGuid(this.OPPORTUNITY_ID))) )
		{
			if ( error )
			{
		...
		if ( SplendidCache.IsInitialized )
		{
			...
			<div style={ {width: '100%'} }>
				...
				<QuotesLineItems ID={ ID } row={ item } onChanged={ this._onChange } ref={ this.lineItems } />
