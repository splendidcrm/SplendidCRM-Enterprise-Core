...
@observer
class PaymentsDetailView extends React.Component<IDetailViewProps, IDetailViewState>
{
		...
		try
		{
			...
			if ( this._isMounted )
			{
				...
				if ( sMODULE_NAME == 'Quotes' || sMODULE_NAME == 'Orders' || sMODULE_NAME == 'Invoices' || sMODULE_NAME == 'Payments' )
				{
					try
					{
						let sSUMMARY_NAME = DETAIL_NAME.replace('.DetailView', '.SummaryView');
						const summaryLayout = DetailView_LoadLayout(sSUMMARY_NAME);
						this.setState({ summaryLayout: summaryLayout });
...
export default withRouter(PaymentsDetailView);
