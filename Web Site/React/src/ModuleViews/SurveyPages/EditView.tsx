...
@observer
export default class SurveyPagesEditView extends React.Component<IEditViewProps, IEditViewState>
{
	...
	private PARENT_TYPE  : string = 'Surveys';
	...
	private load = async () =>
	{
		...
		try
		{
			...
			{
				this.PARENT_ID = queryParams['SURVEY_ID'];
				if ( !Sql.IsEmptyGuid(queryParams['SURVEY_ID']) )
				{
					const d = await EditView_LoadItem('Surveys', this.PARENT_ID);
					let item: any = d.results;
					if ( item != null )
					{
						rowDefaultSearch = {};
						rowDefaultSearch.SURVEY_ID   = item['ID'  ];
						rowDefaultSearch.SURVEY_NAME = item['NAME'];
					}
					else
					{
						this.setState( {error: 'Survey ID [' + this.PARENT_ID + '] was not found.'} );
					}
				}
				else
				{
					this.setState( {error: 'Survey ID was not provided.'} );
				}
			}
