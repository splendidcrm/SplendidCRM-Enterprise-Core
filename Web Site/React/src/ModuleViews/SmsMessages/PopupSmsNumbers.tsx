...
class PopupSmsNumbers extends React.Component<IPopupViewProps, IPopupViewState>
{
	...
	private _onHyperLinkCallback = (MODULE_NAME: string, ID: string, NAME: string, URL: string, row?: any) =>
	{
		...
		callback({ Action: 'SingleSelect', ID, NAME, PHONE_MOBILE });
	}
	...
	private Load = async (sMODULE_NAME, sSORT_FIELD, sSORT_DIRECTION, sSELECT, sFILTER, rowSEARCH_VALUES, nTOP, nSKIP, bADMIN_MODE?, archiveView?) =>
	{
		...
		{
			let dContacts = await ListView_LoadTablePaginated('vwCONTACTS_SmsNumbers', sSORT_FIELD, sSORT_DIRECTION, sSELECT, sFILTER, rowSEARCH_VALUES, nTOP, nSKIP, bADMIN_MODE, archiveView);
			...
			if ( dContacts.results != null )
			{
				...
				{
					dContacts.results[i]['MODULE_TYPE'] = 'Contacts';
		...
		{
			let dLeads = await ListView_LoadTablePaginated('vwLEADS_SmsNumbers', sSORT_FIELD, sSORT_DIRECTION, sSELECT, sFILTER, rowSEARCH_VALUES, nTOP, nSKIP, bADMIN_MODE, archiveView);
			...
			if ( dLeads.results != null )
			{
				...
				{
					dLeads.results[i]['MODULE_TYPE'] = 'Leads';
		...
		{
			let dProspects = await ListView_LoadTablePaginated('vwPROSPECTS_SmsNumbers', sSORT_FIELD, sSORT_DIRECTION, sSELECT, sFILTER, rowSEARCH_VALUES, nTOP, nSKIP, bADMIN_MODE, archiveView);
			...
			if ( dProspects.results != null )
			{
				...
				{
					dProspects.results[i]['MODULE_TYPE'] = 'Prospects';
					d.results.push(dProspects.results[i]);
				}
			}
		}
		if ( SplendidCache.GetUserAccess('Users', 'list', this.constructor.name + '.Load') >= 0 )
		{
			let dUsers = await ListView_LoadTablePaginated('vwUSERS_SmsNumbers', sSORT_FIELD, sSORT_DIRECTION, sSELECT, sFILTER, rowSEARCH_VALUES, nTOP, nSKIP, bADMIN_MODE, archiveView);
			d.__total += dUsers.__total;
			d.__sql   += dUsers.__sql + ';';
			if ( dUsers.results != null )
			{
				for ( let i: number = 0; i < dUsers.results.length; i++ )
				{
					dUsers.results[i]['MODULE_TYPE'] = 'Users';
					d.results.push(dUsers.results[i]);
				}
			}
		}
	...
	public render()
	{
		...
		let EDIT_NAME: string = 'SmsMessages.SearchPopup';
			...
			<Modal show={isOpen} onHide={ this._onClose }>
				<Modal.Body style={{ minHeight: '80vh', minWidth: '80vw' }}>
					...
					<SplendidGrid
						...
						GRID_NAME='SmsMessages.PopupSmsNumbers'
						ADMIN_MODE={ false }
...
export default withRouter(PopupSmsNumbers);
