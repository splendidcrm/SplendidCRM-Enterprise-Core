...
@observer
export default class InvoicesEditView extends React.Component<IEditViewProps, IEditViewState>
{
	...
	private lineItems     = React.createRef<InvoicesLineItems>();
	...
	private load = async () =>
	{
		...
		try
		{
			...
			// 02/04/2020 Paul.  Must include the other conversions, ORDER_ID and QUOTE_ID. 
			this.ORDER_ID = Sql.ToGuid(queryParams['ORDER_ID']);
			this.QUOTE_ID = Sql.ToGuid(queryParams['QUOTE_ID']);
			if ( Sql.IsEmptyGuid(ID) && Sql.IsEmptyGuid(DuplicateID) && Sql.IsEmptyGuid(ConvertID) && Sql.IsEmptyGuid(this.QUOTE_ID) && Sql.IsEmptyGuid(this.ORDER_ID) )
			{
			...
			if ( this._isMounted )
			{
				...
				else if ( !Sql.IsEmptyGuid(this.ORDER_ID) )
				{
					await this.ConvertItem(MODULE_NAME, 'Orders', this.ORDER_ID);
				}
		...
		if ( !Sql.IsEmptyString(sID) )
		{
			try
			{
				...
				if ( this._isMounted )
				{
					...
					{
						// 01/03/2022 Paul.  Should clear INVOICE_NUM not ORDER_NUM. 
						...
						if ( item.LineItems )
						{
							for ( let i: number = 0; i < item.LineItems.length; i++ )
							{
								...
								item.LineItems[i].INVOICE_ID = null;
							}
						}
					}
		...
		try
		{
			...
			switch (sCommandName)
			{
				...
				case 'Cancel':
				{
					...
					{
						history.push(`/Reset/Orders/View/${this.ORDER_ID}`);
					}
					else if ( !Sql.IsEmptyGuid(this.QUOTE_ID) )
					{
	...
	public render()
	{
		...
		// 02/04/2020 Paul.  Must include the other conversions, ORDER_ID and QUOTE_ID. 
		if ( layout == null || (item == null && (!Sql.IsEmptyGuid(ID) || !Sql.IsEmptyGuid(DuplicateID) || !Sql.IsEmptyGuid(ConvertID) || !Sql.IsEmptyGuid(this.QUOTE_ID) || !Sql.IsEmptyGuid(this.ORDER_ID))) )
		{
			if ( error )
			{
		...
		if ( SplendidCache.IsInitialized )
		{
			...
			<div style={ {width: '100%'} }>
				...
				<InvoicesLineItems ID={ ID } row={ item } onChanged={ this._onChange } ref={ this.lineItems } />
