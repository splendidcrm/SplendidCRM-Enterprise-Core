...
@observer
export default class ConfigEditView extends React.Component<IAdminEditViewProps, IAdminEditViewState>
{
	...
	private _onFieldDidMount = (DATA_FIELD: string, component: any): void =>
	{
		...
		try
		{
			if ( !Sql.IsEmptyString(ID) )
			{
				// 03/18/2021 Paul.  The NAME field is read-only for existing config values. 
				if ( DATA_FIELD == 'NAME' )
				{
