...
const MODULE_NAME: string = 'Exchange';
const OUTLOOK_COM: string = 'https://outlook.office.com/';
...
@observer
export default class ExchangeConfigView extends React.Component<IExchangeConfigViewProps, IExchangeConfigViewState>
{
	...
	constructor(props: IExchangeConfigViewProps)
	{
		...
		let BUTTON_NAME : string = MODULE_NAME + '.EditView'  ;
		let MODULE_TITLE: string = L10n.Term(MODULE_NAME + '.LBL_EXCHANGE_SETTINGS');
	...
	private LoadItem = async (layout: any) =>
	{
		try
		{
			...
			// 11/02/2019 Paul.  Exchange.AuthenticationMethod is computed and not stored. 
			let bOAuthEnabled: boolean = !Sql.IsEmptyString(item['Exchange.ClientID']);
			if ( Sql.IsEmptyString(item['Exchange.ClientID']) && Sql.IsEmptyString(item['Exchange.UserName']) )
			{
				bOAuthEnabled = true;
			}
			item['Exchange.AuthenticationMethod'] = (bOAuthEnabled ? 'oauth' : 'username');
			if ( layout.length > 0 )
			{
				for ( let nLayoutIndex = 0; nLayoutIndex < layout.length; nLayoutIndex++ )
				{
					...
					switch ( DATA_FIELD )
					{
						case 'Exchange.ClientID'    :  lay.hidden = !bOAuthEnabled;  break;
						case 'Exchange.ClientSecret':  lay.hidden = !bOAuthEnabled;  break;
						case 'Exchange.UserName'    :  lay.hidden =  bOAuthEnabled;  break;
						case 'Exchange.Password'    :  lay.hidden =  bOAuthEnabled;  break;
					}
				}
			}
			if ( this._isMounted )
			{
				...
				if ( bOAuthEnabled )
				{
					...
					else if ( !Sql.IsEmptyString(queryParams['code']) )
					{
						...
						let res  = await CreateSplendidRequest('Administration/Exchange/Rest.svc/Authorize', 'POST', 'application/octet-stream', sBody);
						let json = await GetSplendidResult(res);
						//console.log((new Date()).toISOString() + ' ' + this.constructor.name + '.Save token', json);
						this.setState({ error: json.d });
						//this.props.history.replace('/Administration/Exchange/ConfigView');
	...
	private _onLayoutLoadedTop = () =>
	{
		...
		let bOAuthEnabled: boolean = (item['Exchange.AuthenticationMethod'] == 'oauth');
		if ( this.dynamicButtonsTop.current != null )
		{
			this.dynamicButtonsTop.current.ShowButton('Authorize', bOAuthEnabled);
		}
	...
	private _onChange = (DATA_FIELD: string, DATA_VALUE: any, DISPLAY_FIELD?: string, DISPLAY_VALUE?: any): void =>
	{
		...
		if ( DATA_FIELD == 'Exchange.AuthenticationMethod' )
		{
			let bOAuthEnabled: boolean = (DATA_VALUE == 'oauth');
			for ( let nLayoutIndex = 0; nLayoutIndex < layout.length; nLayoutIndex++ )
			{
				let lay = layout[nLayoutIndex];
				let DATA_FIELD = lay.DATA_FIELD;
	...
	private _onUpdate = (PARENT_FIELD: string, DATA_VALUE: any, item?: any): void =>
	{
		...
		// 11/04/2019  Paul.  The following are inherent dependencies. 
		if ( PARENT_FIELD == 'Exchange.PushNotifications' )
		{
			Object.keys(this.refMap).map((key) =>
			{
				let ref = this.refMap[key];
				if ( key == 'Exchange.PushNotificationURL' )
				{
					if ( DATA_VALUE )
					{
						if ( Sql.IsEmptyString(ref.current.data.value) )
						{
							let url: string = Crm_Config.SiteURL() + 'ExchangeService2007.asmx';
							ref.current.updateDependancy(PARENT_FIELD, url, 'value', null);
						}
					}
				}
			});
	...
	private Save = async() =>
	{
		...
		let row: any = {};
		let bOAuthEnabled: boolean = false;
		...
		Object.keys(this.refMap).map((key) =>
		{
			...
			{
				// 11/02/2019 Paul.  Exchange.AuthenticationMethod is computed and not stored. 
				if ( data.key == 'Exchange.AuthenticationMethod' )
				{
					bOAuthEnabled = (data.value == 'oauth');
				}
		...
		if ( bOAuthEnabled )
		{
			row['Exchange.UserName'    ] = null;
			row['Exchange.Password'    ] = null;
			// 12/29/2020 Paul.  Don't clear the secret if it is empty as it is never returned to the client anyway. 
			if ( Sql.IsEmptyString(row['Exchange.ClientSecret']) )
			{
				delete row['Exchange.ClientSecret'];
			}
		}
		else
		{
			row['Exchange.ClientID'    ] = null;
			row['Exchange.ClientSecret'] = null;
		}
		try
		{
			if ( this.dynamicButtonsTop.current != null )
	...
	private Page_Command = async (sCommandName, sCommandArguments) =>
	{
		...
		try
		{
			...
			switch (sCommandName)
			{
				...
				case 'Authorize':
				{
					...
					if ( this.Save() )
					{
						...
						try
						{
							...
							let client_id      : string = item['Exchange.ClientID' ];
				...
				case 'TestPushURL':
				{
					let url = SplendidDynamic_EditView.GetRefValue(this.refMap, 'Exchange.PushNotificationURL');
				...
				case 'UseOffice365':
				{
					SplendidDynamic_EditView.SetRefValue(this.refMap, 'Exchange.ServerURL'           , 'https://outlook.office365.com/EWS/Exchange.asmx');
					SplendidDynamic_EditView.SetRefValue(this.refMap, 'Exchange.AuthenticationMethod', 'oauth'                                          );
					break;
				}
				case 'Cancel':
				{
