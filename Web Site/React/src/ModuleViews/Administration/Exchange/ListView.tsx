...
@observer
class ExchangeListView extends React.Component<IAdminListViewProps, IAdminListViewState>
{
	...
	private _onUpdateComplete = (sCommandName) =>
	{
		...
		if ( this.searchView.current != null )
		{
			...
			if ( sCommandName == 'MassDisable' || sCommandName == 'MassEnable' )
			{
				if ( this.splendidGrid.current != null )
				{
	...
	private _onHyperLinkCallback = (MODULE_NAME: string, ID: string, NAME: string, URL: string, row?: any) =>
	{
		...
		// 03/31/2021 Paul.  URL_VALUE may be null for users not enabled. 
		else if ( ID != null )
		{
			let admin: string = '';
	...
	private Load = async (sMODULE_NAME, sSORT_FIELD, sSORT_DIRECTION, sSELECT, sFILTER, rowSEARCH_VALUES, nTOP, nSKIP, bADMIN_MODE?, archiveView?) =>
	{
		...
		{
			sSORT_FIELD     = 'USER_NAME';
			sSORT_DIRECTION = 'asc';
		}
		let arrSELECT: string[] = sSELECT.split(',');
		if ( arrSELECT.indexOf('ASSIGNED_USER_ID') < 0 )
		{
			arrSELECT.push('ASSIGNED_USER_ID');
		}
		if ( arrSELECT.indexOf('USER_ID') < 0 )
		{
			arrSELECT.push('USER_ID');
		}
		// 03/31/2021 Paul.  STATUS is not a valid field name, it is computed below. 
		arrSELECT = arrSELECT.filter(s => s !== 'STATUS'); 
		
		sSELECT = arrSELECT.join(',');
		sFILTER = Sql.ToString(sFILTER);
		let ControlChars = { CrLf: '\r\n' };
		let sIMPERSONATED_TYPE: string = Crm_Config.ToString('Exchange.ImpersonatedType');
		if ( sIMPERSONATED_TYPE == 'SmtpAddress' )
			sFILTER += 'EMAIL1 is not null' + ControlChars.CrLf;
		else if ( sIMPERSONATED_TYPE == 'PrincipalName' )
			sFILTER += 'USER_NAME is not null' + ControlChars.CrLf;
		...
		else if ( sIMPERSONATED_TYPE == 'NoImpersonation' )
		{
			...
			if ( !Sql.IsEmptyString(Crm_Config.ToString('Exchange.ClientID')) )
				sFILTER += 'EMAIL1 is not null' + ControlChars.CrLf;
			else
				sFILTER += 'MAIL_SMTPUSER is not null' + ControlChars.CrLf;
		}
		let d = await ListView_LoadTablePaginated('EXCHANGE_USERS', sSORT_FIELD, sSORT_DIRECTION, sSELECT, sFILTER, rowSEARCH_VALUES, nTOP, nSKIP, bADMIN_MODE, archiveView);
		if ( d.results )
		{
			for ( let i: number = 0; i < d.results.length; i++ )
			{
				let row: any = d.results[i];
				// 03/31/2021 Paul.  We need to treat the USER_ID as the ID for mass update to work properly. 
				row['ID'    ] = row['USER_ID'];
				row['STATUS'] = !Sql.IsEmptyString(row['ASSIGNED_USER_ID']) ? L10n.Term('Exchange.LBL_ENABLED') : L10n.Term('Exchange.LBL_DISABLED');
			}
		}
		return d;
	}
	...
	public templateColumnFormatter = (cell, row, rowIndex, formatExtraData) =>
	{
		if ( this.splendidGrid.current != null )
		{
			let bENABLED: boolean = !Sql.IsEmptyString(row['ASSIGNED_USER_ID']);
			if ( bENABLED )
			{
				return (<span style={ {fontWeight: 'bold'} }>
					{ this.splendidGrid.current.templateColumnFormatter(cell, row, rowIndex, formatExtraData) }
				</span>);
			}
			else
			{
				return this.splendidGrid.current.templateColumnFormatter(cell, row, rowIndex, formatExtraData);
			}
		}
		return null;
	}
	...
	public boundColumnFormatter = (cell, row, rowIndex, formatExtraData) =>
	{
		if ( this.splendidGrid.current != null )
		{
			...
			{
				return (<span style={ {fontWeight: 'bold'} }>
					{ this.splendidGrid.current.boundColumnFormatter(cell, row, rowIndex, formatExtraData) }
				</span>);
			}
			else
			{
				return this.splendidGrid.current.boundColumnFormatter(cell, row, rowIndex, formatExtraData);
			}
		}
		return null;
	}
	...
	private onChangeStatus = async (row: any, ENABLE: boolean) =>
	{
		...
		try
		{
			...
			let res = await CreateSplendidRequest('Administration/Exchange/Rest.svc/Enable', 'POST', 'application/octet-stream', sBody);
			...
			return (
				<span style={ { whiteSpace: 'nowrap'} }>
					...
					? <button
						...
						onClick={ (e) => { e.preventDefault(); this.onChangeStatus(row, bENABLED ? false : true); }  }
						className='button'
						title={ Sql.IsEmptyString(row['ASSIGNED_USER_ID']) ? L10n.Term('Exchange.LBL_DISABLE') : L10n.Term('Exchange.LBL_ENABLE') }
						>
						<FontAwesomeIcon icon={ Sql.IsEmptyString(row['ASSIGNED_USER_ID']) ? 'minus' : 'plus' } size='1x' />
					</button>
					: <input           onClick={ (e) => { e.preventDefault(); this.onChangeStatus(row, bENABLED ? false : true); } } className='listViewTdToolsS1' type='image' src={ this.themeURL + (bENABLED ? 'minus_inline.gif' : 'plus_inline.gif') } style={ {borderWidth: '0px', height: '16px', width: '16px', margin: '2px', verticalAlign: 'middle'} } />
					}
					<a                 onClick={ (e) => { e.preventDefault(); this.onChangeStatus(row, bENABLED ? false : true); } } className='listViewTdToolsS1' style={ {cursor: 'pointer', fontWeight: (bENABLED ? 'bold' : 'normal') } }>{bENABLED ? L10n.Term('Exchange.LBL_DISABLE') : L10n.Term('Exchange.LBL_ENABLE') }</a>
				</span>
			);
		}
		else
	...
	public render()
	{
		...
		if ( SplendidCache.IsInitialized && SplendidCache.AdminMenu )
		{
			...
			<div>
				...
				<DynamicButtons
					key={ MODULE_NAME + '.ListView.buttons' }
					ButtonStyle="MassUpdateHeader"
					VIEW_NAME={ MODULE_NAME + '.ListView' }
					row={ null }
...
export default withRouter(ExchangeListView);
