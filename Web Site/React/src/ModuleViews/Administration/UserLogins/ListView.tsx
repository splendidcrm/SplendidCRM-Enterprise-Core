...
const MODULE_NAME: string = 'UserLogins';
...
@observer
class UserLoginsListView extends React.Component<IAdminListViewProps, IAdminListViewState>
{
	...
	async componentDidMount()
	{
		...
		try
		{
			...
			if ( status == 1 )
			{
				...
				{
					// 04/02/2021 Paul.  Disable mass update panel for UserLogins as there are no buttons.
	...
	private BootstrapColumns = (sLIST_MODULE_NAME, layout, sPRIMARY_MODULE, sPRIMARY_ID) =>
	{
		...
		objDataColumn =
		{
			...
			text           : null,
			dataField      : 'empty1',
			formatter      : this.adminColumnFormatter,
			headerClasses  : 'listViewThS2',
			headerStyle    : {padding: 0, margin: 0},
			headerFormatter: (this.splendidGrid.current != null ? this.splendidGrid.current.renderHeader            : null),
			sort           : false,
	...
	private adminColumnFormatter = (cell, row, rowIndex, formatExtraData) =>
	{
		if ( Sql.ToBoolean(row['IS_ADMIN']) )
		{
			return (
				<img src={ this.themeURL + 'check_inline.gif'} />
			);
		}
		return null;
	}
	...
	private Load = async (sMODULE_NAME, sSORT_FIELD, sSORT_DIRECTION, sSELECT, sFILTER, rowSEARCH_VALUES, nTOP, nSKIP, bADMIN_MODE?, archiveView?) =>
	{
		...
		// 04/14/2021 Paul.  ID is required to export selected items. 
		if ( arrSELECT.indexOf('ID') < 0 )
		{
			arrSELECT.push('ID');
		}
		if ( arrSELECT.indexOf('IS_ADMIN') < 0 )
		{
			arrSELECT.push('IS_ADMIN');
		}
		sSELECT = arrSELECT.join(',');
		let d = await ListView_LoadTablePaginated('USERS_LOGINS', sSORT_FIELD, sSORT_DIRECTION, sSELECT, sFILTER, rowSEARCH_VALUES, nTOP, nSKIP, bADMIN_MODE, archiveView);
		return d;
	}
...
export default withRouter(UserLoginsListView);
