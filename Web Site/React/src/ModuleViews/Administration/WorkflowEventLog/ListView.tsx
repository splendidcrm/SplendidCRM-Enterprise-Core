...
import WorkflowEventPopupView                 from './PopupView'                          ;
...
@observer
class WorkflowEventLogListView extends React.Component<IAdminReadOnlyListViewProps, IAdminReadOnlyListViewState>
{
	...
	private detailView    = React.createRef<WorkflowEventPopupView>();
	...
	private Load = async (sMODULE_NAME, sSORT_FIELD, sSORT_DIRECTION, sSELECT, sFILTER, rowSEARCH_VALUES, nTOP, nSKIP, bADMIN_MODE?, archiveView?) =>
	{
		...
		let d = await ListView_LoadTablePaginated('vwWORKFLOW_RUN_EventLog', sSORT_FIELD, sSORT_DIRECTION, sSELECT, sFILTER, rowSEARCH_VALUES, nTOP, nSKIP, bADMIN_MODE, archiveView);
		return d;
	}
	...
	private viewColumnFormatter = (cell, row, rowIndex, formatExtraData) =>
	{
		if ( !Sql.IsEmptyString(row['WORKFLOW_INSTANCE_ID']) )
		{
			return (<a style={ {cursor: 'pointer', padding: '3px', textDecoration: 'none', whiteSpace: 'nowrap'} } onClick={ () => this._onViewDetails(row) } title={ L10n.Term('.LNK_VIEW') } className='listViewTdToolsS1'>
	...
	private BootstrapColumns = (sLIST_MODULE_NAME, layout, sPRIMARY_MODULE, sPRIMARY_ID) =>
	{
		...
		// 03/10/2021 Paul.  Add column for workflow instance popup. 
		objDataColumn =
		{
			key            : 'editview',
			text           : null,
			dataField      : 'empty2',
			formatter      : this.viewColumnFormatter,
			...
			formatExtraData: {
				data: {
					GRID_NAME : sLIST_MODULE_NAME,
					DATA_FIELD: 'WORKFLOW_INSTANCE_ID',
					fnRender  : null,
					layout    : layout
				}
			}
	...
	public render()
	{
		...
		if ( SplendidCache.IsInitialized && SplendidCache.AdminMenu )
		{
			...
			return (
			<div>
				<WorkflowEventPopupView
					key={ 'WorkflowEventPopupView_' + WORKFLOW_INSTANCE_ID }
					isOpen={ detailsOpen }
...
export default withRouter(WorkflowEventLogListView);
