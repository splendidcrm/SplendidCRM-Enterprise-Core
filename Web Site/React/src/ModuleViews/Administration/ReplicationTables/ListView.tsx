...
@observer
class ReplicationTablesView extends React.Component<IAdminListViewProps, IAdminListViewState>
{
	...
	private Grid_Command = async (sCommandName: string, sCommandArguments: any) =>
	{
		...
		else if ( sCommandName == 'UpdateStats' )
		{
			...
			try
			{
				if ( checkedCount > 0 )
				{
					...
					{
						arrNAME_LIST.push(id);
					}
					const sBody: string = JSON.stringify({ NAME_LIST: arrNAME_LIST });
					let res = await CreateSplendidRequest('Administration/ReplicationTables/Rest.svc/BulkUpdateStats', 'POST', 'application/octet-stream', sBody);
		...
		else if ( sCommandName == 'RunSync' )
		{
			...
			try
			{
				if ( checkedCount > 0 )
				{
					...
					const sBody: string = JSON.stringify({ NAME_LIST: arrNAME_LIST });
					let res = await CreateSplendidRequest('Administration/ReplicationTables/Rest.svc/BulkRunSync', 'POST', 'application/octet-stream', sBody);
					let json = await GetSplendidResult(res);

					this.setState({ error: L10n.Term('.LBL_BACKGROUND_OPERATION'), busy: false });
				}
			}
			catch(error)
			{
	...
	private BootstrapColumns = (sLIST_MODULE_NAME, layout, sPRIMARY_MODULE, sPRIMARY_ID) =>
	{
		...
		objDataColumn =
		{
			...
			text           : null,
			dataField      : 'empty1',
			formatter      : this.updateStatsColumnFormatter,
			headerClasses  : 'listViewThS2',
			headerStyle    : {padding: 0, margin: 0},
		...
		objDataColumn =
		{
			...
			text           : null,
			dataField      : 'empty1',
			formatter      : this.runSyncColumnFormatter,
			headerClasses  : 'listViewThS2',
			headerStyle    : {padding: 0, margin: 0},
			headerFormatter: null,
			sort           : false,
	...
	private onUpdateStats = async (row) =>
	{
		try
		{
			this.setState({ error: null, busy: true });
			let res = await CreateSplendidRequest('Administration/ReplicationTables/Rest.svc/UpdateStats?TABLE_NAME=' + row['TABLE_NAME'], 'POST', 'application/octet-stream', null);
			let json = await GetSplendidResult(res);
			this.setState({  busy: false });
		}
		catch(error)
	...
	private onRunSync = async (row) =>
	{
		try
		{
			this.setState({ error: null, busy: true });
			let res = await CreateSplendidRequest('Administration/ReplicationTables/Rest.svc/RunSync?TABLE_NAME=' + row['TABLE_NAME'], 'POST', 'application/octet-stream', null);
			let json = await GetSplendidResult(res);
			if ( this.searchView.current != null )
			{
				this.searchView.current.SubmitSearch();
	...
	private updateStatsColumnFormatter = (cell, row, rowIndex, formatExtraData) =>
	{
		...
		return (
			<span style={ { whiteSpace: 'nowrap'} }>
				{ nEDIT_ACLACCESS >= 0
				? <a href='#' onClick={ (e) => { e.preventDefault(); this.onUpdateStats(row); } } className='listViewTdToolsS1'>
					<img src={ this.themeURL + 'rightarrow.gif'} alt={ L10n.Term('ReplicationTables.LBL_UPDATE_STATS') } style={ {padding: '3px', width: '24px', height: '24px', borderWidth: '0px'} } />
					{ L10n.Term('ReplicationTables.LBL_UPDATE_STATS') }
				</a>
				: null
				}
			</span>
	...
	private runSyncColumnFormatter = (cell, row, rowIndex, formatExtraData) =>
	{
		let nEDIT_ACLACCESS  : number = SplendidCache.AdminUserAccess(MODULE_NAME, 'edit');
		// 03/17/2021 Paul.  We typcially disable if not subscribed to CurrencyLayer, but we don't have access to this key in the React client. 
		if ( Sql.IsEmptyString(Crm_Config.ToString('CurrencyLayer.AccessKey')) )
		...
		return (
			<span style={ { whiteSpace: 'nowrap'} }>
				{ nEDIT_ACLACCESS >= 0
				? <a href='#' onClick={ (e) => { e.preventDefault(); this.onRunSync(row); } } className='listViewTdToolsS1'>
					<img src={ this.themeURL + 'rightarrow.gif'} alt={ L10n.Term('ReplicationTables.LBL_RUN_SYNC') } style={ {padding: '3px', width: '24px', height: '24px', borderWidth: '0px'} } />
					{ L10n.Term('ReplicationTables.LBL_RUN_SYNC') }
				</a>
				: null
				}
			</span>
	...
	public render()
	{
		...
		const { error, searchLayout, advancedLayout, searchTabsEnabled, duplicateSearchEnabled, searchMode, showUpdatePanel, enableMassUpdate, showSearchView, busy } = this.state;
		...
		if ( SplendidCache.IsInitialized && SplendidCache.AdminMenu )
		{
			...
			<div>
				...
				<SplendidGrid
					...
					SELECTION_FIELD_NAME="TABLE_NAME"
				/>
				{ showUpdatePanel
				? <DynamicMassUpdate
...
export default withRouter(ReplicationTablesView);
