...
const DraggableRow: FC<IDraggableRowProps> = memo(function DraggableRow(props: IDraggableRowProps)
{
	...
	const { id, className, index, length, children, moveDraggableRow, removeRow, dropComplete, addSourceRow, moveDraggableItem, addSourceItem } = props;
	...
	(
		...
		[id, index, moveDraggableRow, removeRow, dropComplete],
	);
	...
	(
		...
		[id, index, length, moveDraggableRow, addSourceRow, moveDraggableItem, addSourceItem],
	);
	...
	return (
			<tr ref={ (node) => connectDragSource(connectDropTarget(node)) }
				...
				<td style={ {...style} }>
					<img src={ themeURL + 'blank.gif' } width={ 20 } height={ 1 } /><br />
					<FontAwesomeIcon className='hiddenHoverItem' icon='sort' size='2x' color='#5A94CE' style={ {paddingLeft: '4px'} } />
				</td>
				{ children }
				<td>
					<img src={ themeURL + 'blank.gif' } width={ 1 } height={ 28 } /><br />
				</td>
			</tr>
	);
});
