...
@observer
class ACLRolesUsers extends React.Component<ISubPanelViewProps, ISubPanelViewState>
{
	...
	constructor(props: ISubPanelViewProps)
	{
		...
		// 01/19/2021 Paul.  The Relationship data may not define sort or primary fields. 
		...
		this.state =
		{
			...
			SORT_FIELD       : 'FULL_NAME', // props.layout.SORT_FIELD,
			SORT_DIRECTION   : 'asc',     // props.layout.SORT_DIRECTION,
			PRIMARY_FIELD    : 'ROLE_ID', // props.layout.PRIMARY_FIELD,
	...
	private Load = async (sMODULE_NAME, sSORT_FIELD, sSORT_DIRECTION, sSELECT, sFILTER, rowSEARCH_VALUES, nTOP, nSKIP, bADMIN_MODE?, archiveView?) =>
	{
		...
		// 01/19/2021 Paul.  The USER_ID field is not normally returned as part of the layout, so we have to add manually. 
		...
		// 01/19/2021 Paul.  Force admin mode. 
		bADMIN_MODE = true;
			...
			return (
				<React.Fragment>
					...
					<Appear onAppearOnce={ (ioe) => this.setState({ subPanelVisible: true }) }>
						{ headerButtons
						? React.createElement(headerButtons, { MODULE_NAME, ID: null, MODULE_TITLE, CONTROL_VIEW_NAME, error, ButtonStyle: 'ListHeader', VIEW_NAME: GRID_NAME, row: item, Page_Command: this.Page_Command, showButtons: !showInlineEdit, onToggle: this.onToggleCollapse, onLayoutLoaded: this._onButtonsLoaded, history: this.props.history, location: this.props.location, match: this.props.match, ref: this.headerButtons })
						: null
						}
					</Appear>
...
export default withRouter(ACLRolesUsers);
