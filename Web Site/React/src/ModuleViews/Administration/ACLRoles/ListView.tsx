...
@observer
class ACLRolesListView extends React.Component<IAdminListViewProps, IAdminListViewState>
{
	...
	private onMakeDefault = async (row: any) =>
	{
		let sFlagName: string = 'default_role';
		let sValue   : string = (row ? row['ID'] : null);
		let rowConfig: any = { NAME: sFlagName, VALUE: sValue, CATEGORY: 'system'}
		try
		{
			await UpdateModule('Config', rowConfig, null, true);
			SplendidCache.SetConfigValue(sFlagName, sValue);
			if ( this.searchView.current != null )
			{
				this.searchView.current.SubmitSearch();
			}
	...
	private defaultColumnFormatter = (cell, row, rowIndex, formatExtraData) =>
	{
		return (
			<span style={ { whiteSpace: 'nowrap'} }>
				{ row['ID'] == Crm_Config.ToString('default_role')
				? <div>
					<button
						style={ {marginRight: '4px'} }
						onClick={ () => this.onMakeDefault(null) }
						className='button'
						title={ L10n.Term('.LBL_REMOVE_BUTTON_TITLE') }
						>
				...
				: <div>
					<button
						style={ {marginRight: '4px'} }
						onClick={ () => this.onMakeDefault(row) }
						className='button'
	...
	{
		return (<div>{ L10n.Term('.LBL_LIST_DEFAULT') }</div>);
	}
	...
	private BootstrapColumns = (sLIST_MODULE_NAME, layout, sPRIMARY_MODULE, sPRIMARY_ID) =>
	{
		...
		objDataColumn =
		{
			...
			text           : null,
			dataField      : 'empty1',
			formatter      : this.defaultColumnFormatter,
			headerClasses  : 'listViewThS2',
			headerStyle    : {padding: 0, margin: 0},
...
export default withRouter(ACLRolesListView);
