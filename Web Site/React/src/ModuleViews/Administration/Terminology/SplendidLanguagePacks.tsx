...
@observer
class SplendidLanguagePacks extends React.Component<ISubPanelViewProps, ISubPanelViewState>
{
	...
	private Load = async (sMODULE_NAME, sSORT_FIELD, sSORT_DIRECTION, sSELECT, sFILTER, rowSEARCH_VALUES, nTOP, nSKIP, bADMIN_MODE?, archiveView?) =>
	{
		...
		let sBody: string = JSON.stringify(obj);
		let res = await CreateSplendidRequest('Administration/Terminology/Rest.svc/GetSplendidLanguagePacks', 'POST', 'application/octet-stream', sBody);
...
export default withRouter(SplendidLanguagePacks);
