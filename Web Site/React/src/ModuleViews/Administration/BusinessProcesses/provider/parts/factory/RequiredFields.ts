...
/**
 * Get all crm:CrmRequiredFields object for a specific business object
 * @param  {ModdleElement} parent
 * @return {ModdleElement} a crm:CrmRequiredFields object
 */
...
/**
 * Get first crm:CrmRequiredFields object for a specific bpmn:ExtensionElements
 * business object.
...
function getPropertiesElementInsideExtensionElements(extensionElements)
{
	...
	{
		return is(elem, 'crm:CrmRequiredFields');
	});
}
...
export default function(element, bpmnFactory, options)
{
	...
	assign(options,
	{
		addLabel: L10n.Term('BusinessProcesses.LBL_BPMN_MODULE_ADD_REQUIRED_FIELD'),
		...
		addElement: function(element, node)
		{
			...
			if ( !properties )
			{
				properties = elementHelper.createElement('crm:CrmRequiredFields', {}, parent, bpmnFactory);
		...
		removeElement: function(element, node, idx)
		{
			...
			{
				// remove crm:CrmRequiredFields if the last existing property has been removed
				if ( !isExtensionElements(parent) )
				{
				...
				else
				{
					...
					{
						if ( is(value, 'crm:CrmRequiredFields') )
						{
