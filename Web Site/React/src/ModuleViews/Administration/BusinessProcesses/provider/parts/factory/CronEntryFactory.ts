...
var cronControl = function(options)
{
	var resource   = new cron(options);
	...
	resource.get = function(element)
	{
		...
		// Property on the StartEvent element. 
		...
		//console.log('CronEntryFactory get ' + this.modelProperty + ' = ' + res[prop]);
		if ( this.cron == null )
		{
			this.SetCron(res[prop]);
			let txtCamunda: any = document.getElementById('camunda-' + resource.id);
			// 07/04/2016 Paul.  jQuery change/trigger is not working. 
			//$('#' + txtCamunda.id).change();
			//$('#' + txtCamunda.id).trigger('change');
	...
	resource.set = function(element, values)
	{
		...
		//console.log('CronEntryFactory set ' + prop + ' = ' + res[prop]);
