...
import DumpXML                                        from '../../../components/DumpXML'                  ;
import DumpXOML                                       from '../../../components/DumpXOML'                 ;

let bDebug: boolean = true;
...
@observer
class BusinessProcessesDetailView extends React.Component<IDetailViewProps, IAdminDetailViewState>
{
	...
	{
		const { ID } = this.props;
		const { item, layout, MODULE_NAME, DETAIL_NAME, SUB_TITLE, auditOpen, error, SVG } = this.state;
			...
			<React.Fragment>
				...
				<div id="content">
					...
					<div style={ {textAlign: 'center'} }>
						<div id="litSVG" dangerouslySetInnerHTML={ { __html: SVG } }></div>
					</div>
					<br />
					...
					<br />
					{ bDebug
					? <div id="divSQL" style={ {marginBottom: '4px'} }>
						<DumpSQL SQL={ item['FILTER_SQL']} />
					</div>
					: null
					}
					{ bDebug
					? <div id="divBPMN" style={ {marginBottom: '4px'} }>
						<DumpXML XML={ item['BPMN']} default_xml={ false } />
					</div>
					: null
					}
					{ bDebug
					? <div id="divXAML" style={ {marginBottom: '4px'} }>
						<DumpXOML XOML={ item['XAML']} default_xoml={ false } />
					</div>
					: null
					}
				</div>
...
export default withRouter(BusinessProcessesDetailView);
