...
@observer
class BusinessProcessesListView extends React.Component<IAdminListViewProps, IAdminListViewState>
{
	...
	private exportColumnFormatter = (cell, row, rowIndex, formatExtraData) =>
	{
		...
		return (<span>
			...
			? <span>
				...
				<span style={ {cursor: 'pointer', padding: '3px', textDecoration: 'none'} } onClick={ () => this._onExportBusinessProcessBPMN(row) } className='listViewTdToolsS1'>{ L10n.Term('BusinessProcesses.LBL_EXPORT_BPMN') }</span>
			</span>
			: null
			}
			{ Security.IS_ADMIN() && nEXPORT_ACLACCESS >= 0
			? <span>
				<img src={ this.themeURL + 'export.gif' } style={ {borderWidth: '0px', height: '10px', width: '10px', paddingLeft: '4px'} } />
				<span style={ {cursor: 'pointer', padding: '3px', textDecoration: 'none'} } onClick={ () => this._onExportBusinessProcessSQL(row) } className='listViewTdToolsS1'>{ L10n.Term('BusinessProcesses.LBL_EXPORT_SQL') }</span>
			</span>
			: null
			}
			...
			? <span>
				...
				<span style={ {cursor: 'pointer', padding: '3px', textDecoration: 'none'} } onClick={ () => this._onDeleteBusinessProcess(row) } className='listViewTdToolsS1'>{ L10n.Term('.LNK_DELETE') }</span>
			</span>
			: null
			}
		</span>);
	...
	private _onDeleteBusinessProcess = async (row) =>
	{
		try
		{
			if ( window.confirm(L10n.Term('.NTC_DELETE_CONFIRMATION')) )
			{
				let MODULE_NAME: string = 'BusinessProcesses';
		...
		{
			console.error((new Date()).toISOString() + ' ' + this.constructor.name + '._onDeleteBusinessProcess', error);
			this.setState({ error });
		}
	}
	...
	private _onExportBusinessProcessBPMN = (row) =>
	{
		window.location.href = Credentials.RemoteServer + 'Administration/BusinessProcesses/export.aspx?id=' + row.ID;
	}

	private _onExportBusinessProcessSQL = (row) =>
	{
		window.location.href = Credentials.RemoteServer + 'Administration/BusinessProcesses/ExportSQL.aspx?id=' + row.ID;
	}
...
export default withRouter(BusinessProcessesListView);
