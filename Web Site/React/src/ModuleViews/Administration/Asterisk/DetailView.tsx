...
const MODULE_NAME: string = 'Asterisk';
...
@observer
class AsteriskDetailView extends React.Component<IDetailViewProps, IAdminDetailViewState>
{
	...
	private DetailView_LoadItem = async (MODULE_NAME: string, ID: string) =>
	{
		let TableName   : string = 'vwCALL_DETAIL_RECORDS';
		let filter      : string = 'ID eq \'' + ID + '\'';
		let res  = await CreateSplendidRequest('Administration/Rest.svc/GetAdminTable?TableName=' + TableName + '&$filter=' + encodeURIComponent(filter) + '&$orderby=ID', 'GET');
	...
	private LoadItem = async (sMODULE_NAME: string, sID: string) =>
	{
		try
		{
			...
			const d = await this.DetailView_LoadItem(sMODULE_NAME, sID);
			if ( this._isMounted )
			{
...
export default withRouter(AsteriskDetailView);
