...
@observer
class SubPanelView extends React.Component<ISubPanelViewProps, ISubPanelViewState>
{
	...
	private BootstrapColumns = (sLIST_MODULE_NAME, layout, sPRIMARY_MODULE, sPRIMARY_ID) =>
	{
		...
		// 07/01/2021 Paul.  View workflow instance data. 
		objDataColumn =
		{
			key            : 'editview',
			text           : null,
			dataField      : 'empty1',
			formatter      : this.viewWorkflowInstanceData,
			headerClasses  : 'listViewThS2',
			headerStyle    : {padding: 0, margin: 0},
			headerFormatter: (this.splendidGrid.current != null ? this.splendidGrid.current.renderHeader : null),
			sort           : false,
	...
	// 11/04/2020 Paul.  Legacy icons means that the remove is on the right. 
	public viewWorkflowInstanceData = (cell, row, rowIndex, formatExtraData) =>
	{
	...
	public render()
	{
		...
		const { detailsOpen, WORKFLOW_INSTANCE_ID } = this.state;
...
export default withRouter(SubPanelView);
