...
@observer
class WorkflowsDetailView extends React.Component<IDetailViewProps, IAdminDetailViewState>
{
	...
	private LoadItem = async (sMODULE_NAME: string, sID: string) =>
	{
		try
		{
			...
			if ( this._isMounted )
			{
				...
				// 07/01/2021 Paul.  Correct boolean issue. workflow_status_dom isues True/False. 
				if ( item != null )
				{
		...
		if ( SplendidCache.IsInitialized && SplendidCache.AdminMenu && layout && item )
		{
			...
			<React.Fragment>
				...
				<div id="content">
					...
					<br />
					<DumpXOML XOML={ item['XOML'] } default_xoml={ false } />
...
export default withRouter(WorkflowsDetailView);
