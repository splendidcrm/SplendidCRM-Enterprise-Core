...
@observer
class WorkflowsActions extends React.Component<ISubPanelViewProps, ISubPanelViewState>
{
	...
	constructor(props: ISubPanelViewProps)
	{
		...
		let GRID_NAME  : string = props.PARENT_TYPE + '.WorkflowActionShells';
		...
		try
		{
			if ( this._isMounted )
			{
				if ( sCommandName == 'Create' || EndsWith(sCommandName, '.Create') )
				{
					...
					// 06/18/2021 Paul.  Inline editing is not enabled. 
...
export default withRouter(WorkflowsActions);
