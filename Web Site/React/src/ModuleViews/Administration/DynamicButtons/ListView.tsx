...
@observer
class DynamicButtonsListView extends React.Component<IAdminListViewProps, IAdminListViewState>
{
	...
	{
		//console.log((new Date()).toISOString() + ' ' + this.constructor.name + '._onSearchViewCallback', row);
		...
		if ( this.splendidGrid.current != null )
		{
			...
			{
				this.setState({ VIEW_NAME: row['VIEW_NAME'].value });
			}
		}
	}
	...
	private _onExport = async (EXPORT_RANGE: string, EXPORT_FORMAT: string) =>
	{
		...
		window.location.href = Credentials.RemoteServer + 'Administration/DynamicButtons/export.aspx?NAME=' + VIEW_NAME;
	}
...
export default withRouter(DynamicButtonsListView);
