...
@observer
class MachineLearningModelsDetailView extends React.Component<IDetailViewProps, IAdminDetailViewState>
{
	...
	private load = async () =>
	{
		...
		try
		{
			...
			if ( status == 1 )
			{
				...
				const layoutEvaluation = DetailView_LoadLayout(MODULE_NAME + '.EvaluationView');
				// 06/19/2018 Paul.  Always clear the item when setting the layout. 
				this.setState({ layout, layoutEvaluation, item: null });
	...
	private LoadItem = async (sMODULE_NAME: string, sID: string) =>
	{
		try
		{
			...
			if ( this._isMounted )
			{
				...
				if ( item != null && !Sql.IsEmptyString(item.EVALUATION_DATA) )
				{
					let evalData: any = JSON.parse(item.EVALUATION_DATA);
					for ( let key in evalData )
					{
						item[key] = evalData[key];
					}
				}
	...
	private Page_Command = async (sCommandName, sCommandArguments) =>
	{
		...
		switch ( sCommandName )
		{
			...
			case 'MachineLearningModels.Train':
			{
				try
				{
					let data: any =
					...
					let res = await CreateSplendidRequest('Administration/MachineLearningModels/Rest.svc/Train', 'POST', 'application/octet-stream', sBody);
					let json = await GetSplendidResult(res);
					this.setState({ error: L10n.Term("MachineLearningModels.LBL_BUSY_TRAINING") });
				}
				catch(error)
				{
					console.error((new Date()).toISOString() + ' ' + this.constructor.name + '.Page_Command ' + sCommandName, error);
			...
			case 'MachineLearningModels.Evaluate':
			{
				try
				{
					let data: any =
					...
					let sBody: string = JSON.stringify(data);
					let res = await CreateSplendidRequest('Administration/MachineLearningModels/Rest.svc/Evaluate', 'POST', 'application/octet-stream', sBody);
					let json = await GetSplendidResult(res);
					this.setState({ error: L10n.Term("MachineLearningModels.LBL_BUSY_EVALUATING") });
				}
				catch(error)
				{
					console.error((new Date()).toISOString() + ' ' + this.constructor.name + '.Page_Command ' + sCommandName, error);
			...
			case 'MachineLearningModels.Predict':
			{
...
private _onSelect = async (value: { Action: string, ID: string, NAME: string, selectedItems: any }) =>
	{
		...
		if ( value.Action == 'SingleSelect' )
		{
			try
			{
				let RECORD_LIST: string[] = [value.ID];
				let data: any =
				{
					ID,
					RECORD_LIST,
				...
				let sBody: string = JSON.stringify(data);
				let res = await CreateSplendidRequest('Administration/MachineLearningModels/Rest.svc/Predict', 'POST', 'application/octet-stream', sBody);
				let json = await GetSplendidResult(res);
				if ( this._isMounted )
				{
					this.setState({ popupOpen: false, error: L10n.Term("MachineLearningModels.LBL_SUCCESS") });
				}
			}
			catch(error)
			{
		...
		else if ( value.Action == 'MultipleSelect' )
		{
			try
			{
				...
				for ( let sRECORD_ID in value.selectedItems )
				{
					RECORD_LIST.push(sRECORD_ID);
				}
				let data: any =
				{
					ID,
	...
	{
		const { ID } = this.props;
		const { item, layout, layoutEvaluation, MODULE_NAME, DETAIL_NAME, SUB_TITLE, auditOpen, popupOpen, error } = this.state;
		...
		if ( SplendidCache.IsInitialized && SplendidCache.AdminMenu && layout && item )
		{
			...
			<React.Fragment>
				...
				<div id="content">
					...
					<br />
					{ !Sql.IsEmptyString(item.EVALUATION_DATA)
					? <React.Fragment>
						<h4>{ L10n.Term('MachineLearningModels.LBL_EVALUATION_INFORMATION') }</h4>
						{ SplendidDynamic_DetailView.AppendDetailViewFields(item, layoutEvaluation, this.refMap, 'tabDetailView', null, this.Page_Command) }
						<br />
					</React.Fragment>
					: null
					}
...
export default withRouter(MachineLearningModelsDetailView);
