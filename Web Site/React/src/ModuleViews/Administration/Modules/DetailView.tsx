...
@observer
class ModulesDetailView extends React.Component<IDetailViewProps, IAdminDetailViewState>
{
		...
		switch ( sCommandName )
		{
			...
			case 'Archive.Build':
			{
				try
				{
					...
					let res = await CreateSplendidRequest('Administration/Rest.svc/BuildModuleArchive', 'POST', 'application/json; charset=utf-8', sBody);
				...
				{
					console.error((new Date()).toISOString() + ' ' + this.constructor.name + '.ModuleArchiveBuild', error);
					this.setState({ error });
					if ( this.headerButtons.current != null )
					{
						this.headerButtons.current.NotBusy();
...
export default withRouter(ModulesDetailView);
