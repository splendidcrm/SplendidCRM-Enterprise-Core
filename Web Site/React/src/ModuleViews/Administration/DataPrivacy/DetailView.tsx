...
@observer
class DataPrivacyDetailView extends React.Component<IDetailViewProps, IAdminDetailViewState>
{
	...
	private LoadItem = async (sMODULE_NAME: string, sID: string) =>
	{
		try
		{
			...
			if ( this._isMounted )
			{
				...
				let res = await CreateSplendidRequest('Administration/DataPrivacy/Rest.svc/GetErasedCount?ID=' + sID, 'GET');
				let json = await GetSplendidResult(res);
				item['ERASED_COUNT'] = json.d;
		...
		switch ( sCommandName )
		{
			...
			case 'DataPrivacy.Complete':
			{
				try
				{
					...
					let res = await CreateSplendidRequest('Administration/DataPrivacy/Rest.svc/Complete', 'POST', 'application/octet-stream', sBody);
					let json = await GetSplendidResult(res);
			...
			case 'DataPrivacy.Reject':
			{
				try
				{
					...
					let sBody: string = JSON.stringify(data);
					let res = await CreateSplendidRequest('Administration/DataPrivacy/Rest.svc/Reject', 'POST', 'application/octet-stream', sBody);
					let json = await GetSplendidResult(res);
					history.push(`/Reset/Administration/${MODULE_NAME}/View/${ID}`);
				}
				catch(error)
			...
			case 'DataPrivacy.Erase':
			{
				try
				{
					...
					let sBody: string = JSON.stringify(data);
					let res = await CreateSplendidRequest('Administration/DataPrivacy/Rest.svc/Erase', 'POST', 'application/octet-stream', sBody);
					let json = await GetSplendidResult(res);
					history.push(`/Reset/Administration/${MODULE_NAME}/View/${ID}`);
				}
				catch(error)
	...
	private _onButtonsLoaded = async () =>
	{
		...
		if ( this.headerButtons.current != null )
		{
			if ( item )
			{
				...
				let nERASED_COUNT: number = Sql.ToInteger(item['ERASED_COUNT']);
				this.headerButtons.current.SetControlClass('DataPrivacy.Complete', 'button DataPrivacyComplete');
				this.headerButtons.current.SetControlClass('DataPrivacy.Erase'   , 'button DataPrivacyErase'   );
				this.headerButtons.current.SetControlClass('DataPrivacy.Reject'  , 'button DataPrivacyReject'  );

				this.headerButtons.current.ShowButton('DataPrivacy.Complete', (sSTATUS == 'Open' && nERASED_COUNT == 0) && (Security.IS_ADMIN() || Security.GetACLRoleAccess('Data Privacy Manager Role')));
				this.headerButtons.current.ShowButton('DataPrivacy.Erase'   , (sSTATUS == 'Open' && nERASED_COUNT >  0) && (Security.IS_ADMIN() || Security.GetACLRoleAccess('Data Privacy Manager Role')));
				this.headerButtons.current.ShowButton('DataPrivacy.Reject'  , (sSTATUS == 'Open'                      ) && (Security.IS_ADMIN() || Security.GetACLRoleAccess('Data Privacy Manager Role')));
				if ( sSTATUS != 'Open' )
...
export default withRouter(DataPrivacyDetailView);
