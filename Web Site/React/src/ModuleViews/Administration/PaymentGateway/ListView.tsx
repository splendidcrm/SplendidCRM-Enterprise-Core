...
@observer
class PaymentGatewayListView extends React.Component<IAdminListViewProps, IAdminListViewState>
{
	...
	private BootstrapColumns = (sLIST_MODULE_NAME, layout, sPRIMARY_MODULE, sPRIMARY_ID) =>
	{
		...
		objDataColumn =
		{
			...
			text           : null,
			dataField      : 'empty1',
			formatter      : this.setDefaultColumnFormatter,
			headerClasses  : 'listViewThS2',
			headerStyle    : {padding: 0, margin: 0},
			headerFormatter: this.defaultHeader,
			sort           : false,
			isDummyField   : true,
			attrs          : { width: '1%' },
			formatExtraData: {
	...
	private onSetDefault = async (row) =>
	{
		try
		{
			let PaymentGateway_ID: string = row['ID'];
			let rowAdmin: any = { PaymentGateway_ID };
			await UpdateAdminConfig(rowAdmin);
			SplendidCache.SetConfigValue('PaymentGateway_ID', PaymentGateway_ID);
			if ( this.searchView.current != null )
			{
				this.searchView.current.SubmitSearch();
			}
	...
	public defaultHeader = (column, colIndex, { sortElement, filterElement }) =>
	{
		return (<div>{ L10n.Term('PaymentGateway.LBL_LIST_DEFAULT') }</div>);
	}

	private setDefaultColumnFormatter = (cell, row, rowIndex, formatExtraData) =>
	{
		const { MODULE_NAME } = this.props;
		...
		let sDefaultGateway: string = Crm_Config.ToString('PaymentGateway_ID').toLowerCase();
		if ( Sql.ToString(row['ID']).toLowerCase() == sDefaultGateway )
		{
			return (
				<span style={ { whiteSpace: 'nowrap'} }>
					<img src={ this.themeURL + 'check_inline.gif'} style={ {padding: '3px', width: '24px', height: '24px', borderWidth: '0px'} } />
					{ L10n.Term('.LBL_YES') }
				</span>
			);
		}
		else
		{
			return (
				<span style={ { whiteSpace: 'nowrap'} }>
					{ nEDIT_ACLACCESS >= 0
					? <a href='#' onClick={ (e) => { e.preventDefault(); this.onSetDefault(row); } }>
						<img src={ this.themeURL + 'decline_inline.gif'} alt={ L10n.Term('.LBL_DEFAULT') } style={ {padding: '3px', width: '24px', height: '24px', borderWidth: '0px'} } />
					</a>
					: null
					}
					{ L10n.Term('.LBL_NO') }
...
export default withRouter(PaymentGatewayListView);
