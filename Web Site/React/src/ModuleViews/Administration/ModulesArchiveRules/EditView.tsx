...
@observer
export default class ModulesArchiveRulesEditView extends React.Component<IAdminEditViewProps, IAdminEditViewState>
{
	...
	{
		let { showPreviewFilter } = this.state;
		...
		if ( this._isMounted )
		{
			this.setState({ editedItem: item, showPreviewFilter });
		}
	}
	...
	private UpdateModule = async (row: any, sID: string) =>
	{
		...
		else
		{
			...
			let sUrl : string = 'Administration/ModulesArchiveRules/Rest.svc/UpdateModule';
			// 02/09/2022 Paul.  Don't need to speicfy ModuleName as it is in the URL. 
	...
	{
		let obj: any = Object.assign({}, this.state.item, this.queryBuilder.current.data);
		obj['$top'            ] = nTOP          ;
		obj['$skip'           ] = nSKIP         ;
		obj['$orderby'        ] = Sql.ToString(sSORT_FIELD + ' ' + sSORT_DIRECTION);
		obj['$select'         ] = Sql.ToString(sSELECT);
		let sBody = JSON.stringify(obj);
		let res = await CreateSplendidRequest('Administration/ModulesArchiveRules/Rest.svc/GetPreviewFilter', 'POST', 'application/octet-stream', sBody);
		let json = await GetSplendidResult(res);
				...
				case 'SaveConcurrency':
				{
					if ( this.validate() )
					{
						...
						try
						{
							...
							{
								// 02/09/2022 Paul.  There is no DetailView, so just return to list. 
								history.push(`/Reset/Administration/${MODULE_NAME}/`);
							}
						}
						catch(error)
						{
	...
	public render()
	{
		...
		const { showPreviewFilter, previewKey } = this.state;
		...
		if ( SplendidCache.IsInitialized && SplendidCache.AdminMenu )
		{
			...
			<div>
				...
				? <QueryBuilder
					row={ currentItem }
					Modules={ currentItem['MODULE_NAME'] }
				...
				? <div style={ {width: '100%'} }>
					<SplendidGrid
						...
						MODULE_NAME={ currentItem['MODULE_NAME'] }
						GRID_NAME={ currentItem['MODULE_NAME'] + '.ListView' }
						SORT_FIELD='NAME'
						SORT_DIRECTION='asc'
						ADMIN_MODE={ false }
