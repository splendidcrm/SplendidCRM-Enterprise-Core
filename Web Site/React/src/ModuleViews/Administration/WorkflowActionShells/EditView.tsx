...
import XomlBuilder                                  from './XomlBuilder'                                ;
...
@observer
export default class WorkflowActionShellsEditView extends React.Component<IAdminEditViewProps, IAdminEditViewState>
{
	...
	private UpdateModule = async (row: any, sID: string) =>
	{
		...
		else
		{
			...
			let res = await CreateSplendidRequest('Administration/WorkflowActionShells/Rest.svc/UpdateModule', 'POST', 'application/octet-stream', sBody);
	...
	public render()
	{
		...
		if ( SplendidCache.IsInitialized && SplendidCache.AdminMenu )
		{
			...
			<div>
				...
				<h4>{ L10n.Term('WorkflowActionShells.LBL_ACTION_TYPES') }</h4>
