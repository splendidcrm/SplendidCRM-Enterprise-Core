...
export default class AdminEditView extends React.Component<IAdminEditViewProps, IAdminEditViewState>
{
	...
	private load = async () =>
	{
		const { MODULE_NAME, ID, rowDefaultSearch, DuplicateID, history } = this.props;
		...
		try
		{
			...
			{
				// 02/02/2021 Paul.  Return back to list. 
				...
				case 'SaveConcurrency':
				{
					...
					if ( nInvalidFields == 0 )
					{
						...
						try
						{
							...
							let res = await CreateSplendidRequest('Administration/Rest.svc/UpdateAdminEditCustomField', 'POST', 'application/octet-stream', sBody);
	...
	public render()
	{
		...
		if ( SplendidCache.IsInitialized && SplendidCache.AdminMenu )
		{
			...
			<div>
				...
				{ currentItem != null && currentItem['DATA_TYPE'] == 'enum'
				? <table cellSpacing={ 0 } cellPadding={ 3 } style={ {backgroundColor: 'white', width: '100%', borderCollapse: 'collapse', border: 'solid 2px black'} }>
					{
						vwPICK_LIST_VALUES.map((item, index) => 
						{
							return (<tr>
								...
								<td style={ { border: 'solid 1px black' } }>{ L10n.ListTerm(currentItem['DROPDOWN_LIST'], item) }</td>
							</tr>);
						})
					}
				</table>
