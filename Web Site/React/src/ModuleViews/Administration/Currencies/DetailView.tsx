...
@observer
class CurrenciesDetailView extends React.Component<IDetailViewProps, IAdminDetailViewState>
{
	...
	private Page_Command = async (sCommandName, sCommandArguments) =>
	{
		...
		switch ( sCommandName )
		{
			...
			case 'Currencies.MakeDefault':
			{
				let row: any = { default_currency: item['ID'] };
				await UpdateAdminConfig(row);
				SplendidCache.SetConfigValue('default_currency', item['ID'] );
				break;
			}
			case 'Currencies.MakeBase':
			{
				let row: any = { base_currency: item['ID'] };
				await UpdateAdminConfig(row);
				SplendidCache.SetConfigValue('base_currency', item['ID'] );
				try
				{
					let data: any = { ISO4217: item['ISO4217'] } ;
...
export default withRouter(CurrenciesDetailView);
