...
@observer
class AuthorizeNetCustomerListView extends React.Component<IAdminListViewProps, IAdminListViewState>
{
	...
	private _onHyperLinkCallback = (MODULE_NAME: string, ID: string, NAME: string, URL: string, row?: any) =>
	{
		...
		history.push(`/Reset/Administration/${MODULE_NAME}/CustomerProfiles/View/${ID}`);
	}
	...
	private Load = async (sMODULE_NAME, sSORT_FIELD, sSORT_DIRECTION, sSELECT, sFILTER, rowSEARCH_VALUES, nTOP, nSKIP, bADMIN_MODE?, archiveView?) =>
	{
		...
		let res = await CreateSplendidRequest('Administration/AuthorizeNet/Rest.svc/CustomerProfiles', 'POST', 'application/octet-stream', sBody);
			...
			<div>
				...
				? <div style={ {display: (showSearchView == 'show' ? 'block' : 'none')} }>
					...
					<SearchView
						...
						EDIT_NAME={ 'AuthorizeNet.SearchCustomerProfiles' }
				...
				<SplendidGrid
					...
					GRID_NAME='AuthorizeNet.CustomerProfiles.ListView'
...
export default withRouter(AuthorizeNetCustomerListView);
