...
@observer
class AzureSqlPricesListView extends React.Component<IAdminListViewProps, IAdminListViewState>
{
	...
	private onUpdateRate = async (row) =>
	{
		try
		{
			...
			let res = await CreateSplendidRequest('Administration/Azure/Rest.svc/UpdateSqlRate', 'POST', 'application/json; charset=utf-8', sBody);
		...
		return (
			<span style={ { whiteSpace: 'nowrap'} }>
				...
				? <a href='#' onClick={ (e) => { e.preventDefault(); this.onUpdateRate(row); } } className='listViewTdToolsS1'>
					<img src={ this.themeURL + 'CurrencyLayer.png'} alt={ L10n.Term('AzureSqlPrices.LBL_UPDATE_RATE') } style={ {padding: '3px', width: '24px', height: '24px', borderWidth: '0px'} } />
					{ L10n.Term('AzureSqlPrices.LBL_UPDATE_RATE') }
				</a>
				: null
				}
			</span>
...
export default withRouter(AzureSqlPricesListView);
