...
@observer
export default class AzureServiceLevelsEditView extends React.Component<IAdminEditViewProps, IAdminEditViewState>
{
	...
	constructor(props: IAdminEditViewProps)
	{
		...
		//console.log((new Date()).toISOString() + ' ' + this.constructor.name + '.constructor ' + EDIT_NAME);
		this.state =
		{
			__total           : 0,
			__sql             : null,
	...
	private load = async () =>
	{
		...
		try
		{
			...
			if ( this._isMounted )
			{
				...
				let obj = new Object();
				obj['TableName'    ] = 'AZURE_PRICE_NAMES';
				obj['$filter'      ] = 'TYPE eq \'VM\'';
				...
				if ( json.d != null && json.d.results != null )
				{
					...
					for ( let i: number = 0; i < json.d.results.length; i++ )
					{
						...
						SplendidCache.SetListTerm('AzureVMPriceNames', row['NAME'], row['NAME'] );
					}
					SplendidCache.SetTerminologyList('AzureVMPriceNames', arrListValues);
				}
				obj = new Object();
				obj['TableName'    ] = 'AZURE_PRICE_NAMES';
				obj['$filter'      ] = 'TYPE eq \'SQL\'';
				obj['$orderby'     ] = 'LIST_ORDER asc';
				obj['$select'      ] = 'NAME';
				sBody = JSON.stringify(obj);
				...
				if ( json.d != null && json.d.results != null )
				{
					...
					for ( let i: number = 0; i < json.d.results.length; i++ )
					{
						let row: any = json.d.results[i];
						arrListValues.push(row['NAME']);
						SplendidCache.SetListTerm('AzureSQLPriceNames', row['NAME'], row['NAME'] );
					}
					SplendidCache.SetTerminologyList('AzureSQLPriceNames', arrListValues);
				}
				...
				case 'SaveConcurrency':
				{
					...
					if ( nInvalidFields == 0 )
					{
						...
						try
						{
							...
							let res = await CreateSplendidRequest('Administration/Azure/Rest.svc/CreateResourceGroup', 'POST', 'application/json; charset=utf-8', sBody);
							let json = await GetSplendidResult(res);
							row.ID = json.d;
							// 02/22/2021 Paul.  A number of admin modules support SaveNew.
							if ( sCommandName == 'SaveNew' )
							{
								history.push(`/Reset/Administration/Azure/${MODULE_NAME}/Edit/`);
							}
							else
							{
								history.push(`/Reset/Administration/Azure/${MODULE_NAME}/List/`);
							}
						}
						catch(error)
						{
