...
@observer
class AzureVmPricesListView extends React.Component<IAdminListViewProps, IAdminListViewState>
{
	...
	private onUpdateRate = async (row) =>
	{
		try
		{
			...
			let res = await CreateSplendidRequest('Administration/Azure/Rest.svc/UpdateVmRate', 'POST', 'application/json; charset=utf-8', sBody);
		...
		return (
			<span style={ { whiteSpace: 'nowrap'} }>
				...
				? <a href='#' onClick={ (e) => { e.preventDefault(); this.onUpdateRate(row); } } className='listViewTdToolsS1'>
					...
					{ L10n.Term('AzureVmPrices.LBL_UPDATE_RATE') }
				</a>
				: null
				}
			</span>
...
export default withRouter(AzureVmPricesListView);
