...
@observer
class DnsNamesDetailView extends React.Component<IDetailViewProps, IAdminDetailViewState>
{
	...
	private LoadItem = async (sMODULE_NAME: string, sID: string) =>
	{
		try
		{
			...
			let res = await CreateSplendidRequest('Administration/Azure/Rest.svc/GetDnsName?NAME=' + encodeURIComponent(sID), 'GET');
		...
		switch ( sCommandName )
		{
			...
			case 'Delete':
			{
				try
				{
					...
					let res = await CreateSplendidRequest('Administration/Azure/Rest.svc/DeleteDnsName', 'POST', 'application/json; charset=utf-8', sBody);
					let json = await GetSplendidResult(res);
					history.push(`/Reset/Administration/Azure/${MODULE_NAME}/List`);
				}
				catch(error)
...
export default withRouter(DnsNamesDetailView);
