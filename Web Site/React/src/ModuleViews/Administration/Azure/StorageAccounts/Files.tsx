...
import { UpdateModule, UpdateRelatedItem, UpdateRelatedList, DeleteRelatedItem, DeleteModuleItem, AdminProcedure } from '../../../../scripts/ModuleUpdate';
...
@observer
class StorageAccountsFiles extends React.Component<ISubPanelViewProps, ISubPanelViewState>
{
	...
	private _onRemove = async (row) =>
	{
		...
		try
		{
			if ( window.confirm(L10n.Term('.NTC_REMOVE_CONFIRMATION')) )
			{
				...
				obj['NAME'         ] = this.props.row['Name'];
				obj['Uri'          ] = row['Uri' ];
				let sBody: string = JSON.stringify(obj);
				let res = await CreateSplendidRequest('Administration/Azure/Rest.svc/DeleteStorageAccountFile', 'POST', 'application/json; charset=utf-8', sBody);
	...
	private Load = async (sMODULE_NAME, sSORT_FIELD, sSORT_DIRECTION, sSELECT, sFILTER, rowSEARCH_VALUES, nTOP, nSKIP, bADMIN_MODE?, archiveView?) =>
	{
		...
		try
		{
			let res = await CreateSplendidRequest('Administration/Azure/Rest.svc/GetStorageAccountFiles?NAME=' + Sql.ToString(row['Name']), 'GET');
			let json = await GetSplendidResult(res);
	...
	{
		const { CONTAINER_NAME } = this.state;
		try
		{
			...
			if ( files.length > 0 )
			{
				...
				else
				{
					...
					reader.onload = async () =>
					{
						...
						{
							let sBody: string = JSON.stringify(obj);
							let res = await CreateSplendidRequest('Administration/Azure/Rest.svc/UploadStorageAccountFile', 'POST', 'application/json; charset=utf-8', sBody);
							let json = await GetSplendidResult(res);
							if ( this._isMounted )
							{
	...
	private _onCONTAINER_NAME_Change = (e) =>
	{
	...
	public render()
	{
		...
		const { RELATED_MODULE, GRID_NAME, TABLE_NAME, SORT_FIELD, SORT_DIRECTION, PRIMARY_FIELD, PRIMARY_ID, error, showCancel, showFullForm, showTopButtons, showBottomButtons, showSearch, showInlineEdit, item, popupOpen, multiSelect, archiveView, open, customView, subPanelVisible, CONTAINER_NAME } = this.state;
			...
			return (
				<React.Fragment>
					...
					<Content pose={ open ? 'open' : 'closed' } style={ {overflow: (open ? 'visible' : 'hidden')} }>
						...
						? <React.Fragment>
							<div id="updIMPORT">
								<input type='text' id='txtCONTAINER_NAME' style={ {width: '200px'} } onChange={ this._onCONTAINER_NAME_Change } value={ CONTAINER_NAME } />&nbsp;
								<input type="file" id="fileIMPORT" style={ {width: '400px'} } onChange={ this._onFILE_Upload } ref={ this.fileUpload } />
							</div>
							<div style={ cssSearch }>
							...
							<SplendidGrid
								...
								RELATED_MODULE='StorageAccounts'
								GRID_NAME={ GRID_NAME }
...
export default withRouter(StorageAccountsFiles);
