...
@observer
class StorageAccountsDetailView extends React.Component<IDetailViewProps, IAdminDetailViewState>
{
	...
	private LoadItem = async (sMODULE_NAME: string, sID: string) =>
	{
		try
		{
			...
			let res = await CreateSplendidRequest('Administration/Azure/Rest.svc/GetStorageAccount?NAME=' + encodeURIComponent(sID), 'GET');
	...
	private Page_Command = async (sCommandName, sCommandArguments) =>
	{
		...
		switch ( sCommandName )
		{
			case 'Edit':
			{
				history.push(`/Reset/Administration/Azure/${MODULE_NAME}/Edit/${ID}`);
				break;
			}
			case 'Duplicate':
			{
				history.push(`/Reset/Administration/Azure/${MODULE_NAME}/Duplicate/${ID}`);
				break;
			}
			case 'Cancel':
			{
			...
			case 'Delete':
			{
				try
				{
					...
					let res = await CreateSplendidRequest('Administration/Azure/Rest.svc/DeleteStorageAccount', 'POST', 'application/json; charset=utf-8', sBody);
					let json = await GetSplendidResult(res);
					history.push(`/Reset/Administration/Azure/${MODULE_NAME}/List`);
				}
				catch(error)
...
export default withRouter(StorageAccountsDetailView);
