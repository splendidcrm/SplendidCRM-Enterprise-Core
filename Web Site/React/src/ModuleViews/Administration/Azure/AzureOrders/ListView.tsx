...
@observer
class AzureOrdersListView extends React.Component<IAdminListViewProps, IAdminListViewState>
{
	...
	private BootstrapColumns = (sLIST_MODULE_NAME, layout, sPRIMARY_MODULE, sPRIMARY_ID) =>
	{
		...
		objDataColumn =
		{
			...
			text           : null,
			dataField      : 'empty1',
			formatter      : this.createColumnFormatter,
			headerClasses  : 'listViewThS2',
			headerStyle    : {padding: 0, margin: 0},
	...
	private onCreateOrder = async (row) =>
	{
		try
		{
	...
	private onDeleteOrder = async (row) =>
	{
		try
		{
	...
	private onCancelOrder = async (row) =>
	{
		try
		{
			let data: any = { ID: row['ID'] } ;
	...
	private createColumnFormatter = (cell, row, rowIndex, formatExtraData) =>
	{
		const { MODULE_NAME } = this.props;
		let nEDIT_ACLACCESS  : number = SplendidCache.AdminUserAccess(MODULE_NAME, 'edit');
		let bALLOW_CREATE: boolean = Sql.ToBoolean(row['ALLOW_CREATE']);
		let bALLOW_DELETE: boolean = Sql.ToBoolean(row['ALLOW_DELETE']);
		let bALLOW_CANCEL: boolean = Sql.ToBoolean(row['ALLOW_CANCEL']);
		return (
			<span style={ { whiteSpace: 'nowrap'} }>
				{ nEDIT_ACLACCESS >= 0 && bALLOW_CREATE
				? <a href='#' onClick={ (e) => { e.preventDefault(); this.onCreateOrder(row); } } className='listViewTdToolsS1'>
					{ L10n.Term('AzureOrders.LNK_CREATE') }
				</a>
				: null
				}
				{ nEDIT_ACLACCESS >= 0 && bALLOW_DELETE
				? <a href='#' onClick={ (e) => { e.preventDefault(); this.onDeleteOrder(row); } } className='listViewTdToolsS1'>
					{ L10n.Term('AzureOrders.LNK_DELETE') }
				</a>
				: null
				}
				{ nEDIT_ACLACCESS >= 0 && bALLOW_CANCEL
				? <a href='#' onClick={ (e) => { e.preventDefault(); this.onCancelOrder(row); } } className='listViewTdToolsS1'>
					{ L10n.Term('AzureOrders.LNK_CANCEL') }
				</a>
				: null
				}
			</span>
	...
	private Load = async (sMODULE_NAME, sSORT_FIELD, sSORT_DIRECTION, sSELECT, sFILTER, rowSEARCH_VALUES, nTOP, nSKIP, bADMIN_MODE?, archiveView?) =>
	{
		...
		if ( d.results )
		{
			for ( let i: number = 0; i < d.results.length; i++ )
			{
				...
				//else
				//{
				//	if ( lstCloudServices.Contains(sCloudServicePrefix.ToLower() + sSERVICE_NAME) )
				//	{
				//		bNAME_EXISTS = true;
				//		row['NAME_EXISTS'] = bNAME_EXISTS;
				//	}
				//}
				row['ALLOW_CREATE'] = !bNAME_EXISTS && sSTATUS.indexOf('Processing') < 0 && sSTATUS.indexOf('Pending') < 0;
				row['ALLOW_DELETE'] =  bNAME_EXISTS && sSTATUS.indexOf('Deleting'  ) < 0 && sSTATUS.indexOf('Pending') < 0;
				row['ALLOW_CANCEL'] =  sSTATUS.indexOf('Processing') > 0;
			}
		}
		return d;
	}
...
export default withRouter(AzureOrdersListView);
