...
@observer
class AzureAppUpdatesFiles extends React.Component<ISubPanelViewProps, ISubPanelViewState>
{
	...
	private Load = async (sMODULE_NAME, sSORT_FIELD, sSORT_DIRECTION, sSELECT, sFILTER, rowSEARCH_VALUES, nTOP, nSKIP, bADMIN_MODE?, archiveView?) =>
	{
		...
		if ( !Sql.IsEmptyString(sDEPLOY_FILES) )
		{
			try
			{
				let res = await CreateSplendidRequest('Administration/Azure/Rest.svc/GetDeployedFiles?MODULE_NAME=' + this.state.RELATED_MODULE + '&ID=' + this.state.PARENT_ID, 'GET');
				let json = await GetSplendidResult(res);
...
export default withRouter(AzureAppUpdatesFiles);
