...
@observer
class AzureAppUpdatesAzureOrders extends React.Component<ISubPanelViewProps, ISubPanelViewState>
{
	...
	private BootstrapColumns = (sLIST_MODULE_NAME, layout, sPRIMARY_MODULE, sPRIMARY_ID) =>
	{
		...
		objDataColumn =
		{
			...
			text           : null,
			dataField      : 'empty1',
			formatter      : this.cancelColumnFormatter,
			headerClasses  : 'listViewThS2',
			headerStyle    : {padding: 0, margin: 0},
	...
	private onCancelOrder = async (row) =>
	{
		...
		try
		{
			let data: any =
			{
				AZURE_ORDER_ID: row['ID'],
				APP_UPDATE_ID : PARENT_ID,
				STATUS: 'Update Cancelled'
			};
			await AdminProcedure('spAZURE_APP_UPDATES_ORDERS_UpdateStatus', data);
			if ( this.searchView.current != null )
			{
				this.searchView.current.SubmitSearch();
			}
	...
	private cancelColumnFormatter = (cell, row, rowIndex, formatExtraData) =>
	{
		const { RELATED_MODULE } = this.state;
		let nEDIT_ACLACCESS   : number = SplendidCache.AdminUserAccess(RELATED_MODULE, 'edit');
		let sAPP_UPDATE_STATUS: string = Sql.ToString(row['APP_UPDATE_STATUS']);
		return (
			<span style={ { whiteSpace: 'nowrap'} }>
				{ nEDIT_ACLACCESS >= 0 && sAPP_UPDATE_STATUS == 'Pending Update'
				? <a href='#' onClick={ (e) => { e.preventDefault(); this.onCancelOrder(row); } } className='listViewTdToolsS1'>
					{ L10n.Term('AzureOrders.LNK_CANCEL') }
					&nbsp;
					{ this.legacyIcons
					? <img src={ this.themeURL + 'set_cancel_inline.gif'} style={ {borderWidth: '0px'} } />
					...
					<Content pose={ open ? 'open' : 'closed' } style={ {overflow: (open ? 'visible' : 'hidden')} }>
						...
						? <React.Fragment>
							...
							<SplendidGrid
								...
								RELATED_MODULE='AzureAppUpdates'
								GRID_NAME={ GRID_NAME }
...
export default withRouter(AzureAppUpdatesAzureOrders);
