...
import { DetailView_LoadItem, DetailView_LoadLayout, DetailView_HideField } from '../../../scripts/DetailView'                ;
...
@observer
class OutboundEmailDetailView extends React.Component<IDetailViewProps, IAdminDetailViewState>
{
	...
	private LoadItem = async (sMODULE_NAME: string, sID: string, layout: any[]) =>
	{
		try
		{
			...
			if ( this._isMounted )
			{
				...
				if ( item != null )
				{
					let MAIL_SENDTYPE = Sql.ToString(item['MAIL_SENDTYPE']);
					if ( !(MAIL_SENDTYPE == 'smtp' || MAIL_SENDTYPE == 'Exchange-Password') )
					{
						DetailView_HideField(layout, 'MAIL_SMTPSERVER'  , true);
						DetailView_HideField(layout, 'MAIL_SMTPPORT'    , true);
						DetailView_HideField(layout, 'MAIL_SMTPAUTH_REQ', true);
						DetailView_HideField(layout, 'MAIL_SMTPSSL'     , true);
						DetailView_HideField(layout, 'MAIL_SMTPUSER'    , true);
					}
				}
				this.setState({ layout, item, SUB_TITLE, __sql: d.__sql });
		...
		switch ( sCommandName )
		{
			...
			case 'ViewLog':
			{
				...
				case 'Test':
				{
					...
					else if ( item['MAIL_SENDTYPE'] == 'GoogleApps' )
					{
	...
	private _onSmtpTest = async () =>
	{
		...
		try
		{
			...
			{
				let obj: any = {};
				obj.from_addr         = item['FROM_ADDR'         ];
				obj.from_name         = item['FROM_NAME'         ];
				obj.mail_sendtype     = item['MAIL_SENDTYPE'     ];
				obj.mail_smtpuser     = item['MAIL_SMTPUSER'     ];
				obj.mail_smtppass     = item['MAIL_SMTPPASS'     ];
				obj.mail_smtpserver   = item['MAIL_SMTPSERVER'   ];
				obj.mail_smtpport     = item['MAIL_SMTPPORT'     ];
				obj.mail_smtpauth_req = item['MAIL_SMTPAUTH_REQ' ];
				obj.mail_smtpssl      = item['MAIL_SMTPSSL'      ];
...
export default withRouter(OutboundEmailDetailView);
