...
const MODULE_NAME: string = 'PayTrace';
...
@observer
class PayTraceListView extends React.Component<IAdminListViewProps, IAdminListViewState>
{
	...
	private Load = async (sMODULE_NAME, sSORT_FIELD, sSORT_DIRECTION, sSELECT, sFILTER, rowSEARCH_VALUES, nTOP, nSKIP, bADMIN_MODE?, archiveView?) =>
	{
		...
		if ( Crm_Config.ToBoolean(MODULE_NAME + '.Enabled') )
		{
			...
			let res = await CreateSplendidRequest('Administration/PayTrace/Rest.svc/Transactions', 'POST', 'application/octet-stream', sBody);
...
export default withRouter(PayTraceListView);
