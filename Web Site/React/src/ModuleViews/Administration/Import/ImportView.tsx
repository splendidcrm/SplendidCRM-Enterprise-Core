...
@observer
export default class DatabaseImportView extends React.Component<IAdminConfigViewProps, IAdminConfigViewState>
{
	...
	private Page_Command = async (sCommandName, sCommandArguments) =>
	{
		...
		const { bTruncate, FILE_DATA, FILE_MIME_TYPE } = this.state;
		switch (sCommandName)
		{
			case 'Save':
			case 'Next':
			{
				try
				{
					...
					let res = await CreateSplendidRequest('Administration/Import/Rest.svc/ImportDatabase', 'POST', 'application/json; charset=utf-8', sBody);
	...
	public render()
	{
		...
		const { bTruncate, MODULE_NAME, BUTTON_NAME, DETAIL_NAME, MODULE_TITLE, error } = this.state;
		if ( SplendidCache.IsInitialized && SplendidCache.AdminMenu )
		{
			...
			<div>
				...
				<div id={!!callback ? null : "content"}>
					<ListHeader TITLE='Administration.LBL_IMPORT_DATABASE_TITLE' />
					<div>
						{ L10n.Term("Terminology.LBL_IMPORT_DATABASE_INSTRUCTIONS") }
					</div>
						...
						<tr>
							<td>
								<table style={ {width: '100%', border: 'none'} }>
									...
									<tr>
										<td className="dataLabel">{ L10n.Term("Administration.LBL_IMPORT_DATABASE_TRUNCATE") }</td>
										<td className="dataField">
