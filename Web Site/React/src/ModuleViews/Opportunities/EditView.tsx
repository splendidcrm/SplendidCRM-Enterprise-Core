...
interface IEditViewState
{
	...
	OpportunitiesMode  : string;
	error              : any;
}

@observer
export default class OpportunitiesEditView extends React.Component<IEditViewProps, IEditViewState>
{
	...
	private lineItems     = React.createRef<RevenueLineItems>();
	private PARENT_ID    : string = null;
	private PARENT_TYPE  : string = null;
	private QUOTE_ID     : string = null;
	private LEAD_ID      : string = null;

	public get data (): any
	{
		const { OpportunitiesMode } = this.state;
		let row: any = {};
	...
	constructor(props: IEditViewProps)
	{
		...
		let OpportunitiesMode: string = Crm_Config.ToString('OpportunitiesMode');
		this.state =
		{
			__total           : 0,
			__sql             : null,
	...
	private load = async () =>
	{
		...
		try
		{
			...
			else if ( !Sql.IsEmptyGuid(queryParams['QUOTE_ID']) )
			{
				...
				rowDefaultSearch = await this.LoadParent('Quotes', this.QUOTE_ID);
			}
			else if ( !Sql.IsEmptyGuid(queryParams['LEAD_ID']) )
			{
		...
		if ( !Sql.IsEmptyString(sPARENT_TYPE) && !Sql.IsEmptyString(sID) )
		{
			try
			{
				...
				if ( this._isMounted && item != null )
				{
					...
					else if ( sPARENT_TYPE == 'Leads' )
					{
						rowDefaultSearch['LEAD_ID'          ] = item['ID'                  ];
						rowDefaultSearch['LEAD_NAME'        ] = item['NAME'                ];
						// 11/29/2017 Paul.  Migrate the Campaign ID for a customer. 
						rowDefaultSearch['CAMPAIGN_ID'      ] = item['CAMPAIGN_ID'         ];
						rowDefaultSearch['CAMPAIGN_NAME'    ] = item['CAMPAIGN_NAME'       ];
					}
					else if ( sPARENT_TYPE == 'Quotes' )
					{
						rowDefaultSearch['ACCOUNT_ID'       ] = item['BILLING_ACCOUNT_ID'  ];
						rowDefaultSearch['ACCOUNT_NAME'     ] = item['BILLING_ACCOUNT_NAME'];
						rowDefaultSearch['AMOUNT'           ] = item['TOTAL_USDOLLAR'      ];
						// 05/01/2013 Paul.  Add Contacts field to support B2C. 
	...
	private Page_Command = async (sCommandName, sCommandArguments) =>
	{
		...
		const { LAST_DATE_MODIFIED, OpportunitiesMode } = this.state;
	...
	public render()
	{
		...
		const { item, layout, EDIT_NAME, SUB_TITLE, OpportunitiesMode, error } = this.state;
