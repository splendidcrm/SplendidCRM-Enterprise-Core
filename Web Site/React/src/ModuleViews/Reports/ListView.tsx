...
class ReportsListView extends React.Component<IListViewProps, IListViewState>
{
	...
	private exportColumnFormatter = (cell, row, rowIndex, formatExtraData) =>
	{
		...
		// 04/29/2022 Paul.  Icons should be 16x16. 
		return (<span>
			{ nEDIT_ACLACCESS >= 0 && row['REPORT_TYPE'] == 'tabular'
			...
			<span style={ {cursor: 'pointer', padding: '3px', textDecoration: 'none'} } onClick={ () => this._onAddDashlet(row) } className='listViewTdToolsS1'>{ L10n.Term('Reports.LNK_ADD_DASHLET') }</span>
			{ Security.IS_ADMIN() && nEXPORT_ACLACCESS >= 0
			? <span>
				<img src={ this.themeURL + 'export.gif' } style={ {borderWidth: '0px', height: '16px', width: '16px', paddingLeft: '4px'} } />
				<span style={ {cursor: 'pointer', padding: '3px', textDecoration: 'none'} } onClick={ () => this._onExportRDL(row) } className='listViewTdToolsS1'>{ L10n.Term('Reports.LBL_EXPORT_RDL') }</span>
			</span>
			: null
			}
			{ Security.IS_ADMIN() && nEXPORT_ACLACCESS >= 0
			? <span>
				<img src={ this.themeURL + 'export.gif' } style={ {borderWidth: '0px', height: '16px', width: '16px', paddingLeft: '4px'} } />
				<span style={ {cursor: 'pointer', padding: '3px', textDecoration: 'none'} } onClick={ () => this._onExportSQL(row) } className='listViewTdToolsS1'>{ L10n.Term('Reports.LBL_EXPORT_SQL') }</span>
			</span>
			: null
			}
		</span>);
	...
	private BootstrapColumns = (sLIST_MODULE_NAME, layout, sPRIMARY_MODULE, sPRIMARY_ID) =>
	{
		...
		{
			key            : 'column' + 'NAME',
			text           : L10n.Term('Reports.LBL_LIST_REPORT_NAME'),
			dataField      : 'NAME',
		...
		// 03/24/2020 Paul.  Manually add MODULE_NAME, REPORT_TYPE as a bound columns. 
		// 05/18/2021 Paul.  Module Name should appear after Report Name. 
		objDataColumn =
		{
			key            : 'column' + 'MODULE_NAME',
			text           : L10n.Term('Reports.LBL_LIST_MODULE_NAME'),
			...
			formatExtraData: {
				data: {
					...
					layout      : 
					{
						...
						HEADER_TEXT    : 'Reports.LBL_LIST_MODULE_NAME',
					}
				}
			}
		};
		...
		objDataColumn =
		{
			key            : 'column' + 'REPORT_TYPE',
			text           : L10n.Term('Reports.LBL_LIST_REPORT_TYPE'),
			...
			formatExtraData: {
				data: {
					GRID_NAME   : sLIST_MODULE_NAME,
					DATA_FIELD  : 'REPORT_TYPE',
					COLUMN_INDEX: 1,
					layout      : 
					{
						COLUMN_TYPE    : 'BoundColumn',
						DATA_FIELD     : 'REPORT_TYPE',
						SORT_EXPRESSION: 'REPORT_TYPE',
						HEADER_TEXT    : 'Reports.LBL_LIST_REPORT_TYPE',
						LIST_NAME      : 'dom_report_types',
					}
				}
			}
		};
		...
		{
			key            : 'column' + 'LAST_RUN',
			text           : L10n.Term('Reports.LBL_LIST_SCHEDULE_LAST_RUN'),
			dataField      : 'LAST_RUN',
			classes        : 'listViewTdLinkS1',
			...
			formatExtraData: {
				data: {
					...
					layout      : 
					{
						COLUMN_TYPE    : 'BoundColumn',
						DATA_FIELD     : 'LAST_RUN',
						SORT_EXPRESSION: 'LAST_RUN',
						HEADER_TEXT    : 'Reports.LBL_LIST_SCHEDULE_LAST_RUN',
					}
				}
			}
		};
	...
	{
		this.props.history.push('/Reports/Duplicate/' + row.ID);
	}

	private _onExportRDL = (row) =>
	{
		window.location.href = Credentials.RemoteServer + 'Reports/ExportRDL.aspx?id=' + row.ID;
	}

	private _onExportSQL = (row) =>
	{
		window.location.href = Credentials.RemoteServer + 'Reports/ExportSQL.aspx?id=' + row.ID;
	}
	...
	private load = async (sMODULE_NAME, sSORT_FIELD, sSORT_DIRECTION, sSELECT, sFILTER, rowSEARCH_VALUES, nTOP, nSKIP, bADMIN_MODE?, archiveView?) =>
	{
		...
		if ( arrSELECT.indexOf('REPORT_TYPE' ) < 0 ) arrSELECT.push('REPORT_TYPE' );
