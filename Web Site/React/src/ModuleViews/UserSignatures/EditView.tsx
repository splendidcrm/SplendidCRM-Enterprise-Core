...
@observer
export default class UserSignaturesEditView extends React.Component<IEditViewProps, IEditViewState>
{
	...
	private PARENT_TYPE  : string = 'Users';
	...
	private load = async () =>
	{
		...
		try
		{
			...
			{
				this.PARENT_ID = Security.USER_ID();
			}
					...
					if ( nInvalidFields == 0 )
					{
						...
						try
						{
							...
							try
							{
								...
								let d = await ListView_LoadTable('vwUSERS_SIGNATURES', 'PRIMARY_SIGNATURE desc, NAME', 'asc', 'ID,NAME,SIGNATURE_HTML,PRIMARY_SIGNATURE', 'USER_ID eq \'' + Security.USER_ID() + '\'', null, false);
								let arrListValues: string[] = [];
								let userSignatures: any = {};
								for ( let i: number = 0; i < d.results.length; i++ )
								{
									let row: any = d.results[i];
									userSignatures[row.ID] = row;
									let sTerm: string = '.UserSignatures.' + row.ID;
									SplendidCache.SetTerminology(sTerm, row.NAME);
									arrListValues.push(row.ID);
								}
								SplendidCache.SetTerminologyList('UserSignatures', arrListValues);
								SplendidCache.SetSIGNATURES(userSignatures);
							}
							catch(error)
							{
								console.error((new Date()).toISOString() + ' ' + this.constructor.name + '.Page_Command vwUSERS_SIGNATURES', error);
							}
