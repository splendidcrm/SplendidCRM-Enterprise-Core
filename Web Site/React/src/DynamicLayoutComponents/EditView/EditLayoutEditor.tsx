...
import EditPropertiesEditor                         from './EditPropertiesEditor'           ;
...
interface IEditLayoutEditorProps
{
	LayoutType      : string;
	ModuleName      : string;
	ViewName        : string;
...
interface IEditLayoutEditorState
{
	layoutName        : string;
...
export default class EditLayoutEditor extends React.Component<IEditLayoutEditorProps, IEditLayoutEditorState>
{
	private _isMounted = false;

	constructor(props: IEditLayoutEditorProps)
	{
		super(props);
	...
	async componentDidUpdate(prevProps: IEditLayoutEditorProps)
	{
	...
	private createItemFromSource = (item) =>
	{
		...
		obj.DISPLAY_FIELD              = item.DISPLAY_FIELD             ;
		obj.CACHE_NAME                 = item.CACHE_NAME                ;
		obj.DATA_REQUIRED              = item.DATA_REQUIRED             ;
		obj.UI_REQUIRED                = item.UI_REQUIRED               ;
		obj.ONCLICK_SCRIPT             = item.ONCLICK_SCRIPT            ;
		obj.FORMAT_SCRIPT              = item.FORMAT_SCRIPT             ;
		obj.FORMAT_TAB_INDEX           = item.FORMAT_TAB_INDEX          ;
		obj.FORMAT_MAX_LENGTH          = item.FORMAT_MAX_LENGTH         ;
		obj.FORMAT_SIZE                = item.FORMAT_SIZE               ;
		obj.FORMAT_ROWS                = item.FORMAT_ROWS               ;
		obj.FORMAT_COLUMNS             = item.FORMAT_COLUMNS            ;
		obj.COLSPAN                    = item.COLSPAN                   ;
		obj.ROWSPAN                    = item.ROWSPAN                   ;
		obj.FIELD_VALIDATOR_ID         = item.FIELD_VALIDATOR_ID        ;
		obj.FIELD_VALIDATOR_MESSAGE    = item.FIELD_VALIDATOR_MESSAGE   ;
		obj.MODULE_TYPE                = item.MODULE_TYPE               ;
		obj.TOOL_TIP                   = item.TOOL_TIP                  ;
		obj.RELATED_SOURCE_MODULE_NAME = item.RELATED_SOURCE_MODULE_NAME;
		obj.RELATED_SOURCE_VIEW_NAME   = item.RELATED_SOURCE_VIEW_NAME  ;
		obj.RELATED_SOURCE_ID_FIELD    = item.RELATED_SOURCE_ID_FIELD   ;
		obj.RELATED_SOURCE_NAME_FIELD  = item.RELATED_SOURCE_NAME_FIELD ;
		obj.RELATED_VIEW_NAME          = item.RELATED_VIEW_NAME         ;
		obj.RELATED_ID_FIELD           = item.RELATED_ID_FIELD          ;
		obj.RELATED_NAME_FIELD         = item.RELATED_NAME_FIELD        ;
		obj.RELATED_JOIN_FIELD         = item.RELATED_JOIN_FIELD        ;
		...
		try
		{
			if ( this._isMounted )
			{
				...
				if ( this._isMounted )
				{
					...
					let TableName   : string = 'EDITVIEWS_FIELDS'  ;
					let filter      : string = 'EDIT_NAME eq \'' + ViewName + '\' and DEFAULT_VIEW eq \'' + DEFAULT_VIEW + '\'';
	...
	private _onSave = async (e) =>
	{
		...
		try
		{
			if ( this._isMounted )
			{
				...
				obj.EDITVIEWS.MODULE_NAME         = layoutProperties.MODULE_NAME       ;
				obj.EDITVIEWS.VIEW_NAME           = layoutProperties.VIEW_NAME         ;
				obj.EDITVIEWS.LABEL_WIDTH         = layoutProperties.LABEL_WIDTH       ;
				obj.EDITVIEWS.FIELD_WIDTH         = layoutProperties.FIELD_WIDTH       ;
				obj.EDITVIEWS.DATA_COLUMNS        = layoutProperties.DATA_COLUMNS      ;
				obj.EDITVIEWS.PRE_LOAD_EVENT_ID   = layoutProperties.PRE_LOAD_EVENT_ID ;
				obj.EDITVIEWS.POST_LOAD_EVENT_ID  = layoutProperties.POST_LOAD_EVENT_ID;
				obj.EDITVIEWS.SCRIPT              = layoutProperties.SCRIPT            ;
				...
				for ( let i = 0; i < rows.length; i++ )
				{
					...
					for ( let j = 0; j < row.columns.length; j++ )
					{
						...
						for ( let k = 0; k < fields.length; k++ )
						{
							...
							layoutField.DISPLAY_FIELD              = activeFields[fields[k]].DISPLAY_FIELD             ;
							layoutField.CACHE_NAME                 = activeFields[fields[k]].CACHE_NAME                ;
							layoutField.DATA_REQUIRED              = activeFields[fields[k]].DATA_REQUIRED             ;
							layoutField.UI_REQUIRED                = activeFields[fields[k]].UI_REQUIRED               ;
							layoutField.ONCLICK_SCRIPT             = activeFields[fields[k]].ONCLICK_SCRIPT            ;
							layoutField.FORMAT_SCRIPT              = activeFields[fields[k]].FORMAT_SCRIPT             ;
							layoutField.FORMAT_TAB_INDEX           = activeFields[fields[k]].FORMAT_TAB_INDEX          ;
							layoutField.FORMAT_MAX_LENGTH          = activeFields[fields[k]].FORMAT_MAX_LENGTH         ;
							layoutField.FORMAT_SIZE                = activeFields[fields[k]].FORMAT_SIZE               ;
							layoutField.FORMAT_ROWS                = activeFields[fields[k]].FORMAT_ROWS               ;
							layoutField.FORMAT_COLUMNS             = activeFields[fields[k]].FORMAT_COLUMNS            ;
							layoutField.COLSPAN                    = activeFields[fields[k]].COLSPAN                   ;
							layoutField.ROWSPAN                    = activeFields[fields[k]].ROWSPAN                   ;
							layoutField.FIELD_VALIDATOR_ID         = activeFields[fields[k]].FIELD_VALIDATOR_ID        ;
							layoutField.FIELD_VALIDATOR_MESSAGE    = activeFields[fields[k]].FIELD_VALIDATOR_MESSAGE   ;
							layoutField.MODULE_TYPE                = activeFields[fields[k]].MODULE_TYPE               ;
							layoutField.TOOL_TIP                   = activeFields[fields[k]].TOOL_TIP                  ;
							layoutField.RELATED_SOURCE_MODULE_NAME = activeFields[fields[k]].RELATED_SOURCE_MODULE_NAME;
							layoutField.RELATED_SOURCE_VIEW_NAME   = activeFields[fields[k]].RELATED_SOURCE_VIEW_NAME  ;
							layoutField.RELATED_SOURCE_ID_FIELD    = activeFields[fields[k]].RELATED_SOURCE_ID_FIELD   ;
							layoutField.RELATED_SOURCE_NAME_FIELD  = activeFields[fields[k]].RELATED_SOURCE_NAME_FIELD ;
							layoutField.RELATED_VIEW_NAME          = activeFields[fields[k]].RELATED_VIEW_NAME         ;
							layoutField.RELATED_ID_FIELD           = activeFields[fields[k]].RELATED_ID_FIELD          ;
							layoutField.RELATED_NAME_FIELD         = activeFields[fields[k]].RELATED_NAME_FIELD        ;
							layoutField.RELATED_JOIN_FIELD         = activeFields[fields[k]].RELATED_JOIN_FIELD        ;
							layoutField.PARENT_FIELD               = activeFields[fields[k]].PARENT_FIELD              ;
							if ( k > 0 )
								layoutField.COLSPAN = -1;
							nFieldIndex++;
							obj.EDITVIEWS_FIELDS.push(layoutField);
						}
					}
				}
				let sBody: string = JSON.stringify(obj);
				//console.log((new Date()).toISOString() + ' ' + this.constructor.name + '._onSave', obj);
				let res  = await CreateSplendidRequest('Administration/Rest.svc/UpdateAdminLayout?TableName=EDITVIEWS_FIELDS&ViewName=' + layoutName, 'POST', 'application/octet-stream', sBody);
				let json = await GetSplendidResult(res);
	...
	{
		const { layoutName } = this.state;
		window.location.href = Credentials.RemoteServer + 'Administration/DynamicLayout/EditViews/export.aspx?NAME=' + layoutName;
	}
		...
		<React.Fragment>
			...
			<div style={{ flex: '2 2 0', border: '1px solid grey', margin: '0 .5em' }}>
				<div style={ {height: '100%', overflowY: 'scroll'} }>
					...
					? <EditPropertiesEditor layoutField={ activeFields[selectedId] } moduleFields={ moduleFields } DATA_COLUMNS={ layoutProperties.DATA_COLUMNS } onEditComplete={ this._onEditPropertiesComplete } MODULE_TERMINOLOGY={ MODULE_TERMINOLOGY } />
					: null
					}
				</div>
			</div>
