...
interface IDynamicListViewProps extends RouteComponentProps<any>
{
	MODULE_NAME: string;
...
interface IDynamicListViewState
{
	customView?: any;
	error?     : any;
}
...
@observer
class DynamicListView extends React.Component<IDynamicListViewProps, IDynamicListViewState>
{
	private _isMounted = false;

	constructor(props: IDynamicListViewProps)
	{
		super(props);
	...
	async componentDidMount()
	{
		...
		try
		{
			...
			if ( status == 1 )
			{
				...
				let module = SplendidCache.Module(this.props.MODULE_NAME, 'DynamicListView.componentDidMount');
				if ( module != null )
				{
					if ( module.IS_ADMIN )
					{
	...
	componentDidUpdate(prevProps: IDynamicListViewProps)
	{
	...
	public render()
	{
		...
		{
			// 06/26/2019 Paul.  Specify a key so that SplendidGrid will get componentDidMount when changing views. 
			return <ListView key={ this.props.MODULE_NAME } {...this.props} />;
		}
		else
		{
			return null;
...
export default withRouter(DynamicListView);
