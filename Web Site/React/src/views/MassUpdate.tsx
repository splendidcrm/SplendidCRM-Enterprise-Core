...
export default class MassUpdate extends React.Component<IMassUpdateProps, IMassUpdateState>
{
	...
	{
		super(props);
		//console.log((new Date()).toISOString() + ' ' + this.constructor.name + '.constructor ' + props.PARENT_TYPE, props.row);
	...
	private _onRulesWizard = async (sCommandArguments) =>
	{
		const { MODULE_NAME } = this.props;
	...
	private _onMailMerge = async (sCommandArguments) =>
	{
		const { MODULE_NAME } = this.props;
		const { selectedItems } = this.state;
		...
		if ( this.ValidateOne() )
		{
			if ( this._isMounted )
			{
				...
				this.props.history.push(`/Reset/MailMerge/${MODULE_NAME}/` + encodeURIComponent(sID));
			}
		}
	}
