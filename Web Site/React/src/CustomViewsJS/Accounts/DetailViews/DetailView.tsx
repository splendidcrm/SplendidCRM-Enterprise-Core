...
//@observer
class AccountsDetailViewJS extends React.Component<IDetailViewProps, IDetailViewState>
{
	private _isMounted     : boolean = false;
	private refMap         : Record<string, React.RefObject<DetailComponent<any, any>>>;
	private auditView        = React.createRef<AuditView>();
	private personalInfoView = React.createRef<PersonalInfoView>();
	...
	private Page_Command = async (sCommandName, sCommandArguments) =>
	{
		...
		switch ( sCommandName )
		{
			...
			case 'PersonalInfo':
			{
				if ( this.personalInfoView.current != null )
				{
					await this.personalInfoView.current.loadData();
					this.setState({ personalInfoOpen: true });
				}
				break;
			}
	...
	private _onPersonalInfoClose = () =>
	{
		this.setState({ personalInfoOpen: false });
	}
	...
	{
		const { MODULE_NAME, ID } = this.props;
		const { item, layout, DETAIL_NAME, SUB_TITLE, auditOpen, personalInfoOpen, activitiesOpen, archiveExists, error } = this.state;
		...
		if ( SplendidCache.IsInitialized && layout && item )
		{
			...
			<React.Fragment>
				...
				<PersonalInfoView
					...
					callback={ this._onPersonalInfoClose }
					MODULE_NAME={ MODULE_NAME }
					NAME={ item.NAME }
					ID={ ID }
					ref={ this.personalInfoView }
				/>
				<ActivitiesPopupView
...
export default withRouter(AccountsDetailViewJS);
