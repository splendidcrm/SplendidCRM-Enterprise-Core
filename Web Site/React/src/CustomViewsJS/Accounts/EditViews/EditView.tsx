...
// 09/04/2020 Paul.  We are getting an error ever since we upgraded mobx. 
// Cannot read property 'componentWillReact' of undefined
//@observer
export default class AccountsEditViewJS extends React.Component<IEditViewProps, IEditViewState>
{
	...
	private load = async () =>
	{
		...
		try
		{
			...
			if ( Sql.IsEmptyGuid(ID) && Sql.IsEmptyGuid(DuplicateID) )
			{
				...
				// 08/10/2020 Paul.  Don't reinitialize if parent is found. 
	...
	private _onUpdate = (PARENT_FIELD: string, DATA_VALUE: any, item?: any): void =>
	{
		...
		if ( PARENT_FIELD == 'BILLING_ADDRESS_POSTALCODE' && item != null )
		{
			if ( !Sql.IsEmptyString(item.CITY) )
			{
				this.UpdateDependancy('BILLING_ADDRESS_CITY', item.CITY, 'value', item);
			}
			if ( !Sql.IsEmptyString(item.STATE) )
			{
				this.UpdateDependancy('BILLING_ADDRESS_STATE', item.STATE, 'value', item);
			}
			if ( !Sql.IsEmptyString(item.COUNTRY) )
			{
				this.UpdateDependancy('BILLING_ADDRESS_COUNTRY', item.COUNTRY, 'value', item);
			}
		}
		else if ( PARENT_FIELD == 'SHIPPING_ADDRESS_POSTALCODE' && item != null )
		{
			if ( !Sql.IsEmptyString(item.CITY) )
			{
				this.UpdateDependancy('SHIPPING_ADDRESS_CITY', item.CITY, 'value', item);
			}
			if ( !Sql.IsEmptyString(item.STATE) )
			{
				this.UpdateDependancy('SHIPPING_ADDRESS_STATE', item.STATE, 'value', item);
			}
			if ( !Sql.IsEmptyString(item.COUNTRY) )
			{
				this.UpdateDependancy('SHIPPING_ADDRESS_COUNTRY', item.COUNTRY, 'value', item);
			}
		}
	}
	...
	public render()
	{
		const { MODULE_NAME, ID, DuplicateID, isSearchView, isUpdatePanel, callback } = this.props;
