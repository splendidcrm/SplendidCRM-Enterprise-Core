...
export default class SevenHeaderButtons extends HeaderButtons
{
					...
					<table className='moduleTitle ModuleHeaderFrame' cellPadding={ 0 } cellSpacing={ 1 } style={ {width: '100%'} }>
						<tr>
							<td style={ {verticalAlign: 'top'} }>
								{ !hideTitle
								? <a className={ 'ModuleHeaderModule ModuleHeaderModule' + MODULE_NAME } href={ sModuleUrl } onClick={ this._onClickModule }>{ MODULE_ABBREVIATION }</a>
								: null
								}
							</td>
							<td style={ {width: '99%'} }>
								{ !Sql.IsEmptyString(ID)
								? <span className='ModuleHeaderName'>
								...
								: <span className='ModuleHeaderName'>{ L10n.Term('.moduleList.' + MODULE_NAME) }</span>
								}
								...
								? <a key={ localKey + '_Add' } href='#' className='utilsLink' onClick={ this._onChangeFavorites }>
									<FontAwesomeIcon icon={ { prefix: 'far', iconName: 'star' } } title={ L10n.Term('.LBL_ADD_TO_FAVORITES') } />
								</a>
								: <a key={ localKey + '_Remove' } href='#' className='utilsLink' onClick={ this._onChangeFavorites }>
									<FontAwesomeIcon icon={ { prefix: 'fas', iconName: 'star' } } color='#FFAD10' title={ L10n.Term('.LBL_REMOVE_FROM_FAVORITES') } />
								</a>
								: null
								}
								...
								? <a key={ localKey + '_Follow' } href='#' className='utilsLink' onClick={ this._onChangeFollowing }>
									<span className='follow'>{ L10n.Term('.LBL_FOLLOW') }</span>
								</a>
								: <a key={ localKey + '_Following' } href='#' className='utilsLink' onClick={ this._onChangeFollowing }>
									<span className='following'>{ L10n.Term('.LBL_FOLLOWING') }</span>
								</a>
								: null
								}
							</td>
							<td style={ {verticalAlign: 'center', textAlign: 'right', paddingTop: 3, paddingLeft: 5, whiteSpace: 'nowrap'} }>
