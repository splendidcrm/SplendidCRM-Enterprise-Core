...
@observer
class ArcticTopNav_Desktop extends React.Component<ITopNavProps, ITopNavState>
{
	...
	*/
	...
	public render()
	{
		...
		let quickCreateIcon = <span style={ {color: 'white', fontSize: '13px', paddingBottom: '15px'} }>
		...
		if ( SplendidCache.IsInitialized && bIsAuthenticated && !bLoading && tabMenus != null )
		{
			...
			userContextMenu = (
				<Nav className="ml-auto" navbar style={ {flexGrow: 0, borderLeft: '1px solid white', borderRight: '1px solid white'} }>
		...
		if ( SplendidCache.IsInitialized )
		{
			...
			return (
<React.Fragment>
<div id="divHeader" style={ {height: '55px'} }>
	<div id="divSixToolbar" className='SixToolbar'>
		<Navbar key={ 'divSixToolbar_' + menuChangeKey.toString() } expand="md" id="topnav" style={ {padding: 0, backgroundColor: '#64707B'} }>
			<Navbar.Toggle onClick={ this.toggle } aria-controls="topnav-nav" />
			<Navbar.Collapse id='topnav-nav' className="otherTabRight" >
				<span style={ {cursor: 'pointer'} } onClick={ (e) => this._onModuleClick('Home') } title={ L10n.Term('.moduleList.Home') }><img src={ sCompanyHomeImage } style={ {borderWidth: 0, width: 55, height: 45} } /></span>
				<Nav navbar>
					{ itemsNav }
				</Nav>
				{ moreItems.length > 0
