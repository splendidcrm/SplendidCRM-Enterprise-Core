...
// 04/25/2021 Paul.  Arctic and Atlantic are nearly identical TopNav classes. 
@observer
class AtlanticTopNav extends React.Component<ITopNavProps, ITopNavState>
{
...
<div id='divHeader' style={ {marginTop: '50px'} }>
					...
					<div id='divTabMenu'>
						<table id='ctlSixToolbar_ctlTabMenu_tblSixMenu' className='tabToolbarFrame' cellSpacing={ 0 } cellPadding={ 0 } style={ {borderCollapse: 'collapse'} } >
							...
							? <td style={ {display: 'inline', verticalAlign: 'bottom'} }>
								<table id='ctlSixToolbar_tabToolbar_more' className='tabToolbarFrame' cellSpacing={ 0 } cellPadding={ 0 }
	...
	<React.Fragment key={ "actionsModule_" + menuChangeKey.toString() }>
		...
		else
		{
			...
			let logoUrl   : string = Sql.IsEmptyString(Crm_Config.ToString('header_logo_image' )) ? '~/App_Themes/Atlantic/images/SplendidCRM_Logo.png' : Crm_Config.ToString('header_logo_image' );
...
export default withRouter(AtlanticTopNav);
