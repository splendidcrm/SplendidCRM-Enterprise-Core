...
class ArcticTopNav extends React.Component<ITopNavProps, ITopNavState>
{
	...
	async componentDidUpdate(prevProps: ITopNavProps)
	{
		...
		if ( this.props.location.pathname != prevProps.location.pathname || Sql.IsEmptyString(this.state.activeModule) )
		{
			...
			// 04/06/2022 Paul.  UnifiedSearch is not an active module, but we still want to display the QuickCreate there. 
			if ( this.state.showInlineEdit && !StartsWith(this.props.location.pathname, '/UnifiedSearch') )
			{
	...
	*/
	...
	{
		//console.log((new Date()).toISOString() + ' ' + this.constructor.name + '._onQuickCreate ' + sMODULE_NAME);
		this.setState(
		{
			showInlineEdit     : true,
				...
				return (<div id='ctlSixToolbar_pnlToolbarUserHover' key='ctlSixToolbar_pnlToolbarUserHover' style={ {position: 'absolute', left, top, zIndex: 1000} } onMouseOut={ (e) => this.moduleTabMouseOut(e, MODULE_NAME) } ref={ this.pnlTabHover }>
					<table cellPadding={ 0 } cellSpacing={ 0 } className='MoreActionsInnerTable'>
						<tr>
							...
							{ bIsAuthenticated && SplendidCache.GetUserAccess('Employees', 'access') >= 0 ? <a href={ Credentials.RemoteServer + Credentials.ReactBase + 'Employees'                } id='usercontext-employees'    key='usercontext-employees'     className='ModuleActionsMenuItems' onClick={ (e) => { e.preventDefault(); this._onEmployees()       ; } }>{ L10n.Term('.LBL_EMPLOYEES'    ) }</a> : null }
							...
							{ bIsAuthenticated && (Security.IS_ADMIN() || Security.IS_ADMIN_DELEGATE()) && !Crm_Config.ToBoolean('disable_admin_classic') ? <a href={ Credentials.RemoteServer + '/' + 'Administration'                } id='usercontext-adminClassic' key='usercontext-admin-classic' className='ModuleActionsMenuItems' onClick={ (e) => { e.preventDefault(); this._onAdminClassicPage(); } }>{ L10n.Term('.LBL_ADMIN_CLASSIC') }</a> : null }
							{ bIsAuthenticated && !Crm_Config.ToBoolean('hide_training')                  ? <a href={ Credentials.RemoteServer + Credentials.ReactBase + 'Home/TrainingPortal'      } id='usercontext-training'     key='usercontext-training'      className='ModuleActionsMenuItems' onClick={ (e) => { e.preventDefault(); this._onTrainingPortal()  ; } }>{ L10n.Term('.LBL_TRAINING'     ) }</a> : null }
					...
					<table cellPadding={ 0 } cellSpacing={ 0 } className='ModuleActionsInnerTable'>
						<tr>
							...
							<td className='ModuleActionsInnerCell'>
								...
								: <span className='ModuleActionsMenuNone'>{ L10n.Term('.LBL_LINK_NONE') }</span>
								}
							</td>
							<td className='ModuleActionsInnerCell'>
	...
	public render()
	{
		const { bIsAuthenticated, txtQuickSearch, nMaxTabs, showInlineEdit, item, error, QUICK_CREATE_MODULE, activeModule, actionsModule, showUnifiedSearch, showQuickCreate, menuChangeKey, unifiedSearchItems, quickCreate } = this.state;
		//console.log((new Date()).toISOString() + ' ' + this.constructor.name + '.render', this.state);
		...
		if ( SplendidCache.IsInitialized )
		{
			...
			let titleQuickCreate = <span className='QuickCreateFirstButton'>
				{ L10n.Term('.LBL_QUICK_SEARCH_NEW_BUTTON') }
				<span className='QuickCreateMoreButton'><FontAwesomeIcon icon='caret-down' size='lg' /></span>
			</span>;
...
<div id='divHeader' style={ {marginTop: '48px'} }>
						...
						<table id='ctlSixToolbar_ctlTabMenu_tblSixMenu' className='tabToolbarFrame' cellSpacing={ 0 } cellPadding={ 0 } style={ {borderCollapse: 'collapse'} } >
							<tr>
								<td id='ctlSixToolbar_tabToolbar_Home'
									className={ activeModule == 'Home' ? 'currentHome' : 'otherHome' }
								...
								>
									<a href={ Credentials.RemoteServer + Credentials.ReactBase + 'Home' } title={ L10n.Term('.moduleList.Home') } onClick={ (e) => { e.preventDefault(); this._onModuleClick('Home'); } }>
					...
					>
						<tr>
							<td className='otherUser' style={ {whiteSpace: 'nowrap'} }>
								...
								<span className='otherTabLink' style={ {paddingRight: '6px'} }>
									<FontAwesomeIcon icon='user' size='lg' className='otherUserIcon' />
									{ Security.FULL_NAME() }
								</span>
	...
	{ showQuickCreate || (showUnifiedSearch && unifiedSearchItems > 0)
	? <div className='UnifiedSearch' style={ {display: 'flex', flexDirection: 'row', justifyContent: 'space-between', width: '100%'} }>
		<div style={ {width: '80%'} }>
			...
			>
				<div id='divUnifiedSearch'>
					...
					/>
					<FontAwesomeIcon id='ctlSixToolbar_btnUnifiedSearch' className='searchButton' icon='search' size='lg' style={ {verticalAlign: 'middle'} } onClick={ this._onUnifiedSearch } title={ L10n.Term('.LBL_SEARCH') } />
				</div>
			</div>
			: null
			}
		...
		<div style={ {textAlign: 'right', marginTop: '5px'} }>
			{ showQuickCreate
			? <NavItem title={ titleQuickCreate }>
				<div id='ctlSixToolbar_pnlToolbarQuickCreate' key='ctlSixToolbar_pnlToolbarQuickCreate' className='QuickCreateHoverFrame'>
					{
						quickCreate.map((item) => 
						(
							<div className='QuickCreateOtherButton'>
								<a key={ 'quick_' + item.MODULE_NAME } onClick={ (e) => { e.preventDefault(); this._onQuickCreate(item.MODULE_NAME); } } >
									<span className='QuickCreateIcon'><FontAwesomeIcon icon='plus' size='lg' /></span>
									{ item.DISPLAY_NAME }
								</a>
							</div>
						))
...
<div key={ 'test_' + menuChangeKey.toString() } >
</div>
	{ showInlineEdit && QUICK_CREATE_MODULE
	? <div key={ 'quickCreateInlineEdit_' + menuChangeKey.toString() } style={ {padding: '.5em'} }>
	...
	<React.Fragment key={ "actionsModule_" + menuChangeKey.toString() }>
		...
		else
		{
			...
			let logoUrl   : string = Sql.IsEmptyString(Crm_Config.ToString('header_logo_image' )) ? '~/App_Themes/Pacific/images/SplendidCRM_Logo.png' : Crm_Config.ToString('header_logo_image' );
