...
@observer
class SugarTopNav_Desktop extends React.Component<ITopNavProps, ITopNavState>
{
	...
	*/
	...
	public render()
	{
		...
		let sCompanyLogo: string = Credentials.RemoteServer + 'Include/images/zSplendidCRM_Logo.gif';
		...
		if ( SplendidCache.IsInitialized && bIsAuthenticated && !bLoading && tabMenus != null )
		{
			...
			(
				<table className="tabFrame" style={ {borderSpacing: 0, padding: 0, height: '25px'} }>
					<tbody><tr>
						<td className={ (tabMenu.MODULE_NAME == activeModule ? 'current' : 'other') + 'TabLeft' }><img src={ themeURL + 'images/blank.gif' } style={ {marginLeft: 'auto', marginRight: 'auto', borderWidth: '0px', height: '25px', width: '5px'} } /></td>
						...
						<td className={ (tabMenu.MODULE_NAME == activeModule ? 'current' : 'other') + 'TabRight' }><img src={ themeURL + 'images/blank.gif' } style={ {marginLeft: 'auto', marginRight: 'auto', borderWidth: '0px', height: '25px', width: '5px'} } /></td>
					</tr></tbody>
				</table>
			));
			...
			actions = this.Actions(activeModule).map((item) => 
			(
				<span style={ {whiteSpace: 'nowrap'} }>
					<a title={ item.label } className="lastViewLink" href="#" onClick={(e) => { e.preventDefault(); this._onAction(activeModule, item) } }>
					<img src={ themeURL + 'images/' + item.IMAGE_NAME } style={ {marginLeft: 'auto', marginRight: 'auto', borderWidth: '0px', width: '16px', height: '16px'} } />
					&nbsp;{ item.label }</a>&nbsp;
				</span>
			));
		}
...
<div id="divHeader">
	...
	<div id="cntLastViewed_ctlActions_divShortcuts" className="lastView" style={ {width: '100%'} }>
...
export default withRouter(SugarTopNav_Desktop);
