...
interface IMyTeamPipelineBySalesStageState
{
...
@observer
class MyTeamPipelineBySalesStage extends React.Component<IDashletProps, IMyTeamPipelineBySalesStageState>
{
	...
	private createChart = () =>
	{
		...
		let arrASSIGNED_USER_ID = DEFAULT_SETTINGS.ASSIGNED_USER_ID;
		if ( arrASSIGNED_USER_ID == null || arrASSIGNED_USER_ID.length == 0 )
		{
			arrASSIGNED_USER_ID = L10n.GetList('AssignedUser');
		}
		try
		{
		...
		{
			console.error((new Date()).toISOString() + ' ' + this.constructor.name + '.createChart', error);
			this.setState({ error });
		}
...
export default withRouter(MyTeamPipelineBySalesStage);
