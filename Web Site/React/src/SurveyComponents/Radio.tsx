...
export default class Radio extends SurveyQuestion<ISurveyQuestionProps, IRadioState>
{
	...
	public get data(): any
	{
		...
		try
		{
			...
			if ( !bValid && Sql.ToBoolean(row.OTHER_ENABLED) && OTHER_VALUE )
			{
	...
	private _onChange = (ANSWER_ID: string): void =>
	{
		...
		try
		{
			...
			{
				// 07/27/2021 Paul.  OTHER_VALUE might be null. 
				if ( OTHER_VALUE == null || !OTHER_VALUE.checked )
				{
					if ( OTHER_VALUE == null )
						OTHER_VALUE = { checked: false, value: null };
					OTHER_VALUE.checked = true;
			...
			else if ( !VALUE[ANSWER_ID].checked )
			{
				const keys = Object.keys(VALUE);
				for ( let key of keys )
				{
	...
	public Summary = () =>
	{
		...
		<div className='SurveyQuestionContent'>
			...
			? <div id={ ID + '_Body' } className='SurveyQuestionBody'>
				<table className='SurveyResultsRadio' cellPadding={ 4 } cellSpacing={ 0 } style={ {border: 'none'} }>
					<tr>
