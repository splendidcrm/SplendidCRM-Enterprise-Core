...
interface ITextboxMultipleState
{
	ID                     : string;
...
export default class TextboxMultiple extends SurveyQuestion<ISurveyQuestionProps, ITextboxMultipleState>
{
			...
			if ( ANSWER_CHOICES )
			{
				...
				for ( let i = 0; i < ANSWER_CHOICES.length; i++ )
				{
					...
					if ( txtValue.length > 0 )
					{
						...
						{
							bValid = this.Validation(txtValue);
							if ( !bValid )
							{
	...
	public Summary = () =>
	{
		...
		<div className='SurveyQuestionContent'>
			...
			? <div id={ ID + '_Body' } className='SurveyQuestionBody'>
				<table className='SurveyResultsTextboxMultiple' cellPadding={ 4 } cellSpacing={ 0 } style={ {border: 'none'} }>
					<tr>
