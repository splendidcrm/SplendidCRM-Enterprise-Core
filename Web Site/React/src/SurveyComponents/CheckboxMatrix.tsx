...
interface ICheckboxMatrixState
{
	ID                     : string;
...
export default class CheckboxMatrix extends SurveyQuestion<ISurveyQuestionProps, ICheckboxMatrixState>
{
	...
	constructor(props: ISurveyQuestionProps)
	{
		...
		{
			ANSWER_CHOICES = 'Shorts\r\nPants\r\nSocks'.split('\r\n');
			COLUMN_CHOICES = 'Small\r\nMedium\r\nLarge'.split('\r\n');
		}
		if ( ANSWER_CHOICES )
		{
			for (let i = 0; i < ANSWER_CHOICES.length; i++)
	...
	private _onChange = (ANSWER_ID: string, COLUMN_ID: string): void =>
	{
		...
		try
		{
			...
			// 12/01/2019 Paul.  Forced Ranking only applies to Rating Scale and Radio Matrix. 
			this.setState({ VALUE });
		}
		catch(error)
		{
	...
	public RenderQuestion = (bDisable: boolean) =>
	{
		...
		if ( row )
		{
			...
			let cssStyle : any    = { transform: 'scale(1.5)', display: 'inline', verticalAlign: 'top', align: 'center', marginTop: '2px', marginBottom: '6px' };
