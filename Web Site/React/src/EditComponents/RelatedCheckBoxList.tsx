...
interface IRelatedCheckBoxListState
{
	ID               : string;
	FIELD_INDEX      : number;
	DATA_FIELD       : string;
...
export default class RelatedCheckBoxList extends EditComponent<IEditComponentProps, IRelatedCheckBoxListState>
{
	public get data(): any
	{
	...
	constructor(props: IEditComponentProps)
	{
		...
		try
		{
			...
			if ( layout != null )
			{
				...
				// 01/17/2018 Paul.  Add DATA_FORMAT to RelatedCheckBoxList support force user selection. 
				...
				if ( !Sql.IsEmptyString(LIST_NAME) )
				{
					...
					if ( arrLIST != null )
					{
						...
						// 01/06/2018 Paul.  Add DATA_FORMAT to RelatedCheckBoxList support multi-select CSV. 
						if ( Math.abs(FORMAT_ROWS) > 0 )
						{
							lstSelected = [];
							// 01/13/2018 Paul.  Terminology is now an ObservableArray. 
	...
	shouldComponentUpdate(nextProps: IEditComponentProps, nextState: IRelatedCheckBoxListState)
	{
	...
	public render()
	{
		...
		try
		{
			...
			{
				return (<span>DATA_FIELD is empty for RelatedCheckBoxList FIELD_INDEX { FIELD_INDEX }</span>);
			}
			else if ( Sql.IsEmptyString(LIST_NAME) )
			{
				return (<div>LIST_NAME is empty for RelatedCheckBoxList DATA_FIELD { DATA_FIELD }</div>);
			}
			else if ( onChanged == null )
			{
				return (<span>onChanged is null for RelatedCheckBoxList DATA_FIELD { DATA_FIELD }</span>);
			}
			// 11/02/2019 Paul.  Hidden property is used to dynamically hide and show layout fields. 
			...
			else
			{
				...
				// 08/03/2017 Paul.  We need a way to insert NONE into the a RelatedCheckBoxList while still allowing multiple rows. 
