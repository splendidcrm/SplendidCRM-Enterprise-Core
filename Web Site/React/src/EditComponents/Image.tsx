...
export default class Image extends EditComponent<IEditComponentProps, IImageState>
{
	public get data(): any
	{
		...
		try
		{
			...
			if ( layout != null )
			{
				...
				if ( row != null )
				{
					...
					if ( DATA_VALUE.indexOf('/') >= 0 || StartsWith(DATA_VALUE, 'http') )
					{
	...
	shouldComponentUpdate(nextProps: IEditComponentProps, nextState: IImageState)
	{
	...
	public render()
	{
		...
		try
		{
			...
			else
			{
				...
				let cssImage    = { objectFit: fit, width: '400px', height: '400px' };
