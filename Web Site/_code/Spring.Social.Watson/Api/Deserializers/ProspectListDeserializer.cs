...
{
	class ProspectListDeserializer : IXmlElementDeserializer
	{
		public object Deserialize(XmlElement xml, XmlElementMapper mapper)
		{
			ProspectList obj = new ProspectList();
			// 01/25/2018 Paul.  Can be a single or a list item serializer. 
			XmlNode xBody = xml.SelectSingleNode("Body");
			if ( xBody != null )
			{
				// 01/26/2017 Paul.  We need to perform the fault check if this is a single item. 
				...
				<LAST_MODIFIED>01/17/18 12:35 AM</LAST_MODIFIED>
				<LAST_CONFIGURED>01/03/18 08:10 PM</LAST_CONFIGURED>
				<CREATED>01/03/18 08:10 PM</CREATED>
				...
				<PARENT_DATABASE_ID>602759</PARENT_DATABASE_ID>
			...
			else
			{
				...
				<LAST_MODIFIED>01/17/18 12:35 AM</LAST_MODIFIED>
				<VISIBILITY>1</VISIBILITY>
				<PARENT_NAME>Main Lead Database</PARENT_NAME>
				<USER_ID>4ff7827e-15fa7cc1721-6681ada67421f5d3ed5e65517ed2e77a</USER_ID>
	...
	class ProspectListListDeserializer : IXmlElementDeserializer
	{
		public object Deserialize(XmlElement xml, XmlElementMapper mapper)
		{
			IList<Spring.Social.Watson.Api.ProspectList> items = new List<Spring.Social.Watson.Api.ProspectList>();
			XmlUtils.FaultCheck(xml);
			XmlNode xBody = xml.SelectSingleNode("Body");
			if ( xBody != null )
			{
				...
				if ( xResult != null )
				{
					...
					{
						Spring.Social.Watson.Api.ProspectList item = mapper.Deserialize<Spring.Social.Watson.Api.ProspectList>(xItem as XmlElement);
						if ( item != null )
							items.Add(item);
					}
				}
	...
	class ProspectListInsertDeserializer : IXmlElementDeserializer
	{
		public object Deserialize(XmlElement xml, XmlElementMapper mapper)
		{
			ProspectListInsert obj = new ProspectListInsert();
			/*
			<Envelope>
				<Body>
					<RESULT>
					<SUCCESS>TRUE</SUCCESS>
					<CONTACT_LIST_ID>672316</CONTACT_LIST_ID>
					<PARENT_FOLDER_ID>440271</PARENT_FOLDER_ID>
					</RESULT>
				</Body>
			</Envelope>
			*/
			...
			obj.RawContent                = xml.InnerXml;
			obj.CONTACT_LIST_ID           = XmlUtils.SelectSingleNode(xml, "CONTACT_LIST_ID"          );
			obj.PARENT_FOLDER_ID          = XmlUtils.SelectSingleNode(xml, "PARENT_FOLDER_ID"         );
			return obj;
		}
	}
