...
namespace Spring.Social.Watson.Api
{
	...
	public class Database : HBase
	{
		...
		public static DataRow ConvertToRow(Spring.Social.Watson.Api.Database obj)
		{
			DataTable dt = Spring.Social.Watson.Api.Database.CreateTable();
			DataRow row = dt.NewRow();
			obj.SetRow(row);
			return row;
		}

		public static DataTable ConvertToTable(IList<Spring.Social.Watson.Api.Database> lists)
		{
			DataTable dt = Spring.Social.Watson.Api.Database.CreateTable();
			if ( lists != null )
			{
				foreach ( Spring.Social.Watson.Api.Database list in lists )
				{
					DataRow row = dt.NewRow();
					dt.Rows.Add(row);
					list.SetRow(row);
