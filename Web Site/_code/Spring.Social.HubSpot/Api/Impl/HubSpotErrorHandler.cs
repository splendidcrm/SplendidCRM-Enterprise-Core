...
{
	class HubSpotErrorHandler : DefaultResponseErrorHandler
	{
		...
		public override void HandleError(Uri requestUri, HttpMethod requestMethod, HttpResponseMessage<byte[]> response)
		{
			...
			// if not otherwise handled, do default handling and wrap with HubSpotApiException
			try
			{
			...
			{
				throw new HubSpotApiException("Error consuming HubSpot REST API.", ex);
			}
		}
		...
		private void HandleClientErrors(HttpResponseMessage<byte[]> response) 
		{
			...
			if ( errorValue.GetValueOrDefault<string>("status") == "error" )
			{
				...
				throw new HubSpotApiException(errorText, HubSpotApiError.ContactExists);
			}
			...
			if ( response.StatusCode == HttpStatusCode.Unauthorized )
			{
				...
				{
					throw new HubSpotApiException("Authorization is required for the operation, but the API binding was created without authorization.", HubSpotApiError.NotAuthorized);
				}
				else if ( errorText == "Could not authenticate with OAuth." )
				{
					throw new HubSpotApiException("The authorization has been revoked.", HubSpotApiError.NotAuthorized);
				}
				else
				{
					throw new HubSpotApiException(errorText ?? response.StatusDescription, HubSpotApiError.NotAuthorized);
				}
			}
			else if ( response.StatusCode == HttpStatusCode.BadRequest )
			{
				throw new HubSpotApiException(errorText, HubSpotApiError.OperationNotPermitted);
			}
			...
			{
				throw new HubSpotApiException(errorText, HubSpotApiError.ResourceNotFound);
			}
			else if ( response.StatusCode == (HttpStatusCode)420 )
			{
				throw new HubSpotApiException("The rate limit has been exceeded.", HubSpotApiError.RateLimitExceeded);
			}
		}
		...
		private void HandleServerErrors(HttpStatusCode statusCode, string errorDetails)
		{
			if ( statusCode == HttpStatusCode.InternalServerError )
			{
				...
				if ( errorValue != null && !errorValue.IsNull && errorValue.GetValueOrDefault<string>("status") == "error" )
				{
					...
					throw new HubSpotApiException(errorText, HubSpotApiError.Server);
				}
				...
				{
					//throw new HubSpotApiException("Something is broken at HubSpot. Please see http://developer.HubSpot.com/ to report the issue.", HubSpotApiError.Server);
				}
			}
			else if ( statusCode == HttpStatusCode.BadGateway )
			{
				throw new HubSpotApiException("HubSpot is down or is being upgraded.", HubSpotApiError.ServerDown);
			}
			else if ( statusCode == HttpStatusCode.ServiceUnavailable )
			{
				throw new HubSpotApiException("HubSpot is overloaded with requests. Try again later.", HubSpotApiError.ServerOverloaded);
			}
		}
