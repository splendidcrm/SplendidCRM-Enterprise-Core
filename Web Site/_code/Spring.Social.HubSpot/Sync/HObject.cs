...
namespace Spring.Social.HubSpot
{
	public class HObject
	{
		...
		#region Properties
		protected Spring.Social.HubSpot.Api.IHubSpot hubSpot;
		public string   HubSpotTableName       ;
		public string   HubSpotTableSort       ;
		...
		public Spring.Social.HubSpot.Api.IHubSpot HubSpot
		{
			get { return this.hubSpot; }
		}
		...
		public HObject(HttpSessionState Session, Security Security, Sql Sql, SqlProcs SqlProcs, ExchangeSecurity ExchangeSecurity, SyncError SyncError, Spring.Social.HubSpot.Api.IHubSpot hubSpot, string sHubSpotTableName, string sHubSpotTableSort, string sCRMModuleName, string sCRMTableName, string sCRMTableSort, bool bCRMAssignedUser)
		{
			...
			this.HubSpotTableName    = sHubSpotTableName   ;
			this.HubSpotTableSort    = sHubSpotTableSort   ;
		...
		// 09/27/2020 Paul.  HubSpot is now using long instead of integers. 
		protected bool Compare(long dLValue, object oRValue, string sFieldName, StringBuilder sbChanges)
		{
			bool bChanged = false;
			long dRValue = Sql.ToLong(oRValue);
			if ( dLValue != dRValue )
			{
				sbChanges.AppendLine(sFieldName + " changed from '" + dLValue.ToString() + "' to '" + dRValue.ToString() + "'.");
				bChanged = true;
		...
		public virtual void SetFromHubSpot(long nId)
		{
			this.Reset();
			this.id = nId;
		}
		...
		public virtual IList<Spring.Social.HubSpot.Api.HBase> SelectModified(DateTime dtStartModifiedDate)
		{
			throw(new Exception("Not implemented."));
		}
				...
				using ( DbDataAdapter da = dbf.CreateDataAdapter() )
				{
					...
					using ( DataTable dt = new DataTable() )
					{
						...
						else
						{
							...
							// 03/25/2010 Paul.  If we find a account, then treat the CRM record as the master and send latest version over to HubSpot. 
							// 05/30/2012 Paul.  There can only be one record mapped to HubSpot, so we don't need to join to the remote key. 
							cmd.Parameters.Clear();
							// 01/27/2015 Paul.  View name changed so as to support Oracle. 
							sSQL = "select vw" + this.CRMTableName + ".ID             " + ControlChars.CrLf
							     + "  from            " + Sql.MetadataName(cmd, "vw" + this.CRMTableName + "_HubSpot") + "  vw" + this.CRMTableName   + ControlChars.CrLf
							     + "  left outer join vw" + this.CRMTableName + "_SYNC" + ControlChars.CrLf
							     + "               on vw" + this.CRMTableName + "_SYNC.SYNC_SERVICE_NAME     = N'HubSpot'   " + ControlChars.CrLf
					...
					using ( DataTable dt = new DataTable() )
					{
						...
						if ( sSYNC_ACTION == "remote new" || sSYNC_ACTION == "remote changed" )
						{
							...
							if (sDIRECTION == "bi-directional" || sDIRECTION == "to crm only" )
							{
								...
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									...
									{
										if ( bVERBOSE_STATUS )
											SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.HubSpotTableName + ".Import: Retrieving " + this.HubSpotTableName + " " + sName + ".");
								...
								if ( bChanged && bVERBOSE_STATUS )
									SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.HubSpotTableName + ".Import: Received " + this.HubSpotTableName + " " + sName + ". " + sbChanges.ToString());
							}
						}
						else if ( (sSYNC_ACTION == "local changed" || sSYNC_ACTION == "local new") && !Sql.IsEmptyGuid(this.LOCAL_ID) )
						{
							...
							if ( dt.Rows.Count > 0 )
							{
								...
								{
									if ( bVERBOSE_STATUS )
										SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.HubSpotTableName + ".Import: Syncing " + this.HubSpotTableName + " " + Sql.ToString(row["NAME"]) + ".");
									bool bChanged = false;
									// 05/18/2012 Paul.  Allow control of sync direction. 
						...
						else if ( sSYNC_ACTION == "local deleted" )
						{
							...
							{
								string sError = "Error deleting HubSpot " + this.HubSpotTableName + " " + sName + "." + ControlChars.CrLf;
								sError += Utils.ExpandException(ex) + ControlChars.CrLf;
								SyncError.SystemMessage("Error", new StackTrace(true).GetFrame(0), sError);
								sbErrors.AppendLine(sError);
							}
							if ( bVERBOSE_STATUS )
								SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.HubSpotTableName + ".Import: Deleting " + sName + ".");
							using ( IDbTransaction trn = Sql.BeginTransaction(con) )
							{
								try
								{
						...
						else if ( sSYNC_ACTION == "remote deleted" && !Sql.IsEmptyGuid(this.LOCAL_ID) )
						{
							...
							{
								if ( bVERBOSE_STATUS )
									SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.HubSpotTableName + ".Import: Deleting " + this.CRMTableName + " " + sName + ".");
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
