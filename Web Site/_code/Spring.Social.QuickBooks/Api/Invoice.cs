...
namespace Spring.Social.QuickBooks.Api
{
	...
	public class Invoice : QOrder
	{
		...
		public DateTime DueDateValue
		{
			get
			{
				return this.DueDate.HasValue ? this.DueDate.Value : DateTime.MinValue;
			}
			set
			{
				this.DueDate = value;
		...
		public static DataTable CreateTable()
		{
			...
			dt.Columns.Add("Deposit"                      , Type.GetType("System.Decimal" ));
			dt.Columns.Add("AllowIPNPayment"              , Type.GetType("System.Boolean" ));
		...
		public void SetRow(DataRow row)
		{
			...
			if ( this.Deposit                     .HasValue ) row["Deposit"                     ] = Sql.ToDBDecimal (this.Deposit                        .Value);
			if ( this.AllowIPNPayment             .HasValue ) row["AllowIPNPayment"             ] = Sql.ToDBBoolean (this.AllowIPNPayment                .Value);
		...
		public static DataRow ConvertToRow(Invoice obj)
		{
			DataTable dt = Invoice.CreateTable();
			DataRow row = dt.NewRow();
			obj.SetRow(row);
			return row;
		}

		public static DataTable ConvertToTable(IList<Invoice> invoices)
		{
			DataTable dt = Invoice.CreateTable();
			if ( invoices != null )
			{
