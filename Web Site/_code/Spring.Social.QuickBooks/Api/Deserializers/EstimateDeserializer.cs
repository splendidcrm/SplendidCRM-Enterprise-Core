...
{
	class EstimateDeserializer : IJsonDeserializer
	{
		public object Deserialize(JsonValue json, JsonMapper mapper)
		{
			// 06/12/2014 Paul.  This method is called for each array item and for a single item. 
			if ( json.ContainsName("Estimate") )
			{
				json = json.GetValue("Estimate");
			}
			...
			obj.ExpirationDate                = json.GetValueOrDefault<DateTime?>("ExpirationDate"              );
			obj.AcceptedBy                    = json.GetValueOrDefault<String   >("AcceptedBy"                  );
			obj.AcceptedDate                  = json.GetValueOrDefault<DateTime?>("AcceptedDate"                );
	...
	class EstimateListDeserializer : IJsonDeserializer
	{
		public object Deserialize(JsonValue json, JsonMapper mapper)
		{
			IList<Estimate> estimates = new List<Estimate>();
			JsonUtils.FaultCheck(json);
			JsonValue jsonResponse = json.GetValue("QueryResponse");
			if ( jsonResponse != null && jsonResponse.IsObject )
			{
				JsonValue jsonEstimates = jsonResponse.GetValue("Estimate");
				if ( jsonEstimates != null && jsonEstimates.IsArray )
				{
					foreach ( JsonValue itemValue in jsonEstimates.GetValues() )
					{
						estimates.Add( mapper.Deserialize<Estimate>(itemValue) );
					}
				}
			}
			return estimates;
