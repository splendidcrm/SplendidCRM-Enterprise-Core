...
{
	class PaymentMethodDeserializer : IJsonDeserializer
	{
		public object Deserialize(JsonValue json, JsonMapper mapper)
		{
			// 06/12/2014 Paul.  This method is called for each array item and for a single item. 
			if ( json.ContainsName("PaymentMethod") )
			{
				json = json.GetValue("PaymentMethod");
			}
	...
	class PaymentMethodListDeserializer : IJsonDeserializer
	{
		public object Deserialize(JsonValue json, JsonMapper mapper)
		{
			...
			{
				JsonValue jsonPaymentMethods = jsonResponse.GetValue("PaymentMethod");
				if ( jsonPaymentMethods != null && jsonPaymentMethods.IsArray )
				{
					foreach ( JsonValue itemValue in jsonPaymentMethods.GetValues() )
					{
						paymentMethods.Add( mapper.Deserialize<PaymentMethod>(itemValue) );
					}
				}
			}
