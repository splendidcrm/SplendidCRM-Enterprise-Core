...
namespace Spring.Social.QuickBooks
{
	public class Item : QObject
	{
		...
		public Item(HttpSessionState Session, Security Security, Sql Sql, SqlProcs SqlProcs, ExchangeSecurity ExchangeSecurity, SyncError SyncError, Spring.Social.QuickBooks.Api.IQuickBooks quickBooks, DataTable dtProductTypes)
			: base(Session, Security, Sql, SqlProcs, ExchangeSecurity, SyncError, quickBooks, "Items", "Name", "ProductTemplates", "PRODUCT_TEMPLATES", "NAME", false, false, false)
		{
		...
		public override bool SetFromCRM(string sID, DataRow row, StringBuilder sbChanges)
		{
			...
			if ( Sql.IsEmptyString(this.ID) )
			{
				...
				this.IncomeAccount        =               Sql.ToString (row["QUICKBOOKS_ACCOUNT"]);
			...
			else
			{
				...
				if ( Compare(this.IncomeAccount ,               Sql.ToString (row["QUICKBOOKS_ACCOUNT"])       , "QUICKBOOKS_ACCOUNT", sbChanges) ) { this.IncomeAccount  =               Sql.ToString (row["QUICKBOOKS_ACCOUNT"])       ;  bChanged = true; }
		...
		{
			this.Reset();
			Spring.Social.QuickBooks.Api.Item obj = this.quickBooks.ItemOperations.GetById(sId);
		...
		{
			Spring.Social.QuickBooks.Api.Item obj = this.quickBooks.ItemOperations.GetById(this.ID);
			obj.Id                    = this.ID           ;
			...
			obj = this.quickBooks.ItemOperations.Update(obj);
			this.RawContent   = obj.RawContent;
			this.ID           = obj.Id;
		...
		public override string Insert()
		{
			...
			Spring.Social.QuickBooks.Api.Item obj = new Spring.Social.QuickBooks.Api.Item();
			obj.Id                    = this.ID           ;
			obj.Name                  = Sql.Truncate(this.Name       ,  100);
			obj.Description           = Sql.Truncate(this.Description, 4000);
			obj.TypeValue             = this.Type         ;
			...
			obj = this.quickBooks.ItemOperations.Insert(obj);
			this.RawContent   = obj.RawContent;
			this.ID           = obj.Id;
			this.TimeModified = obj.MetaData.LastUpdatedTime;
			return this.ID;
		...
		{
			this.quickBooks.ItemOperations.Delete(this.ID);
		}

		public override void Get(string sID)
		{
			IList<Spring.Social.QuickBooks.Api.Item> lst = this.quickBooks.ItemOperations.GetAll("Id = '" + sID + "'", String.Empty);
			if ( lst.Count > 0 )
			{
		...
		{
			IList<Spring.Social.QuickBooks.Api.QBase> lst = this.quickBooks.ItemOperations.GetModified(dtStartModifiedDate);
			return lst;
		}
				...
				if ( acl.IsWriteable() )
				{
					try
					{
						...
						switch ( sColumnName )
						{
							...
							case "QUICKBOOKS_ACCOUNT":  oValue = Sql.ToDBString (this.IncomeAccount       );  break;
							// 03/23/2015 Paul.  Change from MFT_PART_NUM begin the primary field to NAME being the primary field. 
						...
						if ( oValue != null && sColumnName != "MODIFIED_USER_ID" )
						{
							if ( !bChanged )
							{
								switch ( sColumnName )
								{
									...
									case "QUICKBOOKS_ACCOUNT":  bChanged = ParameterChanged(par, oValue,   50, sbChanges);  break;
