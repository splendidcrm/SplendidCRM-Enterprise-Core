...
namespace Spring.Social.QuickBooks
{
	public class Estimate : QOrder
	{
		...
		public Estimate(HttpSessionState Session, Security Security, Sql Sql, SqlProcs SqlProcs, SplendidError SplendidError, ExchangeSecurity ExchangeSecurity, SyncError SyncError, Spring.Social.QuickBooks.Api.IQuickBooks quickBooks, DataTable dtCurrencies, DataTable dtShippers, DataTable dtTaxRates, DataTable dtItems, DataTable dtCustomers, DataTable dtPaymentTerms, string sDiscountAccountId, SplendidCRM.DbProviderFactory dbf, IDbConnection con, bool bShortStateName, bool bShortCountryName)
			: base(Session, Security, Sql, SqlProcs, ExchangeSecurity, SyncError, quickBooks, dtCurrencies, dtShippers, dtTaxRates, dtItems, dtCustomers, dtPaymentTerms, sDiscountAccountId, dbf, con, "Estimates", "Quotes", "QUOTES", "DATE_MODIFIED_UTC", true, bShortStateName, bShortCountryName)
		{
		...
		public override bool SetFromCRM(string sID, DataRow row, StringBuilder sbChanges)
		{
			...
			{
				this.DocNumber          = Sql.ToString  (row["QUOTE_NUM"                  ]);
				this.TxnDate            = Sql.ToDateTime(row["DATE_QUOTE_EXPECTED_CLOSED" ]).Date;
				this.TxnStatus          = Sql.ToString  (row["ORDER_STAGE"                ]);
			}
			else
			{
				if ( Compare(this.DocNumber      , row["QUOTE_NUM"                                 ]      , "QUOTE_NUM"                 , sbChanges) ) { this.DocNumber          = Sql.ToString  (row["QUOTE_NUM"                  ])     ;  bChanged = true; }
				if ( Compare(this.TxnDate        , Sql.ToDateTime(row["DATE_QUOTE_EXPECTED_CLOSED" ]).Date, "DATE_QUOTE_EXPECTED_CLOSED", sbChanges) ) { this.TxnDate            = Sql.ToDateTime(row["DATE_QUOTE_EXPECTED_CLOSED" ]).Date;  bChanged = true; }
				if ( Compare(this.TxnStatus      , row["ORDER_STAGE"                               ]      , "ORDER_STAGE"               , sbChanges) ) { this.TxnStatus          = Sql.ToString  (row["ORDER_STAGE"                ])     ;  bChanged = true; }
			}
			return bChanged;
		}
		...
		public override DataTable GetLineItemsFromCRM(ExchangeSession Session, IDbConnection con, Guid gUSER_ID, bool bRequireItems)
		{
			...
			QuickBooks.EstimateLineItem qoli = new QuickBooks.EstimateLineItem(this.Session, Security, Sql, SqlProcs, ExchangeSecurity, SyncError, quickBooks, vwItems.Table, this.LOCAL_ID, this.ID);
			...
			{
				throw(new Exception("Quote " + this.LOCAL_ID.ToString() + " does not have any valid line items"));
			}
			return dt;
		}
		...
		{
			return new QuickBooks.EstimateLineItem(Session, Security, Sql, SqlProcs, ExchangeSecurity, SyncError, quickBooks, this.Items, Guid.Empty, String.Empty);
		}

		public override void SetFromQuickBooks(string sId)
		{
			this.Reset();
			Spring.Social.QuickBooks.Api.Estimate obj = this.quickBooks.EstimateOperations.GetById(sId);
			...
			foreach ( Spring.Social.QuickBooks.Api.Line line in obj.Lines )
			{
				if ( line.DetailType == Spring.Social.QuickBooks.Api.LineDetailTypeEnum.SalesItemLineDetail && line.Item is Spring.Social.QuickBooks.Api.SalesItemLineDetail )
				{
					...
					{
						EstimateLineItem oLine = new EstimateLineItem(Session, Security, Sql, SqlProcs, ExchangeSecurity, SyncError, quickBooks, this.vwItems.Table, this.LOCAL_ID, this.ID);
		...
		{
			Spring.Social.QuickBooks.Api.Estimate obj = this.quickBooks.EstimateOperations.GetById(this.ID);
			obj.Id                    = this.ID                ;
			obj.DocNumber             = this.DocNumber         ;
			obj.TxnDateValue          = this.TxnDate           ;
			obj.TxnStatus             = this.TxnStatus         ;
			...
			obj = this.quickBooks.EstimateOperations.Update(obj);
			this.RawContent   = obj.RawContent;
			this.ID           = obj.Id;
		...
		public override string Insert()
		{
			...
			Spring.Social.QuickBooks.Api.Estimate obj = new Spring.Social.QuickBooks.Api.Estimate();
			obj.Id                    = this.ID                ;
			obj.DocNumber             = this.DocNumber         ;
			obj.TxnDateValue          = this.TxnDate           ;
			obj.TxnStatus             = this.TxnStatus         ;
			...
			// 03/08/2015 Paul.  We are not going to merge line items.  Just overwrite all line items. 
			SetLineItemsFromCRM(obj);
			obj = this.quickBooks.EstimateOperations.Insert(obj);
			this.RawContent   = obj.RawContent;
			this.ID           = obj.Id;
			this.TimeModified = obj.MetaData.LastUpdatedTime;
			foreach ( Spring.Social.QuickBooks.Api.Line line in obj.Lines )
		...
		{
			this.quickBooks.EstimateOperations.Delete(this.ID);
		}

		public override void Get(string sID)
		{
			IList<Spring.Social.QuickBooks.Api.Estimate> lst = this.quickBooks.EstimateOperations.GetAll("Id = '" + sID + "'", String.Empty);
			if ( lst.Count > 0 )
			{
		...
		{
			IList<Spring.Social.QuickBooks.Api.QBase> lst = this.quickBooks.EstimateOperations.GetModified(dtStartModifiedDate);
			return lst;
		}

		public override void FilterCRM(IDbCommand cmd)
		{
			Sql.AppendParameter(cmd, Sql.ToString(this.DocNumber), "QUOTE_NUM");
			...
			foreach(IDbDataParameter par in spUpdate.Parameters)
			{
				...
				if ( acl.IsWriteable() )
				{
					try
					{
						...
						switch ( sColumnName )
						{
							case "QUOTE_NUM"                  :  oValue = Sql.ToDBString  (this.DocNumber              );  break;
							case "DATE_QUOTE_EXPECTED_CLOSED" :
							...
							case "ORDER_STAGE"                :  oValue = Sql.ToDBString  (this.TxnStatus              );  break;
						}
						...
						if ( oValue != null )
						{
							if ( !bChanged )
							{
								switch ( sColumnName )
								{
									case "QUOTE_NUM"                  :  bChanged = ParameterChanged(par, oValue,   30, sbChanges);  break;
									case "ORDER_STAGE"                :  bChanged = ParameterChanged(par, oValue,   25, sbChanges);  break;
		...
		{
			IDbCommand cmdQUOTES_UpdateTotals = SqlProcs.Factory(trn.Connection, "spQUOTES_UpdateTotals");
			cmdQUOTES_UpdateTotals.Transaction = trn;
			Sql.SetParameter(cmdQUOTES_UpdateTotals, "@ID"              , gID          );
			Sql.SetParameter(cmdQUOTES_UpdateTotals, "@MODIFIED_USER_ID", gUSER_ID     );
			cmdQUOTES_UpdateTotals.ExecuteNonQuery();
		}
	}
}
