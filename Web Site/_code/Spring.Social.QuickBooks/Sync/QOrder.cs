...
namespace Spring.Social.QuickBooks
{
	public class QOrder : QObject
	{
		...
		public List<QuickBooks.LineItem> LineItems ;
		...
		public QOrder(HttpSessionState Session, Security Security, Sql Sql, SqlProcs SqlProcs, ExchangeSecurity ExchangeSecurity, SyncError SyncError, Spring.Social.QuickBooks.Api.IQuickBooks quickBooks, DataTable dtCurrencies, DataTable dtShippers, DataTable dtTaxRates, DataTable dtItems, DataTable dtCustomers, DataTable dtPaymentTerms, string sDiscountAccountId, SplendidCRM.DbProviderFactory dbf, IDbConnection con, string sQuickBooksTableName, string sCRMModuleName, string sCRMTableName, string sCRMTableSort, bool bCRMAssignedUser, bool bShortStateName, bool bShortCountryName)
			: base(Session, Security, Sql, SqlProcs, ExchangeSecurity, SyncError, quickBooks, sQuickBooksTableName, "DocNumber", sCRMModuleName, sCRMTableName, sCRMTableSort, bCRMAssignedUser, bShortStateName, bShortCountryName)
		{
		...
		protected static void SetAddressFromQuickBooks(Spring.Social.QuickBooks.Api.PhysicalAddress addr, bool bShortStateName, bool bShortCountryName, ref string sStreet, ref string sCity, ref string sState, ref string sPostalCode, ref string sCountry, ref string sRawAddress)
		{
			if ( addr != null )
			{
				StringBuilder sbStreet = new StringBuilder();
				if ( !Sql.IsEmptyString(addr.Line1) ) sbStreet.AppendLine(addr.Line1);
				if ( !Sql.IsEmptyString(addr.Line2) ) sbStreet.AppendLine(addr.Line2);
				if ( !Sql.IsEmptyString(addr.Line3) ) sbStreet.AppendLine(addr.Line3);
				if ( !Sql.IsEmptyString(addr.Line4) ) sbStreet.AppendLine(addr.Line4);
				if ( !Sql.IsEmptyString(addr.Line5) ) sbStreet.AppendLine(addr.Line5);
				sStreet     = Sql.ToString(sbStreet.ToString()).Trim();
				sCity       = Sql.ToString(addr.City                  );
				sState      = Sql.ToString(addr.CountrySubDivisionCode);
				sPostalCode = Sql.ToString(addr.PostalCode            );
				sCountry    = Sql.ToString(addr.Country               );
				
				// 02/27/2015 Paul.  QuickBooks is combining address in to Line1-Line5.  Use Google to separate. 
				if ( !Sql.IsEmptyString(sStreet) && Sql.IsEmptyString(sCity) && Sql.IsEmptyString(sState) && Sql.IsEmptyString(sPostalCode) )
				{
					sRawAddress = sbStreet.ToString();
					// 03/14/2015 Paul.  Using Google may lose suite number if not labled properly, so just keep all data in the Street field. 
					// The raw address will used to detect if there was no change so that the parts can be preserved if there is none. 
					//try
					//{
					//	AddressDetails info = new AddressDetails();
					//	GoogleUtils.ConvertAddressV3(sRawAddress, bShortStateName, bShortCountryName, ref info);
					//	sStreet     = info.ADDRESS_STREET    ;
					//	sCity       = info.ADDRESS_CITY      ;
					//	sState      = info.ADDRESS_STATE     ;
					//	sPostalCode = info.ADDRESS_POSTALCODE;
					//	sCountry    = info.ADDRESS_COUNTRY   ;
					//}
					//catch
					//{
					//}
		...
		public virtual void SetLineItemsFromCRM(Spring.Social.QuickBooks.Api.QOrder obj)
		{
			...
			foreach ( LineItem oLine in this.LineItems )
			{
				...
				if ( oLine.ItemType == "Comment" )
				{
					...
					line.DetailType  = Spring.Social.QuickBooks.Api.LineDetailTypeEnum.DescriptionOnly;
					line.Item        = new Spring.Social.QuickBooks.Api.DescriptionLineDetail();
			...
			// In the QuickBooks Online app, select Company Settings, Sales section, Sales form content, then Discount enabled. 
			if ( this.Discount > 0 )
			{
				if ( Sql.IsEmptyString(this.DiscountAccountId) )
					throw(new Exception("DiscountAccountId is empty.  Make sure that CRM setting QuickBooks.DiscountAccount is valid and that QuickBooks Online is enabled for discounts."));
				Spring.Social.QuickBooks.Api.Line line = new Spring.Social.QuickBooks.Api.Line();
				...
				line.DetailType  = Spring.Social.QuickBooks.Api.LineDetailTypeEnum.DiscountLineDetail;
				line.Item        = new Spring.Social.QuickBooks.Api.DiscountLineDetail();
				line.AmountValue = this.Discount;
				(line.Item as Spring.Social.QuickBooks.Api.DiscountLineDetail).DiscountAccountRefValue   = this.DiscountAccountId;
				obj.Lines.Add(line);
			}
			if ( this.ShippingAmount > 0 )
			{
				Spring.Social.QuickBooks.Api.Line line = new Spring.Social.QuickBooks.Api.Line();
				line.DetailType  = Spring.Social.QuickBooks.Api.LineDetailTypeEnum.SalesItemLineDetail;
				line.Item        = new Spring.Social.QuickBooks.Api.SalesItemLineDetail();
		...
		// 03/08/2015 Paul.  We are not going to merge line items.  Just overwrite all line items. 
		/*
		public virtual void MergeLineItemsFromCRM(Spring.Social.QuickBooks.Api.QOrder obj)
		{
			List<LineItem> lstNewItems = new List<LineItem>();
			List<Spring.Social.QuickBooks.Api.Line> lstMatched = new List<Spring.Social.QuickBooks.Api.Line>();
			foreach ( LineItem oLine in this.LineItems )
			{
				bool bFound = false;
				foreach ( Spring.Social.QuickBooks.Api.Line line in obj.Lines )
				{
					...
					else if ( line.DetailType == Spring.Social.QuickBooks.Api.LineDetailTypeEnum.DescriptionOnly && oLine.ItemType == "Comment" )
					{
			...
			// 02/12/2015 Paul.  Remove QuickBooks items that are no longer in CRM, but for only sale items. 
			for ( int i = obj.Lines.Count - 1; i >= 0; i-- )
			{
				Spring.Social.QuickBooks.Api.Line line = obj.Lines[i];
				if ( line.DetailType == Spring.Social.QuickBooks.Api.LineDetailTypeEnum.SalesItemLineDetail )
				{
				...
				else if ( line.DetailType == Spring.Social.QuickBooks.Api.LineDetailTypeEnum.SubTotalLineDetail )
				{
					obj.Lines.Remove(line);
				}
				else if ( line.DetailType == Spring.Social.QuickBooks.Api.LineDetailTypeEnum.DiscountLineDetail )
				{
					// 03/06/2015 Paul.  We are getting blank lines in the middle of the items.  Try forcing shipping and discounts to the bottom. 
					//line.AmountValue = this.Discount;
					//bDiscountFound = true;
				...
				// 03/01/2015 Paul.  QuickBooks is adding a blank line when we send a record. 
		...
		*/
		...
		// 03/08/2015 Paul.  We are not going to merge line items.  Just overwrite all line items. 
		/*
		public virtual bool MergeLineItemsFromCRM(ExchangeSession Session, DataTable dtLineItems, string sSYNC_ACTION, StringBuilder sbChanges)
		{
			...
			if ( sSYNC_ACTION.StartsWith("local") )
			{
				...
				for ( int i = 0; i < dtLineItems.Rows.Count ; i++ )
				{
					...
					if ( !bLineFound )
					{
						QuickBooks.LineItem qoli = this.CreateLineItem();
						qoli.Reset();
//#if !DEBUG
...
//#endif
		...
		*/
		...
		public override bool BuildUpdateProcedure(ExchangeSession Session, IDbCommand spUpdate, DataRow row, Guid gUSER_ID, Guid gTEAM_ID, Guid gASSIGNED_USER_ID, IDbTransaction trn, StringBuilder sbChanges)
		{
			...
			{
				// 02/26/2015 Paul.  We need to use QUICKBOOKS_TAX_VENDOR when setting the TaxCodeTaxRate. 
				vwTaxRates.RowFilter = "QUICKBOOKS_TAX_VENDOR = '" + this.TaxCodeTaxRate + "'";
				if ( vwTaxRates.Count > 0 )
			...
			// 03/12/2015 Paul.  QuickBooks is combining all address fields into a single string array. 
