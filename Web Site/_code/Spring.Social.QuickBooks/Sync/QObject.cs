...
namespace Spring.Social.QuickBooks
{
	public class QObject
	{
		...
		#region Properties
		protected Spring.Social.QuickBooks.Api.IQuickBooks quickBooks;
		public string   QuickBooksTableName    ;
		public string   QuickBooksTableSort    ;
		...
		public Spring.Social.QuickBooks.Api.IQuickBooks QuickBooks
		{
			get { return quickBooks; }
		}
		#endregion

		public QObject(HttpSessionState Session, Security Security, Sql Sql, SqlProcs SqlProcs, ExchangeSecurity ExchangeSecurity, SyncError SyncError, Spring.Social.QuickBooks.Api.IQuickBooks quickBooks, string sQuickBooksTableName, string sQuickBooksTableSort, string sCRMModuleName, string sCRMTableName, string sCRMTableSort, bool bCRMAssignedUser, bool bShortStateName, bool bShortCountryName)
		{
		...
		public virtual void SetFromQuickBooks(string sId)
		{
			this.Reset();
			this.ID = sId;
		}
		...
		public virtual IList<Spring.Social.QuickBooks.Api.QBase> SelectModified(DateTime dtStartModifiedDate)
		{
			throw(new Exception("Not implemented."));
		}
				...
				using ( DbDataAdapter da = dbf.CreateDataAdapter() )
				{
					...
					using ( DataTable dt = new DataTable() )
					{
						...
						else
						{
							...
							sSQL = "select vw" + this.CRMTableName + ".ID             " + ControlChars.CrLf
							     ...
							     + "               on vw" + this.CRMTableName + "_SYNC.SYNC_SERVICE_NAME     = N'QuickBooksOnline'   " + ControlChars.CrLf
					...
					using ( DataTable dt = new DataTable() )
					{
						...
						if ( sSYNC_ACTION == "remote new" || sSYNC_ACTION == "remote changed" || sSYNC_ACTION == "local changed" || sSYNC_ACTION == "local new" )
						{
							...
							{
								cmd.Parameters.Clear();
								if ( this.QuickBooksTableName == "Payments" || this.QuickBooksTableName == "CreditMemos" )
								{
						...
						if ( this is QuickBooks.QOrder )
						{
							dtLineItems = (this as QuickBooks.QOrder).GetLineItemsFromCRM(Session, con, gUSER_ID, false);
						}
						if ( sSYNC_ACTION == "remote new" || sSYNC_ACTION == "remote changed" )
						{
							...
							if (sDIRECTION == "bi-directional" || sDIRECTION == "to crm only" )
							{
								...
								if ( this is QuickBooks.QOrder && dtLineItems != null )
								{
									// 03/08/2015 Paul.  We are not going to merge line items.  Just overwrite all line items. 
									//bChanged = (this as QuickBooks.QOrder).MergeLineItemsFromCRM(Session, dtLineItems, sSYNC_ACTION, sbChanges);
									// 03/08/2015 Paul.  If we are not going to merge line items, then we must assume that all have changed. 
									bChanged |= (this as QuickBooks.QOrder).CompareLineItemsFromCRM(Session, dtLineItems, sSYNC_ACTION, sbChanges);
								}
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									...
									{
										if ( bVERBOSE_STATUS )
											SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.QuickBooksTableName + ".Import: Retrieving " + this.QuickBooksTableName + " " + sName + ".");
...
//#endif
											...
											for ( int i = 0; i < (this as QuickBooks.QOrder).LineItems.Count; i++ )
											{
												LineItem oLineItem = (this as QuickBooks.QOrder).LineItems[i];
												...
												if ( !bChanged && dtLineItems != null && (this as QuickBooks.QOrder).LineItems.Count == dtLineItems.Rows.Count )
												{
...
//#endif
								...
								if ( bChanged && bVERBOSE_STATUS )
									SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.QuickBooksTableName + ".Import: Received " + this.QuickBooksTableName + " " + sName + ". " + sbChanges.ToString());
							}
						}
						else if ( (sSYNC_ACTION == "local changed" || sSYNC_ACTION == "local new") && !Sql.IsEmptyGuid(this.LOCAL_ID) )
						{
							...
							if ( dt.Rows.Count > 0 )
							{
								...
								{
									if ( bVERBOSE_STATUS )
										SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.QuickBooksTableName + ".Import: Syncing " + this.QuickBooksTableName + " " + Sql.ToString(row["NAME"]) + ".");
									bool bChanged = false;
									// 05/18/2012 Paul.  Allow control of sync direction. 
									if (sDIRECTION == "bi-directional" || sDIRECTION == "from crm only" )
									{
										...
										if ( this is QuickBooks.QOrder )
										{
											...
											{
												(this as QuickBooks.QOrder).SetLineItemsFromCRM(Session, con, gUSER_ID, sbChanges);
											}
											else
											{
												//	bChanged |= (this as QuickBooks.QOrder).MergeLineItemsFromCRM(Session, dtLineItems, sSYNC_ACTION, sbChanges);
...
//#endif
						...
						else if ( sSYNC_ACTION == "local deleted" )
						{
							...
							if ( bVERBOSE_STATUS )
								SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.QuickBooksTableName + ".Import: Deleting " + sName + ".");
							using ( IDbTransaction trn = Sql.BeginTransaction(con) )
							{
								try
								{
						...
						else if ( sSYNC_ACTION == "remote deleted" && !Sql.IsEmptyGuid(this.LOCAL_ID) )
						{
							...
							{
								if ( bVERBOSE_STATUS )
									SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.QuickBooksTableName + ".Import: Deleting " + this.CRMTableName + " " + sName + ".");
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
