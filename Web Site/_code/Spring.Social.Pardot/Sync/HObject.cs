...
namespace Spring.Social.Pardot
{
	public class HObject
	{
		...
		#region Properties
		protected Spring.Social.Pardot.Api.IPardot pardot;
		public string   PardotTableName        ;
		public string   PardotTableSort        ;
		...
		public Spring.Social.Pardot.Api.IPardot Pardot
		{
			get { return this.pardot; }
		}
		...
		public HObject(HttpSessionState Session, Security Security, Sql Sql, SqlProcs SqlProcs, ExchangeSecurity ExchangeSecurity, SyncError SyncError, Spring.Social.Pardot.Api.IPardot pardot, string sPardotTableName, string sPardotTableSort, string sCRMModuleName, string sCRMTableName, string sCRMTableSort, bool bCRMAssignedUser)
		{
			...
			this.pardot              = pardot              ;
			this.PardotTableName     = sPardotTableName    ;
			this.PardotTableSort     = sPardotTableSort    ;
		...
		{
			bool bChanged = false;
			if  ( bLValue.HasValue || oRValue != DBNull.Value )
			{
				bool bValue = bLValue.HasValue ? bLValue.Value : false;
				bool bRValue = Sql.ToBoolean(oRValue);
				if ( bValue != bRValue )
				{
					sbChanges.AppendLine(sFieldName + " changed from '" + bValue.ToString() + "' to '" + bRValue.ToString() + "'.");
					bChanged = true;
				}
			}
			return bChanged;
		...
		public virtual void SetFromPardot(string sID)
		{
			this.Reset();
			this.id = sID;
		}
		...
		public virtual IList<Spring.Social.Pardot.Api.Prospect> SelectModified(DateTime dtStartModifiedDate)
		{
			throw(new Exception("Not implemented."));
		}

		public virtual IList<Spring.Social.Pardot.Api.Prospect> SelectDeleted(DateTime dtStartModifiedDate)
		{
			throw(new Exception("Not implemented."));
		}
				...
				using ( DbDataAdapter da = dbf.CreateDataAdapter() )
				{
					...
					using ( DataTable dt = new DataTable() )
					{
						...
						else
						{
							...
							// 03/25/2010 Paul.  If we find a account, then treat the CRM record as the master and send latest version over to Pardot. 
							// 05/30/2012 Paul.  There can only be one record mapped to Pardot, so we don't need to join to the remote key. 
							cmd.Parameters.Clear();
							// 01/27/2015 Paul.  View name changed so as to support Oracle. 
							sSQL = "select vw" + this.CRMTableName + ".ID             " + ControlChars.CrLf
							     + "  from            " + Sql.MetadataName(cmd, "vw" + this.CRMTableName + "_Pardot") + "  vw" + this.CRMTableName   + ControlChars.CrLf
							     + "  left outer join vw" + this.CRMTableName + "_SYNC" + ControlChars.CrLf
							     + "               on vw" + this.CRMTableName + "_SYNC.SYNC_SERVICE_NAME     = N'Pardot'   " + ControlChars.CrLf
					...
					using ( DataTable dt = new DataTable() )
					{
						...
						if ( sSYNC_ACTION == "remote new" || sSYNC_ACTION == "remote changed" )
						{
							...
							if (sDIRECTION == "bi-directional" || sDIRECTION == "to crm only" )
							{
								...
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									...
									{
										if ( bVERBOSE_STATUS )
											SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.PardotTableName + ".Import: Retrieving " + this.PardotTableName + " " + sName + ".");
								...
								if ( bChanged && bVERBOSE_STATUS )
									SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.PardotTableName + ".Import: Received " + this.PardotTableName + " " + sName + ". " + sbChanges.ToString());
							}
						}
						else if ( (sSYNC_ACTION == "local changed" || sSYNC_ACTION == "local new") && !Sql.IsEmptyGuid(this.LOCAL_ID) )
						{
							...
							if ( dt.Rows.Count > 0 )
							{
								...
								{
									if ( bVERBOSE_STATUS )
										SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.PardotTableName + ".Import: Syncing " + this.PardotTableName + " " + Sql.ToString(row["NAME"]) + ".");
									bool bChanged = false;
									// 05/18/2012 Paul.  Allow control of sync direction. 
						...
						else if ( sSYNC_ACTION == "local deleted" )
						{
							...
							{
								string sError = "Error deleting Pardot " + this.PardotTableName + " " + sName + "." + ControlChars.CrLf;
								sError += Utils.ExpandException(ex) + ControlChars.CrLf;
								SyncError.SystemMessage("Error", new StackTrace(true).GetFrame(0), sError);
								sbErrors.AppendLine(sError);
							}
							if ( bVERBOSE_STATUS )
								SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.PardotTableName + ".Import: Deleting " + sName + ".");
							using ( IDbTransaction trn = Sql.BeginTransaction(con) )
							{
								try
								{
						...
						else if ( sSYNC_ACTION == "remote deleted" && !Sql.IsEmptyGuid(this.LOCAL_ID) )
						{
							...
							{
								if ( bVERBOSE_STATUS )
									SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.PardotTableName + ".Import: Deleting " + this.CRMTableName + " " + sName + ".");
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
