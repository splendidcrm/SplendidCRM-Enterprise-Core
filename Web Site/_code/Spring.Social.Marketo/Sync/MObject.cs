...
namespace Spring.Social.Marketo
{
	public class MObject
	{
		...
		#region Properties
		protected Spring.Social.Marketo.Api.IMarketo marketo;
		public string   MarketoTableName       ;
		public string   MarketoTableSort       ;
		public string   MarketoFields          ;
		...
		public Spring.Social.Marketo.Api.IMarketo Marketo
		{
			get { return this.marketo; }
		}
		...
		public MObject(HttpSessionState Session, Security Security, Sql Sql, SqlProcs SqlProcs, SplendidError SplendidError, ExchangeSecurity ExchangeSecurity, SyncError SyncError, Spring.Social.Marketo.Api.IMarketo marketo, string sMarketoTableName, string sMarketoTableSort, string sCRMModuleName, string sCRMTableName, string sCRMTableSort, bool bCRMAssignedUser, string sMarketoFields)
		{
			...
			this.MarketoTableName    = sMarketoTableName   ;
			this.MarketoTableSort    = sMarketoTableSort   ;
			this.MarketoFields       = sMarketoFields      ;
		...
		public virtual void SetFromMarketo(int nId)
		{
			this.Reset();
			this.id = nId;
		}
		...
		public virtual void GetupdatedAt()
		{
			throw(new Exception("Not implemented."));
		}
		...
		public virtual IList<Spring.Social.Marketo.Api.MBase> SelectModified(DateTime dtStartModifiedDate)
		{
			throw(new Exception("Not implemented."));
		}
				...
				using ( DbDataAdapter da = dbf.CreateDataAdapter() )
				{
					...
					using ( DataTable dt = new DataTable() )
					{
						...
						else
						{
							...
							// 03/25/2010 Paul.  If we find a account, then treat the CRM record as the master and send latest version over to Marketo. 
							// 05/30/2012 Paul.  There can only be one record mapped to Marketo, so we don't need to join to the remote key. 
							cmd.Parameters.Clear();
							// 01/27/2015 Paul.  View name changed so as to support Oracle. 
							sSQL = "select vw" + this.CRMTableName + ".ID             " + ControlChars.CrLf
							     + "  from            " + Sql.MetadataName(cmd, "vw" + this.CRMTableName + "_Marketo") + "  vw" + this.CRMTableName   + ControlChars.CrLf
							     + "  left outer join vw" + this.CRMTableName + "_SYNC" + ControlChars.CrLf
							     + "               on vw" + this.CRMTableName + "_SYNC.SYNC_SERVICE_NAME     = N'Marketo'            " + ControlChars.CrLf
					...
					using ( DataTable dt = new DataTable() )
					{
						...
						if ( sSYNC_ACTION == "remote new" || sSYNC_ACTION == "remote changed" )
						{
							...
							if (sDIRECTION == "bi-directional" || sDIRECTION == "to crm only" )
							{
								...
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									...
									{
										if ( bVERBOSE_STATUS )
											SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.MarketoTableName + ".Import: Retrieving " + this.MarketoTableName + " " + sName + ".");
								...
								if ( bChanged && bVERBOSE_STATUS )
									SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.MarketoTableName + ".Import: Received " + this.MarketoTableName + " " + sName + ". " + sbChanges.ToString());
							}
						}
						else if ( (sSYNC_ACTION == "local changed" || sSYNC_ACTION == "local new") && !Sql.IsEmptyGuid(this.LOCAL_ID) )
						{
							...
							if ( dt.Rows.Count > 0 )
							{
								...
								{
									if ( bVERBOSE_STATUS )
										SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.MarketoTableName + ".Import: Syncing " + this.MarketoTableName + " " + Sql.ToString(row["NAME"]) + ".");
									bool bChanged = false;
									// 05/18/2012 Paul.  Allow control of sync direction. 
						...
						else if ( sSYNC_ACTION == "local deleted" )
						{
							...
							{
								string sError = "Error deleting Marketo " + this.MarketoTableName + " " + sName + "." + ControlChars.CrLf;
								sError += Utils.ExpandException(ex) + ControlChars.CrLf;
								SyncError.SystemMessage("Error", new StackTrace(true).GetFrame(0), sError);
								sbErrors.AppendLine(sError);
							}
							if ( bVERBOSE_STATUS )
								SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.MarketoTableName + ".Import: Deleting " + sName + ".");
							using ( IDbTransaction trn = Sql.BeginTransaction(con) )
							{
								try
								{
						...
						else if ( sSYNC_ACTION == "remote deleted" && !Sql.IsEmptyGuid(this.LOCAL_ID) )
						{
							...
							{
								if ( bVERBOSE_STATUS )
									SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.MarketoTableName + ".Import: Deleting " + this.CRMTableName + " " + sName + ".");
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
