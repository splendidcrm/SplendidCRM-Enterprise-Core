...
namespace Spring.Social.Shopify
{
	public class Item : QObject
	{
		...
		public Item(HttpSessionState Session, Security Security, Sql Sql, SqlProcs SqlProcs, ExchangeSecurity ExchangeSecurity, SyncError SyncError, Spring.Social.Shopify.Api.IShopify shopify, DataTable dtProductTypes)
		...
		public override bool SetFromCRM(string sID, DataRow row, StringBuilder sbChanges)
		{
			...
			if ( Sql.IsEmptyString(this.ID) )
			{
				...
				this.IncomeAccount        =               Sql.ToString (row["SHOPIFY_ACCOUNT"]);
			...
			else
			{
				...
				if ( Compare(this.IncomeAccount ,               Sql.ToString (row["SHOPIFY_ACCOUNT"])       , "SHOPIFY_ACCOUNT", sbChanges) ) { this.IncomeAccount  =               Sql.ToString (row["SHOPIFY_ACCOUNT"])       ;  bChanged = true; }
		...
		{
			this.Reset();
			Spring.Social.Shopify.Api.Product obj = this.shopify.ProductOperations.GetById(sId);
		...
		{
			Spring.Social.Shopify.Api.Product obj = this.shopify.ProductOperations.GetById(this.ID);
			obj.Id                    = this.ID           ;
		...
		public override string Insert()
		{
			...
			Spring.Social.Shopify.Api.Product obj = new Spring.Social.Shopify.Api.Product();
			obj.Id                    = this.ID           ;
			obj.Name                  = Sql.Truncate(this.Name       ,  100);
			obj.Description           = Sql.Truncate(this.Description, 4000);
			//obj.TypeValue             = this.Type         ;
		...
		{
			IList<Spring.Social.Shopify.Api.Product> lst = this.shopify.ProductOperations.GetAll("Id = '" + sID + "'");
			if ( lst.Count > 0 )
			{
		...
		{
			IList<Spring.Social.Shopify.Api.Product> products = this.shopify.ProductOperations.GetModified(dtStartModifiedDate);
				...
				if ( acl.IsWriteable() )
				{
					try
					{
						...
						switch ( sColumnName )
						{
							...
							case "SHOPIFY_ACCOUNT":  oValue = Sql.ToDBString (this.IncomeAccount       );  break;
							// 03/23/2015 Paul.  Change from MFT_PART_NUM begin the primary field to NAME being the primary field. 
						...
						if ( oValue != null && sColumnName != "MODIFIED_USER_ID" )
						{
							if ( !bChanged )
							{
								switch ( sColumnName )
								{
									...
									case "SHOPIFY_ACCOUNT":  bChanged = ParameterChanged(par, oValue,   50, sbChanges);  break;
