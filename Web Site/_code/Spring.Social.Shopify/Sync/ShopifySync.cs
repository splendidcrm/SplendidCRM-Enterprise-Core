...
namespace Spring.Social.Shopify
{
	public class ShopifySync
	{
		...
		public ShopifySync(IMemoryCache memoryCache, HttpSessionState Session, Security Security, Sql Sql, SqlProcs SqlProcs, SplendidError SplendidError, SplendidCache SplendidCache, ExchangeSecurity ExchangeSecurity, SyncError SyncError)
		{
		...
		{
			bool bShopifyEnabled = Sql.ToBoolean(Application["CONFIG.Shopify.Enabled"]);
#if DEBUG
			//bShopifyEnabled = true;
#endif
			if ( bShopifyEnabled )
			{
				string sClientID         = Sql.ToString(Application["CONFIG.Shopify.ClientID"        ]);
				string sOAuthAccessToken = Sql.ToString(Application["CONFIG.Shopify.OAuthAccessToken"]);
				bShopifyEnabled = !Sql.IsEmptyString(sOAuthAccessToken) && !Sql.IsEmptyString(sClientID);
			}
			return bShopifyEnabled;
		}

		public Guid ShopifyUserID()
		{
			Guid gShopify_USER_ID = Sql.ToGuid(Application["CONFIG.Shopify.UserID"]);
			if ( Sql.IsEmptyGuid(gShopify_USER_ID) )
				gShopify_USER_ID = new Guid("00000000-0000-0000-0000-000000000012");  // Use special Shopify user. 
			return gShopify_USER_ID;
		}

		public Spring.Social.Shopify.Api.IShopify CreateApi()
		{
			Spring.Social.Shopify.Api.IShopify shopify = null;
			string sShopifyName         = Sql.ToString(Application["CONFIG.Shopify.ShopName"        ]);
			string sShopifyClientID     = Sql.ToString(Application["CONFIG.Shopify.ClientID"        ]);
			string sShopifyClientSecret = Sql.ToString(Application["CONFIG.Shopify.ClientSecret"    ]);
			string sOAuthAccessToken    = Sql.ToString(Application["CONFIG.Shopify.OAuthAccessToken"]);
			
			Spring.Social.Shopify.Connect.ShopifyServiceProvider shopifyServiceProvider = new Spring.Social.Shopify.Connect.ShopifyServiceProvider(sShopifyName, sShopifyClientID, sShopifyClientSecret);
			shopify = shopifyServiceProvider.GetApi(sOAuthAccessToken);
			return shopify;
		}
		...
		{
			bool bSuccess = false;
			string sShopifyShopID        = Sql.ToString(Application["CONFIG.Shopify.ShopID"           ]);
			string sShopifyClientID      = Sql.ToString(Application["CONFIG.Shopify.ClientID"         ]);
			string sShopifyClientSecret  = Sql.ToString(Application["CONFIG.Shopify.ClientSecret"     ]);
			string sOAuthAccessToken  = Sql.ToString(Application["CONFIG.Shopify.OAuthAccessToken" ]);
			string sOAuthRefreshToken = Sql.ToString(Application["CONFIG.Shopify.OAuthRefreshToken"]);
			//string sOAuthExpiresAt              = Sql.ToString(Application["CONFIG.Shopify.OAuthExpiresAt"   ]);
			try
			{
				...
				{
					Spring.Social.Shopify.Connect.ShopifyServiceProvider shopifyServiceProvider = new Spring.Social.Shopify.Connect.ShopifyServiceProvider(sShopifyShopID, sShopifyClientID, sShopifyClientSecret);
					Spring.Social.OAuth2.OAuth2Parameters parameters = new Spring.Social.OAuth2.OAuth2Parameters();
					shopifyServiceProvider.OAuthOperations.RefreshAccessAsync(sOAuthRefreshToken, parameters)
						.ContinueWith<Spring.Social.OAuth2.AccessGrant>(task =>
						{
							...
							else
							{
								...
								Application["CONFIG.Shopify.OAuthAccessToken"] = String.Empty;
								throw(new Exception("Could not refresh Shopify access token.", task.Exception));
							}
							return null;
						}).Wait();
					...
					using ( IDbConnection con = dbf.CreateConnection() )
					{
						...
						using ( IDbTransaction trn = Sql.BeginTransaction(con) )
						{
							try
							{
								Application["CONFIG.Shopify.OAuthAccessToken"  ] = sOAuthAccessToken ;
								Application["CONFIG.Shopify.OAuthRefreshToken" ] = sOAuthRefreshToken;
								//Application["CONFIG.Shopify.OAuthExpiresAt"    ] = sOAuthExpiresAt   ;
								SqlProcs.spCONFIG_Update("system", "Shopify.OAuthAccessToken"  , Sql.ToString(Application["CONFIG.Shopify.OAuthAccessToken"  ]), trn);
								SqlProcs.spCONFIG_Update("system", "Shopify.OAuthRefreshToken" , Sql.ToString(Application["CONFIG.Shopify.OAuthRefreshToken" ]), trn);
								//SqlProcs.spCONFIG_Update("system", "Shopify.OAuthExpiresAt"    , Sql.ToString(Application["CONFIG.Shopify.OAuthExpiresAt"    ]), trn);
								trn.Commit();
								bSuccess = true;
							}
							catch
		...
		private bool ValidateShopify(Spring.Social.Shopify.Api.IShopify shopify)
		{
			IList<Spring.Social.Shopify.Api.Product> products = shopify.ProductOperations.GetModified(DateTime.MinValue);
#if DEBUG
			if ( products != null )
			{
				foreach ( Spring.Social.Shopify.Api.Product product in products )
				{
					Debug.WriteLine(product.ToString());
				}
			}
#endif
			IList<Spring.Social.Shopify.Api.Customer> customers = shopify.CustomerOperations.GetModified(DateTime.MinValue);
#if DEBUG
			if ( customers != null )
			{
				foreach ( Spring.Social.Shopify.Api.Customer customer in customers )
				{
					Debug.WriteLine(customer.ToString());
				}
			}
#endif
			IList<Spring.Social.Shopify.Api.Order> orders = shopify.OrderOperations.GetModified(DateTime.MinValue);
#if DEBUG
			if ( orders != null )
			{
				foreach ( Spring.Social.Shopify.Api.Order order in orders )
				{
		...
		public bool ValidateShopify(string sShopName, string sOAuthClientID, string sOAuthClientSecret, string sOAuthAccessToken, StringBuilder sbErrors)
		{
			bool bValidSource = false;
			try
			{
				Spring.Social.Shopify.Api.IShopify shopify = null;
				Spring.Social.Shopify.Connect.ShopifyServiceProvider shopifyServiceProvider = new Spring.Social.Shopify.Connect.ShopifyServiceProvider(sShopName, sOAuthClientID, sOAuthClientSecret);
				shopify = shopifyServiceProvider.GetApi(sOAuthAccessToken);
				bValidSource = ValidateShopify(shopify);
			}
			catch(Exception ex)
			{
		...
		public bool ValidateShopify(StringBuilder sbErrors)
		{
			bool bValidSource = false;
			try
			{
				Spring.Social.Shopify.Api.IShopify shopify = this.CreateApi();
				bValidSource = ValidateShopify(shopify);
			}
			catch(Exception ex)
			{
...
#pragma warning restore CS1998
		...
		{
			// 02/06/2014 Paul.  New Shopify factory to allow Remote and Online. 
			bool bShopifyEnabled = ShopifyEnabled();
			if ( !bInsideSyncAll && bShopifyEnabled )
			{
				bInsideSyncAll = true;
				try
				{
					...
					if ( !Sql.IsEmptyString(sOAuthAccessToken) )
					{
						...
						if ( DateTime.Now.AddDays(15) > dtOAuthExpiresAt )
						{
							...
							{
								SyncError.SystemMessage("Error", new StackTrace(true).GetFrame(0), "Failed to reconnect Shopify Online Access Token: " + sbErrors.ToString());
						...
						Guid gSHOPIFY_USER_ID = this.ShopifyUserID();
						Shopify.UserSync User = new Shopify.UserSync(Session, Security, SplendidError, ExchangeSecurity, SyncError, this, gSHOPIFY_USER_ID, bSyncAll);
						Sync(User, sbErrors);
					}
				}
				catch(Exception ex)
		...
		{
			Shopify.UserSync User = sender as Shopify.UserSync;
			if ( User != null )
			{
				try
				{
					if ( ShopifyEnabled() )
					{
						...
						if ( shopify != null )
						{
							...
							{
								con.Open();
								Shopify.Item item = new Shopify.Item(Session, Security, Sql, SqlProcs, ExchangeSecurity, SyncError, shopify, SplendidCache.ProductTypes());
								if ( !bInsideItems )
								{
									try
									{
					...
					if ( ShopifyEnabled() )
					{
						...
						if ( shopify != null )
						{
							...
							using ( IDbConnection con = dbf.CreateConnection() )
							{
								...
								{
									Shopify.Customer customer = new Shopify.Customer(Session, Security, Sql, SqlProcs, ExchangeSecurity, SyncError, shopify, dtCustomers, bShortStateName, bShortCountryName);
									if ( !bInsideCustomers )
									{
										try
										{
											...
											using ( DataTable dtItems = PRODUCT_TEMPLATES_SYNC(dbf, con, User.USER_ID) )
											{
												using ( DataTable dtCustomers = ACCOUNTS_SYNC(dbf, con, User.USER_ID) )
												{
													using ( DataTable dtPaymentTerms = PAYMENT_TERMS_SYNC(dbf, con, User.USER_ID) )
													{
														...
														Shopify.Invoice invoice = new Shopify.Invoice(Session, Security, Sql, SqlProcs, SplendidError, ExchangeSecurity, SyncError, shopify, dtCurrencies, dtShippers, dtTaxRates, dtItems, dtCustomers, dtPaymentTerms, sDiscountAccountId, dbf, con, bShortStateName, bShortCountryName);
														if ( !bInsideInvoices )
														{
															try
															{
		...
		private DataTable TAX_RATES_SYNC(SplendidCRM.DbProviderFactory dbf, IDbConnection con, Guid gUSER_ID)
		{
			...
			sSQL = "select SYNC_LOCAL_ID                                 " + ControlChars.CrLf
			     ...
			     + "     , SHOPIFY_TAX_VENDOR                         " + ControlChars.CrLf
		...
		{
			DataTable dt = new DataTable();
			bool bEnableMultiCurrency = Sql.ToBoolean(Application["CONFIG.Shopify.EnableMultiCurrency"]);
		...
		public void Sync(Shopify.UserSync User, StringBuilder sbErrors)
		{
			bool   bVERBOSE_STATUS      = Sql.ToBoolean(Application["CONFIG.Shopify.VerboseStatus"     ]);
			if ( bVERBOSE_STATUS )
				SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), "ShopifySync.Sync Start.");
			...
			if ( shopify != null )
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					try
					{
						...
						bool bSyncInvoices             = Sql.ToBoolean(Application["CONFIG.Shopify.SyncInvoices"       ]);
						bool bEnableMultiCurrency      = Sql.ToBoolean(Application["CONFIG.Shopify.EnableMultiCurrency"]);
						bool bShortStateName           = Sql.ToBoolean(Application["CONFIG.GoogleMaps.ShortStateName"  ]);
						bool bShortCountryName         = Sql.ToBoolean(Application["CONFIG.GoogleMaps.ShortCountryName"]);
						
						DataTable dtShippers = Cache.Get("Shopify.Shippers") as DataTable;
						if ( dtShippers == null )
						{
							dtShippers = SHIPPERS(dbf, con);
							Cache.Set("Shopify.Shippers", dtShippers, DefaultCacheExpiration());
						}
						
						DataTable dtTaxRates = Cache.Get("Shopify.TaxRates") as DataTable;
						if ( dtTaxRates == null )
						{
							dtTaxRates = TAX_RATES_SYNC(dbf, con, User.USER_ID);
							Cache.Set("Shopify.TaxRates", dtTaxRates, DefaultCacheExpiration());
						}
						
						DataTable dtPaymentTypes = Cache.Get("Shopify.PaymentTypes") as DataTable;
						if ( dtPaymentTypes == null )
						{
							dtPaymentTypes = PAYMENT_TYPES_SYNC(dbf, con, User.USER_ID);
							Cache.Set("Shopify.PaymentTypes", dtPaymentTypes, DefaultCacheExpiration());
						}
						
						DataTable dtPaymentTerms = Cache.Get("Shopify.PaymentTerms") as DataTable;
						if ( dtPaymentTerms == null )
						{
							dtPaymentTerms = PAYMENT_TERMS_SYNC(dbf, con, User.USER_ID);
							Cache.Set("Shopify.PaymentTerms", dtPaymentTerms, DefaultCacheExpiration());
						}
						
						DataTable dtItems = Cache.Get("Shopify.Items") as DataTable;
						Shopify.Item item = new Shopify.Item(Session, Security, Sql, SqlProcs, ExchangeSecurity, SyncError, shopify, SplendidCache.ProductTypes());
						if ( bSyncItems && (bSyncInvoices) && (dtItems == null || User.SyncAll) )
						{
							...
							Cache.Set("Shopify.Items", dtItems, DefaultCacheExpiration());
						}
					...
					{
						if ( bVERBOSE_STATUS )
							SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), "ShopifySync.Sync Done.");
					}
				}
			}
		}

		public void Sync(SplendidCRM.DbProviderFactory dbf, IDbConnection con, Shopify.UserSync User, Shopify.QObject qo, StringBuilder sbErrors)
		{
			...
			string sDIRECTION           = Sql.ToString (Application["CONFIG.Shopify.Direction"         ]);
			// 03/09/2015 Paul.  Establish maximum number of records to process at one time. 
			int    nMAX_RECORDS         = Sql.ToInteger(Application["CONFIG.Shopify.MaxRecords"        ]);
			...
			if ( bVERBOSE_STATUS )
				SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), qo.ShopifyTableName + ".Sync: " + sDIRECTION);
			try
			{
				string sSQL = String.Empty;
				...
				IList<Spring.Social.Shopify.Api.QBase> lst = qo.SelectModified(dtStartModifiedDate);
				if ( bVERBOSE_STATUS )
					SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), qo.ShopifyTableName + ".Sync: Query took " + (DateTime.Now - dtStartSelect).Seconds.ToString() + " seconds. Using last modified " + dtStartModifiedDate.ToString());
				if ( lst.Count > 0 )
					SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), qo.ShopifyTableName + ".Sync: " + lst.Count.ToString() + " " + qo.ShopifyTableName + " to import.");
				// 05/31/2012 Paul.  Sorting should not be necessary as the order of the line items should match the display order. 
				foreach ( Spring.Social.Shopify.Api.QBase qb in lst )
				{
					qo.SetFromShopify(qb.Id);
				...
				sSQL = "select vw" + qo.CRMTableName + ".*                                                                   " + ControlChars.CrLf
				     ...
				     + "               on vw" + qo.CRMTableName + "_SYNC.SYNC_SERVICE_NAME     = N'ShopifyOnline'         " + ControlChars.CrLf
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( qo.ShopifyTableName == "Invoices" )
					{
						Shopify.LineItem qoli = (qo as Shopify.QOrder).CreateLineItem();
						...
						cmd.CommandText += "                                   on vw" + qoli.CRMTableName + "_SYNC.SYNC_SERVICE_NAME     = N'ShopifyOnline'           " + ControlChars.CrLf;
					...
					using ( DbDataAdapter da = dbf.CreateDataAdapter() )
					{
						...
						using ( DataTable dt = new DataTable() )
						{
							...
							if ( dt.Rows.Count > 0 && !qo.IsReadOnly )
								SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), qo.ShopifyTableName + ".Sync: " + dt.Rows.Count.ToString() + " " + qo.CRMModuleName + " to send.");
							...
							for ( int i = 0; i < dt.Rows.Count && !qo.IsReadOnly; i++ )
							{
								...
								try
								{
									...
									if ( sSYNC_ACTION == "local new" || Sql.IsEmptyString(sSYNC_REMOTE_KEY) )
									{
										...
										{
											if ( bVERBOSE_STATUS )
												SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), qo.ShopifyTableName + ".Sync: Sending new " + qo.ShopifyTableName + " (" + i.ToString() + ") " + Sql.ToString(row["NAME"]) + ".");
											qo.SetFromCRM(String.Empty, row, sbChanges);
											// 02/7/2015 Paul.  Line items could have changed, so can't use this flag. 
											if ( qo is Shopify.QOrder )
											{
												(qo as Shopify.QOrder).SetLineItemsFromCRM(Session, con, gUSER_ID, sbChanges);
											}
									...
									{
										if ( bVERBOSE_STATUS )
											SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), qo.ShopifyTableName + ".Sync: Binding " + qo.ShopifyTableName + " (" + i.ToString() + ") " + Sql.ToString(row["NAME"]) + ".");
										...
										{
											string sError = "Error retrieving Shopify " + qo.ShopifyTableName + " (" + i.ToString() + ") " + Sql.ToString  (row["NAME"]) + "." + ControlChars.CrLf;
										...
										if ( sSYNC_ACTION == "local changed" )
										{
											...
											if (sDIRECTION == "bi-directional" || sDIRECTION == "from crm only" )
											{
												...
												if ( bVERBOSE_STATUS )
													SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), qo.ShopifyTableName + ".Sync: Sending " + qo.ShopifyTableName + " (" + i.ToString() + ") " + Sql.ToString(row["NAME"]) + ". " + sbChanges.ToString());
												qo.Update();
											}
										}
									}
									...
									else if ( sSYNC_ACTION == "remote deleted" || sSYNC_ACTION == "remote unsync" )
									{
										...
										{
											if ( bVERBOSE_STATUS )
												SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), qo.ShopifyTableName + ".Sync: Deleting " + qo.CRMTableName + " " + Sql.ToString(row["NAME"]) + ".");
											using ( IDbTransaction trn = Sql.BeginTransaction(con) )
											{
												try
												{
													...
													Sql.SetParameter(spSyncDelete, "@SERVICE_NAME"    , "Shopify"    );
								...
								catch(Exception ex)
								{
									...
									string sError = "Error creating Shopify " + qo.ShopifyTableName + " (" + i.ToString() + ") " + Sql.ToString  (row["NAME"]) + "." + ControlChars.CrLf;
				...
				if ( !qo.IsReadOnly )
				{
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						cmd.CommandText += " where SYNC_SERVICE_NAME     = N'ShopifyOnline'                   " + ControlChars.CrLf;
						...
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								da.Fill(dt);
								if ( dt.Rows.Count > 0 )
									SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), qo.ShopifyTableName + ".Sync: " + dt.Rows.Count.ToString() + " " + qo.CRMModuleName + " to delete.");
								for ( int i = 0; i < dt.Rows.Count; i++ )
								{
									...
									string sSYNC_REMOTE_KEY = Sql.ToString(row["SYNC_REMOTE_KEY"]);
									if ( bVERBOSE_STATUS )
										SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), qo.ShopifyTableName + ".Sync: Deleting ShopifyOnline (" + sSYNC_REMOTE_KEY + "), CRM " + qo.CRMModuleName + "(" + gID.ToString() + ").");
									...
									{
										string sError = "Error deleting ShopifyOnline " + qo.ShopifyTableName + " " + sSYNC_REMOTE_KEY + "." + ControlChars.CrLf;
										sError += Utils.ExpandException(ex) + ControlChars.CrLf;
										SyncError.SystemMessage("Error", new StackTrace(true).GetFrame(0), sError);
										sbErrors.AppendLine(sError);
									}
