...
namespace Spring.Social.Shopify
{
	public class Customer : QObject
	{
		...
		public Customer(HttpSessionState Session, Security Security, Sql Sql, SqlProcs SqlProcs, ExchangeSecurity ExchangeSecurity, SyncError SyncError, Spring.Social.Shopify.Api.IShopify shopify, DataTable dtParents, bool bShortStateName, bool bShortCountryName)
			: base(Session, Security, Sql, SqlProcs, ExchangeSecurity, SyncError, shopify, "Customers", "Name", "Accounts", "ACCOUNTS", "NAME", true, bShortStateName, bShortCountryName)
		{
			if ( dtParents != null )
		...
		{
			this.Reset();
			Spring.Social.Shopify.Api.Customer obj = this.shopify.CustomerOperations.GetById(sId);
			/*
		...
		{
			Spring.Social.Shopify.Api.Customer obj = this.shopify.CustomerOperations.GetById(this.ID);
			obj.Id = this.ID;
			/*
			...
			*/
			obj = this.shopify.CustomerOperations.Update(obj);
			this.RawContent   = obj.RawContent;
			this.ID           = obj.Id;
			this.TimeModified = obj.TimeModified;
		}
		...
		public override string Insert()
		{
			...
			Spring.Social.Shopify.Api.Customer obj = new Spring.Social.Shopify.Api.Customer();
			obj.Id = this.ID;
			/*
			// 03/23/2015 Paul.  We are only going to support B2B on the first release. 
			//if ( !Sql.IsEmptyString(this.FamilyName) )
			...
			//obj.JobDescription        = this.JobDescription      ;
			*/
			obj = this.shopify.CustomerOperations.Insert(obj);
			this.RawContent   = obj.RawContent;
			this.ID           = obj.Id;
			this.TimeModified = obj.TimeModified;
			return this.ID;
		...
		{
			this.shopify.CustomerOperations.Delete(this.ID);
		}

		public override void Get(string sID)
		{
			IList<Spring.Social.Shopify.Api.Customer> lst = this.shopify.CustomerOperations.GetAll("Id = '" + sID + "'");
			if ( lst.Count > 0 )
			{
		...
		{
			IList<Spring.Social.Shopify.Api.Customer> customers = this.shopify.CustomerOperations.GetModified(dtStartModifiedDate);
