...
namespace Spring.Social.Shopify
{
	public class Invoice : QOrder
	{
		...
		public Invoice(HttpSessionState Session, Security Security, Sql Sql, SqlProcs SqlProcs, SplendidError SplendidError, ExchangeSecurity ExchangeSecurity, SyncError SyncError, Spring.Social.Shopify.Api.IShopify shopify, DataTable dtCurrencies, DataTable dtShippers, DataTable dtTaxRates, DataTable dtItems, DataTable dtCustomers, DataTable dtPaymentTerms, string sDiscountAccountId, SplendidCRM.DbProviderFactory dbf, IDbConnection con, bool bShortStateName, bool bShortCountryName)
		...
		public override bool SetFromCRM(string sID, DataRow row, StringBuilder sbChanges)
		{
			...
			{
				// 03/09/2015 Paul.  Shopify is truncating the DateTime to just a date.  Do not treat that as a change event. 
		...
		public override DataTable GetLineItemsFromCRM(ExchangeSession Session, IDbConnection con, Guid gUSER_ID, bool bRequireItems)
		{
			...
			Shopify.InvoiceLineItem qoli = new Shopify.InvoiceLineItem(this.Session, Security, Sql, SqlProcs, ExchangeSecurity, SyncError, shopify, vwItems.Table, this.LOCAL_ID, this.ID);
			...
			sSQL = "select vw" + qoli.CRMTableName + ".*                                                                            " + ControlChars.CrLf
			     ...
			     + "               on vw" + qoli.CRMTableName + "_SYNC.SYNC_SERVICE_NAME     = N'ShopifyOnline'                  " + ControlChars.CrLf
					...
					for ( int i = 0; i < dt.Rows.Count ; i++ )
					{
						...
						if ( bVERBOSE_STATUS )
						{
							if ( sSYNC_ACTION == "local new" || Sql.IsEmptyString(sSYNC_REMOTE_KEY) )
								...
								SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), qoli.ShopifyTableName + ".Sync: Binding " + qoli.ShopifyTableName + " (" + i.ToString() + ") " + Sql.ToString(row["NAME"]) + ".");
						}
		...
		{
			return new Shopify.InvoiceLineItem(Session, Security, Sql, SqlProcs, ExchangeSecurity, SyncError, shopify, this.Items, Guid.Empty, String.Empty);
		}

		public override void SetFromShopify(string sId)
		{
			this.Reset();
			Spring.Social.Shopify.Api.Order obj = this.shopify.OrderOperations.GetById(sId);
			...
			*/
			SetAddressFromShopify(obj.billingAddress, this.ShortStateName, this.ShortCountryName, ref this.BillingStreet , ref this.BillingCity , ref this.BillingState , ref this.BillingPostalCode , ref this.BillingCountry , ref this.BillingRawAddress );
			SetAddressFromShopify(obj.shippingAddress, this.ShortStateName, this.ShortCountryName, ref this.ShippingStreet, ref this.ShippingCity, ref this.ShippingState, ref this.ShippingPostalCode, ref this.ShippingCountry, ref this.ShippingRawAddress);
			foreach ( Spring.Social.Shopify.Api.LineItem line in obj.lineItems )
			{
			}
		}
		...
		{
			Spring.Social.Shopify.Api.Order obj = this.shopify.OrderOperations.GetById(this.ID);
			obj.Id                    = this.ID                ;
			/*
		...
		public override string Insert()
		{
			...
			Spring.Social.Shopify.Api.Order obj = new Spring.Social.Shopify.Api.Order();
			obj.Id                    = this.ID                ;
			/*
			obj.DocNumber             = this.DocNumber         ;
			obj.DueDateValue          = this.DueDate           ;
		...
		{
			IList<Spring.Social.Shopify.Api.Order> lst = this.shopify.OrderOperations.GetAll("Id = '" + sID + "'");
			if ( lst.Count > 0 )
			{
				this.SetFromShopify(lst[0].Id);
			}
			else
			{
				this.Deleted = true;
		...
		public override IList<Spring.Social.Shopify.Api.QBase> SelectModified(DateTime dtStartModifiedDate)
		{
			IList<Spring.Social.Shopify.Api.Order> invoices = this.shopify.OrderOperations.GetModified(dtStartModifiedDate);
			List<Spring.Social.Shopify.Api.QBase> lst = new List<Spring.Social.Shopify.Api.QBase>();
			foreach ( Spring.Social.Shopify.Api.Order invoice in invoices )
			{
				lst.Add(invoice);
			}
			return lst;
