...
namespace Spring.Social.SalesFusion
{
	public class Contact : SFObject
	{
		...
		public Contact(HttpSessionState Session, Security Security, Sql Sql, SqlProcs SqlProcs, ExchangeSecurity ExchangeSecurity, SyncError SyncError, SalesFusionSync SalesFusionSync, Spring.Social.SalesFusion.Api.ISalesFusion salesFusion, DataTable dtUSERS, DataTable dtACCOUNTS)
			: base(Session, Security, Sql, SqlProcs, ExchangeSecurity, SyncError, SalesFusionSync, salesFusion, "Contacts", "Name", "Contacts", "CONTACTS", "NAME", true, dtUSERS)
		{
			if ( dtACCOUNTS != null )
		...
		{
			Spring.Social.SalesFusion.Api.Contact obj = this.SalesFusion.ContactOperations.GetById(nId);
			SetFromSalesFusion(obj);
		}

		public void SetFromSalesFusion(Spring.Social.SalesFusion.Api.Contact obj)
		{
			this.Reset();
			...
			this.opt_out             = (obj.opt_out     == "Y" ? true : false);
			this.do_not_call         = (obj.do_not_call == "Y" ? true : false);
		...
		{
			Spring.Social.SalesFusion.Api.Contact obj = this.SalesFusion.ContactOperations.GetById(this.id);
			obj.id                    = this.id                 ;
			...
			this.SalesFusion.ContactOperations.Update(obj);
			obj = this.SalesFusion.ContactOperations.GetById(this.id);
			this.RawContent       = obj.RawContent            ;
			this.id               = obj.id.Value              ;
			this.updated_date     = obj.updated_date.Value    ;
		}
		...
		public override string Insert()
		{
			...
			Spring.Social.SalesFusion.Api.Contact obj = new Spring.Social.SalesFusion.Api.Contact();
			obj.owner_id              = this.owner_id           ;
			obj.crm_id                = this.LOCAL_ID.ToString();
			obj.account_id            = this.account_id         ;
			...
			obj = this.SalesFusion.ContactOperations.Insert(obj);
			// 04/28/2015 Paul.  Insert does not return the last modified date. Get the record again. 
			obj = this.SalesFusion.ContactOperations.GetById(obj.id.Value);
			this.RawContent       = obj.RawContent            ;
			this.id               = obj.id.Value              ;
			this.updated_date     = obj.updated_date.Value    ;
		...
		{
			this.SalesFusion.ContactOperations.Delete(this.id);
		}

		public override void Get(string sID)
		{
			Spring.Social.SalesFusion.Api.Contact obj = this.SalesFusion.ContactOperations.GetById(Sql.ToInteger(sID));
			if ( obj.id.HasValue )
			{
		...
		{
			IList<Spring.Social.SalesFusion.Api.HBase> lst = this.SalesFusion.ContactOperations.GetModified(dtStartModifiedDate);
			return lst;
		}
