...
namespace Spring.Social.SalesFusion
{
	public class SalesFusionSync
	{
		...
		public SalesFusionSync(HttpSessionState Session, Security Security, Sql Sql, SqlProcs SqlProcs, SplendidError SplendidError, ExchangeSecurity ExchangeSecurity, SyncError SyncError)
		{
		...
		public bool SalesFusionEnabled()
		{
			bool bSalesFusionEnabled = Sql.ToBoolean(Application["CONFIG.SalesFusion.Enabled"]);
#if DEBUG
			//bSalesFusionEnabled = true;
#endif
			if ( bSalesFusionEnabled )
			{
				string sUSER_NAME       = Sql.ToString(Application["CONFIG.SalesFusion.UserName"]);
				string sDOMAIN          = Sql.ToString(Application["CONFIG.SalesFusion.Domain"  ]);
				string sPASSWORD        = Sql.ToString(Application["CONFIG.SalesFusion.Password"]);
				bSalesFusionEnabled = !Sql.IsEmptyString(sUSER_NAME) && !Sql.IsEmptyString(sDOMAIN) && !Sql.IsEmptyString(sPASSWORD);
			}
			return bSalesFusionEnabled;
		}

		public Guid SalesFusionUserID()
		{
			Guid gSalesFusion_USER_ID = Sql.ToGuid(Application["CONFIG.SalesFusion.UserID"]);
			if ( Sql.IsEmptyGuid(gSalesFusion_USER_ID) )
				gSalesFusion_USER_ID = new Guid("00000000-0000-0000-0000-00000000000C");  // Use special SalesFusion user. 
			return gSalesFusion_USER_ID;
		}

		public Spring.Social.SalesFusion.Api.ISalesFusion CreateApi()
		{
			Spring.Social.SalesFusion.Api.ISalesFusion salesFusion = null;
			...
			string sDOMAIN          = Sql.ToString(Application["CONFIG.SalesFusion.Domain"  ]);
			string sPASSWORD        = Sql.ToString(Application["CONFIG.SalesFusion.Password"]);
			sPASSWORD = Security.DecryptPassword(sPASSWORD, gCREDIT_CARD_KEY, gCREDIT_CARD_IV);
			
			Spring.Social.SalesFusion.Connect.SalesFusionServiceProvider salesFusionServiceProvider = new Spring.Social.SalesFusion.Connect.SalesFusionServiceProvider();
			salesFusion = salesFusionServiceProvider.GetApi(sUSER_NAME, sDOMAIN, sPASSWORD);
			return salesFusion;
		}

		public bool ValidateSalesFusion(string sUSER_NAME, string sPASSWORD, string sDOMAIN, StringBuilder sbErrors)
		{
			...
			try
			{
				...
				Spring.Social.SalesFusion.Connect.SalesFusionServiceProvider SalesFusionServiceProvider = new Spring.Social.SalesFusion.Connect.SalesFusionServiceProvider();
				salesFusion = SalesFusionServiceProvider.GetApi(sUSER_NAME, sDOMAIN, sPASSWORD);
				salesFusion.ContactOperations.GetPage(1);
				bValidSource = true;
			}
			catch(Exception ex)
			{
		...
		public bool ValidateSalesFusion(StringBuilder sbErrors)
		{
			bool bValidSource = false;
			try
			{
				Spring.Social.SalesFusion.Api.ISalesFusion salesFusion = this.CreateApi();
				salesFusion.ContactOperations.GetPage(1);
				bValidSource = true;
			}
			catch(Exception ex)
...
#pragma warning restore CS1998
		...
		{
			// 02/06/2014 Paul.  New SalesFusion factory to allow Remote and Online. 
			bool bSalesFusionEnabled = SalesFusionEnabled();
			if ( !bInsideSyncAll && bSalesFusionEnabled )
			{
				bInsideSyncAll = true;
				try
				{
					StringBuilder sbErrors = new StringBuilder();
					Spring.Social.SalesFusion.SalesFusionSync.RefreshAccessToken(sbErrors);
					if ( sbErrors.Length == 0 )
					{
						Guid gSalesFusion_USER_ID = SalesFusionSync.SalesFusionUserID(Context.Application);
						SalesFusion.UserSync User = new SalesFusion.UserSync(Session, Security, SplendidError, ExchangeSecurity, SyncError, this, gSalesFusion_USER_ID, bSyncAll);
						Sync(User, sbErrors);
					}
					else
					{
						SyncError.SystemMessage("Error", new StackTrace(true).GetFrame(0), "Failed to refresh SalesFusion Access Token: " + sbErrors.ToString());
					}
				}
				catch(Exception ex)
				{
		...
		public void Sync(SalesFusion.UserSync User, StringBuilder sbErrors)
		{
			bool bVERBOSE_STATUS = Sql.ToBoolean(Application["CONFIG.SalesFusion.VerboseStatus"]);
			if ( bVERBOSE_STATUS )
				SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), "SalesFusionSync.Sync Start.");
			
			Spring.Social.SalesFusion.Api.ISalesFusion salesFusion = SalesFusionSync.CreateApi();
			if ( SalesFusion != null )
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					try
					{
						...
						// 04/28/2015 Paul.  Best practices suggest syncing SalesFusion Contacts to CRM Leads.  We are going to make it configurable. 
						// http://lexnetcg.com/blog/inbound-marketing/SalesFusion-salesforce-integration-best-practices/
						string sSYNC_MODULES = Sql.ToString(Application["CONFIG.SalesFusion.SyncModules"]);
						if ( sSYNC_MODULES == "Contacts" )
						{
							SalesFusion.Company company = new SalesFusion.Company(SalesFusion);
							if ( !bInsideCompanies )
							{
								try
								{
							...
							{
								SalesFusion.Contact contact = new SalesFusion.Contact(SalesFusion, dtCompanies);
								if ( !bInsideContacts )
								{
									try
									{
						...
						{
							SalesFusion.Lead lead = new SalesFusion.Lead(SalesFusion);
							if ( !bInsideContacts )
							{
								try
								{
					...
					{
						if ( bVERBOSE_STATUS )
							SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), "SalesFusionSync.Sync Done.");
					}
				}
			}
		}

		public void Sync(SplendidCRM.DbProviderFactory dbf, IDbConnection con, SalesFusion.UserSync User, SalesFusion.HObject qo, StringBuilder sbErrors)
		{
			...
			bool   bVERBOSE_STATUS      = Sql.ToBoolean(Application["CONFIG.SalesFusion.VerboseStatus"     ]);
			string sCONFLICT_RESOLUTION = Sql.ToString (Application["CONFIG.SalesFusion.ConflictResolution"]);
			string sDIRECTION           = Sql.ToString (Application["CONFIG.SalesFusion.Direction"         ]);
			// 03/09/2015 Paul.  Establish maximum number of records to process at one time. 
			int    nMAX_RECORDS         = Sql.ToInteger(Application["CONFIG.SalesFusion.MaxRecords"        ]);
			Guid   gTEAM_ID             = Sql.ToGuid  (Session["TEAM_ID"]);
			if ( sDIRECTION.ToLower().StartsWith("bi") )
				sDIRECTION = "bi-directional";
			else if ( sDIRECTION.ToLower().StartsWith("to crm"  ) || sDIRECTION.ToLower().StartsWith("from SalesFusion") )
				sDIRECTION = "to crm only";
			else if ( sDIRECTION.ToLower().StartsWith("from crm") || sDIRECTION.ToLower().StartsWith("to SalesFusion"  ) )
				sDIRECTION = "from crm only";
			if ( bVERBOSE_STATUS )
				SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), qo.SalesFusionTableName + ".Sync: " + sDIRECTION);
			try
			{
				string sSQL = String.Empty;
				...
				IList<Spring.Social.SalesFusion.Api.HBase> lst = qo.SelectModified(dtStartModifiedDate);
				if ( bVERBOSE_STATUS )
					SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), qo.SalesFusionTableName + ".Sync: Query took " + (DateTime.Now - dtStartSelect).Seconds.ToString() + " seconds. Using last modified " + dtStartModifiedDate.ToString());
				if ( lst.Count > 0 )
					SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), qo.SalesFusionTableName + ".Sync: " + lst.Count.ToString() + " " + qo.SalesFusionTableName + " to import.");
				// 05/31/2012 Paul.  Sorting should not be necessary as the order of the line items should match the display order. 
				foreach ( Spring.Social.SalesFusion.Api.HBase qb in lst )
				{
					qo.SetFromSalesFusion(qb.id.Value);
				...
				sSQL = "select vw" + qo.CRMTableName + ".*                                                                   " + ControlChars.CrLf
				     ...
				     + "  from            " + Sql.MetadataName(con, "vw" + qo.CRMTableName + "_SalesFusion") + "  vw" + qo.CRMTableName + ControlChars.CrLf
				     + "  left outer join vw" + qo.CRMTableName + "_SYNC                                                     " + ControlChars.CrLf
				     + "               on vw" + qo.CRMTableName + "_SYNC.SYNC_SERVICE_NAME     = N'SalesFusion'         " + ControlChars.CrLf
					...
					using ( DbDataAdapter da = dbf.CreateDataAdapter() )
					{
						...
						using ( DataTable dt = new DataTable() )
						{
							...
							if ( dt.Rows.Count > 0 && !qo.IsReadOnly )
								SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), qo.SalesFusionTableName + ".Sync: " + dt.Rows.Count.ToString() + " " + qo.CRMModuleName + " to send.");
							...
							for ( int i = 0; i < dt.Rows.Count && !qo.IsReadOnly; i++ )
							{
								...
								try
								{
									if ( sSYNC_ACTION == "local new" || Sql.IsEmptyString(sSYNC_REMOTE_KEY) )
									{
										...
										{
											if ( bVERBOSE_STATUS )
												SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), qo.SalesFusionTableName + ".Sync: Sending new " + qo.SalesFusionTableName + " (" + i.ToString() + ") " + Sql.ToString(row["NAME"]) + ".");
									...
									{
										if ( bVERBOSE_STATUS )
											SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), qo.SalesFusionTableName + ".Sync: Binding " + qo.SalesFusionTableName + " (" + i.ToString() + ") " + Sql.ToString(row["NAME"]) + ".");
										...
										try
										{
											...
											// 03/26/2011 Paul.  The SalesFusion remote date can vary by 1 millisecond, so check for local change first. 
										...
										{
											string sError = "Error retrieving SalesFusion " + qo.SalesFusionTableName + " (" + i.ToString() + ") " + Sql.ToString  (row["NAME"]) + "." + ControlChars.CrLf;
										...
										if ( sSYNC_ACTION == "local changed" )
										{
											...
											if (sDIRECTION == "bi-directional" || sDIRECTION == "from crm only" )
											{
												...
												if ( bVERBOSE_STATUS )
													SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), qo.SalesFusionTableName + ".Sync: Sending " + qo.SalesFusionTableName + " (" + i.ToString() + ") " + Sql.ToString(row["NAME"]) + ". " + sbChanges.ToString());
												qo.Update();
											}
										}
									}
										...
										if ( !Sql.IsEmptyString(qo.ID) )
										{
											...
											using ( IDbTransaction trn = Sql.BeginTransaction(con) )
											{
												try
												{
													...
													Sql.SetParameter(spSyncUpdate, "@SERVICE_NAME"            , "SalesFusion"        );
									...
									else if ( sSYNC_ACTION == "remote deleted" || sSYNC_ACTION == "remote unsync" )
									{
										...
										{
											if ( bVERBOSE_STATUS )
												SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), qo.SalesFusionTableName + ".Sync: Deleting " + qo.CRMTableName + " " + Sql.ToString(row["NAME"]) + ".");
											using ( IDbTransaction trn = Sql.BeginTransaction(con) )
											{
												try
												{
													...
													Sql.SetParameter(spSyncDelete, "@SERVICE_NAME"    , "SalesFusion"    );
								...
								catch(Exception ex)
								{
									...
									string sError = "Error creating SalesFusion " + qo.SalesFusionTableName + " (" + i.ToString() + ") " + Sql.ToString  (row["NAME"]) + "." + ControlChars.CrLf;
				...
				if ( !qo.IsReadOnly )
				{
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						cmd.CommandText += " where SYNC_SERVICE_NAME     = N'SalesFusion'                   " + ControlChars.CrLf;
						...
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								da.Fill(dt);
								if ( dt.Rows.Count > 0 )
									SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), qo.SalesFusionTableName + ".Sync: " + dt.Rows.Count.ToString() + " " + qo.CRMModuleName + " to delete.");
								for ( int i = 0; i < dt.Rows.Count; i++ )
								{
									...
									string sSYNC_REMOTE_KEY = Sql.ToString(row["SYNC_REMOTE_KEY"]);
									if ( bVERBOSE_STATUS )
										SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), qo.SalesFusionTableName + ".Sync: Deleting SalesFusion (" + sSYNC_REMOTE_KEY + "), CRM " + qo.CRMModuleName + "(" + gID.ToString() + ").");
									...
									{
										string sError = "Error deleting SalesFusion " + qo.SalesFusionTableName + " " + sSYNC_REMOTE_KEY + "." + ControlChars.CrLf;
										sError += Utils.ExpandException(ex) + ControlChars.CrLf;
										SyncError.SystemMessage("Error", new StackTrace(true).GetFrame(0), sError);
										sbErrors.AppendLine(sError);
									}
