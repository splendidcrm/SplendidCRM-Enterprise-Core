...
namespace Spring.Social.SalesFusion
{
	public class SFObject
	{
		...
		protected SalesFusionSync      SalesFusionSync    ;

		#region Properties
		protected Spring.Social.SalesFusion.Api.ISalesFusion salesFusion;
		public string   SalesFusionTableName   ;
		public string   SalesFusionTableSort   ;
		...
		protected DataView vwUSERS   ;

		public Spring.Social.SalesFusion.Api.ISalesFusion SalesFusion
		{
			get { return this.salesFusion; }
		}

		public string ID
		{
			get { return Sql.ToString(this.id); }
			set { this.id = Sql.ToInteger(value); }
		}
		...
		{
			get { return this.updated_date; }
		}
		#endregion

		public SFObject(HttpSessionState Session, Security Security, Sql Sql, SqlProcs SqlProcs, ExchangeSecurity ExchangeSecurity, SyncError SyncError, SalesFusionSync SalesFusionSync, Spring.Social.SalesFusion.Api.ISalesFusion salesFusion, string sSalesFusionTableName, string sSalesFusionTableSort, string sCRMModuleName, string sCRMTableName, string sCRMTableSort, bool bCRMAssignedUser, DataTable dtUSERS)
		{
			...
			this.SalesFusionSync     = SalesFusionSync    ;

			this.salesFusion          = salesFusion          ;
			this.SalesFusionTableName = sSalesFusionTableName;
			this.SalesFusionTableSort = sSalesFusionTableSort;
			...
			this.vwUSERS = new DataView(dtUSERS);
		}
		...
		public virtual void Reset()
		{
			...
			this.owner_id         = 0                ;
			this.Deleted          = false            ;
			this.updated_date     = DateTime.MinValue;
		...
		protected bool Compare(Int64 dLValue, object oRValue, string sFieldName, StringBuilder sbChanges)
		{
			bool bChanged = false;
			Int64 dRValue = Sql.ToLong(oRValue);
			if ( dLValue != dRValue )
			{
				sbChanges.AppendLine(sFieldName + " changed from '" + dLValue.ToString() + "' to '" + dRValue.ToString() + "'.");
				bChanged = true;
		...
		public virtual void SetFromSalesFusion(int nId)
		{
			this.Reset();
			this.id = nId;
		}
		...
		{
			string sUserName = Sql.ToString (Application["CONFIG.SalesFusion.UserName"]);
			int    nUserID   = Sql.ToInteger(Application["CONFIG.SalesFusion.UserID." + sUserName]);
			if ( nUserID == 0 )
			{
				Spring.Social.SalesFusion.Api.ISalesFusion salesFusion = SalesFusionSync.CreateApi();
				Spring.Social.SalesFusion.Api.User obj = salesFusion.UserOperations.GetByName(sUserName);
				if ( obj != null )
				{
					nUserID = obj.id.Value;
					Application["CONFIG.SalesFusion.UserID." + sUserName] = nUserID;
				}
			}
			return nUserID;
		}

		public string MaxLength(string sValue, int nMax)
		{
		...
		public virtual IList<Spring.Social.SalesFusion.Api.HBase> SelectModified(DateTime dtStartModifiedDate)
		{
			throw(new Exception("Not implemented."));
		}
				...
				using ( DbDataAdapter da = dbf.CreateDataAdapter() )
				{
					...
					using ( DataTable dt = new DataTable() )
					{
						...
						else
						{
							...
							// 03/25/2010 Paul.  If we find a account, then treat the CRM record as the master and send latest version over to SalesFusion. 
							// 05/30/2012 Paul.  There can only be one record mapped to SalesFusion, so we don't need to join to the remote key. 
							cmd.Parameters.Clear();
							// 01/27/2015 Paul.  View name changed so as to support Oracle. 
							sSQL = "select vw" + this.CRMTableName + ".ID             " + ControlChars.CrLf
							     + "  from            " + Sql.MetadataName(cmd, "vw" + this.CRMTableName + "_SalesFusion") + "  vw" + this.CRMTableName   + ControlChars.CrLf
							     + "  left outer join vw" + this.CRMTableName + "_SYNC" + ControlChars.CrLf
							     + "               on vw" + this.CRMTableName + "_SYNC.SYNC_SERVICE_NAME     = N'SalesFusion'        " + ControlChars.CrLf
					...
					using ( DataTable dt = new DataTable() )
					{
						...
						if ( sSYNC_ACTION == "remote new" || sSYNC_ACTION == "remote changed" )
						{
							...
							if (sDIRECTION == "bi-directional" || sDIRECTION == "to crm only" )
							{
								...
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									...
									{
										if ( bVERBOSE_STATUS )
											SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.SalesFusionTableName + ".Import: Retrieving " + this.SalesFusionTableName + " " + sName + ".");
								...
								if ( bChanged && bVERBOSE_STATUS )
									SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.SalesFusionTableName + ".Import: Received " + this.SalesFusionTableName + " " + sName + ". " + sbChanges.ToString());
							}
						}
						else if ( (sSYNC_ACTION == "local changed" || sSYNC_ACTION == "local new") && !Sql.IsEmptyGuid(this.LOCAL_ID) )
						{
							...
							if ( dt.Rows.Count > 0 )
							{
								...
								{
									if ( bVERBOSE_STATUS )
										SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.SalesFusionTableName + ".Import: Syncing " + this.SalesFusionTableName + " " + Sql.ToString(row["NAME"]) + ".");
									bool bChanged = false;
									// 05/18/2012 Paul.  Allow control of sync direction. 
						...
						else if ( sSYNC_ACTION == "local deleted" )
						{
							...
							{
								string sError = "Error deleting SalesFusion " + this.SalesFusionTableName + " " + sName + "." + ControlChars.CrLf;
								sError += Utils.ExpandException(ex) + ControlChars.CrLf;
								SyncError.SystemMessage("Error", new StackTrace(true).GetFrame(0), sError);
								sbErrors.AppendLine(sError);
							}
							if ( bVERBOSE_STATUS )
								SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.SalesFusionTableName + ".Import: Deleting " + sName + ".");
							using ( IDbTransaction trn = Sql.BeginTransaction(con) )
							{
								try
								{
						...
						else if ( sSYNC_ACTION == "remote deleted" && !Sql.IsEmptyGuid(this.LOCAL_ID) )
						{
							...
							{
								if ( bVERBOSE_STATUS )
									SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.SalesFusionTableName + ".Import: Deleting " + this.CRMTableName + " " + sName + ".");
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
