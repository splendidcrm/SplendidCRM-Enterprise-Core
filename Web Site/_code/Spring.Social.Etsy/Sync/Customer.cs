...
namespace Spring.Social.Etsy
{
	public class Customer : QObject
	{
		...
		public Customer(HttpSessionState Session, Security Security, Sql Sql, SqlProcs SqlProcs, ExchangeSecurity ExchangeSecurity, SyncError SyncError, Spring.Social.Etsy.Api.IEtsy shopify, DataTable dtParents, bool bShortStateName, bool bShortCountryName)
		...
		{
			this.Reset();
			Spring.Social.Etsy.Api.Customer obj = this.shopify.CustomerOperations.GetById(sId);
			/*
		...
		{
			Spring.Social.Etsy.Api.Customer obj = this.shopify.CustomerOperations.GetById(this.ID);
			obj.Id = this.ID;
			/*
		...
		public override string Insert()
		{
			...
			Spring.Social.Etsy.Api.Customer obj = new Spring.Social.Etsy.Api.Customer();
			obj.Id = this.ID;
			/*
			// 03/23/2015 Paul.  We are only going to support B2B on the first release. 
			//if ( !Sql.IsEmptyString(this.FamilyName) )
		...
		{
			IList<Spring.Social.Etsy.Api.Customer> lst = this.shopify.CustomerOperations.GetAll("Id = '" + sID + "'");
			if ( lst.Count > 0 )
			{
		...
		{
			IList<Spring.Social.Etsy.Api.Customer> customers = this.shopify.CustomerOperations.GetModified(dtStartModifiedDate);
