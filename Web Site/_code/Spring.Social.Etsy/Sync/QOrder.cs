...
namespace Spring.Social.Etsy
{
	public class QOrder : QObject
	{
		...
		public List<Etsy.LineItem> LineItems ;
		...
		public QOrder(HttpSessionState Session, Security Security, Sql Sql, SqlProcs SqlProcs, ExchangeSecurity ExchangeSecurity, SyncError SyncError, Spring.Social.Etsy.Api.IEtsy shopify, DataTable dtCurrencies, DataTable dtShippers, DataTable dtTaxRates, DataTable dtItems, DataTable dtCustomers, DataTable dtPaymentTerms, string sDiscountAccountId, SplendidCRM.DbProviderFactory dbf, IDbConnection con, string sEtsyTableName, string sCRMModuleName, string sCRMTableName, string sCRMTableSort, bool bCRMAssignedUser, bool bShortStateName, bool bShortCountryName)
			: base(Session, Security, Sql, SqlProcs, ExchangeSecurity, SyncError, shopify, sEtsyTableName, "DocNumber", sCRMModuleName, sCRMTableName, sCRMTableSort, bCRMAssignedUser, bShortStateName, bShortCountryName)
		{
		...
		public override bool SetFromCRM(string sID, DataRow row, StringBuilder sbChanges)
		{
			...
			if ( vwTaxRates.Count > 0 )
				sTaxCodeTaxRate = Sql.ToString(vwTaxRates[0]["ETSY_TAX_VENDOR"]);
		...
		protected static void SetAddressFromEtsy(Spring.Social.Etsy.Api.MailingAddress addr, bool bShortStateName, bool bShortCountryName, ref string sStreet, ref string sCity, ref string sState, ref string sPostalCode, ref string sCountry, ref string sRawAddress)
		{
			if ( addr != null )
			{
		...
		public virtual void SetLineItemsFromCRM(Spring.Social.Etsy.Api.Order obj)
		{
			...
			{
				Spring.Social.Etsy.Api.LineItem line = new Spring.Social.Etsy.Api.LineItem();
				{
					line.id          = oLine.ID             ;
			...
			// In the Etsy Online app, select Company Settings, Sales section, Sales form content, then Discount enabled. 
			if ( this.Discount > 0 )
			{
				if ( Sql.IsEmptyString(this.DiscountAccountId) )
					throw(new Exception("DiscountAccountId is empty.  Make sure that CRM setting Etsy.DiscountAccount is valid and that Etsy Online is enabled for discounts."));
				Spring.Social.Etsy.Api.LineItem line = new Spring.Social.Etsy.Api.LineItem();
				obj.lineItems.Add(line);
			}
		}
		...
		// 03/08/2015 Paul.  We are not going to merge line items.  Just overwrite all line items. 
		/*
		public virtual void MergeLineItemsFromCRM(Spring.Social.Etsy.Api.QOrder obj)
		{
			List<LineItem> lstNewItems = new List<LineItem>();
			List<Spring.Social.Etsy.Api.Line> lstMatched = new List<Spring.Social.Etsy.Api.Line>();
			foreach ( LineItem oLine in this.LineItems )
			{
				bool bFound = false;
				foreach ( Spring.Social.Etsy.Api.Line line in obj.Lines )
				{
					if ( line.DetailType == Spring.Social.Etsy.Api.LineDetailTypeEnum.SalesItemLineDetail )
					{
					...
					else if ( line.DetailType == Spring.Social.Etsy.Api.LineDetailTypeEnum.DescriptionOnly && oLine.ItemType == "Comment" )
					{
			...
			{
				Spring.Social.Etsy.Api.Line line = new Spring.Social.Etsy.Api.Line();
				if ( oLine.ItemType == "Comment" )
				{
					line.Id          = oLine.ID             ;
					line.Description = oLine.ItemDescription;
					line.DetailType  = Spring.Social.Etsy.Api.LineDetailTypeEnum.DescriptionOnly;
					line.Item        = new Spring.Social.Etsy.Api.DescriptionLineDetail();
				}
				else
				{
					line.Id          = oLine.ID             ;
			...
			// 02/12/2015 Paul.  Remove Etsy items that are no longer in CRM, but for only sale items. 
			for ( int i = obj.Lines.Count - 1; i >= 0; i-- )
			{
				Spring.Social.Etsy.Api.Line line = obj.Lines[i];
				if ( line.DetailType == Spring.Social.Etsy.Api.LineDetailTypeEnum.SalesItemLineDetail )
				{
				...
				else if ( line.DetailType == Spring.Social.Etsy.Api.LineDetailTypeEnum.SubTotalLineDetail )
				{
					obj.Lines.Remove(line);
				}
				else if ( line.DetailType == Spring.Social.Etsy.Api.LineDetailTypeEnum.DiscountLineDetail )
				{
					// 03/06/2015 Paul.  We are getting blank lines in the middle of the items.  Try forcing shipping and discounts to the bottom. 
				...
				// 03/01/2015 Paul.  Etsy is adding a blank line when we send a record. 
				else if ( line.DetailType == Spring.Social.Etsy.Api.LineDetailTypeEnum.DescriptionOnly )
				{
			...
			if ( !bDiscountFound && this.Discount > 0 )
			{
				if ( Sql.IsEmptyString(this.DiscountAccountId) )
					throw(new Exception("DiscountAccountId is empty.  Make sure that CRM setting Etsy.DiscountAccount is valid and that Etsy Online is enabled for discounts."));
				Spring.Social.Etsy.Api.Line line = new Spring.Social.Etsy.Api.Line();
				line.DetailType  = Spring.Social.Etsy.Api.LineDetailTypeEnum.DiscountLineDetail;
				line.Item        = new Spring.Social.Etsy.Api.DiscountLineDetail();
				line.AmountValue = this.Discount;
				(line.Item as Spring.Social.Etsy.Api.DiscountLineDetail).DiscountAccountRefValue   = this.DiscountAccountId;
				obj.Lines.Add(line);
			}
			if ( !bShippingFound && this.ShippingAmount > 0 )
			{
				Spring.Social.Etsy.Api.Line line = new Spring.Social.Etsy.Api.Line();
				line.DetailType  = Spring.Social.Etsy.Api.LineDetailTypeEnum.SalesItemLineDetail;
				line.Item        = new Spring.Social.Etsy.Api.SalesItemLineDetail();
		...
		public virtual bool MergeLineItemsFromCRM(ExchangeSession Session, DataTable dtLineItems, string sSYNC_ACTION, StringBuilder sbChanges)
		{
			...
			if ( sSYNC_ACTION.StartsWith("local") )
			{
				...
				for ( int i = 0; i < dtLineItems.Rows.Count ; i++ )
				{
					...
					if ( !bLineFound )
					{
						Etsy.LineItem qoli = this.CreateLineItem();
						qoli.Reset();
//#if !DEBUG
...
//#endif
		...
		*/
		...
		public override bool BuildUpdateProcedure(ExchangeSession Session, IDbCommand spUpdate, DataRow row, Guid gUSER_ID, Guid gTEAM_ID, Guid gASSIGNED_USER_ID, IDbTransaction trn, StringBuilder sbChanges)
		{
			...
			{
				// 02/26/2015 Paul.  We need to use ETSY_TAX_VENDOR when setting the TaxCodeTaxRate. 
				vwTaxRates.RowFilter = "ETSY_TAX_VENDOR = '" + this.TaxCodeTaxRate + "'";
				if ( vwTaxRates.Count > 0 )
			...
			// 03/12/2015 Paul.  Etsy is combining all address fields into a single string array. 
				...
				if ( acl.IsWriteable() )
				{
					try
					{
						...
						switch ( sColumnName )
						{
							...
							case "CURRENCY_ID"                :
								// 03/02/2015 Paul.  Only update if Etsy value is not empty. 
								if ( !Sql.IsEmptyString(this.Currency) )
									if ( vwCurrencies.Count > 0 )
