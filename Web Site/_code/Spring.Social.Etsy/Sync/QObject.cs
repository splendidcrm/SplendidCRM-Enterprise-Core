...
namespace Spring.Social.Etsy
{
	public class QObject
	{
		...
		#region Properties
		protected Spring.Social.Etsy.Api.IEtsy shopify;
		public string   EtsyTableName    ;
		public string   EtsyTableSort    ;
		...
		public Spring.Social.Etsy.Api.IEtsy Etsy
		{
			get { return shopify; }
		}
		#endregion

		public QObject(HttpSessionState Session, Security Security, Sql Sql, SqlProcs SqlProcs, ExchangeSecurity ExchangeSecurity, SyncError SyncError, Spring.Social.Etsy.Api.IEtsy shopify, string sEtsyTableName, string sEtsyTableSort, string sCRMModuleName, string sCRMTableName, string sCRMTableSort, bool bCRMAssignedUser, bool bShortStateName, bool bShortCountryName)
		{
			...
			this.EtsyTableName       = sEtsyTableName     ;
			this.EtsyTableSort       = sEtsyTableSort     ;
		...
		public virtual void SetFromEtsy(string sId)
		{
			this.Reset();
			this.ID = sId;
		}
		...
		public virtual IList<Spring.Social.Etsy.Api.QBase> SelectModified(DateTime dtStartModifiedDate)
		{
			throw(new Exception("Not implemented."));
		}
				...
				using ( DbDataAdapter da = dbf.CreateDataAdapter() )
				{
					...
					using ( DataTable dt = new DataTable() )
					{
						...
						else
						{
							...
							// 03/25/2010 Paul.  If we find a account, then treat the CRM record as the master and send latest version over to Etsy. 
							// 05/30/2012 Paul.  There can only be one record mapped to Etsy, so we don't need to join to the remote key. 
							...
							sSQL = "select vw" + this.CRMTableName + ".ID             " + ControlChars.CrLf
							     ...
							     + "               on vw" + this.CRMTableName + "_SYNC.SYNC_SERVICE_NAME     = N'EtsyOnline'   " + ControlChars.CrLf
					...
					using ( DataTable dt = new DataTable() )
					{
						...
						if ( sSYNC_ACTION == "remote new" || sSYNC_ACTION == "remote changed" || sSYNC_ACTION == "local changed" || sSYNC_ACTION == "local new" )
						{
							...
							{
								cmd.Parameters.Clear();
								if ( this.EtsyTableName == "Payments" || this.EtsyTableName == "CreditMemos" )
								{
						...
						if ( this is Etsy.QOrder )
						{
							dtLineItems = (this as Etsy.QOrder).GetLineItemsFromCRM(Session, con, gUSER_ID, false);
						}
						if ( sSYNC_ACTION == "remote new" || sSYNC_ACTION == "remote changed" )
						{
							...
							if (sDIRECTION == "bi-directional" || sDIRECTION == "to crm only" )
							{
								...
								if ( this is Etsy.QOrder && dtLineItems != null )
								{
									// 03/08/2015 Paul.  We are not going to merge line items.  Just overwrite all line items. 
									//bChanged = (this as Etsy.QOrder).MergeLineItemsFromCRM(Session, dtLineItems, sSYNC_ACTION, sbChanges);
									// 03/08/2015 Paul.  If we are not going to merge line items, then we must assume that all have changed. 
									bChanged |= (this as Etsy.QOrder).CompareLineItemsFromCRM(Session, dtLineItems, sSYNC_ACTION, sbChanges);
								}
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									...
									{
										if ( bVERBOSE_STATUS )
											SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.EtsyTableName + ".Import: Retrieving " + this.EtsyTableName + " " + sName + ".");
...
//#endif
											...
											for ( int i = 0; i < (this as Etsy.QOrder).LineItems.Count; i++ )
											{
												LineItem oLineItem = (this as Etsy.QOrder).LineItems[i];
												...
												if ( !bChanged && dtLineItems != null && (this as Etsy.QOrder).LineItems.Count == dtLineItems.Rows.Count )
												{
...
//#endif
								...
								if ( bChanged && bVERBOSE_STATUS )
									SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.EtsyTableName + ".Import: Received " + this.EtsyTableName + " " + sName + ". " + sbChanges.ToString());
							}
						}
						else if ( (sSYNC_ACTION == "local changed" || sSYNC_ACTION == "local new") && !Sql.IsEmptyGuid(this.LOCAL_ID) )
						{
							...
							if ( dt.Rows.Count > 0 )
							{
								...
								{
									if ( bVERBOSE_STATUS )
										SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.EtsyTableName + ".Import: Syncing " + this.EtsyTableName + " " + Sql.ToString(row["NAME"]) + ".");
									bool bChanged = false;
									// 05/18/2012 Paul.  Allow control of sync direction. 
									if (sDIRECTION == "bi-directional" || sDIRECTION == "from crm only" )
									{
										...
										if ( this is Etsy.QOrder )
										{
											...
											{
												(this as Etsy.QOrder).SetLineItemsFromCRM(Session, con, gUSER_ID, sbChanges);
											}
											else
											{
												//	bChanged |= (this as Etsy.QOrder).MergeLineItemsFromCRM(Session, dtLineItems, sSYNC_ACTION, sbChanges);
...
//#endif
						...
						else if ( sSYNC_ACTION == "local deleted" )
						{
							...
							{
								string sError = "Error deleting Etsy " + this.EtsyTableName + " " + sName + "." + ControlChars.CrLf;
								sError += Utils.ExpandException(ex) + ControlChars.CrLf;
								SyncError.SystemMessage("Error", new StackTrace(true).GetFrame(0), sError);
								sbErrors.AppendLine(sError);
							}
							if ( bVERBOSE_STATUS )
								SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.EtsyTableName + ".Import: Deleting " + sName + ".");
							using ( IDbTransaction trn = Sql.BeginTransaction(con) )
							{
								try
								{
						...
						else if ( sSYNC_ACTION == "remote deleted" && !Sql.IsEmptyGuid(this.LOCAL_ID) )
						{
							...
							{
								if ( bVERBOSE_STATUS )
									SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.EtsyTableName + ".Import: Deleting " + this.CRMTableName + " " + sName + ".");
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
