...
{
	public abstract class EtsyOAuth2ApiBinding : IApiBinding
	{
		...
		protected EtsyOAuth2ApiBinding(string api_key, string accessToken)
		{
			this.api_key      = api_key    ;
			this.accessToken  = accessToken;
			this.restTemplate = new RestTemplate();
			((WebClientHttpRequestFactory)restTemplate.RequestFactory).Expect100Continue = false;
			// 03/13/2022 Paul.  Etsy requires api_key in header as well as Bearer access token. 
			// https://developers.etsy.com/documentation/reference/
			// https://developers.etsy.com/documentation/tutorials/quickstart/
			this.RestTemplate.RequestInterceptors.Add(new EtsyRequestInterceptor("x-api-key", api_key));
			this.restTemplate.RequestInterceptors.Add(new OAuth2RequestInterceptor(accessToken, OAuth2Version.Bearer));
