...
{
	public class MeetingSyncActivityWF4: CodeActivity
	{
		protected override void Execute(CodeActivityContext context)
		{
			...
			try
			{
				...
				Debug.WriteLine("MeetingSyncActivityWF4.Execute " + gAUDIT_ID.ToString());
				...
				using ( DataTable dt = new DataTable() )
				{
					...
					using ( IDbConnection con = dbf.CreateConnection() )
					{
						...
						sSQL = "select *                       " + ControlChars.CrLf
						     + "  from vwMEETINGS_SYNC_ACTIVITY" + ControlChars.CrLf
						     + " where ID in (select ID from vwMEETINGS_AUDIT where AUDIT_ID = @AUDIT_ID)" + ControlChars.CrLf;
					...
					foreach ( DataRow row in dt.Rows )
					{
						...
						{
							SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), "MeetingSyncActivity: GoogleSync, Meeting Changed " + gID.ToString() + " for User " + gUSER_ID.ToString());
							GoogleSync.SyncUser(gUSER_ID);
						}
						if ( bICLOUDEnabled && bICLOUD_SYNC_CALENDAR && !Sql.IsEmptyString(sICLOUD_USERNAME) )
						{
							SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), "MeetingSyncActivity: iCloudSync, Meeting Changed " + gID.ToString() + " for User " + gUSER_ID.ToString());
							iCloudSync.SyncUser(gUSER_ID);
						}
						if ( !Sql.IsEmptyString(sExchangeServerURL) )
						{
							SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), "MeetingSyncActivity: ExchangeSync, Meeting Changed " + gID.ToString() + " for User " + gUSER_ID.ToString());
			...
			catch(Exception ex)
			{
				...
				throw(new Exception("MeetingSyncActivityWF4.Execute failed: " + ex.Message, ex));
			}
		}
	}
}
