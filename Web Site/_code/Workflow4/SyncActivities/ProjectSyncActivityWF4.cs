...
{
	public class ProjectSyncActivityWF4: CodeActivity
	{
		protected override void Execute(CodeActivityContext context)
		{
			...
			try
			{
				...
				Debug.WriteLine("ProjectSyncActivityWF4.Execute " + gAUDIT_ID.ToString());
				...
				using ( DataTable dt = new DataTable() )
				{
					...
					using ( IDbConnection con = dbf.CreateConnection() )
					{
						...
						sSQL = "select *                       " + ControlChars.CrLf
						     + "  from vwPROJECTS_SYNC_ACTIVITY" + ControlChars.CrLf
					...
					foreach ( DataRow row in dt.Rows )
					{
						...
						{
							SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), "ProjectSyncActivity: ExchangeSync, Project Changed " + gID.ToString() + " for User " + gUSER_ID.ToString());
			...
			catch(Exception ex)
			{
				...
				throw(new Exception("ProjectSyncActivityWF4.Execute failed: " + ex.Message, ex));
			}
		}
	}
}
