...
{
	public class ContactSyncActivityWF4: CodeActivity
	{
		protected override void Execute(CodeActivityContext context)
		{
			...
			try
			{
				...
				Debug.WriteLine("ContactSyncActivityWF4.Execute " + gAUDIT_ID.ToString());
				...
				using ( DataTable dt = new DataTable() )
				{
					...
					foreach ( DataRow row in dt.Rows )
					{
						...
						{
							SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), "ContactSyncActivity: GoogleSync, Contact Changed " + gID.ToString() + " for User " + gUSER_ID.ToString());
						...
						{
							SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), "ContactSyncActivity: iCloudSync, Contact Changed " + gID.ToString() + " for User " + gUSER_ID.ToString());
							iCloudSync.SyncUser( gUSER_ID);
						}
						// 01/14/2021 Paul.  A contact can be synchronized normally or as a folder. 
						if ( bEXCHANGE_USER_ENABLED && !Sql.IsEmptyString(sExchangeServerURL) && (Sql.IsEmptyString(sSERVICE_NAME) || sSERVICE_NAME == "Exchange") )
						{
							SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), "ContactSyncActivity: ExchangeSync, Contact Changed " + gID.ToString() + " for User " + gUSER_ID.ToString());
			...
			catch(Exception ex)
			{
				...
				throw(new Exception("ContactSyncActivityWF4.Execute failed: " + ex.Message, ex));
			}
		}
	}
}
