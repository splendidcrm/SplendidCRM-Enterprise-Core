...
{
	public class SplendidTrackingParticipant : TrackingParticipant
	{
		...
		// 09/02/2023 Paul.  DataContractSerializer does not work because System.Xml.Linq.XName is blocked from serialization by Microsoft.
		//private DataContractSerializer variableSerializer;

		public SplendidTrackingParticipant(Sql Sql, SqlProcs SqlProcs)
		{
			this.Sql                 = Sql                ;
		...
		protected override void Track(TrackingRecord record, TimeSpan timeout)
		{
			try
			{
				...
				else if ( record is ActivityStateRecord )
				{
					...
					//Debug.WriteLine("SplendidTrackingParticipant.Track: Activity DisplayName: " + activityStateRecord.Activity.Name + " : ActivityState: " + activityStateRecord.State);
				...
				else if ( record is BookmarkResumptionRecord )
				{
					...
					//Debug.WriteLine("SplendidTrackingParticipant.Track: BookmarkName " + bookmarkRecord.BookmarkName);
				...
				else if ( record is CustomTrackingRecord )
				{
					...
					//Debug.WriteLine("SplendidTrackingParticipant.Track: CustomTrackingName " + customTrackingRecord.Name);
						...
						else
						{
							...
							else
							{
								if ( this.variableSerializer == null )
								{
									...
									//this.variableSerializer = new DataContractSerializer(typeof(Dictionary<XName, object>), dcSettings);
								}
