...
namespace Spring.Social.MailChimp
{
	public class HObject
	{
		...
		#region Properties
		protected Spring.Social.MailChimp.Api.IMailChimp mailChimp;
		public string   MailChimpTableName     ;
		public string   MailChimpTableSort     ;
		...
		public Spring.Social.MailChimp.Api.IMailChimp MailChimp
		{
			get { return this.mailChimp; }
		}
		...
		// 05/29/2016 Paul.  Add merge fields. 
		public HObject(HttpSessionState Session, Security Security, Sql Sql, SqlProcs SqlProcs, ExchangeSecurity ExchangeSecurity, SyncError SyncError, Spring.Social.MailChimp.Api.IMailChimp mailChimp, string sMailChimpTableName, string sMailChimpTableSort, string sCRMModuleName, string sCRMTableName, string sCRMTableSort, bool bCRMAssignedUser, DataTable dtMergeFields)
		{
			...
			this.MailChimpTableName  = sMailChimpTableName ;
			this.MailChimpTableSort  = sMailChimpTableSort ;
		...
		public virtual void Reset()
		{
			...
			this.Deleted          = false            ;
			this.date_created     = DateTime.MinValue;
		...
		public virtual void SetFromMailChimp(string sId)
		{
			this.Reset();
			this.id = sId;
		}
		...
		public virtual IList<Spring.Social.MailChimp.Api.HBase> SelectModified(DateTime dtStartModifiedDate)
		{
			throw(new Exception("Not implemented."));
		}
		...
		public virtual Guid CreateMember(ExchangeSession Session, IDbCommand spUpdate, Guid gUSER_ID, Guid gTEAM_ID, Guid gASSIGNED_USER_ID, IDbTransaction trn, StringBuilder sbChanges)
		{
			throw(new Exception("Not implemented."));
		}
					...
					using ( DataTable dt = new DataTable() )
					{
						...
						else
						{
							...
							sSQL = "select vw" + this.CRMTableName + ".ID" + ControlChars.CrLf
							     + "  from            " + Sql.MetadataName(cmd, "vw" + this.CRMTableName + "_MailChimp") + "  vw" + this.CRMTableName   + ControlChars.CrLf
							     + "  left outer join " + Sql.MetadataName(con, "vw" + this.CRMTableName + "_SYNC") + ControlChars.CrLf
							     + "               on " + Sql.MetadataName(con, "vw" + this.CRMTableName + "_SYNC") + ".SYNC_SERVICE_NAME     = N'MailChimp'                  " + ControlChars.CrLf
					...
					using ( DataTable dt = new DataTable() )
					{
						...
						if ( sSYNC_ACTION == "remote new" || sSYNC_ACTION == "remote changed" )
						{
							if (sDIRECTION == "bi-directional" || sDIRECTION == "to crm only" )
							{
								...
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									...
									{
										if ( bVERBOSE_STATUS )
											SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.MailChimpTableName + ".Import: Retrieving " + this.MailChimpTableName + " " + sName + ".");
										...
										if ( this.IsMember )
										{
											...
											if ( sSYNC_ACTION == "remote new" || Sql.IsEmptyGuid(this.LOCAL_ID) )
												this.LOCAL_ID = this.CreateMember(Session, spUpdate, gUSER_ID, gTEAM_ID, gASSIGNED_USER_ID, trn, sbChanges);
										}
										else
										{
								...
								if ( bChanged && bVERBOSE_STATUS )
									SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.MailChimpTableName + ".Import: Received " + this.MailChimpTableName + " " + sName + ". " + sbChanges.ToString());
							}
						}
						else if ( (sSYNC_ACTION == "local changed" || sSYNC_ACTION == "local new") && !Sql.IsEmptyGuid(this.LOCAL_ID) )
						{
							if ( dt.Rows.Count > 0 )
							{
								...
								{
									if ( bVERBOSE_STATUS )
										SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.MailChimpTableName + ".Import: Syncing " + this.MailChimpTableName + " " + Sql.ToString(row["NAME"]) + ".");
									bool bChanged = false;
						...
						else if ( sSYNC_ACTION == "local deleted" )
						{
							...
							{
								string sError = "Error deleting MailChimp " + this.MailChimpTableName + " " + sName + "." + ControlChars.CrLf;
								sError += Utils.ExpandException(ex) + ControlChars.CrLf;
								SyncError.SystemMessage("Error", new StackTrace(true).GetFrame(0), sError);
								sbErrors.AppendLine(sError);
							}
							if ( bVERBOSE_STATUS )
								SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.MailChimpTableName + ".Import: Deleting " + sName + ".");
							using ( IDbTransaction trn = Sql.BeginTransaction(con) )
							{
								try
								{
						...
						else if ( sSYNC_ACTION == "remote deleted" && !Sql.IsEmptyGuid(this.LOCAL_ID) )
						{
							...
							{
								if ( bVERBOSE_STATUS )
									SyncError.SystemMessage("Warning", new StackTrace(true).GetFrame(0), this.MailChimpTableName + ".Import: Deleting " + this.CRMTableName + " " + sName + ".");
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
