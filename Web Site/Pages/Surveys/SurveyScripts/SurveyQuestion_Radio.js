...
function SurveyQuestion_Radio(row)
{
	this.row = row;
...
SurveyQuestion_Radio.prototype.InitSample = function()
{
	this.row.ANSWER_CHOICES = 'Red\r\nGreen\r\nBlue';
}

SurveyQuestion_Radio.prototype.Value = function()
{
	var bValid = false;
	...
	try
	{
		if ( !Sql.IsEmptyString(this.row.ANSWER_CHOICES) )
		{
			...
			if ( arrANSWER_CHOICES.length > 0 )
			{
				for ( var i = 0; i < arrANSWER_CHOICES.length; i++ )
				{
					...
					var rad = document.getElementById(this.ID + '_' + sANSWER_ID);
		...
		if ( !bValid && Sql.ToBoolean(this.row.OTHER_ENABLED) )
		{
			...
			{
				var radOther = document.getElementById(this.ID + '_Other');
				if ( radOther != null && radOther.checked )
				{
	...
	{
		throw new Error('SurveyQuestionRadio.Value: ' + e.message);
	}
	return arrValue;
}

SurveyQuestion_Radio.prototype.Validate = function(divQuestionError)
{
	var bValid = false;
	var sValue = '';
	try
	...
	{
		throw new Error('SurveyQuestionRadio.Validate: ' + e.message);
		bValid = false;
	}
	return bValid;
}

SurveyQuestion_Radio.prototype.Render = function(divQuestionHeading, divQuestionBody, rowQUESTION_RESULTS, bDisable)
{
	try
	{
		...
		if ( !Sql.IsEmptyString(this.row.ANSWER_CHOICES) )
		{
			...
			if ( arrANSWER_CHOICES.length > 0 )
			{
				...
				//divQuestionBody.appendChild(document.createTextNode(nCellWidth + ', ' + nChoices + ', ' + nColumns + ', ' + nChoicesPerColumn))
				...
				for ( var i = 0; i < arrANSWER_CHOICES.length; i++ )
				{
					...
					rad.id        = this.ID + '_' + sANSWER_ID;
					rad.name      = this.ID;
					rad.type      = 'radio';
					rad.className = 'SurveyAnswerChoiceRadio';
					rad.value     = sANSWER_ID + ',' + arrANSWER_CHOICES[i];
					div.appendChild(rad);
	...
	{
		throw new Error('SurveyQuestionRadio.Render: ' + e.message);
	}
}

SurveyQuestion_Radio.prototype.Report = function(divQuestionHeading, divQuestionBody, rowQUESTION_RESULTS)
{
	try
	{
	...
	{
		throw new Error('SurveyQuestionRadio.Report: ' + e.message);
	}
}

SurveyQuestion_Radio.prototype.Summary = function(divQuestionHeading, divQuestionBody, callback, context)
{
	try
	{
		...
		SurveyResults_LoadTable(sTABLE_NAME, sSORT_FIELD, sSORT_DIRECTION, sSELECT, sFILTER, function(status, message)
		{
			if ( status == 1 )
			{
				...
				if ( rows != null && rows.length > 0 )
				{
					...
					var tbl = document.createElement('table');
					tbl.id          = this.ID;
					tbl.className   = 'SurveyResultsRadio';
					tbl.cellPadding = 4;
					tbl.cellSpacing = 0;
					tbl.border      = 0;
					divQuestionBody.appendChild(tbl);
	...
	{
		throw new Error('SurveyQuestion_Radio.Summary: ' + e.message);
	}
}
