...
function SurveyQuestion_SingleNumerical(row)
{
	this.row = row;
...
SurveyQuestion_SingleNumerical.prototype.InitSample = function()
{
}

SurveyQuestion_SingleNumerical.prototype.Value = function()
{
	var bValid = false;
	...
	{
		throw new Error('SurveyQuestion_SingleNumerical.Value: ' + e.message);
	}
	return arrValue;
}

SurveyQuestion_SingleNumerical.prototype.Validate = function(divQuestionError)
{
	var bValid = false;
	try
	{
		...
		if ( txt != null )
		{
			...
			if ( bValid )
			{
				...
				bValid = !isNaN(nValue);
				// 11/07/2018 Paul.  Value can be positive or negative.  Validation is managed below in SurveyQuestion_Helper_RequiredTypeValidation(). 
				if ( !bValid )
				{
	...
	{
		throw new Error('SurveyQuestion_SingleNumerical.Validate: ' + e.message);
		bValid = false;
	}
	return bValid;
}

SurveyQuestion_SingleNumerical.prototype.Render = function(divQuestionHeading, divQuestionBody, rowQUESTION_RESULTS, bDisable)
{
	try
	{
	...
	{
		throw new Error('SurveyQuestion_SingleNumerical.Render: ' + e.message);
	}
}

SurveyQuestion_SingleNumerical.prototype.Report = function(divQuestionHeading, divQuestionBody, rowQUESTION_RESULTS)
{
	try
	{
	...
	{
		throw new Error('SurveyQuestion_SingleNumerical.Report: ' + e.message);
	}
}

SurveyQuestion_SingleNumerical.prototype.Summary = function(divQuestionHeading, divQuestionBody, callback, context)
{
	try
	{
	...
	{
		throw new Error('SurveyQuestion_SingleNumerical.Summary: ' + e.message);
	}
}
