...
function SurveyQuestion_Date(row)
{
	this.row = row;
...
SurveyQuestion_Date.prototype.InitSample = function()
{
	this.row.ANSWER_CHOICES = 'When can you start work?';
}

SurveyQuestion_Date.prototype.Value = function()
{
	var bValid = false;
	...
	{
		throw new Error('SurveyQuestion_Date.Value: ' + e.message);
	}
	return arrValue;
}

SurveyQuestion_Date.prototype.Validate = function(divQuestionError)
{
	var bValid = false;
	try
	{
		...
		if ( !Sql.IsEmptyString(this.row.ANSWER_CHOICES) )
		{
			...
			for ( var i = 0; i < arrANSWER_CHOICES.length; i++ )
			{
				// 08/17/2018 Paul.  With multiple answers and required flag, all become required. 
				bValid = false;
				var sANSWER_ID = md5(arrANSWER_CHOICES[i]);
				var txt = document.getElementById(this.ID + '_' + sANSWER_ID);
				if ( txt != null )
				{
					...
					if ( sTxtValue.length > 0 )
					{
						...
						sValue += sTxtValue;
						nSelected++;
						bValid = Sql.IsDate(sTxtValue);
						if ( !bValid )
						{
							divQuestionError.innerHTML = this.row.INVALID_DATE_MESSAGE;
							return false;
						}
						// 08/17/2018 Paul.  Required type needs to be checked for each answer. 
						else if ( Sql.ToBoolean(this.row.REQUIRED) )
						{
							var dtValue = Sql.ToDateTime(sTxtValue);
							if ( !Sql.IsEmptyString(this.row.REQUIRED_TYPE) )
							{
								bValid = SurveyQuestion_Helper_RequiredDateValidation(this.row, dtValue);
								if ( !bValid )
								{
									divQuestionError.innerHTML = SurveyQuestion_Helper_RequiredDateMessage(this.row, dtValue);
									return false;
								}
							}
						}
		...
		if ( Sql.ToBoolean(this.row.REQUIRED) )
		{
			...
			{
				if ( !Sql.IsEmptyString(this.row.REQUIRED_TYPE) )
					divQuestionError.innerHTML = SurveyQuestion_Helper_RequiredDateMessage(this.row, null);
				else
	...
	{
		throw new Error('SurveyQuestion_Date.Validate: ' + e.message);
		bValid = false;
	}
	return bValid;
}

SurveyQuestion_Date.prototype.Render = function(divQuestionHeading, divQuestionBody, rowQUESTION_RESULTS, bDisable)
{
	try
	{
		...
		if ( !Sql.IsEmptyString(this.row.ANSWER_CHOICES) )
		{
			...
			if ( arrANSWER_CHOICES.length > 0 )
			{
				...
				for ( var i = 0; i < arrANSWER_CHOICES.length; i++ )
				{
					...
					txt.type      = 'text';
					txt.className = 'SurveyAnswerChoiceDate';
					div.appendChild(txt);
	...
	{
		throw new Error('SurveyQuestion_Date.Render: ' + e.message);
	}
}

SurveyQuestion_Date.prototype.Report = function(divQuestionHeading, divQuestionBody, rowQUESTION_RESULTS)
{
	try
	{
	...
	{
		throw new Error('SurveyQuestion_Date.Report: ' + e.message);
	}
}

SurveyQuestion_Date.prototype.Summary = function(divQuestionHeading, divQuestionBody, callback, context)
{
	try
	{
	...
	{
		throw new Error('SurveyQuestion_Date.Summary: ' + e.message);
	}
}
