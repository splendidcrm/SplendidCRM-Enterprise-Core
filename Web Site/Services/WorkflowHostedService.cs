...
namespace SplendidCRM
{
	...
	public class WorkflowHostedService : IHostedService, IDisposable
	{
		...
		public Task StartAsync(CancellationToken stoppingToken)
		{
			...
			// 09/01/2023 Paul.  Workflow fires every 30 seconds.  Core is quick, so start after 10 seconds. 
		...
		public Task StopAsync(CancellationToken stoppingToken)
		{
			...
			{
				SplendidError SplendidError = scope.ServiceProvider.GetRequiredService<SplendidError>();
				SplendidError.SystemWarning(new StackTrace(true).GetFrame(0), "The Workflow Manager timer is stopping.");
			}
