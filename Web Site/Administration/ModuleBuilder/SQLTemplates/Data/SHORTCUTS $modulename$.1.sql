...
if not exists (select * from SHORTCUTS where MODULE_NAME = '$modulename$' and DELETED = 0) begin -- then
	...
	exec dbo.spSHORTCUTS_InsertOnly null, '$modulename$', '.LNK_ACTIVITY_STREAM'     , '~/$administrationfolder$$modulename$/stream.aspx' , 'ActivityStream.gif', 1,  4, '$modulename$', 'list';
end -- if;
GO
